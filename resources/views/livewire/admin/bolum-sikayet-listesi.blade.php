<div class="premium-card" id="sikayetler-section">
    ...
    <div class="p-4 bg-gray-50/50 border-b border-gray-100">
        <div class="flex flex-wrap items-end gap-4">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                @if($customerId || $status || $startDate || $endDate || $search)
    ...
    <div class="px-4 py-2 bg-amber-50/80 border-b border-amber-100 flex items-center gap-3">
        ...
        <div class="flex flex-wrap gap-2">
            @if($search)
                <span class="inline-flex items-center px-2 py-0.5 bg-white border border-amber-200 rounded-md text-[10px] text-amber-600 font-bold shadow-sm">Arama: "{{ $search }}"</span>
            @endif
            @if($customerId)
                <span class="inline-flex items-center px-2 py-0.5 bg-white border border-amber-200 rounded-md text-[10px] text-amber-600 font-bold shadow-sm">Müşteri: {{ $customers->find($customerId)->name ?? 'Bilinmiyor' }}</span>
            @endif
            @if($status)
                <span class="inline-flex items-center px-2 py-0.5 bg-white border border-amber-200 rounded-md text-[10px] text-amber-600 font-bold shadow-sm">Durum: {{ $status }}</span>
            @endif
            @if($startDate)
                <span class="inline-flex items-center px-2 py-0.5 bg-white border border-amber-200 rounded-md text-[10px] text-amber-600 font-bold shadow-sm">Başlangıç: {{ \Carbon\Carbon::parse($startDate)->format('d.m.Y') }}</span>
            @endif
            @if($endDate)
                <span class="inline-flex items-center px-2 py-0.5 bg-white border border-amber-200 rounded-md text-[10px] text-amber-600 font-bold shadow-sm">Bitiş: {{ \Carbon\Carbon::parse($endDate)->format('d.m.Y') }}</span>
            @endif
        </div>
        <div class="ml-auto text-[10px] text-amber-500 italic font-medium">Toplam {{ $sikayetler->total() }} sonuç listeleniyor</div>
    </div>
    @endif
    ...
    <div class="overflow-x-auto">
        <table class="p-table">
            ...
            <tbody>
                ...
                <tr wire:key="sikayet-{{ $sikayet->id }}">
    ...
    <div class="p-4 bg-gray-50 border-t border-gray-100 text-center">
        <button wire:click="loadMore" wire:loading.attr="disabled" class="text-xs font-black text-indigo-600 hover:text-indigo-800 transition-all flex items-center justify-center gap-2 mx-auto group">
            <span wire:loading.remove>Devamını Göster ({{ $sikayetler->total() - $sikayetler->count() }} kayıt daha)</span>
