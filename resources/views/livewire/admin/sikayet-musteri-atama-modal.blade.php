                ...
                <div class="px-8 py-5">
                    ...
                    <div class="flex p-1.5 bg-gray-100 rounded-2xl mb-8 shadow-inner">
                        <button type="button" wire:click="$set('activeTab', 'mevcut')" 
                            class="flex-1 py-3 text-sm font-bold rounded-xl transition-all duration-300 {{ $activeTab === 'mevcut' ? 'bg-white text-indigo-700 shadow-md transform scale-[1.02]' : 'text-gray-500 hover:text-gray-700 hover:bg-white/50' }}">
                            Mevcut Müşteri
                        </button>
                        <button type="button" wire:click="$set('activeTab', 'yeni')" 
                            class="flex-1 py-3 text-sm font-bold rounded-xl transition-all duration-300 {{ $activeTab === 'yeni' ? 'bg-white text-indigo-700 shadow-md transform scale-[1.02]' : 'text-gray-500 hover:text-gray-700 hover:bg-white/50' }}">
                            Yeni Müşteri Oluştur
                        </button>
                    </div>

                    {{-- TAB 1: MEVCUT MÜŞTERİ --}}
                    @if($activeTab === 'mevcut')
                        <div class="space-y-6 animate-fadeIn">
                            <div>
                                ...
                                @error('selectedCustomerId') <span class="text-red-500 text-xs mt-1 block px-1 font-bold">{{ $message }}</span> @enderror
                            </div>
                            ...
                            <div class="animate-slideUp">
                                ...
                                @error('selectedRepId') <span class="text-red-500 text-xs mt-1 block px-1 font-bold">{{ $message }}</span> @enderror
                            </div>
                            @endif
                        </div>

                    {{-- TAB 2: YENİ MÜŞTERİ OLUŞTUR --}}
                    @else
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="flex items-center space-x-6 p-4 bg-gray-50 rounded-2xl border border-gray-100 group transition-all hover:bg-white hover:shadow-md">
                                    <div class="shrink-0 relative">
                                        @if ($logo)
                                            <img src="{{ $logo->temporaryUrl() }}" class="h-20 w-20 object-cover rounded-2xl border-2 border-indigo-100 shadow-md">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        @error('logo') <span class="text-red-500 text-[10px] font-bold block mt-1 uppercase">{{ $message }}</span> @enderror
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    @error('name') <span class="text-red-500 text-[10px] font-bold block mt-1 uppercase">{{ $message }}</span> @enderror
                                </div>
                                ...
                                <div class="space-y-6">
                                    @foreach($reps as $index => $rep)
                                        <div class="relative bg-white p-5 rounded-2xl border border-indigo-100 shadow-sm transition-all hover:shadow-lg group/rep translate-y-0 hover:-translate-y-0.5">
                                            ...
                                            @if(count($reps) > 1)
                                                <button type="button" wire:click="removeRepRow({{ $index }})" class="absolute -top-3 -right-3 bg-red-100 text-red-600 rounded-xl p-2 hover:bg-red-600 hover:text-white shadow-lg transition-all border-2 border-white opacity-0 group-hover/rep:opacity-100" title="Bu kişiyi sil">
                                            ...
                                            <div class="grid grid-cols-2 gap-4 mb-4">
                                                <div>
                                                    ...
                                                    <input type="text" wire:model="reps.{{ $index }}.name" class="w-full rounded-xl border-gray-100 bg-gray-50 py-2.5 focus:bg-white focus:border-indigo-400 transition-all text-sm font-bold shadow-inner" placeholder="Ad Soyad">
                                                    @error("reps.{$index}.name") <span class="text-red-500 text-[10px] font-bold block mt-1 uppercase">{{ $message }}</span> @enderror
                                                </div>
                                                <div>
                                                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 px-0.5">Ünvan</label>
                                                    <input type="text" wire:model="reps.{{ $index }}.title" class="w-full rounded-xl border-gray-100 bg-gray-50 py-2.5 focus:bg-white focus:border-indigo-400 transition-all text-sm font-bold shadow-inner" placeholder="Örn: Müdür">
                                                </div>
                                            </div>
                                            ...
                                            <div class="grid grid-cols-2 gap-4">
                                                <div>
                                                    ...
                                                    <input type="email" wire:model="reps.{{ $index }}.email" class="w-full rounded-xl border-gray-100 bg-gray-50 py-2.5 focus:bg-white focus:border-indigo-400 transition-all text-sm font-bold shadow-inner" placeholder="mail@ornek.com">
                                                    @error("reps.{$index}.email") <span class="text-red-500 text-[10px] font-bold block mt-1 uppercase">{{ $message }}</span> @enderror
                                                </div>
                                                <div>
                                                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 px-0.5">Telefon</label>
                                                    <input type="tel" wire:model="reps.{{ $index }}.phone" maxlength="15" oninput="this.value = this.value.replace(/[^0-9]/g, '')" class="w-full rounded-xl border-gray-100 bg-gray-50 py-2.5 focus:bg-white focus:border-indigo-400 transition-all text-sm font-bold shadow-inner" placeholder="05...">
                                                </div>
                                            </div>
