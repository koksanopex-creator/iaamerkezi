<div class="space-y-8">
    ...
    {{-- 0. FİLTRELEME BARI (BÖLÜM VE TARİH) --}}
    <div class="space-y-4">
        {{-- BÖLÜM FİLTRELERİ --}}
        {{-- BÖLÜM FİLTRELERİ - MODERN & DARALTILABİLİR --}}
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6" x-data="{ 
            ...
            <div class="overflow-hidden transition-all duration-500" x-show="isOpen || search.length > 0" x-cloak
                ...
                <div class="mt-8 space-y-8 pt-6 border-t border-gray-50">
                    ...
                    <div class="flex flex-wrap gap-3 items-center">
                        <a href="{{ route('dashboard', request()->except('bolum_id')) }}"
                           class="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all duration-300 border
                           {{ !isset($seciliBolumId) || !$seciliBolumId ? 'bg-indigo-600 text-white border-transparent shadow-lg shadow-indigo-100' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50' }}">
                            Genel Toplam (Tümü)
                        </a>
                        ...
                        @if(isset($seciliBolumId) && $seciliBolumId)
                            ...
                            <div class="flex items-center gap-3 px-6 py-2.5 bg-indigo-50 text-indigo-700 rounded-xl border border-indigo-100 shadow-sm font-black text-sm">
                                ...
                                {{ $seciliBolum->ad ?? 'Seçili Birim' }}
                                <a href="{{ route('dashboard', request()->except('bolum_id')) }}" class="ml-2 text-indigo-300 hover:text-indigo-600">
                            ...
                            <div x-show="search === '' || {{ $jsonNames }}.some(name => name.toLowerCase().includes(search.toLowerCase())) || '{{ strtolower($kategoriAd) }}'.includes(search.toLowerCase())"
                                ...
                                <div class="flex flex-col gap-1.5">
                                    @foreach($bolumler as $bolum)
                                        <a href="{{ route('dashboard', array_merge(request()->all(), ['bolum_id' => $bolum->id])) }}"
                                           ...
                                           {{ (isset($seciliBolumId) && $seciliBolumId == $bolum->id) ? 'bg-indigo-50 text-indigo-700 font-bold border border-indigo-100' : 'text-slate-500 hover:bg-slate-50 hover:text-indigo-600' }}">
                                            <span class="truncate">{{ $bolum->ad }}</span>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
            <form action="{{ route('dashboard') }}" method="GET" class="flex flex-wrap items-end gap-4">
                @if(request('bolum_id')) <input type="hidden" name="bolum_id" value="{{ request('bolum_id') }}"> @endif
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('dashboard', request()->only('bolum_id')) }}" class="px-4 py-2 bg-slate-50 text-slate-500 rounded-xl text-sm font-bold hover:bg-slate-100 hover:text-slate-700 transition-all border border-slate-100 flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-all">
            ...
            <h3 class="text-3xl font-black text-gray-800 tracking-tight">{{ number_format($stats['toplam_calisan'], 0) }}</h3>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-all relative overflow-hidden group">
            ...
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="text-right">
                    <p class="text-[10px] font-bold text-indigo-500 uppercase tracking-widest">{{ $stats['tamamlanan_proje'] }} Tamamlanan</p>
                    <p class="text-[9px] text-gray-400 italic">{{ $stats['devam_eden_proje'] }} Devam Eden</p>
                </div>
            </div>
            <p class="text-sm font-bold text-gray-400 uppercase tracking-wider">İAA Projeleri</p>
            <h3 class="text-3xl font-black text-gray-800 tracking-tight">{{ number_format($stats['toplam_proje'], 0) }}</h3>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-all relative overflow-hidden group">
            ...
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="text-right">
                    <p class="text-[10px] font-bold text-red-500 uppercase tracking-widest">{{ $stats['cozulen_sikayet'] }} Çözüldü</p>
                    <p class="text-[9px] text-gray-400 italic">{{ $stats['toplam_sikayet'] - $stats['cozulen_sikayet'] }} Bekleyen</p>
                </div>
            </div>
            <p class="text-sm font-bold text-gray-400 uppercase tracking-wider">Müşteri Şikayetleri</p>
            <h3 class="text-3xl font-black text-gray-800 tracking-tight">{{ number_format($stats['toplam_sikayet'], 0) }}</h3>
        </div>
        ...
        <div class="bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl shadow-lg p-5 text-white relative overflow-hidden">
            ...
            <h3 class="text-3xl font-black tracking-tight">{{ $stats['online_users_list']->count() }}</h3>
