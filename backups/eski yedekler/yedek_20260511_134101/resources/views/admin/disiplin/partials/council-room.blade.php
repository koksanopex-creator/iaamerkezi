@php
    // A. İnteraktif Toplantı Kontrolü (Yeni Yapı)
    $interactiveMeeting = $case->toplantilar()->where('durum', 'tamamlandı')->latest()->first();
...
@endphp
                                            ...
                                            <div class="flex-1">
                                                ...
                                                @if($madde->sorumlu_user_id)
                                                    <div class="mt-3 flex items-center gap-2">
                                                        <span class="text-[10px] font-bold text-slate-400 uppercase uppercase">SORUMLU:</span>
                                    ...
                                    <div class="flex items-start gap-4">
                                        ...
                                        <div class="flex-1">
                                            <div class="flex items-center justify-between">
                                                ...
                                                <span class="text-[10px] font-bold px-2 py-0.5 rounded-full {{ $oy->oy_yonu == 'Ceza Verilsin' ? 'bg-rose-100 text-rose-600' : 'bg-emerald-100 text-emerald-600' }}">
                                                    {{ $oy->oy_yonu }}
                                                </span>
                                            </div>
                                            @if($oy->yorum)
                                                <div class="mt-2 text-sm text-slate-600 italic">"{{ $oy->yorum }}"</div>
                                            @endif
                                        </div>
                                    </div>
                                @endforeach
                    ...
                    @if(Auth::user()->hasRole(['Disiplin Kurulu Üyesi', 'Disiplin Kurulu Başkanı', 'Superadmin', 'Hukuk Admini']) || Auth::user()->can('disiplin.oylama.baslat'))
                        @if(!$case->oylama_aktif)
                            ...
                            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 sticky top-6 text-center space-y-4">
                                ...
                                @if(Auth::user()->hasRole(['Superadmin', 'Hukuk Admini', 'Disiplin Kurulu Başkanı']) || Auth::user()->can('disiplin.oylama.baslat'))
                                    <form id="start-voting-form-{{ $case->id }}" action="{{ route('admin.disiplin.voting.start', $case->id) }}" method="POST">
                                        @csrf
                        ...
                        @else
                            ...
                            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sticky top-6">
                                ...
                                <form action="{{ route('admin.disiplin.vote.save', $case->id) }}" method="POST">
                                    @csrf
                                ...
                                @php $myVote = $case->oylar->where('user_id', Auth::id())->first(); @endphp
                                @if($myVote)
                                    <div class="mt-4 pt-4 border-t border-slate-100 text-center">
                                        <form action="{{ route('admin.disiplin.vote.delete', $case->id) }}" method="POST">
                                            @csrf @method('DELETE')
                                    ...
                                    <div class="flex items-start gap-4">
                                        ...
                                        <div class="flex-1">
                                            <div class="flex items-center justify-between mb-1">
                                                ...
                                                <span class="text-[10px] font-bold px-2.5 py-1 rounded-full ring-1 ring-inset {{ $badgeClass }}">
                                                    {{ $oy->oy_yonu }}
                                                </span>
                                            </div>
                        ...
                        <div class="mt-8 pt-6 border-t border-slate-100">
                            ...
                            <form method="POST" enctype="multipart/form-data" class="bg-slate-50 p-4 rounded-xl border border-slate-200">
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <button type="submit" formaction="{{ route('admin.disiplin.penalty.approve', $case->id) }}" onclick="return confirm('CEZA ONAYLANACAK. Emin misiniz?')" class="bg-rose-600 hover:bg-rose-700 text-white font-bold py-2.5 rounded-lg shadow-sm text-xs flex justify-center items-center gap-2 transition">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                                        Cezayı Onayla
                                    </button>
                                    <button type="submit" formaction="{{ route('admin.disiplin.defense.accept', $case->id) }}" onclick="return confirm('DOSYA KAPATILACAK. Emin misiniz?')" class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 rounded-lg shadow-sm text-xs flex justify-center items-center gap-2 transition">
    ...
    @push('scripts')
    <script>
        function startVotingWithConfirm(caseId) {
            Swal.fire({
