<x-app-layout>
                            ...
                            @if($sikayet->sikayetAltKategori)
                                ...
                                @if(Str::lower(trim($sikayet->sikayetAltKategori->ad)) === 'diğer / belirtilmemiş' || Str::lower(trim($sikayet->sikayetAltKategori->ad)) === 'diğer')
                                    @if($sikayet->sikayet_alt_kategori_diger)
                                        <span class="text-[10px] text-gray-400 italic mt-0.5 text-center px-2 cursor-help" title="{{ $sikayet->sikayet_alt_kategori_diger }}">
                                            ({{ Str::limit($sikayet->sikayet_alt_kategori_diger, 25) }})
                                        </span>
                                    @endif
                                @endif
                            @elseif($sikayet->sikayet_alt_kategori_diger)
                                <span class="text-xs text-gray-500" title="{{ $sikayet->sikayet_alt_kategori_diger }}">{{ Str::limit($sikayet->sikayet_alt_kategori_diger, 25) }}</span>
                            @endif
                        </div>
                    </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    @if($sikayet->iaaProjesi && $sikayet->iaaProjesi->ziyaretPlani)
                        ...
                        @endphp
                        <div class="bg-gradient-to-br from-{{ $ziyaretRenk }}-50 to-white overflow-hidden shadow-sm sm:rounded-xl border mb-6 border-{{ $ziyaretRenk }}-100 relative">
                            <div class="absolute top-0 left-0 w-1 h-full bg-{{ $ziyaretRenk }}-500"></div>
                            <div class="p-6">
                                <h3 class="text-lg font-bold text-gray-900 mb-4 flex justify-between items-center border-b border-{{ $ziyaretRenk }}-100 pb-2">
                                    <div class="flex items-center">
                                        <svg class="w-5 h-5 mr-2 text-{{ $ziyaretRenk }}-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path>
                                        </svg>
                                        Müşteri Ziyareti Bilgileri
                                    </div>
                                    <span class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-{{ $ziyaretRenk }}-100 text-{{ $ziyaretRenk }}-800">
                                        {{ $zStatus }}
                                    </span>
                                </h3>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-[13px] text-gray-700 font-medium w-full">
                                    <!-- Planlanan Tarih -->
                                    <div class="bg-white/50 p-3 rounded-xl border border-{{ $ziyaretRenk }}-100 flex flex-col gap-1">
                                        ...
                                        <span class="font-black text-gray-900 text-sm flex items-center gap-1.5">
                                            ...
                                            {{ \Carbon\Carbon::parse($ziyaret->visit_date)->format('d.m.Y H:i') }}
                                        </span>
                                    </div>
                                    ...
                                    <div class="bg-white/50 p-3 rounded-xl border border-{{ $ziyaretRenk }}-100 flex flex-col gap-1">
                                        ...
                                        <span class="font-black text-gray-900 text-sm">{{ $ziyaret->planner->name ?? 'Bilinmiyor' }}</span>
                                    </div>
                                    ...
                                    <div class="bg-white/50 p-3 rounded-xl border border-{{ $ziyaretRenk }}-100 flex flex-col gap-1">
                                        ...
                                        <span class="font-black text-gray-900 text-sm">
                                            {{ $ziyaret->estimated_return_date ? \Carbon\Carbon::parse($ziyaret->estimated_return_date)->format('d.m.Y') : 'Belirtilmedi' }}
                                        </span>
                                    </div>
                                    ...
                                    <div class="bg-white/50 p-3 rounded-xl border border-{{ $ziyaretRenk }}-100 flex flex-col gap-1">
                                        ...
                                        <span class="font-black text-gray-900 text-sm">
                                            @if($ziyaret->estimated_return_date && $ziyaret->visit_date)
                                                @php
                                    ...
                                    <div class="bg-white/50 p-3 rounded-xl border border-{{ $ziyaretRenk }}-100 flex flex-col gap-1">
                                        ...
                                        <div class="text-sm">
                                            @if($zStatus == 'Tamamlandı')
                                                <p class="text-blue-700 font-medium">
                                                    <span class="font-bold">Ziyarete giden:</span> 
                                                    @if(is_array($ziyaret->visitors) && count($ziyaret->visitors) > 0)
                                                        {{ implode(', ', \App\Models\User::whereIn('id', $ziyaret->visitors)->pluck('name')->toArray()) }}
                                                    @else
                                                        {{ $ziyaret->visitor->name ?? 'Bilinmiyor' }}
                                                    @endif
                                                </p>
                                            @elseif($zStatus == 'Beklemede')
                                                <p class="text-yellow-700 font-medium">
                                                    <span class="font-bold">Onayını bekliyor:</span> {{ $ziyaret->approver_id ? (\App\Models\User::find($ziyaret->approver_id)->name ?? 'Yönetici') : 'Yönetici' }}
                                                    <span class="text-xs text-yellow-600 block">({{ now()->startOfDay()->diffInDays(\Carbon\Carbon::parse($ziyaret->created_at)->startOfDay()) }} gündür bekliyor)</span>
                                                </p>
                                            @elseif($zStatus == 'Revizyon Bekliyor' || $zStatus == 'Revize')
                                                <p class="text-orange-700 font-medium">
                                                    ...
                                                    @else
                                                        {{ $ziyaret->visitor->name ?? 'Bilinmiyor' }}
                                                    @endif
                                                    <span class="text-xs text-orange-600 block">({{ now()->startOfDay()->diffInDays(\Carbon\Carbon::parse($ziyaret->updated_at)->startOfDay()) }} gündür bekliyor)</span>
                                                </p>
                                            @elseif($zStatus == 'Onaylandı')
                                                @php
                                                    $daysLeft = now()->startOfDay()->diffInDays(\Carbon\Carbon::parse($ziyaret->visit_date)->startOfDay(), false);
                                                @endphp
                                                <p class="text-green-700 font-medium">
                                                    @if($daysLeft > 0)
                                                        Ziyarete gidilecek tarihe <span class="font-bold">{{ $daysLeft }} gün</span> kaldı.
                                                    @elseif($daysLeft == 0)
                                                        Ziyaret <span class="font-bold">bugün</span> gerçekleştirilecek.
                                                    @else
                                                        Ziyaret tarihi <span class="font-bold">{{ abs($daysLeft) }} gün</span> geçti.
                                                    @endif
                                                </p>
                                            @elseif($zStatus == 'Reddedildi')
                                                <p class="text-red-700 font-medium">
                                                    <span class="font-bold">Reddeden:</span> {{ $ziyaret->approver_id ? (\App\Models\User::find($ziyaret->approver_id)->name ?? 'Yönetici') : 'Yönetici' }}
                                                </p>
                                            @endif
                                        </div>
                                    </div>
                                    ...
                                    @if($zStatus == 'Tamamlandı')
                                        ...
                                        <div class="bg-white/50 p-3 rounded-xl border border-{{ $ziyaretRenk }}-100 flex flex-col gap-1">
                                            ...
                                            <span class="font-black text-gray-900 text-sm">{{ $ziyaret->completer->name ?? 'Bilinmiyor' }}</span>
                                        </div>
                                        ...
                                        <div class="bg-white/50 p-3 rounded-xl border border-{{ $ziyaretRenk }}-100 flex flex-col gap-1">
                                            ...
                                            <span class="font-black text-gray-900 text-sm">
                                                {{ $ziyaret->completed_at ? \Carbon\Carbon::parse($ziyaret->completed_at)->format('d.m.Y H:i') : '-' }}
                                            </span>
                                        </div>
                                    @endif
                                </div>

                                @if($zStatus == 'Onaylandı' && now()->isAfter(\Carbon\Carbon::parse($ziyaret->visit_date)))
                                ...
                                @if($ziyaret->purpose || $ziyaret->notes)
                                    <div class="mt-4">
                                        <span class="text-[11px] font-bold text-gray-400 uppercase tracking-wider block mb-1">Ziyaret Amacı / Notlar</span>
                                        <p class="text-sm text-gray-700 bg-white/50 p-3 rounded-xl border border-{{ $ziyaretRenk }}-100 whitespace-pre-wrap">{{ $ziyaret->purpose ?? $ziyaret->notes }}</p>
                                    </div>
                                @endif
                            </div>
                        </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100 p-6">
                        ...
                        <div class="mb-4">
                            @if($sikayet->customer_id)
                                <button type="button" 
                                    onclick="Livewire.dispatch('openDigerSikayetlerModal', { customerId: {{ $sikayet->customer_id }}, currentSikayetId: {{ $sikayet->id }} })"
                                    ...
                                    <div class="text-center group-hover:scale-105 transition-transform">
                                        ...
                                        <span class="block text-lg font-bold text-gray-800 group-hover:text-indigo-700">{{ $firmaSikayetSayisi ?? '-' }}</span>
                                    </div>
    ...
    @livewire('admin.sikayetler.musteri-diger-sikayetler-modal')
</x-app-layout>