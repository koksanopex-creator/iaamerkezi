...
<x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">
        {{ __('Dinamik Karşılaştırma Analizi') }}
    </h2>
</x-slot>
...
<div class="space-y-6" wire:key="karsilastirma-raporu">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-5 rounded-2xl shadow-sm border border-gray-100 print-hidden">
        ...
        <div class="flex gap-2">
            <a href="{{ route('admin.musteri-sikayet-analiz-raporu') }}" class="px-4 py-2 bg-gray-100 text-gray-700 font-bold text-xs rounded-xl hover:bg-gray-200 transition shadow-sm border border-gray-200 flex items-center gap-2">
            ...
            <div class="lg:col-span-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
                    ...
                    <div x-data="{ open: false }" class="relative w-full">
                        ...
                        <div x-show="open" @click.away="open = false" x-cloak class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto custom-scrollbar p-2">
                            @foreach($filterData['bolumler'] as $bolum)
                                <label wire:key="kar-bolum-{{ $bolum->id }}" class="flex items-center gap-2 p-2 hover:bg-gray-50 rounded cursor-pointer">
                    ...
                    <div x-data="{ open: false }" class="relative w-full">
                        ...
                        <div x-show="open" @click.away="open = false" x-cloak class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto custom-scrollbar p-2">
                            @foreach($filterData['durumlar'] as $key => $label)
                                <label wire:key="kar-durum-{{ $key }}" class="flex items-center gap-2 p-2 hover:bg-gray-50 rounded cursor-pointer">
                    ...
                    <div x-data="{ open: false }" class="relative w-full">
                        ...
                        <div x-show="open" @click.away="open = false" x-cloak class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto custom-scrollbar p-2">
                            @foreach($filterData['altKategoriler'] as $ak)
                                <label wire:key="kar-altkat-{{ $ak['id'] }}" class="flex items-center gap-2 p-2 hover:bg-gray-50 rounded cursor-pointer">
                ...
                <div>
                    ...
                    <div x-data="{ open: false }" class="relative w-full">
                        ...
                        <div x-show="open" @click.away="open = false" x-cloak class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto custom-scrollbar p-2">
                            @foreach($filterData['takimlar'] as $takim)
                                <label wire:key="kar-takim-{{ $takim->id }}" class="flex items-center gap-2 p-2 hover:bg-gray-50 rounded cursor-pointer">
                                    <input type="checkbox" wire:model.live="takimId" value="{{ $takim->id }}" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                    <span class="text-xs text-gray-700">{{ $takim->ad }}</span>
                                </label>
                            @endforeach
                        </div>
                    </div>
                </div>
                {{-- Görevli Personeller (Squad) --}}
                <div>
                    ...
                    <div x-data="{ open: false }" class="relative w-full">
                        ...
                        <div x-show="open" @click.away="open = false" x-cloak class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto custom-scrollbar p-2">
                            @foreach($filterData['squadUsers'] as $sq)
                                <label wire:key="kar-squad-{{ $sq->id }}" class="flex items-center gap-2 p-2 hover:bg-gray-50 rounded cursor-pointer">
    ...
    {{-- ============================= AKTİF BİLGİ KARTI VE ALGORİTMİK ÖZET ============================= --}}
    ...
    {{-- ============================= E-TİCARET TARZI KIYASLAMA TABLOSU ============================= --}}
            ...
            <div class="flex flex-nowrap min-w-full">
                @foreach($comparisonData as $item)
                    <div class="flex-1 min-w-[280px] max-w-[400px] border-r border-gray-100 last:border-r-0 p-6 flex flex-col hover:bg-slate-50/50 transition-colors group">
                        {{-- Başlık ve Logo/Avatar --}}
                        <div class="text-center mb-6 pb-6 border-b border-gray-100 flex-grow">
                            @if(isset($item['details']['logo']) && $item['details']['logo'])
                                <img src="{{ $item['details']['logo'] }}" class="h-16 mx-auto mb-4 object-contain group-hover:scale-110 transition-transform" alt="{{ $item['label'] }} Logo">
                            @else
                                <div class="w-16 h-16 rounded-2xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-black text-2xl mx-auto mb-4 group-hover:scale-110 transition-transform shadow-sm border border-indigo-100">
                                    {{ mb_substr($item['label'], 0, 1) }}
                                </div>
                            @endif
                            <h3 class="font-black text-lg text-slate-800">{{ $item['label'] }}</h3>
                            @if(isset($item['details']['subtitle']))
                                <p class="text-[10px] uppercase tracking-widest text-slate-400 font-bold mt-1">{{ $item['details']['subtitle'] }}</p>
                            @endif

                            {{-- Personel Detayları --}}
                            @if(isset($item['details']['director']) && $item['details']['director'] !== '-')
                                <div class="mt-4 text-xs bg-white rounded-xl p-3 border border-gray-100 shadow-sm text-left">
                                    <p class="text-gray-500 flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-purple-500"></span><span class="font-bold text-gray-700">Direktör:</span> {{ $item['details']['director'] }}</p>
                                    @if(isset($item['details']['liderler']) && $item['details']['liderler'] !== '-')
                                        <p class="text-gray-500 mt-2 flex items-center gap-2" title="{{ $item['details']['liderler'] }}"><span class="w-1.5 h-1.5 rounded-full bg-blue-500"></span><span class="font-bold text-gray-700">Lider:</span> {{ Str::limit($item['details']['liderler'], 25) }}</p>
                                    @endif
                                </div>
                            @elseif(isset($item['details']['roles']))
                                <div class="mt-4 text-xs bg-white rounded-xl p-3 border border-gray-100 shadow-sm text-left">
                                    <p class="text-gray-500 flex items-center gap-2"><span class="w-1.5 h-1.5 rounded-full bg-amber-500"></span><span class="font-bold text-gray-700">Rol:</span> {{ Str::limit($item['details']['roles'], 25) }}</p>
                                    <p class="text-gray-500 mt-2 truncate flex items-center gap-2" title="{{ $item['details']['email'] }}"><span class="w-1.5 h-1.5 rounded-full bg-gray-400"></span>{{ $item['details']['email'] }}</p>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between items-center bg-gray-50 p-3 rounded-xl border border-gray-100/50">
                                ...
                                <span class="text-base font-black text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-md">{{ $item['kpi']['toplam'] }}</span>
                            </div>
                            <div class="flex justify-between items-center p-3">
                                ...
                                <span class="text-sm font-black flex items-center gap-1">
                                    <span class="text-orange-600 bg-orange-50 px-2 py-0.5 rounded-md">{{ $item['kpi']['acik'] }}</span> 
                                    <span class="text-gray-300">/</span> 
                                    <span class="text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md">{{ $item['kpi']['cozulen'] }}</span>
                                </span>
                            </div>
                            <div class="flex justify-between items-center bg-gray-50 p-3 rounded-xl border border-gray-100/50">
                                <span class="text-xs font-bold text-gray-500 uppercase tracking-wider" title="Ortalama Çözüm Süresi">Ort. Çözüm</span>
                                <span class="text-sm font-black flex items-center gap-1 {{ $item['kpi']['ortCozumSuresi'] > 15 ? 'text-red-600 bg-red-50' : 'text-emerald-600 bg-emerald-50' }} px-2 py-0.5 rounded-md">
                                    {{ $item['kpi']['ortCozumSuresi'] }} <span class="text-[10px] font-bold opacity-70">GÜN</span>
                                </span>
                            </div>
                            <div class="flex justify-between items-center p-3">
                                <span class="text-xs font-bold text-gray-500 uppercase tracking-wider">İade Oranı</span>
                                <span class="text-sm font-black {{ $item['kpi']['iadeOrani'] > 5 ? 'text-red-600 bg-red-50' : 'text-emerald-600 bg-emerald-50' }} px-2 py-0.5 rounded-md">
                                    %{{ $item['kpi']['iadeOrani'] }}
                                </span>
                            </div>
                        </div>
    ...
    <div wire:ignore class="space-y-6">
        ...
        {{-- Çözüm Süresi Trend Kıyaslama (Line Chart) --}}
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
    ...
    <script>
        ...
        Livewire.on('livewire:initialized', () => {
            renderComparisonCharts(@json($comparisonData));
        });
