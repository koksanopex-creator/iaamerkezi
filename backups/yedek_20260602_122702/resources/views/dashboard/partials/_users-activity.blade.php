@php $isEmbedded = $embedded ?? false; $isSidebar = $sidebar ?? false; @endphp

<div class="{{ $isEmbedded ? '' : 'pt-10' }}" 
     x-data="{ 
        showTooltip: null, 
        activeUserLogs: [], 
        tooltipX: 0, 
    ...
    <div class="{{ $isEmbedded ? '' : 'bg-white rounded-3xl border border-indigo-100 shadow-[0_10px_40px_rgba(79,70,229,0.05)]' }} relative">
        
        {{-- BAŞLIK ALANI --}}
        <div class="flex {{ $isSidebar ? 'flex-col' : 'flex-col sm:flex-row sm:items-center' }} justify-between gap-4 {{ $isSidebar ? 'p-3' : 'px-6 py-4' }} border-b border-gray-100 {{ $isEmbedded ? 'bg-gray-50/50' : 'bg-gradient-to-r from-green-50/50 to-emerald-50/50' }}">
            ...
            <div class="flex items-center {{ $isSidebar ? 'justify-between w-full' : 'gap-2' }}">
                <a href="{{ route('logs.login.index') }}"
                   class="{{ $isSidebar ? 'text-[8px] px-1.5 py-1' : 'text-[10px] px-3 py-1.5' }} font-black text-emerald-600 hover:text-emerald-800 uppercase tracking-widest bg-white/80 backdrop-blur-sm rounded-xl border border-green-100 shadow-sm transition-all hover:shadow-md">
                    &larr; {{ $isSidebar ? 'Kayıtlar' : 'Tüm Giriş Kayıtları' }}
                </a>
                <span class="{{ $isSidebar ? 'text-[8px] px-1.5 py-1' : 'text-xs px-4 py-1.5' }} font-black text-emerald-700 bg-white rounded-full border border-green-200 shadow-sm">
                    {{ $onlineKullanicilar->count() }} Aktif
                </span>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
                ...
                <div class="max-h-[500px] overflow-y-auto custom-scrollbar">
                    <table class="w-full text-sm text-left">
                        <tbody class="divide-y divide-gray-50">
                            @forelse($onlineKullanicilar->take(10) as $user)
                                <tr class="bg-white hover:bg-indigo-50/30 transition-colors group relative">
                                    <td class="px-4 py-3 font-medium text-gray-900 flex items-center gap-3 cursor-default">
                                        <div class="flex items-center gap-3 tooltip-trigger"
                                            @mouseenter="updateTooltip({{ json_encode($user->loginActivities->take(10)->map(fn($l) => ['date' => $l->created_at->format('d.m H:i'), 'ip' => $l->ip_address])->toArray()) }}, 'online', {{ $user->id }}, $event)">
                                        ...
                                        <div class="flex flex-col min-w-0 relative">
                                            <a href="{{ route('profile.show', $user->id) }}" class="text-sm font-bold text-gray-800 hover:text-indigo-600 transition-colors truncate">
                                                {{ $user->name }}
                                            </a>
                                            <span class="text-[10px] text-gray-400 font-bold uppercase truncate">{{ $user->unvan ?? ($user->getRoleNames()->first() ?? 'Personel') }}</span>
            ...
            <div class="flex flex-col">
                ...
                <div class="max-h-[500px] overflow-y-auto custom-scrollbar">
                    <table class="w-full text-sm text-left">
                        <tbody class="divide-y divide-gray-50">
                            @foreach($sonAktifKullanicilar->take(10) as $user)
                                <tr class="bg-white hover:bg-indigo-50/30 transition-colors group relative">
                                    <td class="px-4 py-3 font-medium text-gray-900 flex items-center gap-3 cursor-default">
                                        <div class="flex items-center gap-3 tooltip-trigger"
                                            @mouseenter="updateTooltip({{ json_encode($user->loginActivities->take(10)->map(fn($l) => ['date' => $l->created_at->format('d.m H:i'), 'ip' => $l->ip_address])->toArray()) }}, 'last', {{ $user->id }}, $event)">
                                        ...
                                        <div class="flex flex-col min-w-0 relative">
                                            <a href="{{ route('profile.show', $user->id) }}" class="text-sm font-bold text-gray-700 group-hover:text-indigo-600 transition-colors truncate">
                                                {{ $user->name }}
                                            </a>
                                            <span class="text-[10px] text-gray-400 font-bold uppercase truncate">{{ $user->bolum->ad ?? '-' }}</span>
