                                        ...
                                        <template x-for="(res, index) in results" :key="index">
                                            <tr class="hover:bg-slate-50/30 transition-colors group">
                                                    ...
                                                    <div class="flex items-center gap-1.5 flex-wrap">
                                                        <template x-for="(status, role) in res.role_results" :key="role">
                                                            <div class="group/role relative">
                                                                <div class="w-7 h-7 flex items-center justify-center rounded-lg text-[10px] font-black border transition-all cursor-help"
                                                                    :class="{
                                                                        'bg-emerald-50 text-emerald-600 border-emerald-100 ring-emerald-500 hover:ring-2': status.status === 'success',
                                                                        'bg-rose-50 text-rose-600 border-rose-100 ring-rose-500 hover:ring-2': status.status === 'danger',
                                                                        'bg-amber-50 text-amber-600 border-amber-100 ring-amber-500 hover:ring-2': status.status === 'warning',
                                                                        'bg-slate-50 text-slate-400 border-slate-100 ring-slate-400 hover:ring-2': status.status === 'info',
                                                                        'bg-indigo-50 text-indigo-400 border-indigo-100': status.status === 'loading'
                                                                    }"
                                                                    x-text="status.short">
                                                                </div>
                                                                <!-- Tooltip -->
                                                                <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover/role:block z-[60] min-w-[120px]">
                                                                    <div class="bg-slate-800 text-white text-[10px] p-2 rounded-lg shadow-xl">
                                                                        <div class="font-black border-b border-white/20 pb-1 mb-1" x-text="role"></div>
                                                                        <div x-text="status.message + ' (' + status.code + ')'"></div>
                                                                    </div>
                                                ...
                                                <td class="px-6 py-4 text-right">
                                                    <a :href="res.url.startsWith('http') ? res.url : (res.url.startsWith(new URL(baseUrl).pathname) ? baseUrl.replace(new URL(baseUrl).pathname, '') + res.url : baseUrl + (res.url.startsWith('/') ? '' : '/') + res.url)" 
                                                       target="_blank" class="p-2 bg-slate-50 text-slate-400 rounded-lg hover:text-indigo-600 hover:bg-indigo-50 transition-all inline-block">
    ...
    <script>
        function healthChecker() {
            return {
                ...
                async init() {
                    const res = await fetch('{{ route('admin.health.init') }}');
                ...
                async startScan() {
                    ...
                    for (let i = 0; i < total; i++) {
                        ...
                        try {
                            const response = await fetch('{{ route('admin.health.scan') }}', {
                                method: 'POST',
                                headers: {
                            ...
                            this.summary.total++;
                            this.summary.fail += data.fail_count;
                            this.summary.success += (Object.keys(data.result.role_results).length - data.fail_count);
                ...
                async fetchModelIssues() {
                    const res = await fetch('{{ route('admin.health.scan') }}', {
                        method: 'POST',
