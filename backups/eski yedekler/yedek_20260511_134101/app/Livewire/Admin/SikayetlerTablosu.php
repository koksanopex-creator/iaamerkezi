...
class SikayetlerTablosu extends Component
{
    ...
    {
        $query->when(!empty($this->filtreDurum), fn($q) => $q->whereIn('musteri_durum', $this->filtreDurum));
        $query->when(!empty($this->filtreOncelik), fn($q) => $q->whereIn('musteri_oncelik', $this->filtreOncelik));
        $query->when(!empty($this->filtreTakim), fn($q) => $q->whereIn('atanan_cozum_takimi_id', $this->filtreTakim));
        ...
        $query->when(!empty($this->filtreEkleyen), fn($q) => $q->whereIn('olusturan_kurul_uyesi_id', $this->filtreEkleyen));
        ...
        $query->when(!empty($this->filtreKategori), fn($q) => $q->whereIn('sikayet_kategorisi_id', $this->filtreKategori));
        $query->when(!empty($this->filtreKonumTipi), fn($q) => $q->whereIn('konum_tipi', $this->filtreKonumTipi));
        ...
        $query->when(!empty($this->filtreProjeDurumu), function ($q) {
            $q->whereHas('iaaProjesi', function ($subQ) {
                $subQ->whereIn('durum', $this->filtreProjeDurumu);
            });
        });
    ...
    public function getFilterSummaryProperty()
    {
        ...
        if (!empty($this->filtreDurum)) {
            $summary[] = "Durumu " . implode(', ', $this->filtreDurum);
        }
        
        if (!empty($this->filtreOncelik)) {
            $summary[] = "Önceliği " . implode(', ', $this->filtreOncelik);
        }
        
        if (!empty($this->filtreKategori)) {
            $kategoriAdlari = SikayetKategori::whereIn('id', $this->filtreKategori)->pluck('ad')->toArray();
            $summary[] = "Kategorisi " . implode(', ', $kategoriAdlari);
        }
        
        if (!empty($this->filtreTakim)) {
            $takimAdlari = Takim::whereIn('id', $this->filtreTakim)->pluck('ad')->toArray();
        ...
        if (!empty($this->filtreProjeDurumu)) {
            ...
            $readableStatuses = array_map(fn($s) => $statusMap[$s] ?? $s, $this->filtreProjeDurumu);
