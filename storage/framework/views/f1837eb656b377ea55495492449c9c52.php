<div class="w-full">
    <div class="bg-gradient-to-br from-white to-gray-50 p-6 rounded-xl shadow-lg border border-gray-200">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <?php if($iaa->visit_planned): ?>
                    ...
                    <a href="#ziyaret-bilgileri-alani" onclick="document.getElementById('ziyaret-bilgileri-alani').scrollIntoView({behavior: 'smooth'})" class="relative inline-flex items-center px-3 py-1.5 bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 rounded-lg text-xs font-bold transition-all shadow-sm transform hover:scale-105">
                        <span class="absolute -top-1 -right-1 flex h-3 w-3 <?php echo e($visitDotClass == 'hidden' ? 'hidden' : ''); ?>">
                          <span class="animate-ping absolute inline-flex h-full w-full rounded-full <?php echo e($visitDotClass); ?> opacity-75"></span>
                          <span class="relative inline-flex rounded-full h-3 w-3 <?php echo e($visitDotClass); ?>"></span>
                        </span>
                ...
                <?php if($iaa->musteriSikayeti): ?>
                    <a href="<?php echo e(auth()->user()->is_personnel ? route('admin.sikayetler.show', $iaa->musteriSikayeti->id) : route('iaa.sikayetler.show', $iaa->musteriSikayeti->id)); ?>" class="inline-flex items-center px-3 py-1.5 bg-orange-50 text-orange-700 hover:bg-orange-100 border border-orange-200 rounded-lg text-xs font-bold transition-all shadow-sm transform hover:scale-105">
        ...
        <dl class="grid grid-cols-1 md:grid-cols-<?php echo e($iaa->musteriSikayeti ? 4 : 5); ?> gap-4">
            <div class="bg-white rounded-lg p-3 shadow-sm border border-gray-100">
                ...
                <dd class="text-sm font-bold text-gray-900">
                    <a href="<?php echo e(route('takimlar.show', $takim->id)); ?>" class="hover:text-indigo-600 hover:underline transition-colors flex items-center gap-1">
                        <?php echo e($takim->ad); ?>
                    ...
                    <dd class="text-sm font-bold text-gray-900">
                        <?php if($displayLider): ?>
                            <div class="flex flex-col gap-1">
                                <a href="<?php echo e(route('profile.show', $displayLider->id)); ?>" class="hover:text-indigo-600 hover:underline transition-colors flex items-center gap-1">
                                    <?php echo e($displayLider->name); ?>

                                    <?php if(method_exists($displayLider, 'trashed') && $displayLider->trashed()): ?>
                                        <span class="text-[10px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded border border-red-200">İşten Ayrıldı<?php echo e($displayLider->termination_date ? ' (' . \Carbon\Carbon::parse($displayLider->termination_date)->format('d.m.Y') . ')' : ''); ?></span>
                                    <?php endif; ?>
                                    <svg class="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
                                </a>

                                <?php if($leaderChanged): ?>
                                    <div class="inline-flex items-center gap-1.5 px-2 py-1 bg-orange-50 text-orange-700 border border-orange-100 rounded text-[10px] font-bold leading-none w-fit">
                                        ...
                                        <span>Önceki Lider: <?php echo e($originalLider->name); ?> (<?php echo e($originalLider->trashed() ? 'İşten Ayrıldı' . ($originalLider->termination_date ? ' - ' . \Carbon\Carbon::parse($originalLider->termination_date)->format('d.m.Y') : '') : 'Görev Değişikliği'); ?>)</span>
                                    </div>
                                <?php endif; ?>
                            </div>
                        <?php else: ?>
            ...
            <?php if(!$iaa->musteriSikayeti): ?>
                <div class="bg-white rounded-lg p-3 shadow-sm border border-gray-100 italic">
                    ...
                    <dd class="text-sm font-bold text-gray-900">
                        <a href="<?php echo e(route('iaa.show', $iaa->id)); ?>" target="_blank" class="text-amber-700 hover:text-amber-900 hover:underline transition-colors flex items-center gap-1 font-bold">
                            İncele
            ...
            <div class="bg-gradient-to-r from-red-50 to-orange-50 rounded-lg p-3 shadow-sm border-2 border-red-200 flex flex-col justify-between"
                x-data="extensionTimer('<?php echo e($assignment->due_date ? \Carbon\Carbon::parse($assignment->due_date)->toISOString() : ''); ?>')"
                x-init="start()">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <dd class="text-sm font-bold text-red-700"><?php echo e(\Carbon\Carbon::parse($assignment->due_date)->format('d.m.Y H:i')); ?></dd>
                    </div>
                </div>
                ...
                <?php if(in_array($iaa->durum, ['Tamamlandı', 'talep_olarak_kapatildi', 'hatali_bildirim_olarak_kapatildi'])): ?>
                    <div class="mt-2 text-sm font-bold text-green-600 flex items-center gap-1.5">
                        ...
                        <span><?php echo e($iaa->completion_duration_in_days ?? 'İşlem'); ?></span> <?php echo e($iaa->durum == 'Tamamlandı' ? 'tamamlandı' : 'kapatıldı'); ?>
                    ...
                    <?php if(isset($statusDate)): ?>
                        <div class="mt-1 text-[11px] font-medium text-gray-500">
                            Gerçekleşen: <?php echo e(\Carbon\Carbon::parse($statusDate)->format('d.m.Y H:i')); ?>

                            <?php if($assignment->due_date && \Carbon\Carbon::parse($statusDate)->startOfDay()->gt(\Carbon\Carbon::parse($assignment->due_date)->startOfDay())): ?>
                                <?php
                                    $gecikmeGun = \Carbon\Carbon::parse($assignment->due_date)->startOfDay()->diffInDays(\Carbon\Carbon::parse($statusDate)->startOfDay());
                                ?>
                                <?php if($gecikmeGun > 0): ?>
                                    <span class="text-red-500 font-bold ml-1">(<?php echo e($gecikmeGun); ?> gün gecikti)</span>
                                <?php endif; ?>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                ...
                <?php if($isComplaint && $isLeaderOrAdminHeader && $ekSureGosterilebilir && in_array($extensionDurum, [null, 'Reddedildi']) && !$iaa->musteriSikayeti->trashed()): ?>
                    <div class="mt-3">
                        <button type="button" @click="$dispatch('open-extension-modal')" class="w-full inline-flex justify-center items-center px-3 py-1.5 text-xs font-bold rounded-lg text-orange-700 bg-orange-100 hover:bg-orange-200 border border-orange-300 transition-colors shadow-sm">
                            ...
                            <?php if($extensionDurum == 'Reddedildi'): ?> Yeni Ek Süre Talep Et <?php else: ?> Ek Süre Talep Et <?php endif; ?>
                        </button>
                    </div>
                <?php endif; ?>

                <?php if($isComplaint && $canSeeEvaluating): ?>
                    <?php if($extensionDurum == 'Reddedildi'): ?>
                    ...
                    <?php elseif($extensionDurum == 'Onaylandı'): ?>
                        <div class="mt-3">
                    ...
                    <?php elseif($extensionDurum == 'Talep Edildi'): ?>
                        <?php if($isApprover): ?>
                            <div class="mt-3">
                ...
                <?php if(isset($statusDate)): ?>
                    <div class="flex items-center">
                        <div class="flex items-center gap-2 text-xs font-bold text-slate-600 bg-white shadow-sm px-4 py-2.5 rounded-xl border border-slate-200">
                            ...
                            <span class="text-[13px] tracking-tight"><?php echo e(\Carbon\Carbon::parse($statusDate)->format('d.m.Y H:i')); ?></span>
                        </div>
                    </div>
                <?php endif; ?>
...
</div><?php /**PATH /var/www/kys_koksan/iaa/resources/views/proje-calisma-alani/partials/_project-header.blade.php ENDPATH**/ ?>