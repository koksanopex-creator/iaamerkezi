...
<x-app-layout>
    ...
    <div class="matrix-page-root" x-data="{
        ...
        toggleRelation(userId, type, value, state) {
            ...
            let url = '{{ route('admin.disiplin.hukuk-matrisi.update', ['user' => ':userId']) }}';
        ...
        <div class="info-bar">
            ...
            <div class="stats">
                ...
                <div class="vdiv"></div>
                <div class="stat">
                    <div class="stat-n i">{{ $personel->filter(fn($p) => $p->hasRole('Hukuk Yöneticisi'))->count() }}</div>
                            ...
                            @foreach($personel as $p)
                                <th class="th-person col-p">
                                    <div class="avatar shadow-sm">
                                        ...
                                        @else
                                            {{ strtoupper(substr($p->name, 0, 1)) }}{{ strtoupper(substr(strrchr($p->name, ' '), 1, 1)) }}
                                        @endif
                                        @if($p->hasRole('Hukuk Yöneticisi'))
                                            <div class="avatar-badge">HY</div>
                                        @endif
                                    </div>
                                    <div class="p-name" title="{{ $p->name }}">{{ $p->name }}</div>
                                    <div class="p-dept">{{ $p->bolum->ad ?? 'Bölüm Yok' }}</div>
                                    <button 
                                        @click="toggleRelation('{{ $p->id }}', 'role', 'Hukuk Yöneticisi', !{{ $p->hasRole('Hukuk Yöneticisi') ? 'true' : 'false' }})"
                                        class="mgr-btn {{ $p->hasRole('Hukuk Yöneticisi') ? 'on' : 'off' }}">
                                        {{ $p->hasRole('Hukuk Yöneticisi') ? 'YÖNETİCİ' : 'YAP' }}
                                    </button>
                                </th>
                            @endforeach
                        </tr>
                    ...
                    <tbody>
                        ...
                        @foreach($managedPermissions as $groupName => $perms)
                            ...
                            @foreach($perms as $slug => $label)
                                <tr>
                                    <td class="td-icon col-icon bg-{{ $color }}">
                                        <div class="group-stripe">
                                            <div class="group-dot dot-{{ $color }}"></div>
                                        </div>
                                    </td>
                                    <td class="td-label col-label">
                                        @if($loop->first)
                                            <div class="label-group gc-{{ $color }}">{{ $groupName }}</div>
                                        @endif
                                    ...
                                    @foreach($personel as $p)
                                        <td class="td-switch col-p">
                                            ...
                                            <label class="toggle">
                                                <input type="checkbox" 
                                                    @if($hasThisPerm) checked @endif
                                                    @if(!$p->hasRole('Hukuk Yöneticisi')) disabled @endif
