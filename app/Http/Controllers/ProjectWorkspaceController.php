...
class ProjectWorkspaceController extends Controller
{
    ...
    {
        $result = $this->talepYonetimService->decideRequestByDirector($request, $id);
        return back()->with($result['status'], $result['message']);
    }
    ...
    public function recallRequest($id)
    {
        $result = $this->talepYonetimService->recallRequest($id);
        return back()->with($result['status'], $result['message']);
    }

    public function recallRequestByQuality($id)
    {
        $result = $this->talepYonetimService->recallRequestByQuality($id);
        return back()->with($result['status'], $result['message']);
    }

    public function recallRequestByDirector($id)
    {
        try {
            $result = $this->talepYonetimService->recallRequestByDirector($id);
            return back()->with($result['status'], $result['message']);
        } catch (\Exception $e) {
            return back()->with('error', 'İşlem sırasında bir hata oluştu: ' . $e->getMessage());
        }
    ...
    public function recallRequestBySuperadmin($id)
    {
        try {
            $result = $this->talepYonetimService->recallRequestBySuperadmin($id);
            return back()->with($result['status'], $result['message']);
        } catch (\Exception $e) {
            return back()->with('error', 'İşlem sırasında bir hata oluştu: ' . $e->getMessage());
        }
