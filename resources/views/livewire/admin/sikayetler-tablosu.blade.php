            ...
            <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200/50 overflow-hidden">
                ...
                <div class="border-b border-gray-200/70 bg-gradient-to-br from-gray-50/80 to-white" x-data="{ open: false }">
                    ...
                    <div x-show="open"
                         ...
                         style="display: none;" {{-- Başlangıçta gizli olması için --}}
                         >
                        ...
                        {{-- Puan Filtresi --}}
                        <div> {{-- mb-5 kaldırıldı, son grup olduğu için --}}
                            <div class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($sikayetler as $sikayet)
                                <tr class="hover:bg-blue-50 transition-colors duration-200">
                                    <td class="px-6 py-4 whitespace-nowrap"><span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-blue-100 text-blue-600 font-semibold text-sm">{{ ($sikayetler->currentPage() - 1) * $sikayetler->perPage() + $loop->iteration }}</span></td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-400 to-indigo-600 flex items-center justify-center text-white font-semibold flex-shrink-0">{{ strtoupper(substr($sikayet->musteri_adi, 0, 1)) }}</div>
                                            <span class="ml-3 text-sm font-medium text-gray-900">{{ $sikayet->musteri_adi }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $sikayet->cozumTakimi->ad ?? '—' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $sikayet->created_at->format('d.m.Y') }} <span class="block text-xs text-gray-400">{{ $sikayet->created_at->format('H:i') }}</span></td>
                    ...
                    <div x-data="{ openLogs: false }" class="bg-white rounded-xl border border-gray-200/80 shadow-sm hover:shadow-lg transition-all duration-300 p-4 md:p-6 group animate-slide-up">
                            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 gap-3">
                                <div class="flex items-center gap-4">
                                    <span class="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 text-white font-bold text-sm shadow-md group-hover:shadow-lg transition-shadow duration-200">
                                        #{{ ($sikayetler->currentPage() - 1) * $sikayetler->perPage() + $loop->iteration }} {{-- Sayfalama ile uyumlu sıra no --}}
                                    </span>
                            ...
                            <div class="sm:ml-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-3 text-sm bg-gray-50/70 rounded-lg p-3 border border-gray-200/60">
                                <div class="flex flex-col gap-1">
                                    ...
                                    {{-- Alt Kategori (Her Zaman Görünür) --}}
                                    <div class="flex items-center gap-1.5 text-gray-500 text-xs ml-5">
                                        ...
                                        @elseif($sikayet->sikayet_alt_kategori_diger)
                                            <span class="bg-yellow-50 px-2 py-0.5 rounded text-yellow-800 border border-yellow-200" title="{{ $sikayet->sikayet_alt_kategori_diger }}">
                                                Diğer: {{ Str::limit($sikayet->sikayet_alt_kategori_diger, 15) }}
                                            </span>
                                        @else
                                    ...
                                    <div class="flex items-center gap-1.5 text-gray-600">
                                        ...
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold {{ $sikayet->oncelik_badge_class }}">
                                            {{ $sikayet->musteri_oncelik ?? 'Normal' }}
                                        </span>
                                    </div>
                                    <div class="flex items-center gap-1.5 text-gray-600">
                                    ...
                                    {{-- MÜŞTERİ GERİ BİLDİRİMİ (VARSA GÖSTER) --}}
                                        ...
                                        <div class="mt-2 flex items-start gap-2 p-2 rounded-lg border {{ $fbRenk }}">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                @if($sikayet->musteri_feedback_note)
                                                    <p class="text-xs mt-0.5 italic opacity-90 truncate" title="{{ $sikayet->musteri_feedback_note }}">
                                                        "{{ $sikayet->musteri_feedback_note }}"
                                                    </p>
                                                @endif
                                                
                                                {{-- GÜVENLİK UYARISI (Varsa) --}}
                                                @if($sikayet->feedback_by_user_id)
                                                    <div class="mt-1 pt-1 border-t border-red-200 text-[9px] text-red-700 font-bold flex items-center gap-1">
                                                        ...
                                                        Dikkat: Personel ({{ \App\Models\User::find($sikayet->feedback_by_user_id)->name ?? '?' }})
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
