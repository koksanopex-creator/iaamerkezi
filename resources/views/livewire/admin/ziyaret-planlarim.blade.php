...
<div class="py-6 px-4 sm:px-6 lg:px-8 bg-gray-50 min-h-screen">
    <div class="max-w-7xl mx-auto">
        ...
        {{-- Tabs --}}
        @if(Auth::user()->hasRole(['Superadmin', 'Yönetim', 'Direktör', 'Bölüm Kalite Yöneticisi']))
        <div class="mb-6 border-b border-gray-200">
            <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                <button wire:click="setTab('my_visits')" class="{{ $activeTab === 'my_visits' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors flex items-center gap-2">
                    <svg class="h-5 w-5 {{ $activeTab === 'my_visits' ? 'text-indigo-500' : 'text-gray-400' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                    Ziyaretlerim
                </button>
                <button wire:click="setTab('pending_approval')" class="{{ $activeTab === 'pending_approval' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors flex items-center gap-2">
                    <svg class="h-5 w-5 {{ $activeTab === 'pending_approval' ? 'text-indigo-500' : 'text-gray-400' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    Onay Bekleyenler
                </button>
            </nav>
        </div>
                            ...
                            <tr class="hover:bg-indigo-50/30 transition duration-150 group">
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="flex-shrink-0 h-10 w-10 bg-white rounded-lg border border-gray-100 p-1 flex items-center justify-center overflow-hidden shadow-sm">
                                            @if($complaint && !empty($complaint->customer->logo_path))
                                                <img src="{{ asset('storage/' . $complaint->customer->logo_path) }}" alt="Logo" class="max-h-full max-w-full object-contain">
                                            @else
                                                <div class="h-full w-full bg-indigo-50 rounded flex items-center justify-center text-indigo-700 font-bold text-xs">
                                                    {{ strtoupper(substr($complaint ? ($complaint->customer->name ?? $complaint->musteri_adi) : 'M', 0, 1)) }}
                                                </div>
                                            @endif
                                        </div>
                                        <div class="flex flex-col min-w-0">
                                            <span class="text-sm font-bold text-gray-800 truncate" title="{{ $complaint ? ($complaint->customer->name ?? $complaint->musteri_adi) : 'Müşteri Bilinmiyor' }}">
                                                {{ $complaint ? ($complaint->customer->name ?? $complaint->musteri_adi) : 'Müşteri Bilinmiyor' }}
                                            </span>
                                            @if(!empty($visit->customer_product_id))
                                                <span class="text-[11px] text-indigo-600 font-bold bg-indigo-50 self-start px-2 py-0.5 rounded-full mt-1 truncate max-w-full" title="{{ $visit->customer_product_id }}">{{ $visit->customer_product_id }}</span>
                                            @else
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-8 w-8 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-700 font-bold text-xs">
                                            {{ strtoupper(substr($visit->visitor_name ?? 'P', 0, 1)) }}
                                        </div>
                                        <div class="ml-3">
                                            <div class="text-xs font-bold text-gray-900">{{ $visit->visitor_name ?? 'Dinamik Personel' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-4">
                                ...
                                <td class="px-4 py-4 text-center text-sm font-medium">
                                    <div class="flex justify-center items-center gap-2">
                                        ...
                                        @endif
                                        @if($visit->iaa_id)
                                            <a href="{{ route('proje.workspace.show', $visit->iaa_id) }}" target="_blank" class="p-2 text-indigo-600 hover:bg-indigo-600 hover:text-white rounded-xl transition shadow-sm border border-indigo-100">
