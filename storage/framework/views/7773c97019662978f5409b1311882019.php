<div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
    <div>
        <h2 class="font-bold text-2xl text-gray-800 leading-tight flex items-center gap-3">
            <span class="text-gray-500">#<?php echo e($case->id); ?></span>
            <?php echo e($case->dosya_no ?? 'Dosya No Bekleniyor'); ?>
            ...
            ?>
            <span class="px-3 py-1 text-sm rounded-full <?php echo e($statusClass); ?> font-semibold shadow-sm">
                <?php echo e(strtoupper(str_replace('_', ' ', $case->status))); ?>

            </span>
        </h2>
        <p class="text-sm text-gray-500 mt-1">
            İlgili Çalışan: <span class="font-bold text-gray-700"><?php echo e($case->calisan->name); ?></span> 
            (<?php echo e($case->calisan->email); ?>)
        </p>

        
        <p class="text-xs text-gray-400 mt-1 flex items-center gap-2">
            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            <span>Oluşturan: <span class="font-semibold"><?php echo e($case->creator->name ?? 'Sistem'); ?></span></span>
            <span class="text-gray-300">|</span>
            <span><?php echo e($case->created_at->format('d.m.Y H:i')); ?></span>
        </p>
    </div>
    ...
    <div class="flex gap-2 items-center">
        ...
        <?php if($case->status == 'hukuk_incelemesinde' && (auth()->user()->can('arabuluculuk.approve_legal') || auth()->user()->hasRole('Superadmin'))): ?>
            <div x-data="{ openDecision: false }" class="relative">
                ...
                <div x-show="openDecision" @click.away="openDecision = false" class="absolute right-0 mt-2 w-72 bg-white rounded-md shadow-xl z-50 border border-gray-200 p-4" style="display: none;">
                    <form action="<?php echo e(route('admin.arabuluculuk.submitDecision', $case->id)); ?>" method="POST">
                        <?php echo csrf_field(); ?>
        ...
        <?php if($case->status == 'yonetim_onayinda' && (auth()->user()->can('arabuluculuk.approve_board') || auth()->user()->hasRole('Superadmin'))): ?>
        ...
        <?php if($case->status == 'arabulucuda' && (
        ...
        <a href="<?php echo e(route('admin.arabuluculuk.index')); ?>" class="bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 px-4 py-2 rounded-lg shadow-sm font-medium transition">
            &larr; Listeye Dön
        </a>
    </div>
</div><?php /**PATH C:\Users\celal.karaman\Desktop\Projelerim\iaa_projesi\resources\views/admin/arabuluculuk/parcalar/ust-baslik-ve-butonlar.blade.php ENDPATH**/ ?>