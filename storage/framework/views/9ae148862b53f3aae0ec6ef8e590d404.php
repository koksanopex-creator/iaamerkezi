...
<?php if($iaa->musteriSikayeti): ?>
    ...
    <div id="iade-hurda-alani" class="mt-8 mb-8 bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md relative">
        ...
        <?php if($isLeader && $isOnayBekliyor): ?>
            <div class="absolute top-4 right-4 z-10">
                <form action="<?php echo e(route('proje.recallSubmission', $iaa->id)); ?>" method="POST">
                    <?php echo csrf_field(); ?>
        ...
        <div class="p-6 md:p-8">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-base font-bold text-gray-800 mt-0.5"><?php echo e($iade->urun_turu); ?></p>
                    </div>
                </div>
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-base font-bold text-gray-800 mt-0.5"><?php echo e($iade->iade_sebebi); ?></p>
                    </div>
                </div>
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="w-full">
                        ...
                        <div class="flex items-baseline gap-1 mt-0.5">
                            <span class="text-xl font-black text-red-600"><?php echo e(floatval($iade->miktar)); ?></span> 
                            <span class="text-gray-400 text-sm">/</span> 
                            <span class="text-lg font-bold text-gray-700"><?php echo e(floatval($iade->toplam_parti_miktari)); ?></span> 
                            <span class="text-xs font-bold text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded"><?php echo e($iade->birim); ?></span>
                        </div>
                        ...
                        <div class="w-full bg-gray-100 rounded-full h-2 mt-2 overflow-hidden border border-gray-200">
                            <div class="bg-gradient-to-r from-red-500 to-pink-600 h-2 rounded-full" style="width: <?php echo e($oran); ?>%"></div>
                        </div>
                        <div class="flex justify-between items-center mt-1">
                            <p class="text-[10px] font-bold text-red-500">%<?php echo e(number_format($oran, 1)); ?> İade</p>
                            <p class="text-[10px] text-gray-400">Kalan: <?php echo e($toplam - $miktar); ?> <?php echo e($iade->birim); ?></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-bold text-gray-800 mt-0.5">
                            <?php echo e($iade->iade_tarihi ? $iade->iade_tarihi->format('d.m.Y') : '-'); ?>

                        </p>
                        <p class="text-[10px] text-gray-500 mt-0.5 truncate max-w-[150px]" title="<?php echo e($iaa->musteriSikayeti->customer->address); ?>">
                            <?php echo e($iaa->musteriSikayeti->customer->address ?? 'Lokasyon Yok'); ?>
            ...
            <?php if((Auth::check() && Auth::user()->is_personnel == 1) || $iade->musteri_gorebilir_mi): ?>

            <?php if($isLeader || (Auth::check() && (Auth::user()->hasRole('Superadmin') || Auth::user()->hasRole('Yonetim')))): ?>
                
                <div class="mt-8 flex flex-col sm:flex-row items-center justify-between bg-slate-50 border border-slate-200 rounded-xl p-4 gap-4 transition-all duration-300" id="customer-visibility-wrapper-<?php echo e($iade->id); ?>">
                    <div class="flex items-center gap-3">
                        <div id="visibility-icon-container-<?php echo e($iade->id); ?>" class="flex items-center justify-center w-8 h-8 rounded-full <?php echo e($iade->musteri_gorebilir_mi ? 'bg-green-100 text-green-600' : 'bg-gray-200 text-gray-500'); ?>">
                            <?php if($iade->musteri_gorebilir_mi): ?>
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-gray-500" id="visibility-status-text-<?php echo e($iade->id); ?>">
                                Aşağıdaki ek notlar ve belgeler şu an müşteri tarafından <strong class="<?php echo e($iade->musteri_gorebilir_mi ? 'text-green-600' : 'text-gray-600'); ?>"><?php echo e($iade->musteri_gorebilir_mi ? 'GÖRÜLEBİLİYOR' : 'GÖRÜLEMİYOR (GİZLİ)'); ?></strong>.
                            </p>
                        </div>
                    </div>
                    <button type="button" 
                            id="toggle-vis-btn-<?php echo e($iade->id); ?>"
                            onclick="toggleMusteriGorebilirMi(<?php echo e($iade->id); ?>, this)" 
                            class="flex items-center gap-1.5 px-4 py-2 border rounded-lg shadow-sm text-xs font-bold transition-all shrink-0 <?php echo e($iade->musteri_gorebilir_mi ? 'bg-white border-red-200 text-red-600 hover:bg-red-50 hover:border-red-300' : 'bg-white border-green-200 text-green-600 hover:bg-green-50 hover:border-green-300'); ?>">
                        <?php if($iade->musteri_gorebilir_mi): ?>
                    ...
                    <?php if($iade->dosya_yolu): ?>
                        <?php if($isImage): ?>
                            ...
                            <div class="group relative w-full h-40 bg-gray-100 rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                                <img src="<?php echo e(asset('storage/' . $iade->dosya_yolu)); ?>" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" alt="İade Belgesi">
                                
                                
                                <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                                    <a href="<?php echo e(asset('storage/' . $iade->dosya_yolu)); ?>" target="_blank" class="inline-flex items-center px-4 py-2 bg-white text-gray-900 rounded-lg font-bold text-xs shadow-lg hover:bg-gray-100 transform hover:scale-105 transition-all">
                        ...
                        <?php else: ?>
                            ...
                            <a href="<?php echo e(asset('storage/' . $iade->dosya_yolu)); ?>" target="_blank" class="block group">
                                <div class="flex items-center p-4 bg-white border border-gray-200 rounded-xl hover:border-indigo-300 hover:shadow-md transition-all duration-200">
                                    ...
                                    <div class="ml-3 overflow-hidden">
                                        ...
                                        <p class="text-xs text-gray-500 uppercase"><?php echo e($fileExtension); ?> Dosyası</p>
                                    </div>
                                </div>
                            </a>
                        <?php endif; ?>
...
<?php endif; ?>
<?php /**PATH C:\Users\celal.karaman\Desktop\Projelerim\iaa_projesi\resources\views/proje-calisma-alani/partials/_return-details-card.blade.php ENDPATH**/ ?>