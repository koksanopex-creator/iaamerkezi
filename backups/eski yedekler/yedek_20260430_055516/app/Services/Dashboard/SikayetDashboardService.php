    ...
    public function getLeaderStats(User $user, $filters = [])
    {
        ...
        if ($stats['has_teams']) {
            ...
            $onayBekleyenler = $sikayetler->filter(function($sikayet) {
                ...
                if ($sikayet->iaaProjesi && in_array($sikayet->iaaProjesi->durum, ['Bölüm Onayı Bekliyor', 'Yönetici Onayı Bekliyor', 'talep_onayi_bekliyor_kalite', 'talep_onayi_bekliyor_superadmin', 'Direktör Onayı Bekliyor'])) {
                    return true;
                }
                return false;
