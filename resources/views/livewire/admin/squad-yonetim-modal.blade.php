            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                ...
                                <p class="text-sm text-gray-500 mt-1">{{ Str::limit($projeBasligi, 40) }}</p>
                                ...
                                <div class="mt-4 relative">
                                    ...
                                    @if(!empty($bulunanKullanicilar))
                                        <ul class="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                                            @foreach($bulunanKullanicilar as $user)
                                                <li class="text-gray-900 cursor-default select-none relative py-2 pl-3 pr-9 hover:bg-indigo-50 flex justify-between items-center">
                                                    <div class="flex items-center">
                                                        <span class="font-medium block truncate">{{ $user->name }}</span>
                                                        {{-- Burada pivot kontrolü YOK çünkü bunlar arama sonucu --}}
                                                        <span class="text-xs text-gray-400 ml-2">({{ $user->bolum->ad ?? '-' }})</span>
                                                    </div>
                                                    <button wire:click="uyeEkle({{ $user->id }})" class="text-white bg-indigo-600 hover:bg-indigo-700 px-2 py-1 rounded text-xs">Davet Et</button>
                                                </li>
                                            @endforeach
                                        </ul>
                                    @endif
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6">
                        <h4 class="text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Mevcut Ekip ({{ count($mevcutUyeListesi) }})</h4>
                        ...
                        <div class="space-y-2 max-h-60 overflow-y-auto">
                            @foreach($mevcutUyeListesi as $uye)
                                <div class="flex justify-between items-center bg-white p-3 rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                                    ...
                                    {{-- Sol Taraf: Profil ve İsim --}}
                                    <div class="flex items-center gap-3">
                                        <div class="relative">
                                            @if($uye->profile_photo_path)
                                                <img src="{{ asset('storage/'.$uye->profile_photo_path) }}" class="w-10 h-10 rounded-full object-cover border">
                                            @else
                                            ...
                                            {{-- Durum İkonu (Sağ Alt Köşe) --}}
                                            @if(isset($uye->pivot))
                                                @if($uye->pivot->rol == 'Lider')
                                                ...
                                                @elseif($uye->pivot->durum == 'onaylandi')
                                                ...
                                                @elseif($uye->pivot->durum == 'bekliyor')
                                                ...
                                                @elseif($uye->pivot->durum == 'reddedildi')
                                    ...
                                    @if($uye->id != $liderId)
                                        <div class="flex items-center gap-2">
                                            ...
                                            @else
                                                {{-- Normal üyeyi çıkarma --}}
                                                <button wire:click="uyeCikar({{ $uye->id }})" class="text-gray-400 hover:text-red-600 transition-colors" title="Ekip'ten Çıkar">
