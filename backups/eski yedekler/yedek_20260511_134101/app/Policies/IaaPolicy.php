...
class IaaPolicy
{
    ...
    public function view(User $user, Iaa $iaa): bool
    {
        ...
        // --- rules1.md KURAL 8 ve 111: İAA vs ŞİKAYET AYRIMI ---
        $isIaa = !empty($iaa->oneri);
        $isSikayet = !$isIaa;

        // Kural 1.6: Şikayet Projeleri İçin Ek Roller
        if ($isSikayet) {
            ...
            // Bölüm Lideri Yetkisi (Kendi bölümüne ait şikayet projeleri)
            if ($user->hasRole('Bölüm Lideri') && $user->bolum_id && $iaa->bolum_id == $user->bolum_id) {
                return true;
            }
        }
