<x-guest-layout wrapperClass="w-[90%]">
    {{-- ========================================== --}}
    {{-- 🚨 HATA ÖNLEYİCİ YAMA (ÇOK ÖNEMLİ) 🚨 --}}
    {{-- ========================================== --}}
    <script>
    ...
    <form action="{{ route('public.sikayet.store') }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
            ...
            <div class="group col-span-1">
                ...
                <input type="text" name="musteri_adi" id="musteri_adi"
                    value="{{ old('musteri_adi', auth()->user()?->name) }}" required
            ...
            <div class="group col-span-1">
                ...
                <input type="email" name="musteri_iletisim" id="musteri_iletisim"
                    value="{{ old('musteri_iletisim', auth()->user()?->email) }}" required
                    class="block w-full border-gray-300 rounded-lg shadow-sm focus:ring-red-500 focus:border-red-500 transition duration-150 ease-in-out pl-4 pr-4 py-2.5 text-gray-900"
                ...
                <div class="mt-2 flex items-center space-x-6">
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="radio" name="konum_tipi" value="Yurt İçi"
                            class="form-radio text-red-600 focus:ring-red-500" {{ old('konum_tipi', 'Yurt İçi') == 'Yurt İçi' ? 'checked' : '' }}>
                        <span class="ml-2 text-sm text-gray-700">Yurt İçi</span>
                    </label>
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="radio" name="konum_tipi" value="Yurt Dışı"
                            class="form-radio text-red-600 focus:ring-red-500" {{ old('konum_tipi') == 'Yurt Dışı' ? 'checked' : '' }}>
            ...
            {{-- ============================================================== --}}
            {{-- === KATEGORİ VE ALT KATEGORİ ALANI (Alpine.js) === --}}
            {{-- ============================================================== --}}
            <div class="col-span-1 md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6" x-data="categorySystem()"
                ...
                <div class="group">
                    ...
                    @error('sikayet_kategorisi_id') <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                    @enderror
                </div>

                {{-- 2. Alt Kategori (Dinamik) --}}
                <div class="group" x-show="subCategories.length > 0 || showOtherOption" style="display: none;">
                    ...
                    @error('sikayet_alt_kategori_id') <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                    @enderror
                </div>

                {{-- 3. Diğer Açıklama Alanı --}}
                <div class="group md:col-span-2 bg-yellow-50 p-4 rounded border border-yellow-200"
                    ...
                    @error('sikayet_alt_kategori_diger') <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                    @enderror
                </div>
            </div>
            {{-- ============================================================== --}}
            ...
            <div class="col-span-1 md:col-span-2 group">
                ...
                <textarea name="musteri_sikayet_detayi" id="musteri_sikayet_detayi" rows="5" required
                    ...
                    placeholder="Lütfen yaşadığınız sorunu tüm detaylarıyla açıklayınız...">{{ old('musteri_sikayet_detayi') }}</textarea>
    ...
    <script>
        ...
        function categorySystem() {
            return {
                selectedCategory: '{{ old('sikayet_kategorisi_id') }}',
                selectedSubCategory: '{{ old('sikayet_alt_kategori_id', old('sikayet_alt_kategori_diger') ? 'other' : '') }}',
                otherText: '{{ old('sikayet_alt_kategori_diger') }}',
                subCategories: [],
                ...
                fetchSubCategories(keepSelection = false) {
                    ...
                    // === URL DÜZELTMESİ ===
                    var baseUrl = "{{ asset('api/get-alt-kategoriler') }}";
