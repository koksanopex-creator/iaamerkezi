...
<?php if(Auth::user()->hasRole(['Superadmin', 'Hukuk Admini', 'Hukuk Yöneticisi', 'Yonetim', 'Yönetim'])): ?>
            ...
            <button @click="showLogs = !showLogs" class="w-full px-8 py-6 flex items-center justify-between hover:bg-slate-50 transition-colors group">
                <div class="flex items-center gap-4">
                    ...
                    <div class="text-left">
                        ...
                        <p class="text-[10px] text-slate-400 font-bold">TOPLAM <?php echo e($case->logs->count()); ?> ADET KAYIT BULUNMAKTADIR</p>
                    </div>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <?php $__empty_1 = true; $__currentLoopData = $case->logs()->with('user')->orderBy('created_at', 'desc')->get(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr class="hover:bg-white transition-colors group/row">
                                    <td class="px-8 py-5">
                                        <div class="text-xs font-black text-slate-700"><?php echo e($log->created_at->format('d.m.Y')); ?></div>
                                        <div class="text-[10px] text-slate-400 font-bold"><?php echo e($log->created_at->format('H:i')); ?></div>
                                    </td>
                                    <td class="px-8 py-5">
                                        <div class="flex items-center gap-3">
                                            <?php if($log->user): ?>
                                                <img src="<?php echo e($log->user->profile_photo_url); ?>" class="w-8 h-8 rounded-lg border border-slate-200 shadow-sm object-cover">
                                                <div>
                                                    <a href="<?php echo e(url('/kullanici-profil/'.$log->user_id)); ?>" class="text-xs font-black text-slate-900 hover:text-indigo-600 transition-colors"><?php echo e($log->user->name); ?></a>
                                                    <p class="text-[9px] text-slate-400 font-bold uppercase tracking-tighter"><?php echo e($log->user->bolum->ad ?? '-'); ?></p>
                                                </div>
                                            <?php else: ?>
                                    ...
                                    <td class="px-8 py-5">
                                        ...
                                        ?>
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-lg bg-<?php echo e($badgeColor); ?>-50 text-<?php echo e($badgeColor); ?>-700 text-[10px] font-black uppercase tracking-widest border border-<?php echo e($badgeColor); ?>-100">
                                            <?php echo e($log->eylem); ?>

                                        </span>
                                    </td>
                                    <td class="px-8 py-5">
                                        <p class="text-xs text-slate-600 font-medium leading-relaxed italic">"<?php echo e($log->aciklama); ?>"</p>
                                        <?php if($log->eski_metin): ?>
...
<?php endif; ?>
<?php /**PATH C:\Users\celal.karaman\Desktop\Projelerim\iaa_projesi\resources\views/admin/disiplin/partials/logs.blade.php ENDPATH**/ ?>