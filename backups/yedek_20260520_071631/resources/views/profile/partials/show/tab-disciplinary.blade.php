...
@if($canViewDiscipline)
    <div x-show="activeTab === 'disiplin'" x-transition:enter="transition ease-out duration-300 transform" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" style="display: none;">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            {{-- YENİ TUTANAK BUTONU (Sadece Yöneticiler) --}}
            @if(Auth::user()->hasRole(['Superadmin', 'Hukuk Yöneticisi', 'Bölüm Lideri']) && Auth::id() != $user->id)
                <a href="{{ route('admin.disiplin.create') }}" class="group inline-flex items-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-indigo-700 transition shadow-md hover:shadow-lg">
        ...
        @if($profileCases->isEmpty())
            
            {{-- MODERN EMPTY STATE (BOŞ DURUM) --}}
                ...
                @foreach($profileCases as $case)
                    ...
                    <a href="{{ $detailLink }}" class="block group">
                        <div class="bg-white rounded-xl border border-gray-200 p-5 shadow-sm hover:shadow-md transition-all duration-200 relative overflow-hidden">
                            ...
                            {{-- Sol Durum Çizgisi --}}
                            <div class="absolute left-0 top-0 bottom-0 w-1.5 {{ explode(' ', $statusStyles)[0] }}"></div>
                            ...
                            <div class="flex items-start justify-between">
                                <div class="flex gap-4">
                                    ...
                                    <div class="flex-shrink-0">
                                        <div class="w-12 h-12 rounded-lg bg-gray-50 flex flex-col items-center justify-center border border-gray-100 text-gray-600 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition">
                                            ...
                                            <span class="text-lg font-black leading-none">#{{ $case->id }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        <h4 class="text-base font-bold text-gray-900 group-hover:text-indigo-600 transition flex items-center gap-2">
                                            {{ $case->behavior->category->ad ?? 'Genel Disiplin' }}
                                        </h4>
                                        <p class="text-sm text-gray-500 mt-1 line-clamp-1">
                                            {{ Str::limit($case->behavior->tanim ?? 'İhlal tanımı bulunamadı.', 80) }}
                                        </p>
                                        
                                        <div class="flex items-center gap-4 mt-2 text-xs text-gray-400">
                                            <span class="flex items-center gap-1">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                                {{ $case->olay_tarihi->format('d.m.Y') }}
                                            </span>
                                            <span class="flex items-center gap-1">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                                                Raportör: {{ $case->reporter->name ?? 'Sistem' }}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="flex flex-col items-end gap-2">
                                    <span class="px-3 py-1 rounded-full text-xs font-bold border {{ str_replace('border-', 'border-opacity-20 ', $statusStyles) }}">
                                        {{ $case->durum }}
                                    </span>
