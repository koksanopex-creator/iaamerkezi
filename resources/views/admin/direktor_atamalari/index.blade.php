<x-app-layout>
        ...
        <div class="flex justify-between items-center" x-data="{ showAddModal: false }">
            ...
            <div class="flex items-center gap-4">
                ...
                <span class="text-xs text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                    Toplam {{ $direktorler->count() }} Direktör
                </span>
            </div>

            {{-- HIZLI DİREKTÖR EKLEME MODALI --}}
            <div x-data="{ open: false }" 
                ...
                <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" 
                        ...
                        <form action="{{ route('admin.direktorler.store') }}" method="POST">
                            @csrf
    ...
    <div class="py-10">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div x-data="{ 
                    selectedUserId: {{ $direktorler->first()->id ?? 'null' }}, 
                    search: '' 
                 }" 
                        ...
                        @else
                            <ul class="divide-y divide-gray-100">
                                @foreach ($direktorler as $direktor)
                                    <li x-show="search === '' || '{{ strtolower($direktor->name) }}'.includes(search.toLowerCase())"
                                        @click="selectedUserId = {{ $direktor->id }}"
                                        :class="selectedUserId === {{ $direktor->id }} ? 'bg-indigo-50 border-l-4 border-indigo-600' : 'hover:bg-gray-50 border-l-4 border-transparent'"
                                        ...
                                        <div class="p-4 flex items-center">
                                            <div class="flex-shrink-0 mr-3">
                                                <div class="h-10 w-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold text-sm group-hover:bg-indigo-200 transition">
                                                    {{ strtoupper(substr($direktor->name, 0, 1)) }}{{ strtoupper(substr(explode(' ', $direktor->name)[1] ?? '', 0, 1)) }}
                                                </div>
                                            </div>
                                            <div class="flex-1 min-w-0">
                                                <p class="text-sm font-semibold text-gray-900 truncate" 
                                                   :class="selectedUserId === {{ $direktor->id }} ? 'text-indigo-700' : ''">
                                                    {{ $direktor->name }}
                                                </p>
                                                <p class="text-xs text-gray-500 truncate">{{ $direktor->email }}</p>
                                            </div>
                                            <div x-show="selectedUserId === {{ $direktor->id }}">
                ...
                <div class="w-full md:w-2/3 bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col overflow-hidden relative">
                    ...
                    {{-- Her direktör için Form (Sadece seçili olan görünür) --}}
                    @foreach ($direktorler as $direktor)
                        <div x-show="selectedUserId === {{ $direktor->id }}" x-cloak
                            ...
                            <div class="px-6 py-5 border-b border-gray-200 flex justify-between items-center bg-gray-50">
                                <div>
                                    <h3 class="text-lg font-bold text-gray-900">{{ $direktor->name }}</h3>
                            ...
                            {{-- Form İçeriği --}}
                            <form action="{{ route('admin.direktorler.update', $direktor->id) }}" method="POST" class="flex flex-col flex-1 min-h-0">
                                @csrf
                                            ...
                                            <label class="relative flex items-start p-4 rounded-lg border border-gray-200 hover:bg-gray-50 cursor-pointer transition-colors duration-200 group"
                                                ...
                                                <div class="flex items-center h-5">
                                                    <input type="checkbox" 
                                                           name="bolumler[]" 
                                                           value="{{ $bolum->id }}" 
                                                           {{ $direktor->yonetilenBolumler->contains($bolum->id) ? 'checked' : '' }}
                                                ...
                                                <div class="ml-3 text-sm">
                                                    ...
                                                    @if($bolum->director && $bolum->director_id != $direktor->id)
                                                        <p class="text-[10px] text-red-500 mt-1 italic">
                                                            Şu anki: {{ $bolum->director->name }}
                                                        </p>
                                                    @endif
                                                </div>
                                            </label>
