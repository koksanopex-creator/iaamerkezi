...
<div class="space-y-6" wire:key="analiz-raporu">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-5 rounded-2xl shadow-sm border border-gray-100 print-hidden">
        <div>
            ...
            <div class="mt-2 flex items-center gap-2">
                ...
                @endphp
                @if(!auth()->user()->hasRole('Superadmin') && !auth()->user()->hasRole('Yonetim') && !auth()->user()->hasRole('Yönetim'))
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('admin.musteri-sikayet-karsilastirma') }}" class="px-4 py-2 bg-indigo-50 text-indigo-700 font-bold text-xs rounded-xl hover:bg-indigo-100 transition shadow-sm border border-indigo-200 flex items-center gap-2">
    ...
    <div class="bg-gradient-to-r from-indigo-50/50 via-white to-white rounded-2xl shadow-sm border border-indigo-100 print-hidden p-3 flex flex-col xl:flex-row items-start xl:items-center gap-4 mb-2">
        ...
        <div class="flex gap-2 overflow-x-auto custom-scrollbar w-full pb-1">
            ...
            @endphp
            @foreach($navLinks as $id => $link)
                <a href="#{{ $id }}" class="flex-shrink-0 flex items-center px-3 py-1.5 text-[10.5px] font-bold text-slate-500 bg-white border border-slate-200 rounded-lg {{ $link['hover'] }} transition-colors">
                    <span class="w-1.5 h-1.5 rounded-full {{ $link['dot'] }} mr-1.5"></span>
                    {{ $link['label'] }}
                </a>
            @endforeach
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 print-hidden">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-6">
            ...
            {{-- Boş alan (Gridi tamamlamak için) --}}
            <div class="hidden lg:block"></div>

            {{-- ================= GRUP 2: ŞİKAYET ÖZELLİKLERİ ================= --}}
            ...
            <div>
                ...
                <div x-data="{ open: false }" class="relative w-full">
                    <button @click="open = !open" type="button" class="w-full bg-gray-50 border border-gray-200 text-xs rounded-xl py-2 px-3 text-left flex justify-between items-center focus:ring-indigo-500 focus:border-indigo-500">
                        <span class="truncate">{{ count($bolumId) > 0 ? count($bolumId) . ' Seçili' : '-- Tüm Bölümler --' }}</span>
                    ...
                    <div x-show="open" @click.away="open = false" x-cloak class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto custom-scrollbar p-2">
                        @foreach($filterData['bolumler'] as $bolum)
                            <label wire:key="filter-bolum-{{ $bolum->id }}" class="flex items-center gap-2 p-1.5 hover:bg-gray-50 rounded cursor-pointer">
                                <input type="checkbox" wire:model.live="bolumId" value="{{ $bolum->id }}" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                <span class="text-xs text-gray-700">{{ $bolum->ad }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
            ...
            <div>
                ...
                <div x-data="{ open: false }" class="relative w-full">
                    <button @click="open = !open" type="button" class="w-full bg-gray-50 border border-gray-200 text-xs rounded-xl py-2 px-3 text-left flex justify-between items-center focus:ring-indigo-500 focus:border-indigo-500">
                        <span class="truncate">{{ count($altKategoriId) > 0 ? count($altKategoriId) . ' Seçili' : '-- Tüm Alt Kategoriler --' }}</span>
                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                    <div x-show="open" @click.away="open = false" x-cloak class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto custom-scrollbar p-2">
                        @foreach($filterData['altKategoriler'] as $ak)
                            <label wire:key="filter-altkat-{{ $ak['id'] }}" class="flex items-center gap-2 p-1.5 hover:bg-gray-50 rounded cursor-pointer">
                                <input type="checkbox" wire:model.live="altKategoriId" value="{{ $ak['id'] }}" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                <span class="text-xs text-gray-700">{{ $ak['ad'] }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
            ...
            <div>
                ...
                <div x-data="{ open: false }" class="relative w-full">
                    <button @click="open = !open" type="button" class="w-full bg-gray-50 border border-gray-200 text-xs rounded-xl py-2 px-3 text-left flex justify-between items-center focus:ring-indigo-500 focus:border-indigo-500">
                        <span class="truncate">{{ count($durum) > 0 ? count($durum) . ' Seçili' : '-- Tüm Durumlar --' }}</span>
                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                    <div x-show="open" @click.away="open = false" x-cloak class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto custom-scrollbar p-2">
                        @foreach($filterData['durumlar'] as $key => $label)
                            <label wire:key="filter-durum-{{ $key }}" class="flex items-center gap-2 p-1.5 hover:bg-gray-50 rounded cursor-pointer">
                                <input type="checkbox" wire:model.live="durum" value="{{ $key }}" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
            ...
            <div>
                ...
                <div x-data="{ open: false }" class="relative w-full">
                    <button @click="open = !open" type="button" class="w-full bg-gray-50 border border-gray-200 text-xs rounded-xl py-2 px-3 text-left flex justify-between items-center focus:ring-indigo-500 focus:border-indigo-500">
                        <span class="truncate">{{ count($oncelik) > 0 ? count($oncelik) . ' Seçili' : '-- Tüm Öncelikler --' }}</span>
                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                    <div x-show="open" @click.away="open = false" x-cloak class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto custom-scrollbar p-2">
                        @foreach($filterData['oncelikler'] as $o)
                            <label wire:key="filter-oncelik-{{ $o }}" class="flex items-center gap-2 p-1.5 hover:bg-gray-50 rounded cursor-pointer">
                                <input type="checkbox" wire:model.live="oncelik" value="{{ $o }}" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                <span class="text-xs text-gray-700">{{ $o }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
            ...
            <div>
                ...
                <div x-data="{ open: false }" class="relative w-full">
                    <button @click="open = !open" type="button" class="w-full bg-gray-50 border border-gray-200 text-xs rounded-xl py-2 px-3 text-left flex justify-between items-center focus:ring-indigo-500 focus:border-indigo-500">
                        <span class="truncate">{{ count($takimId) > 0 ? count($takimId) . ' Seçili' : '-- Tüm Takımlar --' }}</span>
                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                    <div x-show="open" @click.away="open = false" x-cloak class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto custom-scrollbar p-2">
                        @foreach($filterData['takimlar'] as $tkm)
                            <label wire:key="filter-takim-{{ $tkm->id }}" class="flex items-center gap-2 p-1.5 hover:bg-gray-50 rounded cursor-pointer">
                                <input type="checkbox" wire:model.live="takimId" value="{{ $tkm->id }}" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                <span class="text-xs text-gray-700">{{ $tkm->ad }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
            ...
            <div>
                ...
                <div x-data="{ open: false }" class="relative w-full">
                    <button @click="open = !open" type="button" class="w-full bg-gray-50 border border-gray-200 text-xs rounded-xl py-2 px-3 text-left flex justify-between items-center focus:ring-indigo-500 focus:border-indigo-500">
                        <span class="truncate">{{ count($squadUserId) > 0 ? count($squadUserId) . ' Seçili' : '-- Tüm Personeller --' }}</span>
                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                    <div x-show="open" @click.away="open = false" x-cloak class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto custom-scrollbar p-2">
                        @foreach($filterData['squadUsers'] as $sq)
                            <label wire:key="filter-squad-{{ $sq->id }}" class="flex items-center gap-2 p-1.5 hover:bg-gray-50 rounded cursor-pointer">
                                <input type="checkbox" wire:model.live="squadUserId" value="{{ $sq->id }}" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                <span class="text-xs text-gray-700">{{ $sq->name }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
            ...
            <div>
                ...
                <div x-data="{ open: false }" class="relative w-full">
                    <button @click="open = !open" type="button" class="w-full bg-gray-50 border border-gray-200 text-xs rounded-xl py-2 px-3 text-left flex justify-between items-center focus:ring-indigo-500 focus:border-indigo-500">
                        <span class="truncate">{{ count($customerId) > 0 ? count($customerId) . ' Seçili' : '-- Tüm Müşteriler --' }}</span>
                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                    <div x-show="open" @click.away="open = false" x-cloak class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto custom-scrollbar p-2">
                        @foreach($filterData['musteriler'] as $m)
                            <label wire:key="filter-musteri-{{ $m->id }}" class="flex items-center gap-2 p-1.5 hover:bg-gray-50 rounded cursor-pointer">
                                <input type="checkbox" wire:model.live="customerId" value="{{ $m->id }}" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                <span class="text-xs text-gray-700">{{ $m->name }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
            ...
            <div>
                ...
                <div x-data="{ open: false }" class="relative w-full">
                    <button @click="open = !open" type="button" class="w-full bg-gray-50 border border-gray-200 text-xs rounded-xl py-2 px-3 text-left flex justify-between items-center focus:ring-indigo-500 focus:border-indigo-500">
                        <span class="truncate">{{ count($konumTipi) > 0 ? count($konumTipi) . ' Seçili' : '-- Tümü --' }}</span>
                        <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                    <div x-show="open" @click.away="open = false" x-cloak class="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-60 overflow-y-auto custom-scrollbar p-2">
                        @foreach($filterData['konumTipleri'] as $kt)
                            <label wire:key="filter-konum-{{ $kt }}" class="flex items-center gap-2 p-1.5 hover:bg-gray-50 rounded cursor-pointer">
                                <input type="checkbox" wire:model.live="konumTipi" value="{{ $kt }}" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                <span class="text-xs text-gray-700">{{ $kt }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 hover:shadow-md transition-all group">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-black text-gray-900">{{ number_format($kpi['toplam']) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-orange-100 p-4 hover:shadow-md transition-all group">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-black text-gray-900">{{ number_format($kpi['acik']) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-emerald-100 p-4 hover:shadow-md transition-all group">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-black text-gray-900">{{ number_format($kpi['cozulen']) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-rose-100 p-4 hover:shadow-md transition-all group">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-black text-gray-900">{{ number_format($kpi['gecikmis']) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-blue-100 p-4 hover:shadow-md transition-all group">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-black text-gray-900">{{ $kpi['ortCozumSuresi'] }} <span class="text-xs font-medium text-gray-400">gün</span></p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-purple-100 p-4 hover:shadow-md transition-all group">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-xl font-black text-gray-900">%{{ $kpi['iadeOrani'] }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="detay-tablosu" class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden no-break mb-6">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h4 class="text-sm font-black text-slate-700 uppercase tracking-wider flex items-center gap-2">
                ...
                <span class="text-[10px] font-bold text-gray-400 normal-case">({{ $detayTablosu->total() }} kayıt)</span>
            </h4>
        </div>
        <div class="w-full">
            <table class="w-full divide-y divide-gray-200 text-[11px]">
                <thead class="bg-gray-50/80">
                    <tr>
                        ...
                        <th class="px-3 py-3 text-left font-bold text-gray-500 uppercase tracking-wider whitespace-nowrap">{{ $tarihAlani == 'musteri_sikayet_tarihi' ? 'Bildirim' : ($tarihAlani == 'musteri_cozum_son_tarihi' ? 'Çözüm' : 'Giriş') }}</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse($detayTablosu as $sikayet)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            <td class="px-3 py-3 font-bold text-indigo-600 whitespace-nowrap">#{{ $sikayet->id }}</td>
                            <td class="px-3 py-3">
                                <div class="max-w-[140px] xl:max-w-[180px] truncate" title="{{ $sikayet->musteri_sikayet_konusu }}">{{ $sikayet->musteri_sikayet_konusu }}</div>
                            </td>
                            <td class="px-3 py-3 whitespace-nowrap">
                                @if($sikayet->sikayetKategori && $sikayet->sikayetKategori->bolum)
                                    <div class="flex items-center gap-1.5">
                                        @if($sikayet->sikayetKategori->bolum->logo_yolu)
                                            <img src="{{ asset('storage/'.$sikayet->sikayetKategori->bolum->logo_yolu) }}" class="w-5 h-5 object-contain rounded border border-gray-100" alt="{{ $sikayet->sikayetKategori->bolum->ad }}">
                                        @endif
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-blue-50 text-blue-700 leading-tight">{{ $sikayet->sikayetKategori->bolum->ad }}</span>
                                    </div>
                                @else
                            ...
                            <td class="px-3 py-3">
                                <div class="flex items-center gap-1.5">
                                    @if($sikayet->customer && $sikayet->customer->logo_path)
                                        <img src="{{ asset('storage/'.$sikayet->customer->logo_path) }}" class="w-5 h-5 rounded-full object-cover border border-gray-200">
                                    @else
                                    ...
                                    @endif
                                    @if($sikayet->customer)
                                        <a href="{{ route('musteri.profil.show', $sikayet->customer_id) }}" class="text-indigo-600 hover:text-indigo-800 font-medium max-w-[100px] xl:max-w-[140px] truncate block" title="{{ $sikayet->customer->name }}">
                                            {{ $sikayet->customer->name }}
                                        </a>
                                    @else
                                        <span class="max-w-[100px] xl:max-w-[140px] truncate block" title="{{ $sikayet->musteri_adi }}">{{ $sikayet->musteri_adi }}</span>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td class="px-3 py-3 align-middle">{!! $sikayet->musteri_durum_badge_analiz !!}</td>
                            <td class="px-3 py-3 align-middle justify-center whitespace-nowrap">{!! $sikayet->musteri_sure_bilgisi !!}</td>
                            <td class="px-3 py-3 align-middle whitespace-nowrap">
                                <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold {{ $sikayet->oncelik_badge_class }}">{{ $sikayet->musteri_oncelik ?? '-' }}</span>
                            </td>
                            <td class="px-3 py-3 text-right space-y-1">
                                <a href="{{ route('admin.sikayetler.show', $sikayet->id) }}" target="_blank" class="inline-flex items-center px-2 py-1 bg-blue-50 text-blue-600 hover:bg-blue-100 hover:text-blue-700 rounded text-[9px] font-bold transition-colors w-full justify-center whitespace-nowrap">Detay</a>
                                @if($sikayet->iaa_id)
                                    <a href="{{ route('proje.workspace.show', $sikayet->iaa_id) }}" target="_blank" class="inline-flex items-center px-2 py-1 bg-purple-50 text-purple-600 hover:bg-purple-100 hover:text-purple-700 rounded text-[9px] font-bold transition-colors w-full justify-center whitespace-nowrap">Proje</a>
                                @endif
                            </td>
                        </tr>
                    @empty
    ...
    <div id="opex-analizleri" class="grid grid-cols-1 gap-6 mb-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-red-100 p-5 relative">
            <div class="absolute top-5 right-5 text-[9px] text-gray-400 font-medium bg-gray-50 px-2 py-0.5 rounded border border-gray-100 hidden xl:block print-hidden max-w-[200px] truncate" title="{{ $this->getActiveFilterText() }}">{{ $this->getActiveFilterText() }}</div>
...
{{-- ============================= YETKİ MATRİSİ MODALI ============================= --}}
@if($isSuperadmin && $showYetkiModal)
<div class="fixed inset-0 z-[999] flex items-center justify-center bg-black/50 backdrop-blur-sm" wire:key="yetki-modal">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col" @click.away="$wire.closeYetkiModal()">
        ...
        <div class="overflow-y-auto flex-1 p-6 space-y-6">
            {{-- Rapor Türü Seçimi (Tabs) --}}
            <div class="flex items-center gap-2 border-b border-gray-200 pb-2">
                <button wire:click="$set('yetkiReportName', 'analiz_raporu')"
                        class="px-4 py-2 text-xs font-bold rounded-t-lg transition border-b-2 {{ $yetkiReportName === 'analiz_raporu' ? 'border-amber-500 text-amber-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                    Analiz Raporu Yetkileri
                </button>
                <button wire:click="$set('yetkiReportName', 'karsilastirma_raporu')"
                        class="px-4 py-2 text-xs font-bold rounded-t-lg transition border-b-2 {{ $yetkiReportName === 'karsilastirma_raporu' ? 'border-amber-500 text-amber-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                    Karşılaştırma Raporu Yetkileri
                </button>
            </div>

            {{-- Bildirimler --}}
            @if(session('yetkiSuccess'))
                <div class="bg-green-50 border border-green-200 text-green-700 text-xs font-bold p-3 rounded-xl flex items-center gap-2">
                    ...
                    {{ session('yetkiSuccess') }}
                </div>
            @endif
            @if(session('yetkiError'))
            ...
            {{-- BÖLÜM 1: MEVCUT YETKİLİ KULLANICILAR --}}
            <div>
                <h4 class="text-sm font-black text-gray-700 uppercase tracking-wider mb-3 flex items-center gap-2">
                    <span class="w-1.5 h-4 bg-emerald-500 rounded-full"></span>
                    Yetkili Kullanıcılar
                    <span class="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-lg">{{ $yetkiliKullanicilar->count() }} Kişi</span>
                </h4>
                <div class="bg-gray-50 rounded-xl border border-gray-200 overflow-hidden">
                    <table class="w-full text-xs">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse($yetkiliKullanicilar as $yetki)
                                <tr class="hover:bg-white transition" wire:key="yetki-user-{{ $yetki->user_id }}">
                                    <td class="px-4 py-3">
                                        <div class="flex items-center gap-2">
                                            <img src="{{ $yetki->user->profile_photo_url ?? '' }}" class="w-7 h-7 rounded-full object-cover border border-gray-200" alt="">
                                            <span class="font-bold text-gray-800">{{ $yetki->user->name ?? '-' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-4 py-3 text-gray-500">
                                        <div class="flex flex-col">
                                            <span class="text-[10px] font-bold text-indigo-600">{{ $yetki->user->roles->pluck('name')->implode(', ') }}</span>
                                            <span class="text-[10px] text-gray-400">{{ $yetki->user->bolum->ad ?? '-' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-4 py-3">
                                        @if($yetkiEditUserId == $yetki->user_id)
                                            <div class="flex flex-col gap-1">
                                                <select wire:model.live="yetkiDataScope" class="text-[10px] bg-white border border-amber-300 rounded-lg py-1 px-2 focus:ring-amber-500">
                                                    @foreach(\App\Models\ReportRoleAuthorization::DATA_SCOPE_OPTIONS as $key => $label)
                                                        <option value="{{ $key }}">{{ $label }}</option>
                                                    @endforeach
                                                </select>
                                                @if($yetkiDataScope === 'specific_departments')
                                                    <div class="max-h-24 overflow-y-auto custom-scrollbar bg-white rounded-lg p-1 border border-gray-200">
                                                        @foreach($tumBolumler as $bolum)
                                                            <label class="flex items-center gap-1 py-0.5 cursor-pointer">
                                                                <input type="checkbox" wire:model.live="yetkiSpecificDeptIds" value="{{ $bolum->id }}" class="rounded border-gray-300 text-amber-600 w-3 h-3">
                                                                <span class="text-[9px] text-gray-600">{{ $bolum->ad }}</span>
                                                            </label>
                                                        @endforeach
                                                    </div>
                                                @endif
                                        ...
                                        @else
                                            ...
                                            @endphp
                                            <span class="px-2 py-1 rounded-lg font-bold text-[10px] {{ $scopeColors[$yetki->data_scope] ?? 'bg-gray-100' }}">
                                                {{ $scopeLabels[$yetki->data_scope] ?? $yetki->data_scope }}
                                            </span>
                                            @if($yetki->data_scope === 'specific_departments' && !empty($yetki->specific_department_ids))
                                                @php $bolumAdlari = \App\Models\Bolum::whereIn('id', $yetki->specific_department_ids)->pluck('ad')->toArray(); @endphp
                                                <div class="text-[9px] text-gray-400 mt-0.5">{{ implode(', ', $bolumAdlari) }}</div>
                                            @endif
                                        @endif
                                    </td>
                                    <td class="px-4 py-3 text-center">
                                        <div class="flex items-center justify-center gap-1">
                                            <button wire:click="editUserYetki({{ $yetki->user_id }})" class="p-1.5 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 transition" title="Düzenle">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                            </button>
                                            <button wire:click="deleteUserYetki({{ $yetki->user_id }})" wire:confirm="Bu kullanıcının yetkisini kaldırmak istediğinize emin misiniz?" class="p-1.5 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 transition" title="Sil">
            ...
            <div class="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
                ...
                @if($yetkiFilterRole && $filtrelenmisKullanicilar->count() > 0)
                    <div class="bg-gray-50 rounded-xl border border-gray-200 max-h-60 overflow-y-auto custom-scrollbar">
                        <table class="w-full text-xs">
                            <thead class="bg-gray-100 sticky top-0">
                                <tr>
                                    <th class="px-3 py-2 text-left w-8">
                                        <input type="checkbox"
                                            @if(count($yetkiSelectedUserIds) === $filtrelenmisKullanicilar->count() && $filtrelenmisKullanicilar->count() > 0) checked @endif
                                            wire:click="$set('yetkiSelectedUserIds', {{ count($yetkiSelectedUserIds) === $filtrelenmisKullanicilar->count() ? '[]' : $filtrelenmisKullanicilar->pluck('id')->toJson() }})"
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach($filtrelenmisKullanicilar as $kul)
                                    <tr class="hover:bg-white transition" wire:key="filter-user-{{ $kul->id }}">
                                        <td class="px-3 py-2">
                                            <input type="checkbox" wire:model.live="yetkiSelectedUserIds" value="{{ $kul->id }}" class="rounded border-gray-300 text-amber-600 focus:ring-amber-500">
                                        </td>
                                        <td class="px-3 py-2">
                                            <div class="flex items-center gap-2">
                                                <img src="{{ $kul->profile_photo_url }}" class="w-6 h-6 rounded-full object-cover" alt="">
                                                <span class="font-bold text-gray-700">{{ $kul->name }}</span>
                                            </div>
                                        </td>
                                        <td class="px-3 py-2 text-gray-400">{{ $kul->bolum->ad ?? '-' }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    ...
                    @if(count($yetkiSelectedUserIds) > 0)
                        <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-3">
                            <p class="text-xs font-bold text-amber-700">
                                <span class="bg-amber-500 text-white px-2 py-0.5 rounded-lg mr-1">{{ count($yetkiSelectedUserIds) }}</span>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                @if($yetkiBulkDataScope === 'specific_departments')
                                    <div>
                                        ...
                                        <div class="grid grid-cols-2 gap-1 max-h-32 overflow-y-auto custom-scrollbar bg-white rounded-xl p-2 border border-amber-200">
                                            @foreach($tumBolumler as $bolum)
                                                <label class="flex items-center gap-1 p-1 cursor-pointer hover:bg-amber-50 rounded">
                                                    <input type="checkbox" wire:model.live="yetkiBulkSpecificDeptIds" value="{{ $bolum->id }}" class="rounded border-gray-300 text-amber-600 w-3 h-3">
                                                    <span class="text-[10px] text-gray-600">{{ $bolum->ad }}</span>
                                                </label>
                                            @endforeach
                                        </div>
                                    </div>
                    ...
                    @endif
                @elseif($yetkiFilterRole && $filtrelenmisKullanicilar->count() === 0)
                    <div class="text-center py-4 text-gray-400 text-xs">
                        "{{ $yetkiFilterRole }}" rolünde eklenecek kullanıcı bulunamadı (tümü zaten yetkili olabilir).
                    </div>
                @endif
            </div>
