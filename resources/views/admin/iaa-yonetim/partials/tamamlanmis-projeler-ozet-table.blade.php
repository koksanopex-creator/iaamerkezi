                ...
                <tbody class="block sm:table-row-group">
                    @forelse ($iaas as $iaa)
                        <tr class="block mb-4 border bg-white border-gray-200 rounded-lg sm:table-row sm:mb-0 sm:border-0 sm:border-b sm:border-gray-100 hover:bg-gray-50">
                            <td class="p-4 align-middle font-semibold">{{ $iaa->baslik }} <br> <span class="text-xs text-gray-500 font-normal">{{ $iaa->atananTakim->ad ?? 'N/A' }}</span></td>
                            <td class="p-4 align-middle text-sm text-gray-600">{{ $iaa->iaaTalebi ? \Carbon\Carbon::parse($iaa->iaaTalebi->start_date)->format('d.m.Y') : '-' }}</td>
                            <td class="p-4 align-middle text-sm text-gray-600">{{ $iaa->onaylanma_tarihi ? \Carbon\Carbon::parse($iaa->onaylanma_tarihi)->format('d.m.Y') : '-' }}</td>
