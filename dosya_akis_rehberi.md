...
# BÖLÜM A: GLOBAL PROJE KURALLARI (RULES.md)

## A.1. ENVIRONMENT & DEPLOYMENT
...
- Hardcoded URL/path kullanmak YASAK: ❌ `/storage/...` ❌ `/admin/...` ❌ sabit string URL
- Her zaman kullan: ✅ `asset()` ✅ `url()` ✅ `route()` ✅ `Storage::url()`
- Amaç: ortam değiştiğinde linkler kırılmamalı

## A.2. RESPONSIVE DESIGN
...
## A.4. FILE UPLOAD STRUCTURE
- Zorunlu yol: `storage/{ana_klasor}/{kullanici_veya_firma}/{kayit_id}/{tarih_saat}_{random}.{ext}`
- Tarih formatı: `dd.MM.yyyy_HH.mm`
- Sona 2 karakter random eklenmeli, uzantı korunmalı

## A.5. USER & CUSTOMER PROFILE LINKING
- Kullanıcı ismi geçen her yerde clickable olmalı → `/kullanici-profil/{id}`
- Müşteri ismi geçen her yerde clickable olmalı → `/musteri-profil/{id}`

## A.6. DIRECTOR PERMISSIONS
- Direktör yetkisi: `bolumler` tablosundaki `director_id` üzerinden belirlenir.
- Sorumlu olduğu bölümlere ait HER şeyi görür, tüm hareketlerden haberdar olur.

## A.7. ROLE INTELLIGENCE RULE
- Yetki kontrolleri controller içinde `if` ile yapılmaz.
- User modeline eklenmeli: `isDirector()`, `isDepartmentLeader()`, `getResponsibleDepartments()`

## A.8. COMPLAINT vs IAA AYRIMI
- Tüm kayıtlar `iaas` tablosunda. `oneri` dolu → IAA, `oneri` boş → ŞİKAYET.

## A.9. QUERY & PERFORMANCE
- N+1 YASAK, her zaman `->with()` kullan. Büyük veri: pagination zorunlu. Filtreleme DB seviyesinde yapılmalı.

## A.10. LOGGING
- Her işlem loglanmalı: `user_id`, işlem, `entity_id`, tarih/saat.
- UI: son 10 kayıt, "Tümünü Gör" butonu, küçük scroll modal.

## A.11. BACKWARD COMPATIBILITY
- Mevcut kod silinmez, özellik kaldırılmaz. Sadece ekleme yapılır.

## A.12. AI BEHAVIOR & STRICT MODE
- Eksik kod YASAK. `// ...` YASAK. `sen tamamla` YASAK. Her feature tam ve çalışır halde teslim edilmeli.
