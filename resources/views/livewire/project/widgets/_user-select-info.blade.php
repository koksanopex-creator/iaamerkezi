...
<div class="space-y-3">
    <label for="widget-{{ $index }}-user" class="block text-lg font-semibold text-gray-800">
        {{ !empty($config['title']) ? $config['title'] : 'Kişi(ler)' }}
    ...
    <div wire:ignore class="opacity-0 transition-opacity duration-300" x-data x-init="
                ...
                $refs.selectEl.addEventListener('change', () => {
                    $wire.set('formData.{{ $index }}.info_user_ids', Array.from($refs.selectEl.selectedOptions).map(option => option.value));
        ...
        <select x-ref="selectEl"
            ...
            @foreach($users as $user)
                ...
                <option value="{{ $user->id }}" 
                        data-custom-properties='{{ $customProps }}'
                        @if(in_array($user->id, $selectedIds)) selected @endif>
                    {{ $user->name }}
                </option>
            @endforeach
        </select>
    </div>
    @error("formData.{$index}.info_user_ids") <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
</div>