<div x-data="{ 
...
}" wire:poll.3s class="<?php echo e($inModal ? '' : 'mt-12 border-t border-gray-200 pt-8'); ?>">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
            ...
            <div>
                ...
                <div class="flex items-center flex-wrap gap-x-3 gap-y-1 mt-1">
                    <p class="text-slate-500 text-sm font-bold flex items-center gap-1.5">
                        ...
                        İlgili Personel: <span class="text-slate-900 font-black"><?php echo e($case->user->name); ?></span>
                    </p>
                    <span class="hidden md:inline text-slate-300">•</span>
                    <!--[if BLOCK]><![endif]--><?php if($case->durum == 'Karar Verildi' || $case->durum == 'Dosya Kapatıldı'): ?>
        ...
        <!--[if BLOCK]><![endif]--><?php if(Auth::user()->hasAnyRole(['Superadmin', 'Disiplin Kurulu Başkanı', 'Disiplin Kurulu Üyesi', 'Hukuk Admini', 'Hukuk Yöneticisi'])): ?>
            <div class="flex flex-wrap md:flex-nowrap items-center justify-start md:justify-end gap-2 w-full md:w-auto shrink-0">
                <div class="flex items-center bg-slate-100/50 p-1 rounded-2xl border border-slate-200">
                    <button wire:click="togglePresentationMode" class="group flex items-center justify-center gap-2 px-3 md:px-4 py-2.5 <?php echo e($presentationMode ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-white text-slate-600 hover:bg-slate-50'); ?> rounded-xl transition-all">
                        <svg class="w-4 h-4 <?php echo e($presentationMode ? 'text-white' : 'text-indigo-500'); ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
    ...
    <!--[if BLOCK]><![endif]--><?php if($case->oylama_notu): ?>
        <div class="mb-10">
            <div class="bg-indigo-50/50 border border-indigo-100 rounded-[2rem] p-6 shadow-sm flex items-start gap-5 ring-1 ring-white">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-indigo-800/80 text-sm font-bold leading-relaxed italic">"<?php echo e($case->oylama_notu); ?>"</p>
                </div>
            </div>
        </div>
    <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-3">
            <!--[if BLOCK]><![endif]--><?php if($case->durum == 'Kurulda'): ?>
                <!--[if BLOCK]><![endif]--><?php if(!$case->oylama_aktif): ?>
            ...
            <?php else: ?>
                <div class="bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 p-10 text-center text-slate-300">
                    ...
                    <?php if(Auth::user()->hasAnyRole(['Superadmin', 'Disiplin Kurulu Başkanı', 'Hukuk Admini', 'Hukuk Yöneticisi']) && $case->durum === 'Karar Verildi'): ?>
        ...
        <div class="lg:col-span-6 space-y-8">
            ...
            <div class="bg-white rounded-[2rem] p-7 border border-slate-100 shadow-xl shadow-slate-200/30 flex justify-between items-center relative overflow-hidden group">
                ...
                <div class="flex-1">
                    ...
                    <div class="flex items-center gap-3">
                        <!--[if BLOCK]><![endif]--><?php if($canSeeResults): ?>
                            <?php
                            ...
                            ?>
                            <div class="flex items-center gap-3">
                                <span class="px-4 py-1.5 <?php echo e($statusLabel['bg']); ?> <?php echo e($statusLabel['text']); ?> <?php echo e($statusLabel['border']); ?> border-2 rounded-2xl text-xl font-black uppercase tracking-tighter shadow-sm">
                                    <?php echo e($leaderText); ?>

                                </span>
                                <!--[if BLOCK]><![endif]--><?php if($votesUsed > 0 && $leaderText != 'Eşitlik' && $leaderText != 'Bekleniyor'): ?>
                                    <span class="text-sm font-black text-slate-300">(<?php echo e($max); ?>/<?php echo e($votesUsed); ?> Oy)</span>
                                <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                            </div>
                        <?php else: ?>
                            <div class="flex items-center gap-2">
                ...
                <div class="text-right">
                    ...
                    <div class="flex flex-col items-end">
                        <span class="text-3xl font-black text-slate-900 tracking-tight leading-none mb-1">
                            <?php echo e($votesUsed); ?><span class="text-slate-300 text-lg mx-1">/</span><?php echo e($totalMembersCount); ?>

                        </span>
                        <span class="text-[11px] font-black text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-lg border border-indigo-100">
                            %<?php echo e($participationRate); ?> Katılım
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/20 border border-slate-50 p-8">
                <div class="flex items-center justify-between mb-8">
                    ...
                    <span class="px-3 py-1 bg-slate-50 text-slate-400 text-[10px] font-black rounded-xl border border-slate-100"><?php echo e($votesUsed); ?> Görüş Kayıtlı</span>
                </div>

                <div class="space-y-5 max-h-[600px] overflow-y-auto pr-3 custom-scrollbar">
                    <!--[if BLOCK]><![endif]--><?php $__empty_1 = true; $__currentLoopData = $caseVotes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $oy): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            ...
                            <div class="flex items-start gap-5">
                                ...
                                <div class="flex-1">
                                    <div class="flex items-center justify-between mb-1">
                                        <span class="text-base font-black text-slate-900 tracking-tight"><?php echo e($oy->user->name); ?></span>
                                        <!--[if BLOCK]><![endif]--><?php if($canSeeResults): ?>
                                            <span class="text-[10px] font-black px-3 py-1.5 rounded-xl <?php echo e($oy->oy_yonu == 'Ceza Verilsin' ? 'bg-rose-50 text-rose-600 border border-rose-100' : ($oy->oy_yonu == 'Ceza Verilmesin' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-amber-50 text-amber-600 border border-amber-100')); ?>">
                                    ...
                                    <div class="flex items-center gap-2 mb-4">
                                        ...
                                        <span class="text-[10px] font-bold text-slate-300 tracking-tight"><?php echo e($oy->created_at->format('d.m.Y H:i')); ?></span>
                                    </div>
                                    
                                    <!--[if BLOCK]><![endif]--><?php if($oy->yorum): ?>
            ...
            <?php if(Auth::user()->hasAnyRole(['Superadmin', 'Disiplin Kurulu Başkanı', 'Hukuk Admini']) && $case->durum === 'Kurulda'): ?>
                    ...
                    <div class="relative z-10">
                        ...
                        <div class="mb-4 pb-4 border-b border-slate-200">
                            ...
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <button wire:click="toggleManualIntervention" class="flex items-center gap-2 px-4 py-2 <?php echo e($manualInterventionMode ? 'bg-amber-100 text-amber-700 border-amber-200' : 'bg-white text-slate-500 border-slate-200 shadow-sm'); ?> border rounded-xl text-[9px] font-black uppercase tracking-widest transition-all">
                                    Manuel Müdahale: <?php echo e($manualInterventionMode ? 'Açık' : 'Kapalı'); ?>
                            ...
                            <div class="bg-indigo-600 rounded-[1.5rem] p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xl shadow-indigo-100/50 border border-indigo-500/50">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <span class="block text-base font-black text-white tracking-tight uppercase leading-none"><?php echo e($case->sistem_oneri_ceza); ?></span>
                                    </div>
                                </div>
                                <!--[if BLOCK]><![endif]--><?php if($manualInterventionMode): ?>
                                    <div class="flex-1 max-w-sm">
                                        <select wire:model="selectedManualPenalty" class="w-full bg-white/10 border-2 border-white/20 text-white text-[10px] font-black py-2 px-3 rounded-lg focus:ring-amber-500 focus:border-amber-500 appearance-none shadow-sm backdrop-blur-md">
                                            <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $penaltyScale; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <option value="<?php echo e($p->ceza_adi); ?>" class="text-slate-800"><?php echo e($p->ceza_adi); ?> (<?php echo e($p->min_puan); ?>-<?php echo e($p->max_puan); ?> Puan)</option>
        ...
        <div class="lg:col-span-3 space-y-8">
            ...
            <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/20 border border-slate-50 p-8">
                ...
                <!--[if BLOCK]><![endif]--><?php if($votesUsed > 0): ?>
                    <!--[if BLOCK]><![endif]--><?php if($canSeeResults): ?>
                        <div class="space-y-8">
                            ...
                            <div class="group">
                                <div class="flex justify-between text-xs mb-3">
                                    ...
                                    <span class="font-black text-rose-600 bg-rose-50 px-2 py-0.5 rounded-lg"><?php echo e($votesPenalty); ?> Oy (%<?php echo e(number_format($percPenalty, 0)); ?>)</span>
                                </div>
                                <div class="w-full bg-slate-50 rounded-full h-3 relative overflow-hidden shadow-inner border border-slate-100">
                                    <div class="absolute inset-y-0 left-0 bg-gradient-to-r from-rose-400 to-rose-600 rounded-full shadow-lg transition-all duration-1000" style="width: <?php echo e($percPenalty); ?>%"></div>
                                </div>
                            </div>
                            ...
                            <div class="group">
                                <div class="flex justify-between text-xs mb-3">
                                    ...
                                    <span class="font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-lg"><?php echo e($votesNoPenalty); ?> Oy (%<?php echo e(number_format($percNoPenalty, 0)); ?>)</span>
                                </div>
                                <div class="w-full bg-slate-50 rounded-full h-3 relative overflow-hidden shadow-inner border border-slate-100">
                                    <div class="absolute inset-y-0 left-0 bg-gradient-to-r from-emerald-400 to-emerald-600 rounded-full shadow-lg transition-all duration-1000" style="width: <?php echo e($percNoPenalty); ?>%"></div>
                                </div>
                            </div>
                            ...
                            <div class="group">
                                <div class="flex justify-between text-xs mb-3">
                                    ...
                                    <span class="font-black text-amber-600 bg-amber-50 px-2 py-0.5 rounded-lg"><?php echo e($votesInvestigation); ?> Oy (%<?php echo e(number_format($percInvestigation, 0)); ?>)</span>
                                </div>
                                <div class="w-full bg-slate-50 rounded-full h-3 relative overflow-hidden shadow-inner border border-slate-100">
                                    <div class="absolute inset-y-0 left-0 bg-gradient-to-r from-amber-300 to-amber-500 rounded-full shadow-lg transition-all duration-1000" style="width: <?php echo e($percInvestigation); ?>%"></div>
                                </div>
                            </div>
                        </div>
                    <?php else: ?>
            ...
            <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/20 border border-slate-50 p-8">
                <div class="flex justify-between items-center mb-8">
                    ...
                    <!--[if BLOCK]><![endif]--><?php if($case->durum === 'Karar Verildi'): ?>
                    ...
                    <?php elseif($waitingVotes > 0): ?>
                        <span class="bg-amber-50 text-amber-700 text-[9px] font-black px-3 py-1 rounded-full border border-amber-100 animate-pulse tracking-tighter uppercase"><?php echo e($waitingVotes); ?> BEKLENİYOR</span>
                    <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                </div>
                <div class="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                    <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $allCouncilMembers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $member): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php $voted = in_array($member->id, $votedUserIds); ?>
                        <div class="group flex items-center justify-between p-3 rounded-2xl <?php echo e($voted ? 'bg-emerald-50/30 border-emerald-100' : 'bg-slate-50/50 border-slate-50'); ?> border-2 transition-all duration-300">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex flex-col">
                                    <span class="text-xs font-black text-slate-800 tracking-tight"><?php echo e($member->name); ?></span>
                                    <!--[if BLOCK]><![endif]--><?php if($member->hasRole('Disiplin Kurulu Başkanı')): ?>
                                        <span class="text-[8px] font-black text-indigo-500 uppercase tracking-widest">BAŞKAN</span>
                                    <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                                </div>
                            </div>
                            <!--[if BLOCK]><![endif]--><?php if($voted): ?>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex items-center gap-4">
                            ...
                            <div class="flex-1">
                                ...
                                <span class="block text-sm font-bold text-slate-800 leading-tight"><?php echo e($case->user->name); ?></span>
                            </div>
                            <div class="text-right">
                                ...
                                <span class="inline-flex items-center gap-1 text-sm font-black text-slate-900 bg-white px-3 py-1 rounded-xl shadow-sm border border-slate-100">
                                    ...
                                    <?php echo e(number_format($case->user->toplam_puan, 0, ',', '.')); ?>
                        ...
                        <div class="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <span class="block text-sm font-bold text-slate-800"><?php echo e($case->olay_tarihi->format('d.m.Y')); ?></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-10">
                        <div class="flex items-center justify-between mb-4 px-1">
                            ...
                            <div class="text-[10px] font-bold text-slate-400 italic">
                                Raporlayan: <span class="text-slate-600"><?php echo e($case->reporter->name); ?></span> | Tarih: <?php echo e($case->created_at->format('d.m.Y H:i')); ?>

                            </div>
                        </div>
                        <div class="bg-white border-2 border-slate-50 rounded-3xl p-6 shadow-inner min-h-[150px]">
                            <p class="text-sm text-slate-700 font-bold leading-relaxed whitespace-pre-wrap">"<?php echo e($case->olay_aciklamasi); ?>"</p>
                        </div>
                    </div>

                    <div>
                        <h4 class="text-xs font-black text-slate-900 uppercase tracking-widest mb-4">Ekli Belgeler & Kanıtlar</h4>
                        <!--[if BLOCK]><![endif]--><?php if(!empty($case->kanit_dosyalari)): ?>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $case->kanit_dosyalari; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kanit): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <a href="<?php echo e(asset('storage/' . $kanit)); ?>" target="_blank" class="flex items-center gap-4 p-4 bg-slate-50 border border-slate-100 rounded-2xl hover:bg-white hover:shadow-lg transition-all group">
                                        ...
                                        <div class="flex-1 truncate">
                                            <span class="block text-xs font-black text-slate-700 truncate"><?php echo e(basename($kanit)); ?></span>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        <div class="bg-rose-50 rounded-2xl p-4 border-2 border-rose-100 text-center shadow-sm">
                            ...
                            <div class="flex items-center justify-center gap-1">
                                <span class="text-3xl font-black text-rose-900 tracking-tighter"><?php echo e($case->hesaplanan_puan); ?></span>
                        ...
                        <div class="bg-indigo-50 rounded-2xl p-4 border-2 border-indigo-100 text-center shadow-sm flex flex-col justify-center">
                            ...
                            <span class="text-xs font-black text-indigo-900 leading-tight uppercase"><?php echo e($case->sistem_oneri_ceza); ?></span>
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 gap-1.5">
                            ...
                            <div class="flex items-start gap-3 p-3 bg-white border border-slate-100 rounded-xl">
                                ...
                                <div class="flex-1">
                                    ...
                                    <span class="text-[10px] font-bold text-slate-700 leading-snug"><?php echo e($case->behavior->category->ad ?? 'Genel'); ?> - <?php echo e($case->behavior->tanim); ?></span>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-1.5">
                                ...
                                <div class="flex items-center justify-between p-2.5 bg-slate-50/50 border border-slate-100 rounded-xl">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-[10px] font-black text-slate-800 truncate max-w-[80px]"><?php echo e($case->impact->ad ?? 'Genel'); ?></span>
                                    </div>
                                    <span class="text-[10px] font-black text-rose-600">+<?php echo e($case->impact->puan ?? 0); ?></span>
                                </div>
                                ...
                                <div class="flex items-center justify-between p-2.5 bg-slate-50/50 border border-slate-100 rounded-xl">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-[10px] font-black text-slate-800 truncate max-w-[80px]"><?php echo e($case->scope->ad ?? 'Standart'); ?></span>
                                    </div>
                                    <span class="text-[10px] font-black text-rose-600">+<?php echo e($case->scope->puan ?? 0); ?></span>
                                </div>
                                ...
                                <div class="flex items-center justify-between p-2.5 bg-slate-50/50 border border-slate-100 rounded-xl">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-[10px] font-black text-slate-800"><?php echo e($case->tekrar_sayisi ?? 1); ?>. Kez</span>
                                    </div>
                                    <!--[if BLOCK]><![endif]--><?php if(($case->tekrar_sayisi ?? 1) > 1): ?>
                                        <span class="px-1.5 py-0.5 bg-rose-600 text-white text-[9px] font-black rounded-lg">x<?php echo e($case->tekrar_sayisi); ?></span>
                                    <?php else: ?>
    ...
    <div x-show="showDefenseModal" x-cloak class="fixed inset-0 z-[100] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showDefenseModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block align-bottom bg-white rounded-[2.5rem] text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full">
                ...
                <div class="px-8 py-10">
                    <!--[if BLOCK]><![endif]--><?php if($case->savunma_tarihi): ?>
                        <div class="bg-amber-50 rounded-2xl p-4 border-2 border-amber-100 mb-6 flex items-center justify-between shadow-sm">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <span class="text-sm font-black text-amber-900 tracking-tight"><?php echo e($case->savunma_tarihi->format('d.m.Y H:i')); ?></span>
                                </div>
                            </div>
                        ...
                        <div class="mb-8">
                            ...
                            <div class="bg-white border-2 border-slate-50 rounded-3xl p-6 shadow-inner relative overflow-hidden">
                                ...
                                <p class="text-sm text-slate-700 font-bold leading-relaxed whitespace-pre-wrap italic">"<?php echo e($mainContent); ?>"</p>
                                
                                <!--[if BLOCK]><![endif]--><?php if($noteContent): ?>
                        ...
                        <div>
                            ...
                            <!--[if BLOCK]><![endif]--><?php if(!empty($case->savunma_dosyalari)): ?>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $case->savunma_dosyalari; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sDosya): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <a href="<?php echo e(asset('storage/' . $sDosya)); ?>" target="_blank" class="flex items-center gap-4 p-4 bg-slate-50 border border-slate-100 rounded-2xl hover:bg-white hover:shadow-lg transition-all group">
                                            ...
                                            <div class="flex-1 truncate">
                                                <span class="block text-xs font-black text-slate-700 truncate"><?php echo e(basename($sDosya)); ?></span>
                                                <span class="block text-[9px] text-slate-400 font-bold uppercase">Görüntüle</span>
                                            </div>
                                        </a>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><!--[if ENDBLOCK]><![endif]-->
...
<script>
    window.handleResolveAction = function(type) {
        if (type === 'approve') {
            const manualMode = window.Livewire.find('<?php echo e($_instance->getId()); ?>').get('manualInterventionMode');
            const penalty = manualMode ? window.Livewire.find('<?php echo e($_instance->getId()); ?>').get('selectedManualPenalty') : '<?php echo e($case->sistem_oneri_ceza); ?>';
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.Livewire.find('<?php echo e($_instance->getId()); ?>').resolveCase('approve');
                }
            });
            return;
        }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.Livewire.find('<?php echo e($_instance->getId()); ?>').resolveCase(type);
            }
        });
    }
    ...
    window.handlePostponeAction = function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.Livewire.find('<?php echo e($_instance->getId()); ?>').postponeCase(result.value.date, result.value.location, result.value.reason);
            }
        });
    }
    ...
    window.handleRevertDecision = function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.Livewire.find('<?php echo e($_instance->getId()); ?>').revertDecision();
            }
        });
    }
</script>
<?php $__env->stopPush(); ?>
<?php /**PATH C:\Users\celal.karaman\Desktop\Projelerim\iaa_projesi\resources\views/livewire/admin/disiplin/disiplin-oylama-paneli.blade.php ENDPATH**/ ?>