...
>
    ...
    {{-- 1. SON GELEN ŞİKAYET KARTI --}}
    @php $last = $sonSikayetler->first(); @endphp
    @if($last)
        <div class="rounded-lg border border-red-200 bg-red-50/60 p-3 shadow-sm relative overflow-hidden group">
            ...
            <div class="flex justify-between items-center mb-2 relative z-10">
                ...
                <a href="{{ route('admin.sikayetler.show', $last->id) }}" class="text-[11px] font-bold text-indigo-600 hover:text-indigo-800 transition flex items-center gap-1">
    ...
    <div class="flex flex-col gap-2 border-b border-gray-200 pb-4">
        ...
        <div class="text-center mt-1">
             <span x-show="mode === 'month'" class="text-xs font-semibold text-gray-500 uppercase tracking-widest">
                {{ now()->locale('tr')->translatedFormat('F Y') }} Verileri
            </span>
    ...
    <div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            @foreach([
                ...
                <div @click="toggleTab('{{ $kpiItem['key'] }}')" 
                     ...
                     :class="activeTab === '{{ $kpiItem['key'] }}' ? 'ring-2 ring-{{ $kpiItem['color'] }}-500 ring-offset-2 bg-{{ $kpiItem['color'] }}-50' : 'hover:bg-gray-50'">
                    
                    <div class="absolute -right-2 -bottom-2 text-{{ $kpiItem['color'] }}-50 opacity-20 group-hover:opacity-40 transition-opacity transform group-hover:scale-110">
                        <svg class="w-16 h-16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $kpiItem['icon'] }}"/></svg>
                    </div>

                    <div class="relative z-10 flex flex-col items-center justify-center h-full">
                        <p class="text-[10px] font-bold text-{{ $kpiItem['color'] }}-500 uppercase tracking-widest mb-1">{{ $kpiItem['label'] }}</p>
                        <div class="flex items-baseline gap-1">
                            <p x-show="mode === 'month'" class="text-3xl font-black text-gray-800" x-transition.opacity>
                                {{ $kpiMonthly[$kpiItem['data_key'] ?? $kpiItem['key']] }}
                            </p>
                            <p x-show="mode === 'all'" class="text-3xl font-black text-gray-800" x-transition.opacity>
                                {{ $kpi[$kpiItem['data_key'] ?? $kpiItem['key']] }}
                            </p>
                            @if(isset($kpiItem['unit'])) <span class="text-xs font-bold text-gray-400">{{ $kpiItem['unit'] }}</span> @endif
                        </div>
                        
                        <div class="mt-2 transition-transform duration-300" :class="activeTab === '{{ $kpiItem['key'] }}' ? 'rotate-180 text-{{ $kpiItem['color'] }}-600' : 'text-gray-300'">
        ...
        >
            ...
            <div class="flex justify-between items-center mb-3 border-b pb-2" :class="'border-' + activeColor + '-200'">
                <h5 class="font-bold text-gray-800 flex items-center gap-2">
                    ...
                    {{-- DÜZELTME BURADA YAPILDI: activeTab ? ... : '' --}}
    ...
    {{-- 3. MÜŞTERİ MEMNUNİYET İSTATİSTİKLERİ --}}
    <div>
        <h4 class="font-bold text-gray-700 text-sm uppercase mb-3 flex items-center gap-2">
            <span class="w-1 h-4 bg-gray-800 rounded-full"></span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3 w-full">
            ...
            <div class="p-4 bg-green-50 border border-green-100 rounded-xl flex items-center justify-between shadow-sm hover:shadow-md transition group cursor-pointer">
                <div>
                    ...
                    <p class="text-2xl font-black text-green-700 group-hover:scale-110 transition-transform origin-left">{{ $musteriKararIstatistikleri['onay_orani'] }}</p>
                </div>
            ...
            <div class="p-4 bg-red-50 border border-red-100 rounded-xl flex items-center justify-between shadow-sm hover:shadow-md transition group cursor-pointer">
                <div>
                    ...
                    <p class="text-2xl font-black text-red-700 group-hover:scale-110 transition-transform origin-left">{{ $musteriKararIstatistikleri['red_orani'] }}</p>
                </div>
            ...
            <div class="p-4 bg-yellow-50 border border-yellow-100 rounded-xl flex items-center justify-between shadow-sm hover:shadow-md transition group cursor-pointer">
                <div>
                    ...
                    <p class="text-2xl font-black text-yellow-700 group-hover:scale-110 transition-transform origin-left">{{ $musteriKararIstatistikleri['revizyon'] }}</p>
                </div>
