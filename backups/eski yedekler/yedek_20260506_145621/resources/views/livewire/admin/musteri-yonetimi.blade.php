                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @forelse($selectedCustomer->users as $rep)
                                        <tr wire:key="rep-row-{{ $rep->id }}" class="transition-colors {{ $rep->trashed() ? 'bg-red-50' : 'hover:bg-gray-50' }}">
                                            @if($editingRepId === $rep->id)
                                                ...
                                                <td class="px-6 py-3 text-right whitespace-nowrap align-top">
                                                    <div class="flex justify-end space-x-2" wire:key="edit-actions-{{ $rep->id }}">
                                                        <button wire:click="updateRepresentative" wire:key="btn-save-{{ $rep->id }}" class="text-emerald-600 hover:text-emerald-800 p-2 rounded-lg hover:bg-emerald-50 transition-colors" title="Kaydet">
                                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                                        </button>
                                                        <button wire:click="cancelEditRepresentative" wire:key="btn-cancel-{{ $rep->id }}" class="text-gray-400 hover:text-gray-600 p-2 rounded-lg hover:bg-gray-100 transition-colors" title="İptal">
                                                    ...
                                                    <div class="flex justify-end space-x-2" wire:key="view-actions-{{ $rep->id }}">
                                                        ...
                                                        @if(!$rep->trashed())
                                                            ...
                                                            @if(!auth()->user()->hasRole('Müşteri Şikayeti Kurulu'))
                                                                <button wire:click="deleteRepresentative({{ $rep->id }})" wire:key="btn-delete-{{ $rep->id }}" wire:confirm="Bu yetkiliyi silmek istediğinize emin misiniz?" class="text-rose-600 hover:text-rose-900 p-2 rounded-lg hover:bg-rose-50 transition-colors" title="Sil">
