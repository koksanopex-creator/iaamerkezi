...
<head>
    ...
    <title>İyileştirme Önerisi Yap - <?php echo e(config('app.name', 'Laravel')); ?></title>
...
<body class="antialiased">
    ...
    <div class="relative container mx-auto max-w-4xl py-12 px-4 z-10">
        ...
        <div class="text-center mb-10 floating-animation">
            <a href="<?php echo e(url('/')); ?>"><img src="<?php echo e(asset('storage/logos/2mIKZO0DYbIDjSJdjfN1IpO7jkTqEcSOh886xYH5.png')); ?>" class="h-16 sm:h-20 w-auto mx-auto mb-6 drop-shadow-lg" alt="Köksan Logo"></a>
        ...
        <?php if(session('success')): ?><div class="mb-8 bg-green-100 border-l-4 border-green-500 text-green-800 p-5 rounded-r-lg shadow-lg" role="alert"><p class="font-bold text-lg">🎉 Harika!</p><p class="mt-1"><?php echo e(session('success')); ?></p></div><?php endif; ?>
        <?php if($errors->any()): ?><div class="mb-8 bg-red-100 border-l-4 border-red-500 text-red-800 p-5 rounded-r-lg shadow-lg" role="alert"><p class="font-bold text-lg">⚠️ Lütfen aşağıdaki hataları düzeltin:</p><ul class="mt-3 list-disc list-inside space-y-1"><?php $__currentLoopData = $errors->all(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $error): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><li><?php echo e($error); ?></li><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></ul></div><?php endif; ?>

        <div class="bg-white/90 backdrop-blur-xl rounded-3xl p-6 sm:p-10 shadow-2xl border border-gray-200/80">
            <form x-data="fileUploader()" action="<?php echo e(route('guest.iaa.store')); ?>" method="POST" enctype="multipart/form-data">
                <?php echo csrf_field(); ?>
                ...
                <div class="space-y-10">
                    ...
                    <div class="bg-white/50 p-6 rounded-2xl border border-gray-200/80">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div class="input-group"><label for="guest_name" class="block text-sm font-semibold text-gray-800 mb-2">Adınız Soyadınız <span class="text-red-500">*</span></label><input type="text" name="guest_name" id="guest_name" value="<?php echo e(old('guest_name')); ?>" required class="block w-full px-4 py-3 rounded-xl shadow-sm transition-all duration-300"></div>
                            <div class="input-group"><label for="guest_email" class="block text-sm font-semibold text-gray-800 mb-2">E-posta Adresiniz</label><input type="email" name="guest_email" id="guest_email" value="<?php echo e(old('guest_email')); ?>" class="block w-full px-4 py-3 rounded-xl shadow-sm transition-all duration-300"></div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white/50 p-6 rounded-2xl border border-gray-200/80">
                         ...
                         <div class="space-y-6">
                             <div class="input-group"><label for="baslik" class="block text-sm font-semibold text-gray-800 mb-2">Konu / Başlık <span class="text-red-500">*</span></label><input type="text" name="baslik" id="baslik" value="<?php echo e(old('baslik')); ?>" required class="block w-full px-4 py-3 rounded-xl shadow-sm transition-all duration-300"></div>
                             <div class="input-group"><label for="ilgili_alan" class="block text-sm font-semibold text-gray-800 mb-2">İlgili Alan / Departman <span class="text-red-500">*</span></label><input type="text" name="ilgili_alan" id="ilgili_alan" value="<?php echo e(old('ilgili_alan')); ?>" required placeholder="Örn: Üretim, Lojistik..." class="block w-full px-4 py-3 rounded-xl shadow-sm transition-all duration-300"></div>
                             <div class="input-group"><label for="mevcut_durum" class="block text-sm font-semibold text-gray-800 mb-2">Mevcut Durum / Problem Tanımı <span class="text-red-500">*</span></label><textarea name="mevcut_durum" id="mevcut_durum" rows="5" required class="block w-full px-4 py-3 rounded-xl shadow-sm resize-none transition-all duration-300"><?php echo e(old('mevcut_durum')); ?></textarea></div>
                             <div class="input-group"><label for="oneri" class="block text-sm font-semibold text-gray-800 mb-2">İyileştirme Öneriniz</label><textarea name="oneri" id="oneri" rows="5" class="block w-full px-4 py-3 rounded-xl shadow-sm resize-none transition-all duration-300"><?php echo e(old('oneri')); ?></textarea></div>
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-6 border-t border-gray-200/80">
                                <div class="input-group sm:col-span-2"><label for="oneren_kazanc_miktar" class="block text-sm font-semibold text-gray-800 mb-2">Tahmini Kazanç</label><input type="number" name="oneren_kazanc_miktar" id="oneren_kazanc_miktar" value="<?php echo e(old('oneren_kazanc_miktar')); ?>" placeholder="Örn: 5000" class="block w-full px-4 py-3 rounded-xl shadow-sm transition-all duration-300"></div>
                                <div class="input-group"><label for="oneren_kazanc_birim" class="block text-sm font-semibold text-gray-800 mb-2">Para Birimi</label><select name="oneren_kazanc_birim" id="oneren_kazanc_birim" class="block w-full px-4 py-3 rounded-xl shadow-sm transition-all duration-300 bg-white"><option value="TL" <?php if(old('oneren_kazanc_birim') == 'TL'): echo 'selected'; endif; ?>>TL</option><option value="USD" <?php if(old('oneren_kazanc_birim') == 'USD'): echo 'selected'; endif; ?>>USD</option><option value="EUR" <?php if(old('oneren_kazanc_birim') == 'EUR'): echo 'selected'; endif; ?>>EUR</option></select></div>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-6">
                                <div class="input-group sm:col-span-2"><label for="oneren_butce_miktar" class="block text-sm font-semibold text-gray-800 mb-2">Tahmini Bütçe</label><input type="number" name="oneren_butce_miktar" id="oneren_butce_miktar" value="<?php echo e(old('oneren_butce_miktar')); ?>" placeholder="Örn: 1000" class="block w-full px-4 py-3 rounded-xl shadow-sm transition-all duration-300"></div>
                                <div class="input-group"><label for="oneren_butce_birim" class="block text-sm font-semibold text-gray-800 mb-2">Para Birimi</label><select name="oneren_butce_birim" id="oneren_butce_birim" class="block w-full px-4 py-3 rounded-xl shadow-sm transition-all duration-300 bg-white"><option value="TL" <?php if(old('oneren_butce_birim') == 'TL'): echo 'selected'; endif; ?>>TL</option><option value="USD" <?php if(old('oneren_butce_birim') == 'USD'): echo 'selected'; endif; ?>>USD</option><option value="EUR" <?php if(old('oneren_butce_birim') == 'EUR'): echo 'selected'; endif; ?>>EUR</option></select></div>
                            </div>
                        </div>
                    </div>
...
</html><?php /**PATH C:\Users\celal.karaman\Desktop\Projelerim\iaa_projesi\resources\views/guest/iaa/create.blade.php ENDPATH**/ ?>