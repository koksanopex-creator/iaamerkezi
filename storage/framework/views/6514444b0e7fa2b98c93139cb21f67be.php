...
<?php if($case->status == 'kapatildi'): ?>
    <div class="bg-red-50 border-l-4 border-red-500 p-4 mb-6">
        <div class="flex">
            ...
            <div class="ml-3"><p class="text-sm text-red-700"><strong>Dosya Kapatıldı:</strong> Bu dosya kapatıldığı için artık yeni belge yüklenemez.</p></div>
        </div>
    </div>
<?php elseif(auth()->id() == $case->created_by && $case->status == 'odeme_bekliyor' && !auth()->user()->can('arabuluculuk.approve_legal')): ?>
...
<?php else: ?>
    <div class="bg-gray-50 p-4 rounded-lg mb-6 border border-gray-200">
        ...
        <form action="<?php echo e(route('admin.arabuluculuk.uploadFile', $case->id)); ?>" method="POST" enctype="multipart/form-data" class="flex gap-4 items-end">
            <?php echo csrf_field(); ?>
            <div class="flex-1">
                ...
                <select name="doc_type" class="w-full border-gray-300 rounded text-sm">
                    <?php if($case->status == 'taslak' && (auth()->id() == $case->created_by || auth()->user()->hasRole('Superadmin') || auth()->user()->can('arabuluculuk.approve_legal'))): ?>
                    ...
                    <?php endif; ?>
                    <?php if($case->status != 'taslak'): ?> 
                        <?php if(auth()->user()->can('arabuluculuk.approve_legal') || auth()->user()->can('arabuluculuk.assign_mediator') || auth()->user()->hasRole('Superadmin')): ?>
...
<?php if($case->files->count() > 0): ?>
    <div class="grid grid-cols-1 gap-3">
        <?php $__currentLoopData = $case->files; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $file): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="flex justify-between items-center p-3 border rounded hover:bg-gray-50 <?php echo e($file->locked ? 'bg-red-50 border-red-200' : ''); ?>">
                <div class="flex items-center gap-3">
                    <?php if(Str::endsWith($file->dosya_yolu, '.pdf')): ?>
                    ...
                    <?php elseif(Str::endsWith($file->dosya_yolu, ['.doc', '.docx'])): ?>
                    ...
                    <div>
                        <p class="font-bold text-sm text-gray-800">
                            <?php echo e($file->orijinal_adi); ?> 
                            <span class="text-xs text-gray-500 bg-gray-200 px-1 rounded ml-1"><?php echo e($file->doc_type); ?></span>
                        </p>
                        <p class="text-xs text-gray-500"><?php echo e($file->created_at->format('d.m.Y H:i')); ?> - <?php echo e($file->uploader->name ?? 'Sistem'); ?></p>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                    <?php if($file->locked): ?>
                    ...
                    <?php endif; ?>
                    <a href="<?php echo e(asset('storage/' . $file->dosya_yolu)); ?>" target="_blank" class="text-sm font-bold text-blue-600 hover:underline">İndir</a>
                    <?php if(auth()->user()->hasRole('Superadmin') || (auth()->id() == $case->created_by && $case->status == 'taslak') || (auth()->id() == $file->uploaded_by && $case->status != 'kapatildi')): ?>
                        <form action="<?php echo e(route('admin.arabuluculuk.deleteFile', ['file' => $file->id])); ?>" method="POST" onsubmit="return confirm('Silmek istediğinize emin misiniz?');">
...
<?php endif; ?><?php /**PATH C:\Users\celal.karaman\Desktop\Projelerim\iaa_projesi\resources\views/admin/arabuluculuk/parcalar/sekme-dosyalar.blade.php ENDPATH**/ ?>