...
class ProjeTamamlamaService
{
    ...
    public function completeWithReturn(Request $request, $id)
    {
        ...
        DB::transaction(function () use ($request, $iaa) {
            ...
            // 3. Log
            $logEylemi = $isQualityManager ? 'Bölüm Onaylandı (İadeli - Direktör Onayına Sevk)' : 'Bölüm Onayına Gönderildi (İadeli)';
            $logAciklama = $isQualityManager 
                ? Auth::user()->name . " projeyi tamamlayarak onayladı ve iade bilgileriyle direktör onayına sundu."
                : Auth::user()->name . " projeyi tamamladı ve iade bilgileriyle onaya sundu.";
