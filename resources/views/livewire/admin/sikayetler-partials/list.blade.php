            ...
            <tbody class="divide-y divide-gray-100">
                @forelse ($sikayetler as $index => $sikayet)
                    ...
                    <tr wire:key="list-{{ $sikayet->id }}" class="hover:bg-indigo-50/30 transition-colors group {{ $isCustomerEntry ? 'bg-red-50/30' : '' }} {{ $isSelected ? 'bg-indigo-50' : '' }}">
                        ...
                        {{-- 7. İşlemler --}}
                        <td class="px-4 py-4 text-right space-x-0.5 whitespace-nowrap">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                @if(!$sikayet->customer_id)
                                    ...
                                    @if($canAssign)
                                        <button onclick="Livewire.dispatch('openMusteriAtamaModal', { sikayetId: {{ $sikayet->id }} })"
                                            class="p-1.5 rounded-lg text-emerald-600 hover:bg-emerald-50 transition-colors" title="Müşteri Ata">
                                ...
                                @role('Superadmin|Müşteri Şikayeti Kurulu|Müşteri Şikayeti Kurulu - Yurt İçi|Müşteri Şikayeti Kurulu - Yurt Dışı|Müşteri Şikayeti Kurulu Yöneticisi|Müşteri Şikayeti Kurulu Yöneticisi - Yurt İçi|Müşteri Şikayeti Kurulu Yöneticisi - Yurt Dışı')
                                    <button x-data="{ loading: false }"
                                        ...
                                        :disabled="loading"
                                        :class="loading ? 'opacity-50 cursor-wait bg-amber-50' : 'hover:bg-amber-50'"
                                        class="p-1.5 rounded-lg text-amber-600 transition-colors" title="Yönet">
                                        ...
                                        <svg x-show="loading" style="display: none;" class="animate-spin w-4 h-4 text-amber-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg>
                                    </button>
                                @endrole

                                {{-- 3. DETAY --}}
                                <a href="{{ route('admin.sikayetler.show', $sikayet) }}" 
                                    class="inline-flex items-center gap-1 p-1.5 rounded-lg text-blue-600 hover:bg-blue-50 transition-colors" title="Detay">
                                ...
                                @can('update', $sikayet)
                                    <a href="{{ route('admin.sikayetler.edit', $sikayet) }}"
                                        class="p-1.5 rounded-lg text-indigo-600 hover:bg-indigo-50 transition-colors" title="Düzenle">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                @if($sikayet->iaaProjesi)
                                    <a href="{{ route('proje.workspace.show', $sikayet->iaaProjesi->id) }}" target="_blank"
                                        class="p-1.5 rounded-lg text-purple-600 hover:bg-purple-50 transition-colors" title="Projeye Git">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
