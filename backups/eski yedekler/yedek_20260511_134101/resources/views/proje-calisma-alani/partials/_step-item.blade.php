        ...
        @else
                    ...
                    @if($isLeader && !$isCompleted && $canEdit)
                        <form action="{{ route('proje.workspace.assignUserToStep', ['iaa' => $iaa->id, 'step' => $step->id]) }}" method="POST">
                            ...
                            <select name="user_id" onchange="this.form.submit()" class="appearance-none bg-white border border-gray-300 text-gray-700 text-xs rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 block w-40 py-1.5 pl-3 pr-8 cursor-pointer hover:border-gray-400 transition-colors">
                                    ...
                                    @php $gosterilenIds = []; @endphp
                                    {{-- 1. Proje Ekibi (Squad) Üyeleri --}}
                                    @foreach($iaa->projeEkibi as $ekipUyesi)
                                        @if($ekipUyesi->pivot->durum == 'onaylandi')
                                            ...
                                            @php $gosterilenIds[] = $ekipUyesi->id; @endphp
                                        @endif
                                    @endforeach
                                    {{-- 2. Takım Üyeleri (Squad'da olmayanlar) --}}
                                    @if($takim)
                                        @foreach($takim->users as $takimUyesi)
                                            @if(!in_array($takimUyesi->id, $gosterilenIds))
                                                <option value="{{ $takimUyesi->id }}" {{ ($sorumluUser && $sorumluUser->id == $takimUyesi->id) ? 'selected' : '' }}>
                                                    {{ $takimUyesi->name }}{{ $takimUyesi->id == $takim->lider_user_id ? ' (Lider)' : '' }}
                                                </option>
                                                @php $gosterilenIds[] = $takimUyesi->id; @endphp
                                            @endif
                                        @endforeach
                                    @endif
                                    {{-- 3. Lider (henüz listede yoksa) --}}
                                    @if($takim && $takim->lider && !in_array($takim->lider_user_id, $gosterilenIds))
            ...
            @if($isCompleted && $progressUpdate)
                @php
                    ...
                    $stepAssignment = $stepAssignments[$step->id] ?? null;
                    $isAssignedToSomeoneElse = false;
                    
                    if ($isSafeIaa && $stepAssignment && auth()->check()) {
                        $isAssignedToMe = ($stepAssignment->user_id == auth()->id());
            ...
            @if($isCurrent)
                ...
                @if($canEdit && !$isCustomerView && !$isAssignedToSomeoneElse)
                ...
                @else
                    {{-- İZLEYİCİ (Yetkisiz Personel veya Başkasına Atanmış Adım) --}}
                    @if(!$isCustomerView)
                                ...
                                <div class="ml-3">
                                    <p class="text-sm text-blue-700">
                                        ...
                                        @if($isAssignedToSomeoneElse)
                                            Bu adım <strong>{{ \App\Models\User::find($stepAssignment->user_id)->name }}</strong> kullanıcısına atanmıştır. Sadece sorumlu kişi veya lider müdahale edebilir.
                                        @else
