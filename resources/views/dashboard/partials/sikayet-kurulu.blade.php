...
<div class="bg-white rounded-2xl shadow-lg overflow-hidden mb-8 border border-gray-200/50">
                ...
                <li class="hover:bg-gray-50/70 transition-colors duration-150">
                    <a href="{{ route('admin.sikayetler.show', $sikayet) }}" class="flex items-center justify-between p-4 sm:p-6 space-x-4">
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-semibold text-indigo-700 truncate" title="{{ $sikayet->musteri_sikayet_konusu }}">{{ $sikayet->musteri_sikayet_konusu }}</p>
                            <p class="text-sm text-gray-500 flex flex-wrap items-center gap-x-3 gap-y-1 mt-1">
                                <span class="inline-flex items-center gap-1.5"><svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg><span>{{ $sikayet->musteri_adi }}</span></span>
                            </p>
                        </div>
    ...
    <div class="bg-gray-50/70 px-6 py-4 border-t border-gray-200/70 text-center">
        <a href="{{ route('admin.sikayetler.index') }}" class="text-sm font-semibold text-indigo-600 hover:text-indigo-800 transition-colors">Tüm Şikayetleri Yönet →</a>
    </div>
</div>