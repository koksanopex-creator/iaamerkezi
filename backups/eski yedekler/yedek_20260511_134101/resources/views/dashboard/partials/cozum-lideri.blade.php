                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex items-center gap-2 flex-wrap mt-2">
                                    ...
                                    @if($sikayet->iaaProjesi)
                                        @if(str_contains($sikayet->iaaProjesi->durum, 'talep'))
                                            <span class="text-[10px] font-black bg-purple-600 text-white px-2 py-0.5 rounded shadow-sm flex items-center gap-1">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                                TALEP SÜRECİ
                                            </span>
                                        @elseif(str_contains($sikayet->iaaProjesi->durum, 'hatali_bildirim'))
                                            <span class="text-[10px] font-black bg-orange-500 text-white px-2 py-0.5 rounded shadow-sm flex items-center gap-1">
                                    ...
                                    {{-- Kimin Onayı Bekleniyor Bilgisi --}}
                                                                        @php
                                        $bekleyenKisi = "Sistem";
                                        $beklemeRengi = "text-purple-600 bg-purple-50 border-purple-200";
                                        if ($sikayet->iaaProjesi && str_contains($sikayet->iaaProjesi->durum, 'bekliyor')) {
                                            $bekleyenKisi = $sikayet->iaaProjesi->aktif_asama_metni ?? 'Bölüm / Yönetici Onayı';
                                        } else {
                                            $bekleyenKisi = $sikayet->musteri_durum;
                                        }
                                        $gecenGunTutari = ceil($sikayet->updated_at->diffInHours(now()) / 24);
                                        $gecenGun = $gecenGunTutari == 0 ? 1 : $gecenGunTutari; // Bugün ise 1. gün dolsun.
                                    @endphp
                                    <span class="text-[11px] font-bold {{ $beklemeRengi }} border px-2 py-0.5 rounded-md flex items-center gap-1" title="Son İşlem: {{ $sikayet->updated_at->format('d.m.Y H:i') }}">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse(isset($stats['ziyaretler']) ? $stats['ziyaretler'] : [] as $index => $ziyaret)
                            <tr class="hover:bg-indigo-50/30 transition-colors toggle-row {{ $index >= 5 ? 'hidden' : '' }}" data-index="{{ $index }}">
                                <td class="px-6 py-4">
                                    <div class="font-bold text-gray-800">{{ $ziyaret->iaa->iaa_name ?? '-' }}</div>
                                    <div class="text-[11px] text-gray-500">{{ $ziyaret->iaa->musteriSikayetleri->first()->customer->firma_adi ?? '-' }}</div>
                                </td>
                                <td class="px-6 py-4">
                                    @if($ziyaret->visitor)
                                        <div class="flex items-center gap-2">
