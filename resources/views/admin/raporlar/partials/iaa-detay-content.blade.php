...
<div class="space-y-4 text-sm max-h-[70vh] overflow-y-auto pr-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div class="bg-gray-50 p-3 rounded-lg border border-gray-200">
            ...
            <p class="font-medium text-gray-900">
                ...
                @else
                    {{ $iaa->guest_name }} <span class="text-xs bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full">Misafir</span>
                @endif
            </p>
        </div>
        
        {{-- YENİ: SORUMLU BÖLÜM ALANI --}}
        <div class="bg-blue-50 p-3 rounded-lg border border-blue-100">
            ...
            <p class="font-bold text-blue-900 text-lg">
                {{ $iaa->bolum->ad ?? $iaa->ilgili_alan ?? 'Genel' }}
            </p>
        </div>
    </div>

    {{-- YENİ: EĞER ATANDI İSE TAKIM BİLGİSİ --}}
    @if($iaa->atananTakim)
        <div class="bg-indigo-50 p-4 rounded-lg border border-indigo-100 mt-2">
            <div class="flex items-center gap-2 mb-2">
                ...
                <h4 class="font-bold text-indigo-900">Atanan Çözüm Takımı: {{ $iaa->atananTakim->ad }}</h4>
            </div>
    ...
    <div class="grid grid-cols-2 gap-4 text-xs text-gray-500 mt-2">
        ...
        @if($iaa->puan)
            <p class="text-right"><span class="bg-yellow-100 text-yellow-800 px-2 py-1 rounded font-bold">Hesaplanan Puan: {{ $iaa->puan }}</span></p>
        @endif
    </div>

    <div class="mt-4 p-4 bg-gray-50 rounded-md border">
        <h4 class="font-semibold mb-2 text-gray-700">Mevcut Durum:</h4>
        <p class="prose prose-sm max-w-none text-gray-600">{!! nl2br(e($iaa->mevcut_durum)) !!}</p>
    </div>

    <div class="mt-4 p-4 bg-green-50 rounded-md border border-green-100">
        <h4 class="font-semibold mb-2 text-green-800">Öneri:</h4>
        <p class="prose prose-sm max-w-none text-gray-700">{!! nl2br(e($iaa->oneri)) !!}</p>
    </div>
    ...
    <div class="mt-4">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
            @foreach($iaa->resimler as $resim)
                <a href="{{ asset('storage/' . $resim->dosya_yolu) }}" target="_blank" class="block group relative overflow-hidden rounded-lg shadow-sm border border-gray-200">
                    <img src="{{ asset('storage/' . $resim->dosya_yolu) }}" alt="İAA Resmi" class="object-cover w-full h-28 transform group-hover:scale-110 transition-transform duration-500">
    ...
    <div class="mt-8 pt-6 border-t border-gray-200 flex flex-col sm:flex-row justify-end gap-3 no-print">
        ...
        {{-- 1. ÖNERİ DETAYINA GİT (HER ZAMAN GÖRÜNÜR) --}}
        <a href="{{ url('iyilestirme/' . $iaa->id) }}" target="_blank" class="inline-flex items-center justify-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150">
            <svg class="w-4 h-4 mr-2 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
            Öneri Detayına Git
        </a>

        {{-- 2. PROJEYE GİT (SADECE ATANDI veya TAMAMLANDI İSE) --}}
        @if(in_array($iaa->durum, ['Atandı', 'Tamamlandı', 'Revize Ediliyor', 'Yönetici Onayı Bekliyor']))
            <a href="{{ url('proje-calisma-alani/' . $iaa->id) }}" target="_blank" class="inline-flex items-center justify-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
