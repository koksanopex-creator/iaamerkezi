            ...
            <div class="flex items-center">
                ...
                <div class="ml-4 pr-8">
                    <p class="text-sm text-blue-800 leading-relaxed">
                        <span class="font-bold text-blue-900">{{ auth()->user()->observers->pluck('name')->join(', ') }}</span> 
                        ...
                        <a href="{{ route('profile.edit') }}" class="inline-flex items-center font-black underline ml-2 hover:text-blue-600 transition-colors group/link">
                            Ayarları Yönet
...
<div class="grid grid-cols-1 {{ isset($isSidebar) && $isSidebar ? '' : 'md:grid-cols-2 xl:grid-cols-3' }} gap-6 mb-8">
                ...
                <div class="space-y-2">
                    @forelse($stats['son_iaa_projelerim'] as $proje)
                        <div class="flex justify-between items-center text-sm">
                            ...
                            <span class="text-gray-500 text-xs bg-gray-100 px-2 py-1 rounded-md whitespace-nowrap">
                                {{ optional($proje->onaya_gonderilme_tarihi ?? $proje->onaylanma_tarihi)->format('d.m.Y') ?? '-' }}
                            </span>
                        </div>
                    @empty
...
@include('dashboard.partials.top5-performers', ['isSidebar' => $isSidebar ?? false])

{{-- DOĞUM GÜNLERİ --}}
@include('dashboard.partials.birthdays', ['isSidebar' => $isSidebar ?? false])