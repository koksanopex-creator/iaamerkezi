...
class KullaniciPuanService
{
    ...
    public function calculateScoreInRange(User $user, $startDate = null, $endDate = null)
    {
        ...
        $squadProjeIds = $user->gorevliOlduguProjeler()->wherePivot('durum', 'onaylandi')->pluck('iaas.id');

        $projeQuery = Iaa::where('durum', 'Tamamlandı')
            ->where('puan', '>', 0)
            ->where(function ($query) use ($takimIdleri, $user, $squadProjeIds) {
                $query->where('tamamlayan_lider_id', $user->id) // 1. LİDER (SABİTLENMİŞ)
                    ->orWhereIn('id', $squadProjeIds)          // 2. SQUAD / DONDURULMUŞ ÜYELER
                    ->orWhere(function ($sub) use ($takimIdleri) {
                        // 3. PASİF TAKIM PUANI (Sadece Öneriler)
                        // Önemli: Eğer kişi tamamlayan lider değilse ve squad'da yoksa, 
                        // sadece takımı üzerinden puan alabilir.
