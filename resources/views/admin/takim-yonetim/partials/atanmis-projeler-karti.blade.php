<div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border border-gray-200">
    <div class="p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">Takımın Mevcut Projeleri ({{ $takim->atanmisProjeler->count() }})</h3>
        
        <div class="space-y-3">
            @forelse ($takim->atananProjeler as $proje)
            <a href="{{ route('proje.workspace.show', $proje) }}" class="block bg-gray-50 p-4 rounded-xl border border-gray-200 hover:bg-indigo-50 hover:border-indigo-200 transition-colors">
                <p class="font-bold text-gray-800">{{ $proje->baslik }}</p>
                    <p class="font-bold text-gray-800">{{ $proje->baslik }}</p>
                    <div class="mt-2 flex items-center justify-between text-sm text-gray-600">
                        <span class="font-semibold px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-800">
                            {{ $proje->durum }}
                        </span>
                        {{-- Bitiş Tarihini göstermek için pivot tablodan veri çekmemiz gerekecek. Şimdilik bu kısmı basitleştiriyoruz --}}
                    </div>
            </a>
            @empty
