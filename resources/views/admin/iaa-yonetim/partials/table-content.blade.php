<div class="bg-gradient-to-br from-{{ $color }}-50 via-white to-{{ $color }}-100 overflow-hidden shadow-xl sm:rounded-2xl border border-{{ $color }}-200">
    
    <div class="p-6 sm:p-8 text-gray-900">
        
        {{-- Üst Başlık ve Toplam Sayı --}}
                    ...
                    @forelse ($iaas as $iaa)
                        <tr class="block mb-4 border bg-white border-gray-200 rounded-lg sm:table-row sm:mb-0 sm:border-0 sm:border-b sm:border-gray-100 hover:bg-{{ $color }}-50 transition-colors group">
                            ...
                            <td class="p-4 align-middle text-center sm:table-cell">
                                <span class="text-lg font-bold text-gray-300 group-hover:text-{{ $color }}-400 transition-colors">
                                    {{ $loop->iteration }}
                                </span>
                            </td>
                            ...
                            <td class="flex justify-between items-start p-3 border-t sm:border-0 sm:table-cell sm:p-4 align-middle">
                                ...
                                <div class="text-right sm:text-left w-full">
                                    <p class="text-gray-800 font-medium group-hover:text-{{ $color }}-700 transition-colors inline-flex items-center flex-wrap gap-2">
                            ...
                            {{-- ÖNEREN ALANI (GÜNCELLENEN KISIM - TIKLANABİLİR YAPILDI) --}}
                            @if(in_array($type, ['onay', 'havuz', 'reddedilmis']))
                                <td class="flex justify-between items-center p-3 border-t sm:border-0 sm:table-cell sm:p-4 align-middle">
                                    <span class="font-semibold text-sm text-gray-500 sm:hidden">Öneren:</span>
                                    <div class="text-right sm:text-left">
                                        @if ($iaa->gonderen)
                                            {{-- BURASI DEĞİŞTİ: DIV YERİNE A KULLANILDI --}}
                                            <a href="{{ route('profile.show', $iaa->gonderen->id) }}" class="inline-flex items-center space-x-2 group/user hover:opacity-80 transition-opacity" title="Profili Görüntüle">
                                                <div class="w-8 h-8 bg-white border border-gray-200 text-{{ $color }}-600 rounded-full flex items-center justify-center shadow-sm group-hover/user:bg-{{ $color }}-50 group-hover/user:border-{{ $color }}-300 transition-colors">
                                                    <span class="text-xs font-bold">{{ substr($iaa->gonderen->name, 0, 1) }}</span>
                                                </div>
                                                <span class="text-sm font-medium text-gray-700 group-hover/user:text-{{ $color }}-700 group-hover/user:underline decoration-{{ $color }}-300 transition-colors">{{ $iaa->gonderen->name }}</span>
                                            </a>
                                        @else
                                            <div class="inline-flex items-center space-x-2">
                                                ...
                                                <div>
                                                    <span class="text-sm font-medium text-gray-700 block">{{ $iaa->guest_name }}</span>
                                        ...
                                        @if($iaa->atananTakim)
                                             <div class="inline-flex items-center space-x-2">
                                                <div class="w-8 h-8 bg-white border border-gray-200 text-green-600 rounded-full flex items-center justify-center shadow-sm">
                                                    <span class="text-xs font-bold">{{ Str::substr($iaa->atananTakim->ad, 0, 1) }}</span>
                                                </div>
                            ...
                            <td class="flex justify-between items-center p-3 border-t sm:border-0 sm:table-cell sm:p-4 align-middle">
                                ...
                                <span class="text-right sm:text-left text-sm text-gray-600 tabular-nums font-medium">
                                    ...
                                    @if($type === 'havuz') {{ $iaa->onaylanma_tarihi ? \Carbon\Carbon::parse($iaa->onaylanma_tarihi)->format('d.m.Y') : '-' }} @endif
                                    @if($type === 'reddedilmis') {{ $iaa->updated_at->format('d.m.Y') }} @endif
                                </span>
                            </td>
                            
                            {{-- İŞLEMLER ALANI (BUTONLAR) --}}
                            <td class="p-3 border-t sm:border-0 sm:table-cell sm:p-4 align-middle">
                                <div class="flex flex-col sm:flex-row sm:justify-end sm:space-x-2 space-y-2 sm:space-y-0">
                                    @include('admin.iaa-yonetim.partials.actions', ['type' => $type, 'iaa' => $iaa])
                                </div>
                            </td>
                        </tr>
                    @empty
