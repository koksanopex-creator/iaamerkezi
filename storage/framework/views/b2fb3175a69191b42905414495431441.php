...
<div x-data="{ activeKategori: 'all' }" class="space-y-8 animate-fade-in-up">
    ...
    <?php if(isset($stats['onay_bekleyen_ziyaretler']) && count($stats['onay_bekleyen_ziyaretler']) > 0): ?>
        <div x-data="{ showZiyaretAlert: true }" x-show="showZiyaretAlert" x-transition class="bg-indigo-50 border-l-4 border-indigo-500 p-4 rounded-md shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 relative">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    <p class="text-sm text-indigo-800">
                        Onayınızı bekleyen <span class="font-bold"><?php echo e(count($stats['onay_bekleyen_ziyaretler'])); ?> adet</span> müşteri ziyareti planı bulunmaktadır. Lütfen inceleyip onaylayınız.
                    </p>
                </div>
            </div>
            <div class="flex items-center gap-4 shrink-0 mt-2 sm:mt-0">
                <a href="#bolum-ziyaretleri-tablosu" class="text-sm font-bold text-indigo-700 hover:text-indigo-900 underline bg-indigo-100/50 px-3 py-1 rounded-md transition-colors">
                    İncele &rarr;
                </a>
                <button @click="showZiyaretAlert = false" class="text-indigo-400 hover:text-indigo-600 focus:outline-none bg-transparent hover:bg-indigo-100 p-1.5 rounded-full transition-colors" title="Kapat">
    ...
    <div class="bg-white/70 backdrop-blur-md rounded-2xl p-4 border border-gray-100 shadow-sm">
        <div class="flex items-center gap-3 flex-wrap">
            ...
            <a href="#bolum-ziyaretleri-tablosu"
               class="relative inline-flex items-center gap-1.5 px-3 py-1.5 bg-teal-50 text-teal-700 rounded-lg text-xs font-bold border border-teal-100 hover:bg-teal-100 hover:shadow-sm transition-all group">
                <?php if(isset($stats['onay_bekleyen_ziyaretler']) && count($stats['onay_bekleyen_ziyaretler']) > 0): ?>
                    <span class="absolute -top-1.5 -right-1.5 flex h-3.5 w-3.5">
                        <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-red-400 opacity-75"></span>
                        <span class="relative inline-flex rounded-full h-3.5 w-3.5 bg-red-500 border-2 border-white shadow-sm"></span>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <a href="<?php echo e(route('admin.sikayetler.index', ['tab' => 'onay_bekleyenler'])); ?>"
           class="group relative bg-gradient-to-br from-purple-600 to-indigo-700 rounded-2xl p-5 shadow-lg hover:shadow-2xl transform hover:-translate-y-1 transition-all duration-300 overflow-hidden">
            ...
            <div class="relative z-10 text-white">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <?php if(($stats['bolum_onay_sayisi'] ?? 0) > 0): ?>
                        <span class="bg-white/20 px-2.5 py-0.5 rounded-full text-[10px] font-bold animate-pulse"><?php echo e($stats['bolum_onay_sayisi']); ?> Bekleyen</span>
                    <?php endif; ?>
                </div>
                <p class="text-indigo-100 text-xs font-medium">Onayınızı Bekleyen Projeler</p>
                <h3 class="text-3xl font-bold mt-1"><?php echo e($stats['bolum_onay_sayisi'] ?? 0); ?></h3>
        ...
        <a href="<?php echo e(route('admin.sikayetler.index', ['tab' => 'islemde'])); ?>"
           class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md hover:border-yellow-200 transition-all duration-300 group">
            ...
            <h3 class="text-2xl font-bold text-gray-800"><?php echo e($stats['islemdeki_sikayet'] ?? 0); ?></h3>
            ...
            <div class="mt-3 w-full bg-gray-100 rounded-full h-1">
                <?php
                    $oran = ($stats['toplam_sikayet'] ?? 0) > 0
                        ...
                        : 0;
                ?>
                <div class="bg-yellow-400 h-1 rounded-full transition-all duration-500" style="width: <?php echo e($oran); ?>%"></div>
            </div>
        </a>

        
        <a href="<?php echo e(route('admin.sikayetler.index', ['tab' => 'cozulmus'])); ?>"
           class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-md hover:border-green-200 transition-all duration-300 group">
            ...
            <h3 class="text-2xl font-bold text-gray-800"><?php echo e($stats['cozulen_sikayet'] ?? 0); ?></h3>
            <p class="text-green-600 text-[11px] mt-0.5 font-medium flex items-center">
                <?php if(($stats['toplam_sikayet'] ?? 0) > 0): ?>
                    <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path></svg>
                    %<?php echo e(round((($stats['cozulen_sikayet'] ?? 0) / ($stats['toplam_sikayet'] ?? 1)) * 100)); ?> Başarı
                <?php else: ?>
                    %0 Başarı
                <?php endif; ?>
            </p>
    ...
    <div id="sorumlu-alanlar-bolumu" class="scroll-mt-24 bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
        ...
        <div class="px-6 py-6 border-b border-gray-100 bg-gradient-to-br from-indigo-600 to-purple-700 relative overflow-hidden">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
                <div class="flex-1">
                    <div class="flex items-center gap-4 mb-2">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-indigo-100 font-medium opacity-80 uppercase tracking-widest">Kategori bazlı şikayet ve proje istatistikleri • <?php echo e($yonetilenKategoriler->count()); ?> Kategori</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm w-full lg:w-auto">
                    <form method="GET" action="<?php echo e(url()->current()); ?>" class="flex flex-col lg:flex-row items-stretch lg:items-center gap-4">
                        <div class="flex items-center gap-2 flex-1">
                            <input type="date" name="areas_start_date" value="<?php echo e(request('areas_start_date')); ?>" class="w-full text-xs border-gray-200 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 py-2.5 px-3 shadow-inner bg-gray-50/50">
                            <span class="text-gray-300">-</span>
                            <input type="date" name="areas_end_date" value="<?php echo e(request('areas_end_date')); ?>" class="w-full text-xs border-gray-200 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 py-2.5 px-3 shadow-inner bg-gray-50/50">
                        </div>
                        <div class="flex items-center gap-2">
                            ...
                            <?php if(request('areas_start_date') || request('areas_end_date')): ?>
                                <a href="<?php echo e(url()->current()); ?>" class="p-2.5 bg-gray-50 text-gray-400 hover:text-indigo-500 hover:bg-indigo-50 rounded-lg transition-colors border border-gray-100 flex items-center justify-center" title="Filtreyi Temizle">
        ...
        <div class="p-6 lg:p-8">
            ...
            <div class="bg-gray-100/50 p-1.5 rounded-2xl border border-gray-100 flex flex-wrap gap-1 mb-6">
                ...
                <?php $__currentLoopData = $yonetilenKategoriler; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <button @click="activeKategori = 'kat-<?php echo e($kat->id); ?>'"
                            :class="activeKategori === 'kat-<?php echo e($kat->id); ?>' ? 'bg-white text-indigo-600 shadow-md border-gray-100 scale-105' : 'text-gray-500 hover:text-gray-700 hover:bg-white/50 border-transparent'"
                            class="px-4 py-2.5 rounded-xl text-xs font-black transition-all border duration-300">
                        <?php echo e($kat->ad); ?>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <?php $__currentLoopData = $yonetilenKategoriler; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $kat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    ...
                    ?>
                    <div x-show="activeKategori === 'all' || activeKategori === 'kat-<?php echo e($kat->id); ?>'"
                        ...
                        <div class="relative z-10">
                            ...
                            <div class="flex items-center justify-between mb-6">
                                <div class="flex items-center gap-3">
                                    ...
                                    <h5 class="font-black text-gray-900 group-hover/card:text-indigo-600 transition-colors uppercase tracking-wide text-base lg:text-lg"><?php echo e($kat->ad); ?></h5>
                                </div>
                                <a href="<?php echo e(route('admin.sikayetler.index', ['filtreKategori' => [$kat->id]])); ?>" class="p-2.5 bg-gray-50 text-gray-400 rounded-xl group-hover/card:bg-indigo-600 group-hover/card:text-white transition-all shadow-sm hover:scale-110" title="Tüm şikayetleri göster">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <a href="<?php echo e(route('admin.sikayetler.index', ['filtreKategori' => [$kat->id]])); ?>"
                                   class="bg-gray-50/50 rounded-xl p-4 border border-gray-100/50 hover:bg-white hover:shadow-lg hover:border-blue-200 transition-all duration-300 block cursor-pointer">
                                    ...
                                    <p class="text-2xl lg:text-3xl font-black text-gray-900"><?php echo e($katStats['toplam']); ?></p>
                                </a>
                                <a href="<?php echo e(route('admin.sikayetler.index', ['filtreKategori' => [$kat->id], 'tab' => 'islemde'])); ?>"
                                   class="bg-amber-50/30 rounded-xl p-4 border border-amber-100/30 hover:bg-white hover:shadow-lg hover:border-amber-200 transition-all duration-300 block cursor-pointer">
                                    ...
                                    <p class="text-2xl lg:text-3xl font-black text-amber-800"><?php echo e($katStats['islemde']); ?></p>
                                </a>
                                <a href="<?php echo e(route('admin.sikayetler.index', ['filtreKategori' => [$kat->id], 'tab' => 'onay_bekleyenler'])); ?>"
                                   class="bg-purple-50/30 rounded-xl p-4 border border-purple-100/30 hover:bg-white hover:shadow-lg hover:border-purple-200 transition-all duration-300 block cursor-pointer">
                                    ...
                                    <p class="text-2xl lg:text-3xl font-black text-purple-900"><?php echo e($katStats['onay_bekleyen']); ?></p>
                                </a>
                                <a href="<?php echo e(route('admin.sikayetler.index', ['filtreKategori' => [$kat->id], 'tab' => 'cozulmus'])); ?>"
                                   class="bg-green-50/30 rounded-xl p-4 border border-green-100/30 hover:bg-white hover:shadow-lg hover:border-green-200 transition-all duration-300 block cursor-pointer">
                                    ...
                                    <p class="text-2xl lg:text-3xl font-black text-green-900"><?php echo e($katStats['cozulen']); ?></p>
                                </a>
                            </div>
                            ...
                            <div class="mt-6 pt-5 border-t border-gray-100 flex items-center justify-end">
                                <div class="flex items-center gap-2">
                                    <div class="text-right">
                                        ...
                                        <p class="text-sm font-black text-indigo-600 leading-none">%<?php echo e($katStats['toplam'] > 0 ? round(($katStats['cozulen'] / $katStats['toplam']) * 100) : 0); ?></p>
                                    </div>
    ...
    <div id="listeler-bolumu" class="scroll-mt-24 grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
            <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                ...
                <a href="<?php echo e(route('admin.sikayetler.index', ['tab' => 'onay_bekleyenler'])); ?>" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-purple-600 text-white rounded-lg text-xs font-bold hover:bg-purple-700 transition-all shadow-sm">
            ...
            <div class="flex-1 overflow-y-auto max-h-[500px]">
                <?php if(($stats['onay_bekleyen_liste'] ?? collect())->isNotEmpty()): ?>
                                ...
                                <tr class="hover:bg-purple-50/50 transition-colors group">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="min-w-0">
                                                <div class="flex items-center gap-2">
                                                    <p class="font-bold text-gray-800 group-hover:text-purple-700 transition-colors truncate"><?php echo e(Str::limit($proje->baslik, 45)); ?></p>
                                                ...
                                                <p class="text-[10px] text-gray-400 font-bold flex items-center gap-1.5 mt-1.5 flex-wrap">
                                                    <span class="flex items-center gap-1 bg-gray-50 px-1.5 py-0.5 rounded border border-gray-100">
                                                        ...
                                                        <?php echo e($proje->atananTakim->ad ?? 'Takım Yok'); ?>

                                                    </span>
                                                    <?php if(isset($proje->atananTakim->lider)): ?>
                                                        <span class="text-gray-200">•</span>
                                                        <a href="<?php echo e(route('profile.show', $proje->atananTakim->lider->id)); ?>" class="flex items-center gap-1 text-gray-500 hover:text-purple-600 transition-colors">
                                                            ...
                                                            <?php echo e($proje->atananTakim->lider->name); ?>
                                                    ...
                                                    <span class="flex items-center gap-1">
                                                        ...
                                                        <?php echo e($proje->updated_at->diffForHumans()); ?>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
            ...
            <div class="flex-1 overflow-y-auto max-h-[500px] p-4 space-y-3">
                <?php if(($stats['son_departman_sikayetleri'] ?? collect())->isNotEmpty()): ?>
                        ...
                        <div class="rounded-xl border border-gray-200 hover:border-blue-300 hover:shadow-md bg-white transition-all duration-300 group/item overflow-hidden">
                            ...
                            <div class="px-4 pt-4 pb-3">
                                <div class="flex items-start justify-between gap-3 mb-2.5">
                                    <a href="<?php echo e($baslikLink); ?>" class="text-sm font-bold text-gray-800 hover:text-blue-600 transition-colors leading-snug flex-1">
                                        <?php echo e(Str::limit($sikayet->musteri_sikayet_konusu, 70)); ?>
                                ...
                                <div class="flex items-center gap-2 flex-wrap">
                                    ...
                                    <?php if($sikayet->sikayetKategori): ?>
                                        <a href="<?php echo e(route('admin.sikayetler.index', ['filtreKategori' => [$sikayet->sikayet_kategorisi_id]])); ?>"
                                           class="px-2 py-0.5 rounded text-[10px] font-bold transition-all hover:scale-105 hover:shadow-sm <?php echo e($isSorumlu ? 'bg-blue-600 text-white shadow-sm hover:bg-blue-700' : 'bg-gray-100 text-gray-600 border border-gray-200 hover:bg-gray-200'); ?>">
                            ...
                            <div class="px-4 py-2.5 bg-gray-50/70 border-t border-gray-100 flex items-center justify-between gap-3 flex-wrap">
                                ...
                                <?php if($sikayet->customer_id): ?>
                                    <a href="<?php echo e(route('musteri.profil.show', $sikayet->customer_id)); ?>"
                                       class="flex items-center gap-1.5 text-[11px] font-bold text-blue-700 hover:text-blue-900 transition-colors group/customer">
                                        ...
                                        <span class="group-hover/customer:underline"><?php echo e(Str::limit($sikayet->musteri_adi, 25)); ?></span>
                                ...
                                <?php else: ?>
                                    <span class="flex items-center gap-1.5 text-[11px] font-bold text-blue-700">
                                        <svg class="w-3.5 h-3.5 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                                        <?php echo e(Str::limit($sikayet->musteri_adi, 25)); ?>
                                ...
                                <div class="flex items-center gap-2 text-[11px] font-bold text-gray-500">
                                    ...
                                    <span><?php echo e($sikayet->cozumTakimi->ad ?? 'Atanmadı'); ?></span>
                                    <?php if(isset($sikayet->cozumTakimi->lider)): ?>
                                        <span class="text-gray-300">|</span>
                                        <a href="<?php echo e(route('profile.show', $sikayet->cozumTakimi->lider->id)); ?>"
                            ...
                            <?php if($workflow && $workflow['toplam'] > 0): ?>
                                ...
                                ?>
                                <div class="px-4 py-2.5 border-t border-gray-100 <?php echo e($progressBg); ?>">
                                    <div class="flex justify-between items-center mb-1.5">
                                        <div class="flex flex-col">
                                            <span class="text-[9px] uppercase font-bold text-gray-400 leading-none mb-0.5">Şu Anki Aşama</span>
                                            <span class="text-[11px] font-bold <?php echo e($progressText); ?> leading-none"><?php echo e($aktifAsamaMetni); ?></span>
                                        </div>
                                        <span class="text-[10px] font-black text-gray-500"><?php echo e($workflow['tamamlanan']); ?>/<?php echo e($workflow['toplam']); ?> Adım</span>
                                    </div>
                                    <div class="w-full <?php echo e($progressBarBg); ?> rounded-full h-1.5 overflow-hidden">
                                        <div class="<?php echo e($progressBarFill); ?> h-1.5 rounded-full transition-all duration-500" style="width: <?php echo e($workflow['yuzde']); ?>%"></div>
                                    </div>
                                </div>
                            <?php endif; ?>
                        </div>
    ...
    <?php if(isset($iadeVerileri) && count($iadeVerileri) > 0): ?>
        <div id="iade-tablosu-bolumu" class="scroll-mt-24" x-data="{ iadeShowAll: false }">
            <?php echo $__env->make('dashboard.partials.iadeler-tablosu', ['iadeLimit' => 5], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        </div>
    <?php endif; ?>
...
$__html = app('livewire')->mount($__name, $__params, 'dept-visit-table-kalite-'.Auth::id(), $__slots ?? [], get_defined_vars());
...
<?php /**PATH C:\Users\celal.karaman\Desktop\Projelerim\iaa_projesi\resources\views/dashboard/partials/bolum-yoneticisi.blade.php ENDPATH**/ ?>