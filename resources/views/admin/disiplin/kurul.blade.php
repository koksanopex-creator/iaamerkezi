<x-app-layout>
    ...
    <div class="py-6 space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" x-data="{ 
        activeTab: {{ json_encode($activeTab ?: request('activeTab', 'overview')) }},
        meetingSearch: '',
        meetingStatusFilter: '',
                                            ...
                                            <div class="mt-1 space-y-1">
                                                @foreach($iMeeting->kararMaddeleri->take(2) as $m)
                                                    <p class="text-[9px] font-bold text-slate-500 line-clamp-1 flex items-center gap-1">
                                                        ...
                                                        <span class="truncate">{{ $m->icerik }}</span>
                                                    </p>
                                                @endforeach
                                            </div>
                                        @else
                                            <p class="text-[9px] font-black text-purple-400 uppercase tracking-tighter truncate mt-0.5">{{ $karar->behavior->tanim ?? 'Disiplin Vukuatı' }}</p>
                                        @endif
                    ...
                    <div class="flex items-center justify-between px-2">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div class="px-3 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-black uppercase tracking-widest rounded-full ring-1 ring-indigo-100">
                                {{ $paginatedGenelBakis->total() }} Toplam Kayıt
                            </div>
                            
                            {{-- Filtreler (Tablo üstünde) --}}
                        ...
                        <div class="overflow-x-auto custom-scrollbar">
                            <table class="w-full text-left border-collapse min-w-[1000px]">
                                ...
                                <tbody class="divide-y divide-gray-50">
                                    @forelse($paginatedGenelBakis as $t)
                                        <tr class="hover:bg-indigo-50/20 transition-all group" 
                                            x-show="(meetingSearch === '' || {{ json_encode(strtolower($t->baslik)) }}.includes(meetingSearch.toLowerCase())) && (meetingStatusFilter === '' || '{{ $t->durum }}' === meetingStatusFilter)">
                                            ...
                                            <td class="px-5 py-6 whitespace-nowrap">
                                                @if($t->baslangic_tarihi && $t->durum !== 'gündem_bekliyor')
                                                    <div class="text-gray-800 font-black">{{ \Carbon\Carbon::parse($t->baslangic_tarihi)->translatedFormat('d F Y') }}</div>
                                                    <div class="text-[11px] text-gray-400 font-bold mt-0.5">{{ \Carbon\Carbon::parse($t->baslangic_tarihi)->format('H:i') }}</div>
                                                @else
                                            ...
                                            <td class="px-5 py-6">
                                                <div class="flex items-center gap-2">
                                                    @if($t->olusturan)
                                                        ...
                                                        <span class="text-[11px] font-black text-gray-700">{{ $t->olusturan->name }}</span>
                                                    @else
                                            ...
                                            <td class="px-5 py-6">
                                                <div class="flex -space-x-3 hover:-space-x-1 transition-all duration-300">
                                                    @forelse($t->katilimcilar->take(5) as $kat)
                                                        @if($kat->user)
                                                            <img src="{{ $kat->user->profile_photo_url }}" class="w-8 h-8 rounded-full border-2 border-white shadow-sm ring-1 ring-gray-100" title="{{ $kat->user->name }}">
                                                        @endif
                                                    @empty
                                            ...
                                            <td class="px-5 py-6 whitespace-nowrap">
                                                ...
                                                @endphp
                                                <span class="px-3 py-1 rounded-full text-[9px] font-black tracking-widest ring-1 {{ $statusClasses }}">
                                                    {{ $statusLabel }}
                                                </span>
                                            </td>
                                            <td class="px-5 py-6 text-right">
                                                @if($t->type === 'meeting')
                                                    <a href="{{ route('admin.disiplin.kurul.toplanti.show', $t->original_id) }}" class="inline-flex items-center justify-center w-10 h-10 rounded-2xl bg-gray-50 text-gray-400 hover:bg-indigo-600 hover:text-white transition-all shadow-sm group/btn">
                                                ...
                                                @else
                                                    <a href="{{ route('admin.disiplin.show', $t->original_id) }}" class="inline-flex items-center justify-center w-10 h-10 rounded-2xl bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white transition-all shadow-sm group/btn">
                            ...
                            <tbody class="divide-y divide-gray-50">
                                @forelse($paginatedTakvim as $t)
                                    <tr class="hover:bg-indigo-50/20 transition-all group"
                                        ...
                                        <td class="px-5 py-6">
                                            <div class="font-black text-gray-800 text-base group-hover:text-indigo-600 transition-colors">{{ $t->baslik }}</div>
                                            <div class="text-[10px] text-gray-400 font-bold mt-1 uppercase tracking-tighter">{{ $t->tur }} | {{ $t->yer ?: 'Lokasyon Belirtilmedi' }}</div>
                                        </td>
                                        <td class="px-5 py-6 whitespace-nowrap">
                                        ...
                                        <td class="px-5 py-6">
                                            <div class="flex items-center gap-1.5 flex-wrap">
                                                @forelse($t->disiplinDosyalari as $dosya)
                                                    <span class="px-2 py-0.5 bg-gray-50 text-gray-500 rounded text-[10px] font-bold border border-gray-100">#{{ $dosya->id }}</span>
                                                @empty
                                        ...
                                        <td class="px-5 py-6">
                                            ...
                                            @endphp
                                            <span class="px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-[0.1em] ring-1 {{ $color }}">
                                                {{ $t->durum }}
                                            </span>
                                        </td>
                                        <td class="px-5 py-6 text-right">
                                            <div class="flex items-center justify-end gap-2">
                                                <a href="{{ route('admin.disiplin.kurul.toplanti.show', $t->original_id) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-black text-white hover:bg-indigo-600 transition-all">
            ...
            <div x-show="activeTab === 'archive'" x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 translate-y-8" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-8">
                <div class="flex items-center justify-between px-2">
                    ...
                    <div class="px-4 py-1.5 bg-purple-50 text-purple-600 text-[10px] font-black uppercase tracking-widest rounded-full ring-1 ring-purple-100">
                        {{ $arsivdekiDosyalar->total() }} Toplam Kayıt
                    </div>
                </div>
                            ...
                            <tbody class="divide-y divide-gray-50">
                                @forelse($arsivdekiDosyalar as $case)
                                    <tr class="hover:bg-purple-50/20 transition-all group">
                                        ...
                                        <td class="px-6 py-6">
                                            <div class="text-xs font-black text-gray-700 leading-relaxed">{{ $case->behavior_text }}</div>
                                            @if($case->subject)
    ...
    <x-modal name="plan-meeting-modal" :show="false" focusable>
        <form action="{{ route('admin.disiplin.kurul.toplanti.store') }}" method="POST" class="bg-white rounded-3xl overflow-hidden" x-data="{ modalTab: 'kurul' }">
            ...
            <div class="max-h-[80vh] overflow-y-auto divide-y divide-gray-100">
                ...
                {{-- SATIR 3: İlgili Disiplin Dosyaları --}}
                <div class="px-8 py-5">
                    ...
                    <div class="max-h-40 overflow-y-auto bg-gray-50 border border-gray-200 rounded-xl p-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
                        @foreach($secilebilirDosyalar as $case)
                            <label class="flex items-start gap-3 p-3 bg-white rounded-lg border border-gray-100 cursor-pointer hover:border-emerald-300 transition-colors group">
                                <input type="checkbox" name="disiplin_dosyalari[]" value="{{ $case->id }}" class="mt-1 w-4 h-4 rounded text-emerald-600 border-gray-300 focus:ring-emerald-500">
                                <div class="flex-1">
                                    <div class="text-sm font-bold text-gray-800 group-hover:text-emerald-700 transition-colors">{{ $case->user->name }}</div>
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="text-[10px] text-gray-500 font-mono">#{{ $case->id }}</span>
                                        <span class="px-1.5 py-0.5 rounded bg-gray-100 text-gray-600 text-[9px] font-black uppercase">{{ $case->durum }}</span>
                                    </div>
                                </div>
                            </label>
                        @endforeach
            ...
            <form action="{{ route('admin.disiplin.kurul.toplanti.update', $t) }}" method="POST" class="bg-white rounded-3xl overflow-hidden" x-data="{ modalTab: 'kurul' }">
                ...
                <div class="divide-y divide-gray-100">
                        ...
                        <div class="max-h-40 overflow-y-auto bg-gray-50 border border-gray-200 rounded-xl p-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
                            @foreach($secilebilirDosyalar as $case)
                                <label class="flex items-start gap-3 p-3 bg-white rounded-lg border border-gray-100 cursor-pointer hover:border-indigo-300 transition-colors group">
                                    <input type="checkbox" name="disiplin_dosyalari[]" value="{{ $case->id }}" {{ $t->disiplinDosyalari->contains($case->id) ? 'checked' : '' }} class="mt-1 w-4 h-4 rounded text-indigo-600 border-gray-300 focus:ring-indigo-500">
                                    <div class="flex-1">
                                        <div class="text-sm font-bold text-gray-800 group-hover:text-indigo-700 transition-colors">{{ $case->user->name }}</div>
                                        <div class="flex items-center gap-2 mt-1">
                                            <span class="text-[10px] text-gray-500 font-mono">#{{ $case->id }}</span>
                                            <span class="px-1.5 py-0.5 rounded bg-gray-100 text-gray-600 text-[9px] font-black uppercase">{{ $case->durum }}</span>
                                        </div>
                    ...
                    <div class="px-8 py-5">
                        ...
                        <textarea name="icerik" rows="2" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm font-medium transition-all outline-none resize-none">{{ $t->icerik }}</textarea>
                    </div>

                    {{-- SATIR 4: Katılımcı Yönetimi (DÜZENLEME) --}}
                    <div class="px-8 py-5">
                        <div class="flex flex-wrap items-center justify-between gap-4 mb-4">
                            <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Katılımcı Yönetimi</label>
                        ...
                        <div x-show="modalTab === 'kurul'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-56 overflow-y-auto pr-1">
                                @foreach($tumUyeler as $uye)
                                    @php $isKatilimci = $t->katilimcilar->contains('user_id', $uye->id); @endphp
                                    <label class="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-gray-50 hover:bg-indigo-50 border border-transparent {{ $isKatilimci ? 'border-indigo-200 bg-indigo-50/30' : '' }} hover:border-indigo-200 cursor-pointer transition-all group">
                                        <input type="checkbox" name="katilimcilar[]" value="{{ $uye->id }}" {{ $isKatilimci ? 'checked' : '' }} class="w-4 h-4 rounded text-indigo-600 border-gray-300 focus:ring-indigo-500 flex-shrink-0"/>
                                        <img src="{{ $uye->profile_photo_url }}" alt="{{ $uye->name }}" class="w-8 h-8 rounded-full object-cover border border-gray-200 flex-shrink-0">
                                        <div class="min-w-0">
                                            <p class="text-sm font-semibold text-gray-800 truncate group-hover:text-indigo-700">{{ $uye->name }}</p>
                                            <p class="text-[10px] text-gray-400 uppercase tracking-wide">
                                                @if($baskanlar->contains($uye)) Başkan @else Kurul Üyesi @endif
                                            </p>
                                        </div>
                        ...
                        <div x-show="modalTab === 'sistem'" x-data="{ search: '' }" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1">
                                @foreach($tumPersonel as $sUser)
                                    @php $isKatilimci = $t->katilimcilar->contains('user_id', $sUser->id); @endphp
                                    <label x-show="'{{ strtolower($sUser->name) }} {{ strtolower($sUser->bolum->ad ?? '') }}'.includes(search.toLowerCase())" class="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-gray-50 hover:bg-indigo-50 border border-transparent {{ $isKatilimci ? 'border-indigo-200 bg-indigo-50/30' : '' }} hover:border-indigo-200 cursor-pointer transition-all group">
                                        <input type="checkbox" name="sistem_katilimcilari[]" value="{{ $sUser->id }}" {{ $isKatilimci ? 'checked' : '' }} class="w-4 h-4 rounded text-indigo-600 border-gray-300 focus:ring-indigo-500 flex-shrink-0"/>
                                        <img src="{{ $sUser->profile_photo_url }}" alt="{{ $sUser->name }}" class="w-8 h-8 rounded-full object-cover border border-gray-200 flex-shrink-0">
                                        <div class="min-w-0">
                                            <p class="text-sm font-semibold text-gray-800 truncate group-hover:text-indigo-700">{{ $sUser->name }}</p>
                                            <p class="text-[10px] text-gray-400 uppercase tracking-wide">{{ $sUser->bolum->ad ?? 'Bölüm yok' }}</p>
                        ...
                        {{-- Panel: Dış Katılımcı --}}
                        @php $disKat = $t->katilimcilar->where('rol', 'davetli')->first(); @endphp
                        <div x-show="modalTab === 'dis'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0">
                            <div class="bg-amber-50 border border-amber-100 rounded-2xl p-5">
                                <p class="text-xs text-amber-700 font-medium mb-4">Sisteme kayıtlı olmayan bir kişiyi e-posta ile davet edebilirsiniz.</p>
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-[10px] font-bold text-amber-600 uppercase tracking-widest mb-2">Ad Soyad</label>
                                        <input type="text" name="dis_katilimci_adi" value="{{ $disKat ? $disKat->dis_katilimci_adi : '' }}" placeholder="Dış katılımcı adı" class="w-full bg-white border border-amber-200 focus:border-amber-400 focus:ring-2 focus:ring-amber-100 text-gray-800 rounded-xl px-4 py-3 text-sm font-medium transition-all outline-none"/>
                                    </div>
                                    <div>
                                        <label class="block text-[10px] font-bold text-amber-600 uppercase tracking-widest mb-2">E-posta Adresi</label>
                                        <input type="email" name="dis_katilimci_email" value="{{ $disKat ? $disKat->dis_katilimci_email : '' }}" placeholder="ornek@sirket.com" class="w-full bg-white border border-amber-200 focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 text-gray-800 rounded-xl px-4 py-3 text-sm font-medium transition-all outline-none"/>
                                    </div>
                                </div>
                            </div>
                        </div>
