...
trait ComplaintNotificationTrait
{
    ...
    public function sendNewComplaintNotification(MusteriSikayeti $sikayet)
    {
        ...
        // 6. Müşteri Temsilcileri (Bu firmadan olan tüm temsilciler)
        if ($sikayet->customer_id) {
            $musteriTemsilcileri = User::where('customer_id', $sikayet->customer_id)
                ->where(function($q) {
                    $q->whereHas('roles', function($rq) {
                        $rq->where('name', 'like', '%Müşteri Temsilcisi%');
                    })->orWhere('unvan', 'like', '%Müşteri Temsilcisi%');
                })->get();
