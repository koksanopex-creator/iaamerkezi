<x-app-layout>
        ...
        <div class="flex items-center space-x-3">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Şikayet No: <span
                        class="font-semibold text-gray-700">#{{ $sikayet->id }}</span></p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto">
            <div class="bg-white overflow-hidden shadow-2xl rounded-xl sm:rounded-2xl">
                ...
                <div class="p-4 sm:p-8">
                    ...
                    {{-- === ANA FORM === --}}
                    {{-- === ANA FORM === --}}
                    <form action="{{ route('admin.sikayetler.update', $sikayet) }}" method="POST"
                        enctype="multipart/form-data" x-data="fileUploadComponent()">
                        @csrf
                        @method('PUT')

                        {{-- IZGARA YAPISI: Dikey boşluk (gap-y) artırıldı --}}
                        {{-- YENİ: x-data="categorySystem()" ana grid alanına taşındı --}}
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-x-6 gap-y-8" x-data="categorySystem()"
                                    ...
                                    <div class="lg:col-span-2">
                                        @if($tamYetki)
                                            <livewire:admin.sikayet-musteri-secimi
                                                ...
                                                :selected-ek-rep-ids="$sikayet->ekYetkililer->pluck('id')->toArray()" />
                                        @else
                                            {{-- Kısıtlı Yetki: Sadece Salt Okunur Göster --}}
                                                ...
                                                <div class="flex items-start">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p class="text-sm font-semibold text-gray-800">
                                                            {{ $sikayet->customer->name ?? 'Belirtilmemiş' }}</p>
                                                        <input type="hidden" name="customer_id"
                                                            value="{{ $sikayet->customer_id }}">
                                                        <input type="hidden" name="yetkili_user_id"
                                                            value="{{ $sikayet->yetkili_user_id }}">
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
                                ...
                                @else
                                    ...
                                    <div class="group flex flex-col mb-6">
                                        ...
                                        <input type="text" name="musteri_adi" id="musteri_adi"
                                            value="{{ old('musteri_adi', $sikayet->musteri_adi) }}" required
                                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 pl-4 py-3 text-gray-900 block {{ !$tamYetki ? 'bg-gray-100 cursor-not-allowed' : '' }}"
                                            {{ !$tamYetki ? 'readonly' : '' }}>
                                        @error('musteri_adi') <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                                        @enderror
                                    </div>
                            ...
                            <div class="lg:col-span-2 grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                                    ...
                                    <div class="relative">
                                        ...
                                        @else
                                            ...
                                            <input type="hidden" name="sikayet_kategorisi_id"
                                                value="{{ $sikayet->sikayet_kategorisi_id }}">
                                        @endif
                                    </div>
                                ...
                                <div class="group flex flex-col" x-show="subCategories.length > 0 || showOtherOption"
                                    ...
                                    <div class="relative">
                                        {{-- [YENİ]: Herkese açık yapıldı (Kısmi yetkililer Alt Kategori
                                        değiştirebilsin) --}}
                                ...
                                <div class="group flex flex-col lg:col-span-2 bg-gray-50 p-4 rounded border border-gray-200"
                                    ...
                                    <input type="text" name="sikayet_alt_kategori_diger" id="sikayet_alt_kategori_diger"
                                        value="{{ old('sikayet_alt_kategori_diger', $sikayet->sikayet_alt_kategori_diger) }}"
                                ...
                                <div class="relative">
                                    ...
                                    @if($tamYetki)
                                        <select name="musteri_oncelik" id="musteri_oncelik"
                                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 pl-4 pr-10 py-3 text-gray-900 appearance-none bg-white block">
                                            <option value="Düşük" {{ $currentOncelik == 'Düşük' ? 'selected' : '' }}>🟢 Düşük
                                            </option>
                                            <option value="Normal" {{ $currentOncelik == 'Normal' ? 'selected' : '' }}>🟡
                                                Normal</option>
                                            <option value="Yüksek" {{ $currentOncelik == 'Yüksek' ? 'selected' : '' }}>🟠
                                                Yüksek</option>
                                            <option value="Acil" {{ $currentOncelik == 'Acil' ? 'selected' : '' }}>🔴 Acil
                                            </option>
                                        </select>
                                    ...
                                    @else
                                        <div
                                            ...
                                            @if($currentOncelik == 'Düşük') 🟢 Düşük @endif
                                            @if($currentOncelik == 'Normal') 🟡 Normal @endif
                                            @if($currentOncelik == 'Yüksek') 🟠 Yüksek @endif
                                            @if($currentOncelik == 'Acil') 🔴 Acil @endif
                                        </div>
                                        <input type="hidden" name="musteri_oncelik" value="{{ $sikayet->musteri_oncelik }}">
                                    @endif
                                </div>
                            ...
                            <div class="group flex flex-col mb-6">
                                ...
                                @if($tamYetki)
                                    <input type="date" name="musteri_sikayet_tarihi" id="musteri_sikayet_tarihi"
                                        value="{{ old('musteri_sikayet_tarihi', $sikayet->musteri_sikayet_tarihi->format('Y-m-d')) }}"
                                        required
                                ...
                                @else
                                    <div
                                        ...
                                        {{ $sikayet->musteri_sikayet_tarihi->format('d.m.Y') }}
                                    </div>
                                    <input type="hidden" name="musteri_sikayet_tarihi"
                                        value="{{ $sikayet->musteri_sikayet_tarihi->format('Y-m-d') }}">
                                @endif
                                    ...
                                    <div class="mt-2 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                                        @foreach ($sikayet->dosyalar as $dosya)
                                            <div x-show="!deletedFileIds.includes({{ $dosya->id }})"
                                                ...
                                                @if($tamYetki || $dosya->yukleyen_id == auth()->id())
                                                    <button type="button"
                            ...
                            <div class="group flex flex-col lg:col-span-2 mb-6">
                                ...
                                @error('dosyalar.*') <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                                @enderror
                ...
                {{-- MAKAM VE SUPERADMIN İÇİN ŞİKAYET GEÇMİŞİ TABLOSU --}}
                @hasanyrole('Superadmin|Yönetim')
                <div class="p-4 sm:p-8 bg-white border-t border-gray-100 rounded-b-xl sm:rounded-b-2xl">
                    ...
                    @if(isset($loglar) && $loglar->count() > 0)
                                    ...
                                    <div class="flex-1 min-w-0 bg-gray-50 rounded-xl p-4 sm:p-5 border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
                                        <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-2 sm:mb-1 gap-2">
                                            <div class="flex items-center gap-2">
                                                <span class="font-bold text-gray-900 text-sm sm:text-base">{{ optional($log->user)->name ?? 'Bilinmeyen Kullanıcı' }}</span>
    ...
    <script>
        ...
        function categorySystem() {
            return {
                selectedCategory: @json(old('sikayet_kategorisi_id', $sikayet->sikayet_kategorisi_id ?? '')),
                selectedSubCategory: '', // Başlangıçta boş, fetch içinde dolacak
                otherText: @json(old('sikayet_alt_kategori_diger', $sikayet->sikayet_alt_kategori_diger ?? '')),
                ...
                dbSubId: @json($sikayet->sikayet_alt_kategori_id),
                dbOther: @json($sikayet->sikayet_alt_kategori_diger),
                    ...
                    fetch(apiUrl)
                        ...
                        .then(data => {
                            ...
                            if (keepSelection) {
                                ...
                                let oldSub = @json(old('sikayet_alt_kategori_id'));
