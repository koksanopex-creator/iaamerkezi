            ...
            @foreach($sonSikayetler->take(5) as $index => $sikayet)
                ...
                <a href="{{ route('admin.sikayetler.show', $sikayet->id) }}" target="_blank"
                    ...
                    <div class="flex items-center gap-1 text-sm w-full overflow-hidden font-medium">
                        <span class="font-bold uppercase text-xs opacity-80">[{{ $sikayet->sikayetKategori->ad ?? 'Genel' }}]</span>
                        <span class="mx-1 opacity-50">-</span>
                        <span class="font-bold whitespace-nowrap">{{ $tarih }}</span>
                        <span class="whitespace-nowrap opacity-80">&nbsp;tarihinde&nbsp;</span>
                        <span class="font-bold whitespace-nowrap">{{ Str::limit($sikayet->musteri_adi, 20) }}</span>
                        <span class="whitespace-nowrap opacity-80">&nbsp;tarafından&nbsp;</span>
                        <span class="italic font-bold truncate">"{{ Str::limit($sikayet->musteri_sikayet_konusu, 40) }}"</span>
                        <span class="whitespace-nowrap opacity-80">&nbsp;şikayeti. Durumu:&nbsp;</span>
                        <span class="bg-white/60 px-2 py-0.5 rounded text-[10px] font-black uppercase shadow-sm whitespace-nowrap">{{ $sikayet->musteri_durum }}</span>
                        @if($isGecikmis) <span class="ml-2 bg-red-600 text-white px-2 py-0.5 rounded text-[10px] font-black animate-pulse shadow-sm whitespace-nowrap">GECİKTİ!</span> @endif
                        @if($sikayet->musteri_feedback) <span class="ml-2 px-2 py-0.5 rounded text-[10px] font-bold uppercase border bg-white/50">{{ $sikayet->musteri_feedback }}</span> @endif
                    </div>
                </a>
            @endforeach
        </div>
