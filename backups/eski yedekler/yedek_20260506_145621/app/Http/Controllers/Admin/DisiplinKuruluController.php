...
class DisiplinKuruluController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $uyeStats = $tumUyeler->map(function ($uye) use ($baslangic, $bitis, $bolumId, $toplamToplanti, $baskanlar) {
            ...
            foreach ($tumDavaIdleri as $caseId) {
                ...
                if ($resmiOy) {
                    ...
                    if ($sonCanliOy) {
                        $finalOy = in_array($sonCanliOy->oy, ['lehte', 'evet']) ? 'lehte'
                                 : (in_array($sonCanliOy->oy, ['aleyhte', 'hayir']) ? 'aleyhte' : 'cekimser');
                    }
                }
        ...
        $planlananToplantiSayisi = DisiplinKuruluToplanti::whereIn('durum', ['planlandı', 'devam_ediyor'])->count();

        $tamamlananToplantiSayisi = DisiplinKuruluToplanti::where('durum', 'tamamlandı')->count();
