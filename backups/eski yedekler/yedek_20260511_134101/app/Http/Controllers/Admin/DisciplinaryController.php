...
class DisciplinaryController extends Controller
{
    public function index(Request $request)
    {
        ...
        // C. BÖLÜM YETKİLİLERİ (Liderler, İSG, Güvenlik vb.)
        else if ($user->hasRole('Bölüm Lideri') || $user->can_issue_disciplinary || ($user->bolum && $user->bolum->is_disciplinary_global)) {
    ...
    public function approvePenalty(Request $request, $id)
    {
        ...
        $imzaliNot = $not . ' (İşlemi Yapan: ' . Auth::user()->name . ' - ' . now()->format('d.m.Y H:i') . ')';
        $case->update([
            'durum' => 'Karar Verildi',
        ...
        return redirect()->route('admin.disiplin.show', $case->id)
            ->with('success', 'Ceza onaylandı, puan düşüldü ve karar dosyaya işlendi.');
    }
    ...
    public function acceptDefense(Request $request, $id)
    {
        ...
        return redirect()->route('admin.disiplin.show', $case->id)
            ->with('success', 'Savunma haklı bulundu ve kabul edildi. Personelden puan düşülmedi.');
    }
    ...
    public function startVoting(Request $request, $id)
    {
        ...
        return redirect()->route('admin.disiplin.show', [$case->id, 'tab' => 'kurul'])
            ->with('success', 'Oylama başarıyla başlatıldı. Kurul üyelerine bildirim gönderildi.')
            ->with('tab', 'kurul');
    }
    ...
    public function revokeDecision($id)
    {
        ...
        try {
            ...
            // --- İLGİLİ TOPLANTIYI TAMAMEN SİL ---
            // Müdür değerlendirmesine dönüldüğü için kurul süreci sıfırlanmalı
            DisiplinKuruluToplanti::where('disciplinary_case_id', $case->id)->delete();
    ...
    public function saveVote(Request $request, $id)
    {
        ...
        $toplanti = DisiplinKuruluToplanti::where('disciplinary_case_id', $case->id)
        ...
        return redirect()->route('admin.disiplin.show', $id)
            ->with('success', 'Oyunuz ve görüşünüz başarıyla kaydedildi.')
            ->with('tab', 'kurul');
    }
    ...
    public function deleteVote($id)
    {
        ...
        if ($vote) {
            $vote->delete();
            return redirect()->route('admin.disiplin.show', $id)
                ->with('success', 'Oyunuz geri çekildi.')
                ->with('tab', 'kurul');
        }
    ...
    {
        try {
            DisiplinKuruluToplanti::where('disciplinary_case_id', $case->id)
                ->whereIn('durum', ['planlandı', 'devam_ediyor'])
                ->update([
                    'durum'        => 'tamamlandı',
                    'bitirilme_at' => now(),
            ...
            DisiplinKuruluToplantiKatilimci::where('toplanti_id', function($query) use ($case) {
                $query->select('id')
                      ->from('disiplin_kurulu_toplanti')
                      ->where('disciplinary_case_id', $case->id)
