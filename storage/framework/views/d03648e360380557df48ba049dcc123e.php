        ...
        <table class="block sm:table min-w-full">
            <thead class="hidden sm:table-header-group bg-gradient-to-r from-gray-50 to-blue-50/30">
                <tr class="text-left text-sm font-bold text-gray-700 tracking-wide">
                    ...
                    <th class="px-6 py-6 text-center">Tamamlayan</th> 
            ...
            <tbody class="block sm:table-row-group divide-y divide-gray-100">
                <!--[if BLOCK]><![endif]--><?php $__empty_1 = true; $__currentLoopData = $iaas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $iaa): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr x-data @click="$dispatch('open-modal', 'detay-modal-<?php echo e($iaa->id); ?>')" class="block mb-6 border bg-gradient-to-r from-white to-blue-50/20 border-gray-200 rounded-2xl sm:table-row sm:mb-0 sm:border-0 sm:border-b sm:border-gray-100 hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-300 cursor-pointer group hover:shadow-xl hover:scale-[1.02] transform-gpu hover:-translate-y-1">
                        
                        <td class="flex justify-between items-center p-4 sm:table-cell sm:p-6 align-middle text-center"><span class="font-bold text-sm text-gray-600 sm:hidden">No:</span><div class="w-10 h-10 bg-gradient-to-br from-blue-500 to-indigo-600 text-white rounded-xl flex items-center justify-center font-bold text-sm group-hover:scale-110 transition-transform"><?php echo e($startNumber + $index + 1); ?></div></td>
                        <td class="flex justify-between items-start p-4 border-t sm:border-0 sm:table-cell sm:p-6 align-middle"><span class="font-bold text-sm text-gray-600 sm:hidden">Başlık:</span><div class="text-right sm:text-left max-w-md"><p class="text-gray-800 font-bold text-lg group-hover:text-indigo-700 transition-colors leading-tight"><?php echo e($iaa->baslik); ?></p></div></td>
                        <td class="flex justify-between items-center p-4 border-t sm:border-0 sm:table-cell sm:p-6 align-middle"><span class="font-bold text-sm text-gray-600 sm:hidden">Öneren:</span><div class="text-right sm:text-left"><!--[if BLOCK]><![endif]--><?php if($iaa->gonderen): ?><div class="flex items-center gap-3"><div class="w-10 h-10 bg-gradient-to-br from-green-400 to-blue-500 rounded-full flex items-center justify-center text-white font-bold text-sm"><?php echo e(substr($iaa->gonderen->name, 0, 2)); ?></div><div><p class="font-bold text-gray-800"><?php echo e($iaa->gonderen->name); ?></p><p class="text-sm text-gray-500 bg-gray-100 px-2 py-1 rounded-full inline-block"><?php echo e($iaa->gonderen->bolum->ad ?? 'Bölüm Yok'); ?></p></div></div><?php else: ?><div class="flex items-center gap-3"><div class="w-10 h-10 bg-gradient-to-br from-gray-400 to-gray-600 rounded-full flex items-center justify-center text-white"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg></div><div><p class="font-bold text-gray-800"><?php echo e($iaa->guest_name); ?></p><span class="text-xs text-white bg-gradient-to-r from-orange-500 to-red-500 px-3 py-1 rounded-full font-semibold">👤 Misafir</span></div></div><?php endif; ?><!--[if ENDBLOCK]><![endif]--></div></td>
                        ...
                        <td class="flex justify-between items-center p-4 border-t sm:border-0 sm:table-cell sm:p-6 align-middle">
                            ...
                            <div class="w-full text-right sm:text-center">
                                ...
                                <span class="px-4 py-2 inline-flex items-center gap-2 text-sm font-bold rounded-2xl 
                                            <?php echo e($this->getDurumBadgeClasses($iaa->durum)); ?>">
                                    
                                    
                                    <!--[if BLOCK]><![endif]--><?php switch($iaa->durum):
                                        case ('Onay Bekliyor'): ?> <div class="w-2 h-2 bg-yellow-500 rounded-full animate-pulse"></div> <?php break; ?>
                                        <?php case ('Atandı'): ?> <div class="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div> <?php break; ?>
                                        <?php case ('Reddedildi'): ?> <div class="w-2 h-2 bg-red-500 rounded-full"></div> <?php break; ?>
                                        <?php default: ?> <div class="w-2 h-2 bg-gray-500 rounded-full"></div>
                        ...
                        <td class="flex justify-between items-center p-4 border-t sm:border-0 sm:table-cell sm:p-6 align-middle">
                            ...
                            <div class="w-full text-center">
                                <!--[if BLOCK]><![endif]--><?php if($iaa->durum == 'Tamamlandı' && $iaa->atananTakim): ?>
                                    <div x-data="{ tooltip: false }" class="relative inline-block">
                                        ...
                                        <div x-show="tooltip" 
                                            ...
                                            <ul class="list-disc list-inside">
                                                <!--[if BLOCK]><![endif]--><?php $__empty_2 = true; $__currentLoopData = $iaa->atananTakim->uyeler; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $uye): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_2 = false; ?>
                                                    <li><?php echo e($uye->name); ?></li>
...
<?php endif; ?>
<?php $component->withAttributes(['name' => 'detay-modal-'.e($iaa->id).'','max-width' => '2xl']); ?>
            <div class="p-8 bg-gradient-to-br from-white to-blue-50/20">
                <?php echo $__env->make('admin.raporlar.partials.iaa-detay-content', ['iaa' => $iaa], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            </div>
...
<script>
    document.addEventListener('livewire:initialized', () => {
        window.dispatchEvent(new CustomEvent('updateChart', {
            detail: <?php echo json_encode($initialChartData, 15, 512) ?>
        }));
    });
</script><?php /**PATH C:\Users\celal.karaman\Desktop\Projelerim\iaa_projesi\resources\views/livewire/admin/raporlar-tablosu.blade.php ENDPATH**/ ?>