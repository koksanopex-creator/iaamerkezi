<div class="space-y-8">
        ...
        <div class="relative flex flex-col md:flex-row items-center justify-between gap-8">
            <div class="text-white">
                ...
                <div class="flex items-center gap-4 text-indigo-100 opacity-90 text-xs font-bold mb-4">
                    ...
                    <a href="{{ route('admin.health.logs', ['type' => 'veri']) }}" class="flex items-center gap-1 text-white hover:underline">
        ...
        @if(count($inconsistencies) > 0)
            <div class="mt-8 grid grid-cols-1 md:grid-cols-4 gap-4 animate-fade-in">
                <div class="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20">
                    ...
                    <p class="text-2xl font-black text-white">{{ $scanResults['dates'] }} <span class="text-xs font-normal opacity-60">Kayıt</span></p>
                </div>
                <div class="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20">
                    <p class="text-[10px] font-bold text-indigo-200 uppercase tracking-widest mb-1">Durum Uyuşmazlığı</p>
                    <p class="text-2xl font-black text-white">{{ $scanResults['statuses'] }} <span class="text-xs font-normal opacity-60">Kayıt</span></p>
                </div>
                <div class="bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/20">
                    <p class="text-[10px] font-bold text-indigo-200 uppercase tracking-widest mb-1">Bölüm Sapmaları</p>
                    <p class="text-2xl font-black text-white">{{ $scanResults['departments'] }} <span class="text-xs font-normal opacity-60">Kayıt</span></p>
                </div>
                <div class="bg-amber-400/20 backdrop-blur-md rounded-2xl p-4 border border-amber-400/30">
                    ...
                    <p class="text-2xl font-black text-white">{{ $scanResults['points'] }} <span class="text-xs font-normal opacity-60">Kayıt</span></p>
                </div>
            </div>
        @endif
    </div>
    ...
    @if (session()->has('info'))
        <div class="p-6 rounded-3xl bg-indigo-50 border border-indigo-100 flex flex-col items-center gap-3 text-center animate-fade-in">
            ...
            <p class="text-sm text-indigo-700 max-w-md">{{ session('info') }}</p>
        </div>
    @endif
    ...
    @if(count($inconsistencies) > 0)
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden animate-fade-in">
            <div class="p-6 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                ...
                <span class="text-xs font-bold text-slate-400">Toplam {{ count($inconsistencies) }} uyuşmazlık bulundu.</span>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        @foreach($inconsistencies as $item)
                            <tr class="hover:bg-slate-50/30 transition-colors group">
                                ...
                                <td class="px-6 py-4">
                                    <span class="text-xs font-medium text-rose-500 line-through opacity-60">{{ $item['old_value'] }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-xs font-bold text-emerald-600">{{ $item['new_value'] }}</span>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <button wire:click="calibrateSingle({{ $loop->index }})" wire:loading.attr="disabled" class="p-2 bg-emerald-50 text-emerald-600 rounded-lg hover:bg-emerald-100 transition-all flex items-center gap-1 group/btn" title="Sadece Bu Kaydı Kalibre Et">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                            <span class="text-[10px] font-black hidden group-hover/btn:block uppercase">Onayla</span>
                                        </button>
                                        <a href="{{ $item['url'] }}" target="_blank" class="p-2 bg-slate-50 text-slate-400 rounded-lg hover:text-indigo-600 hover:bg-indigo-50 transition-all inline-block" title="İlgili Kayda Git">
    ...
    @elseif(!$isScanning && !session()->has('info'))
                            ...
                            <tr class="hover:bg-slate-50/30 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-xs font-bold text-slate-700">{{ $log->causer->name ?? 'Bilinmeyen' }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span class="text-[10px] font-black uppercase text-indigo-400">{{ $log->type }}</span>
                                        <span class="text-xs font-bold text-slate-600">{{ $log->model_type }} #{{ $log->model_id }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2 text-[10px] font-medium">
                                        <span class="text-rose-400 line-through">{{ $log->old_value }}</span>
                                        ...
                                        <span class="text-emerald-600 font-black">{{ $log->new_value }}</span>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
