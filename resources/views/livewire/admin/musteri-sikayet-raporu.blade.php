<div class="space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
        ...
        {{-- Talep Kapanan --}}
        <div wire:click="setFilter('talep_kapatilan')" class="bg-white p-3 rounded-xl shadow-sm border border-gray-100 cursor-pointer transition-all hover:shadow-md hover:scale-[1.02] flex flex-col justify-between h-24 {{ $activeFilter === 'talep_kapatilan' ? 'ring-2 ring-purple-500 bg-purple-50' : '' }}">
            <div class="flex justify-between items-start">
            ...
            <div class="text-2xl font-black text-purple-600">{{ $kpi['talep_kapatilan'] }}</div>
        </div>

        {{-- Hatalı --}}
        <div wire:click="setFilter('hatali_bildirim')" class="bg-white p-3 rounded-xl shadow-sm border border-gray-100 cursor-pointer transition-all hover:shadow-md hover:scale-[1.02] flex flex-col justify-between h-24 {{ $activeFilter === 'hatali_bildirim' ? 'ring-2 ring-orange-500 bg-orange-50' : '' }}">
            <div class="flex justify-between items-start">
            ...
            <div class="text-2xl font-black text-orange-500">{{ $kpi['hatali_bildirim'] }}</div>
        </div>
                            ...
                            @forelse ($sonSikayetler as $sikayet)
                                ...
                                <tr class="{{ $rowBg }} {{ $rowBar }} transition-all duration-200">
                                    ...
                                    <td class="px-4 py-4 text-sm font-medium text-gray-900">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('admin.sikayetler.show', $sikayet) }}" target="_blank" class="truncate max-w-[180px] block hover:text-indigo-600 transition-colors font-bold" title="{{ $sikayet->musteri_adi }}">
                                                {{ $sikayet->musteri_adi }}
                                            </a>
                                            @if($isExternal)
                                    ...
                                    <td class="px-4 py-4 text-sm text-gray-700 truncate">
                                        ...
                                        <div class="mt-2 flex items-center flex-nowrap gap-1.5 overflow-x-auto no-scrollbar">
                                            ...
                                            @endif
                                            @if(isset($visitStats['visits_by_complaint'][$sikayet->id]))
                                    ...
                                    <td class="px-4 py-4 whitespace-nowrap text-sm">
                                        <div class="flex flex-col gap-1">
                                            ...
                                            @endphp
                                            <div class="{{ $isOverdue ? 'text-red-700 font-black' : 'text-gray-600 font-medium' }} flex flex-col gap-2 py-1" title="Hedef Çözüm Tarihi">
                                                <span>{{ $sikayet->musteri_cozum_son_tarihi ? $sikayet->musteri_cozum_son_tarihi->format('d.m.Y') : 'N/A' }}</span>
                                                @if($isOverdue)
                                                    <span class="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-black bg-red-100 text-red-600 border border-red-200 animate-pulse uppercase tracking-tighter w-fit shadow-sm">
                                                        ⚠️ Gecikti ({{ (int) $sikayet->musteri_cozum_son_tarihi->diffInDays(now()) }} Gün)
                                                    </span>
                                                @endif
                                            </div>
                                            @if($sikayet->kurul_onay_tarihi || $sikayet->musteri_onay_tarihi)
                            ...
                            <div class="rounded-lg shadow border {{ $rowBg }} {{ $rowBar }} p-4 space-y-3">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-1">
                                        <a href="{{ route('admin.sikayetler.show', $sikayet) }}" target="_blank" class="text-sm font-medium text-gray-700 truncate hover:text-indigo-600 transition-colors" title="{{ $sikayet->musteri_adi }}">{{ $sikayet->musteri_adi }}</a>
                                    ...
                                    <div class="text-sm">
                                        ...
                                        @php
                                            $isOverdue = $sikayet->musteri_cozum_son_tarihi && 
                                                        ...
                                                        !in_array($sikayet->musteri_durum, ['Çözümlendi', 'Kapatıldı']);
                                        @endphp
                                        <span class="font-semibold {{ $isOverdue ? 'text-red-700 font-black' : 'text-gray-500' }}">
    ...
    {{-- === YENİ: EN ÇOK GECİKENLER LİSTESİ === --}}
    @if($enCokGecikenler->count() > 0)
    <div class="bg-white rounded-xl shadow-lg border border-red-100 overflow-hidden mt-8">
        <div class="px-6 py-4 bg-red-50 border-b border-red-100 flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.sikayet-raporlari.tum-liste', ['gecikmis' => 1]) }}" target="_blank" class="text-xs font-black text-red-700 hover:underline flex items-center gap-1">
                    Tümünü Gör 
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-50">
                    @foreach($enCokGecikenler as $geciken)
                        <tr class="hover:bg-red-50/40 transition-colors group">
                            <td class="px-4 py-4">
                                <div class="flex flex-col">
                                    <span class="text-sm font-bold text-gray-900 group-hover:text-red-700 transition-colors truncate max-w-[200px]">{{ $geciken->musteri_adi }}</span>
                                    <span class="text-[10px] text-gray-500 truncate max-w-[200px] font-medium">{{ $geciken->musteri_sikayet_konusu }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-4 text-center">
                                <span class="text-xs font-bold text-gray-600">{{ $geciken->created_at->format('d.m.Y') }}</span>
                            </td>
                            <td class="px-4 py-4 text-center">
                                <div class="flex flex-col">
                                    <span class="text-xs font-bold text-red-600">{{ $geciken->hesaplanan_deadline->format('d.m.Y') }}</span>
                                    <span class="text-[9px] font-black {{ $geciken->deadline_type == 'Manuel' ? 'text-blue-500' : 'text-orange-500' }} uppercase">{{ $geciken->deadline_type }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-4">
                                <div class="flex flex-col">
                                    <span class="text-[11px] font-bold text-indigo-600 truncate max-w-[120px]">{{ $geciken->cozumTakimi->ad ?? 'Atanmamış' }}</span>
                                    <span class="text-[9px] text-gray-400 font-bold uppercase truncate max-w-[120px]">{{ $geciken->sikayetKategori->bolum->ad ?? '-' }}</span>
                                </div>
                            </td>
                            <td class="px-4 py-4 text-center">
                                <div class="inline-flex flex-col items-center bg-red-100 px-3 py-1 rounded-lg border border-red-200">
                                    <span class="text-lg font-black text-red-700 leading-none">{{ $geciken->gecikme_gunu }}</span>
                                    <span class="text-[9px] font-black text-red-500 uppercase">GÜN</span>
                                </div>
                            </td>
                            <td class="px-4 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route('admin.sikayetler.show', $geciken) }}" target="_blank" class="p-1.5 bg-white border border-red-100 text-red-600 rounded-lg hover:bg-red-600 hover:text-white transition-all shadow-sm">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                    </a>
                                    @if($geciken->iaa_id)
                                        <a href="{{ route('proje.workspace.show', $geciken->iaa_id) }}" target="_blank" class="p-1.5 bg-white border border-purple-100 text-purple-600 rounded-lg hover:bg-purple-600 hover:text-white transition-all shadow-sm">
        ...
        <div class="px-6 py-3 bg-gray-50 border-t border-gray-100 flex justify-center">
            <a href="{{ route('admin.sikayet-raporlari.tum-liste', ['gecikmis' => 1]) }}" target="_blank" class="inline-flex items-center gap-2 px-4 py-2 bg-red-600 text-white text-xs font-black rounded-lg hover:bg-red-700 transition-all shadow-md">
