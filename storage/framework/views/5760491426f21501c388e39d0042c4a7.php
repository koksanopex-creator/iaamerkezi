     ...
     <?php $__env->slot('header', null, []); ?> 
        <div class="flex items-center justify-between">
            <h2 class="font-black text-2xl text-slate-800 leading-tight tracking-tighter flex items-center gap-3">
                ...
                <?php echo e(__('Dış Avukat Portalı')); ?>
            ...
            <div class="flex items-center gap-3">
                <div class="hidden md:flex flex-col text-right">
                    ...
                    <span class="text-xs font-bold text-emerald-600 flex items-center justify-end gap-1">
                        ...
                        Aktif Kayıtlı Avukat: <?php echo e($totalLawyers); ?>

                    </span>
                </div>
                <a href="<?php echo e(route('admin.dis_avukatlar.create')); ?>" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-slate-900 text-white font-bold py-2.5 px-5 rounded-xl transition-all shadow-lg hover:shadow-indigo-200 transform hover:-translate-y-0.5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between group hover:border-indigo-200 transition-all">
                    <div>
                        ...
                        <h3 class="text-3xl font-black text-slate-900 leading-none group-hover:text-indigo-600 transition-colors"><?php echo e($totalLawyers); ?></h3>
                    </div>
                ...
                <div class="md:col-span-3 bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center">
                    <form action="<?php echo e(route('admin.dis_avukatlar.index')); ?>" method="GET" class="w-full flex gap-3">
                        ...
                        <?php if($search): ?>
                            <a href="<?php echo e(route('admin.dis_avukatlar.index')); ?>" class="bg-rose-50 text-rose-600 px-4 py-2.5 rounded-xl font-bold hover:bg-rose-100 transition-all flex items-center">Sıfırla</a>
                        <?php endif; ?>
                    </form>
                </div>
            </div>
                ...
                <div class="overflow-x-auto min-h-[400px]">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-50">
                            <?php $__empty_1 = true; $__currentLoopData = $lawyers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $avukat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr class="hover:bg-slate-50 transition-colors group">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-600 font-black border border-indigo-100">
                                                <?php echo e(strtoupper(substr($avukat->name, 0, 1))); ?>

                                            </div>
                                            <div>
                                                <p class="text-sm font-black text-slate-800 leading-tight"><?php echo e($avukat->name); ?></p>
                                    ...
                                    <td class="px-6 py-4">
                                        <p class="text-sm font-bold text-slate-700 mb-0.5"><?php echo e($avukat->email); ?></p>
                                        <p class="text-xs text-slate-400 italic">Tel: <?php echo e($avukat->telefon ?? 'Tanımlanmamış'); ?></p>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="space-y-3">
                                            
                                            <?php if($avukat->addedBy): ?>
                                                <div class="flex items-center gap-2 group/user cursor-pointer">
                                                    <a href="<?php echo e(route('profile.show', $avukat->addedBy->id)); ?>" class="flex items-center gap-2">
                                                        <img src="<?php echo e($avukat->addedBy->profile_photo_url ?? 'https://ui-avatars.com/api/?name='.urlencode($avukat->addedBy->name).'&color=7F9CF5&background=EBF4FF'); ?>" 
                                                             class="w-6 h-6 rounded-full border border-slate-200 group-hover/user:border-indigo-400 transition-colors object-cover" 
                                                             title="Ekleyen: <?php echo e($avukat->addedBy->name); ?>">
                                                        <span class="text-xs font-bold text-slate-600 underline decoration-slate-200 hover:text-indigo-600 hover:decoration-indigo-200 transition-all"><?php echo e($avukat->addedBy->name); ?></span>
                                                    </a>
                                            ...
                                            <?php if($avukat->updatedBy): ?>
                                                <div class="flex flex-col gap-1 border-t border-slate-50 pt-2">
                                                    <div class="flex items-center gap-2 group/user cursor-pointer">
                                                        <a href="<?php echo e(route('profile.show', $avukat->updatedBy->id)); ?>" class="flex items-center gap-2">
                                                            <img src="<?php echo e($avukat->updatedBy->profile_photo_url ?? 'https://ui-avatars.com/api/?name='.urlencode($avukat->updatedBy->name).'&color=F59E0B&background=FEF3C7'); ?>" 
                                                                 class="w-6 h-6 rounded-full border border-slate-200 group-hover/user:border-amber-400 transition-colors object-cover" 
                                                                 title="Son Düzenleyen: <?php echo e($avukat->updatedBy->name); ?>">
                                                            <span class="text-[11px] font-bold text-slate-500 hover:text-amber-600 transition-all italic"><?php echo e($avukat->updatedBy->name); ?></span>
                                                        </a>
                                                    ...
                                                    <p class="text-[9px] text-slate-400 ml-8 font-medium leading-none">
                                                        <?php echo e($avukat->updated_at->translatedFormat('d F Y')); ?>

                                                        <span class="text-slate-300">(<?php echo e($avukat->updated_at->diffForHumans()); ?>)</span>
                                                    </p>
                                                </div>
                                            <?php endif; ?>
                                        </div>
                                    ...
                                    <td class="px-6 py-4">
                                        <p class="text-xs font-black text-slate-500 uppercase"><?php echo e($avukat->created_at->translatedFormat('d F Y')); ?></p>
                                        <p class="text-[10px] text-slate-400 tracking-tighter"><?php echo e($avukat->created_at->diffForHumans()); ?></p>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            <?php if($canEdit): ?>
                                                <a href="<?php echo e(route('admin.dis_avukatlar.edit', $avukat->id)); ?>" class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all" title="Bilgileri Düzenle">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                                </a>
                                                <form action="<?php echo e(route('admin.dis_avukatlar.destroy', $avukat->id)); ?>" method="POST" onsubmit="return confirm('Bu avukat kaydını silmek istediğinize emin misiniz?')" class="inline">
                                                    <?php echo csrf_field(); ?>
...
<?php endif; ?><?php /**PATH C:\Users\celal.karaman\Desktop\Projelerim\iaa_projesi\resources\views/admin/dis_avukatlar/index.blade.php ENDPATH**/ ?>