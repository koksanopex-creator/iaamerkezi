                    ...
                    <tr class="bg-white hover:bg-blue-50/30 transition-colors group">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex flex-col">
                                <a href="{{ route('proje.workspace.show', $visit->iaa->id) }}" target="_blank" class="text-sm font-bold text-gray-900 hover:text-blue-600 transition-colors">
                                    #{{ $visit->iaa->id }} - {{ Str::limit($visit->iaa->id_label ?? $visit->iaa->baslik, 30) }}
                                </a>
                                <span class="text-[10px] text-gray-400 font-medium mt-0.5">
                                     {{ $visit->iaa->atananTakim->ad ?? '-' }}
                                </span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-center whitespace-nowrap">
                             <div class="inline-flex flex-col items-center justify-center bg-gray-50 rounded-lg p-1.5 border border-gray-100 group-hover:bg-white transition-colors">
                                <span class="text-xs font-black text-gray-800">{{ $visit->visit_date->format('d.m.Y') }}</span>
                                <span class="text-[9px] text-gray-400 font-bold uppercase">{{ $visit->visit_date->translatedFormat('l') }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                             <div class="flex items-center gap-2">
                                <div class="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-xs font-bold text-blue-600 border border-blue-200">
                                    {{ substr($visit->iaa->musteriSikayeti->customer->name ?? '-', 0, 1) }}
                                </div>
                                <a href="{{ $visit->iaa->musteriSikayeti->customer ? route('musteri.profil.show', $visit->iaa->musteriSikayeti->customer->id) : '#' }}" target="_blank" class="text-xs font-bold text-gray-700 hover:text-blue-600 hover:underline transition-colors truncate max-w-[150px]">
                                    {{ $visit->iaa->musteriSikayeti->customer->name ?? '-' }}
                                </a>
                             </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex flex-col gap-1">
                                <div class="flex items-center gap-1.5" title="Planlayan">
                                    ...
                                    <span class="text-[10.5px] font-bold text-gray-700">{{ $visit->planner->name ?? 'Bilinmeyen' }}</span>
                                </div>
                                <div class="flex items-center gap-1.5" title="Ziyaretçi">
                                    ...
                                    <span class="text-[10.5px] font-bold text-gray-600">{{ $visit->visitor->name ?? $visit->visitor_name ?? '-' }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-center whitespace-nowrap">
                            ...
                            <div class="flex flex-col items-center">
                                <span class="group/status relative px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border {{ $colorClass }} cursor-default">
                                    {{ $visit->status }}
                                    @if($visit->status === 'Onaylandı' && $visit->approver)
                                ...
                                @elseif($visit->status === 'Beklemede')
                                    @if(Auth::user()->hasRole('Bölüm Kalite Yöneticisi') || Auth::user()->hasRole('Superadmin'))
                                    ...
                                    @endif
                                @elseif($visit->status === 'Direktör Onayı Bekliyor')
                        ...
                        <td class="px-6 py-4 text-right whitespace-nowrap">
                            <div class="flex items-center justify-end gap-2">
                                {{-- Geri Al Butonu (Superadmin veya Yetkili Direktör) --}}
                                @php
                                    $user = Auth::user();
                                ...
                                @if($canAction)
                                    @if($visit->status === 'Beklemede' || $visit->status === 'Direktör Onayı Bekliyor' || $visit->status === 'Onay Bekliyor')
                                        {{-- Onayla --}}
                                        <button wire:click="openApproveModal({{ $visit->id }})" 
                                        ...
                                        {{-- Revize İste --}}
                                        <button wire:click="openRejectModal({{ $visit->id }}, 'revision')" 
                                        ...
                                        {{-- Reddet --}}
                                        <button wire:click="openRejectModal({{ $visit->id }}, 'reject')" 
                                    ...
                                    @elseif(in_array($visit->status, ['Onaylandı', 'Reddedildi', 'Revize İsteniyor']))
                                        {{-- Geri Al --}}
                                        <button wire:click="undoAction({{ $visit->id }})" 
                                ...
                                <a href="{{ route('proje.workspace.show', $visit->iaa->id) }}" target="_blank" 
        ...
        <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-center gap-3">
            @if(!$showAll)
                ...
                <button wire:click="toggleShowAll"
                    ...
                    Devamını Yükle
                    <span class="bg-indigo-100 text-indigo-600 px-2 py-0.5 rounded-lg text-[10px] font-black">+{{ $totalCount - $initialLimit }}</span>
                </button>
            @else
                {{-- Devamını Gizle --}}
                <button wire:click="toggleShowAll"
    ...
    <div class="fixed inset-0 z-[100] overflow-y-auto">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        <div class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full {{ $actionType === 'revision' ? 'bg-purple-100' : 'bg-rose-100' }} sm:mx-0 sm:h-10 sm:w-10">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            <h3 class="text-lg leading-6 font-bold text-gray-900">{{ $actionType === 'revision' ? 'Revize İste' : 'Ziyaret Planını Reddet' }}</h3>
                            <div class="mt-4">
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                    <button type="button" wire:click="processRejectOrRevision" class="w-full inline-flex justify-center rounded-xl border border-transparent shadow-sm px-4 py-2 {{ $actionType === 'revision' ? 'bg-purple-600 hover:bg-purple-700' : 'bg-rose-600 hover:bg-rose-700' }} text-base font-bold text-white focus:outline-none sm:w-auto sm:text-sm transition-colors">
                        {{ $actionType === 'revision' ? 'Revize İste' : 'Reddet' }}
                    </button>
