<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
    ...
    <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex justify-between items-center">
        ...
        <span class="text-xs font-mono text-gray-400">Dosya #{{ $case->id }}</span>
    </div>
    ...
    <div class="p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            {{-- 1. PERSONEL --}}
            <a href="{{ url('/kullanici-profil/'.$case->user_id) }}" class="flex items-start gap-3 group">
                ...
                <div>
                    ...
                    <p class="font-bold text-gray-900 group-hover:text-indigo-600 transition-colors">{{ $case->user->name }}</p>
                    <p class="text-xs text-gray-500">{{ $case->user->bolum->ad ?? 'Bölüm Yok' }}</p>
                </div>
            </a>
            ...
            <div class="flex items-start gap-3">
                ...
                <div>
                    ...
                    <p class="font-bold text-gray-900">{{ $case->behavior->category->ad ?? 'Genel' }}</p>
                    <p class="text-xs text-red-500 font-medium">{{ Str::limit($case->behavior->tanim, 40) }}</p>
                </div>
            </div>

            {{-- 3. TARİH VE RAPORLAYAN --}}
            <div class="flex items-start gap-3">
                <div class="flex-shrink-0">
                ...
                <div>
                    ...
                    <p class="font-bold text-gray-900">{{ $case->olay_tarihi->format('d.m.Y') }}</p>
                    <div class="mt-1 flex flex-col gap-0.5">
                        <a href="{{ url('/kullanici-profil/'.$case->reporter_id) }}" class="text-xs text-gray-500 hover:text-blue-600 transition-colors">
                            <span class="font-bold text-slate-700">Raporlayan:</span> {{ $case->reporter->name }}
                        </a>
                        <span class="text-[10px] text-slate-400 italic">
                            <span class="font-bold">Raporlama Tarihi:</span> {{ $case->created_at->format('d.m.Y H:i') }}
                        </span>
                    </div>
                </div>
            </div>
            ...
            @else
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    @foreach($case->kanit_dosyalari as $kanit)
                        <a href="{{ asset('storage/' . $kanit) }}" target="_blank" class="group flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-2xl hover:border-indigo-500 hover:bg-indigo-50/30 hover:shadow-lg hover:shadow-indigo-500/10 transition-all duration-300">
