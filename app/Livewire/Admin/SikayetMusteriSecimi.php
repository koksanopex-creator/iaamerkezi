...
class SikayetMusteriSecimi extends Component
{
    ...
    public function mount($preselectedCustomerId = null, $selectedRepId = null, $selectedEkRepIds = [])
    {
        ...
        // 1. Tüm Müşterileri Çek (Yetkiye Göre Filtreli)
        $user = auth()->user();
        if (!$user->is_personnel) {
            ...
            $this->customers = Customer::whereIn('id', $validCustomerIds)->orderBy('name')->get();
        } else {
        ...
        if ($preselectedCustomerId) {
            ...
            // [DÜZELTME] O firmaya ait TÜM yetkilileri (Pivot tablo dahil) yükle
            $customer = Customer::find($preselectedCustomerId);
            $this->representatives = $customer ? $customer->users()->wherePivot('is_active', true)->get() : collect();
    ...
    public function storeNewCustomer()
    {
        ...
        foreach ($this->reps as $repData) {
            ...
            // Müşteriyi (Firmayı) ve Kullanıcıyı pivot tabloda birbirine bağla
            $customer->users()->attach($user->id, ['is_active' => true, 'unvan' => $repData['title']]);

            // SSO Senkronizasyonu
            try {
                app(\App\Services\CentralSsoSyncService::class)->syncUser($user, $password, 'customer');
            } catch (\Exception $ssoEx) {
                \Illuminate\Support\Facades\Log::error('Central SSO Sync failed for fast customer representative user: ' . $ssoEx->getMessage());
            }
        ...
        $this->successMessage = "Müşteri ve " . count($createdUserIds) . " yetkili eklendi. Şifreler: " . implode(' | ', $passwords);
    }
