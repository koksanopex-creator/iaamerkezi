    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            @if(auth()->user()->hasRole('Superadmin') || auth()->user()->canAny(['arabuluculuk.create_ihtiyari', 'arabuluculuk.create_zorunlu']))
                <a href="{{ route('admin.arabuluculuk.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-lg shadow-md transition duration-150 ease-in-out flex items-center">
                                ...
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-3">
                                                <div class="text-sm font-medium text-gray-900">{{ $case->calisan->name ?? 'Silinmiş Kullanıcı' }}</div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @endif
                                        <div class="text-xs text-gray-500 mt-1">Yöneten: {{ ucfirst($case->owner_role) }}</div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @if($case->external_lawyer_id)
                                            <div class="text-xs text-purple-600 font-semibold">Av. {{ $case->externalLawyer->name ?? '' }} (Dış)</div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        ...
                                        @endphp
                                        <span class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-md {{ $color }}">
                                            {{ str_replace('_', ' ', strtoupper($case->status)) }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <a href="{{ route('admin.arabuluculuk.show', $case->id) }}" class="text-indigo-600 hover:text-indigo-900 font-bold">Detay &rarr;</a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
