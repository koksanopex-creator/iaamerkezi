...
use App\Http\Controllers\Admin\SikayetHatirlatmaController;
use App\Http\Controllers\Admin\SikayetHatirlaticiController;
...
Route::get('/preview-disiplin-mail', function () {
    $toplanti = \App\Models\DisiplinKuruluToplanti::latest()->first();
    $oldData = [
        'baslik' => 'Eski Toplantı Başlığı',
        'baslangic_tarihi' => now()->subDay()->toDateTimeString(),
        'yer' => 'Eski Toplantı Salonu'
    ];
    $newData = [
    ...
    return (new \App\Notifications\DisiplinToplantiRevizyonNotification($toplanti, $oldData, $newData, 'Test Kullanıcısı'))
        ->toMail(auth()->user());
})->middleware(['auth']);
...
// --- MERKEZİ SSO YÖNLENDİRMELERİ ---
$centralSsoUrl = rtrim(env('CENTRAL_SSO_URL', 'http://localhost:8001'), '/');
Route::get('/sso-login', function (\Illuminate\Http\Request $request) use ($centralSsoUrl) { 
    $appCode = env('CENTRAL_SSO_APP_CODE', 'iaa-yonetim-sistemi');
    $callbackUrl = route('sso.login'); // The callback route where Central SSO will send the token
    
    // Yönlendirme URL'ine uygulamanın kodunu ve dönüş yapacağı rotayı ekliyoruz
    return redirect($centralSsoUrl . '/sso-entry?app_code=' . $appCode . '&redirect_url=' . urlencode($callbackUrl)); 
})->name('login');
Route::post('/sso-logout', function (\Illuminate\Http\Request $request) use ($centralSsoUrl) { 
    Auth::guard('web')->logout();
    $request->session()->invalidate();
    $request->session()->regenerateToken();
    
    // Herkesi (Personel veya Müşteri fark etmeksizin) merkezi API'den de çıkış yapmaya yönlendiriyoruz.
    // Aksi takdirde SSO'da session açık kaldığı için tekrar giriş yap butonuna basınca şifresiz içeri alıyor.
...
Route::controller(SsoController::class)->prefix('sso')->name('sso.')->group(function () {
    ...
    Route::post('/basvuru-guncelle', 'basvuruGuncelle')->name('basvuru_guncelle');
    Route::post('/hatirlat', 'hatirlat')->name('hatirlat');
});
// ==========================================================================
...
Route::middleware(['auth', BlockCustomerAccess::class])->group(function ()
{
    ...
    Route::post('/sikayetler/kaydet', [SikayetController::class, 'store'])->name('iaa.sikayetler.store');
    Route::get('/sikayetler/{sikayet}', [SikayetController::class, 'show'])
        ->name('iaa.sikayetler.show')
        ->whereNumber('sikayet');
    
    // Müşteri Hatırlatmalarım
    Route::get('/hatirlatmalarim', [SikayetHatirlatmaController::class, 'musteriIndex'])->name('iaa.hatirlatmalarim.index');
    Route::get('/musteri-hatirlatmalari/{hatirlatma}', [SikayetHatirlatmaController::class, 'musteriShow'])->name('iaa.hatirlatmalarim.show');
    Route::post('/musteri-hatirlatmalari/{sikayet}/dashboard-gonder', [SikayetHatirlatmaController::class, 'dashboardGonder'])->name('iaa.hatirlatmalarim.dashboardGonder');
});
...
Route::middleware(['auth', BlockCustomerAccess::class])->group(function ()
{
    ...
    Route::post('/dashboard/switch-customer', [DashboardController::class, 'switchCustomer'])->name('dashboard.switch-customer');
    ...
    Route::get('/kullanici-puanlari/{user}', [DashboardController::class, 'kullaniciPuanlari'])->name('profile.puanlar')->withTrashed();
    ...
    Route::get('/kullanici-profil/{user}', [ProfileController::class, 'show'])->name('profile.show')->withTrashed();
    ...
    Route::post('takim-istekleri/{davetiye}/reddet', [TakimController::class, 'istekReddet'])->name('takimlar.istekReddet');
    ...
    Route::post('/proje/{id}/talep-karar-direktor', [ProjectWorkspaceController::class, 'decideRequestByDirector'])->name('proje.decideRequestByDirector');
    ...
    // Talep Geri Alma Rotaları
    Route::post('/proje/{id}/talep-geri-al', [ProjectWorkspaceController::class, 'recallRequest'])->name('proje.recallRequest');
    Route::post('/proje/{id}/talep-kalite-geri-al', [ProjectWorkspaceController::class, 'recallRequestByQuality'])->name('proje.recallRequestByQuality');
    Route::post('/proje/{id}/talep-direktor-geri-al', [ProjectWorkspaceController::class, 'recallRequestByDirector'])->name('proje.recallRequestByDirector');
    Route::post('/proje/{id}/talep-superadmin-geri-al', [ProjectWorkspaceController::class, 'recallRequestBySuperadmin'])->name('proje.recallRequestBySuperadmin');
    ...
    // =============================================================
    // === PROJE TAMAMLAMA VE İŞLEMLERİ (YENİ) ===
    // =============================================================
    ...
    Route::get('proje-calisma-alani/{id}/tamamla-iadeli', function($id) { return redirect()->route('proje.workspace.show', $id); });
    ...
    Route::get('proje-calisma-alani/{id}/tamamla-iadesiz', function($id) { return redirect()->route('proje.workspace.show', $id); });
...
// admin/users/bekleyen-basvurular rotası (SSO Onay Mekanizması)
Route::get('users/bekleyen-basvurular', [UserController::class, 'onayBekleyenler'])->name('users.onay_bekleyenler');
Route::get('users/export', [UserController::class, 'exportExcel'])->name('users.export');
Route::post('users/import-preview', [UserController::class, 'importPreview'])->name('users.import_preview');
Route::post('users/import-confirm', [UserController::class, 'importConfirm'])->name('users.import_confirm');
Route::post('users/{id}/onayla', [UserController::class, 'basvuruOnayla'])->name('users.basvuru_onayla');
Route::post('users/{id}/geri-al', [UserController::class, 'basvuruGeriAl'])->name('users.basvuru_gerial');
Route::post('users/merkezden-cek', [UserController::class, 'merkezdenCek'])->name('users.merkezden_cek');
Route::post('users/{id}/reddet', [UserController::class, 'basvuruReddet'])->name('users.basvuru_reddet');



    // =================================================================
    // === MAIL BİLDİRİM LOGLARI (Yetki kontrolü controller içinde) ===
    // =================================================================
    ...
    Route::patch('/mail-logs/{id}/mark-resolved', [MailLogController::class, 'markResolved'])->name('mail-logs.mark-resolved');


    // =================================================================
    // === YENİ EKLENEN: MÜŞTERİŞİMİ (Superadmin + Yönetim + Kurul) ===
    // =================================================================
    Route::get('/musteriler/toplu-mail', \App\Livewire\Admin\TopluMusteriMail::class)->name('musteriler.toplu-mail');
    Route::get('/musteriler/toplu-mail-loglari', \App\Livewire\Admin\TopluMailLoglari::class)->name('musteriler.toplu-mail-loglari');
    ...
    // =================================================================
    Route::middleware(['role:Superadmin|Bölüm Lideri|Yonetim|Direktör|Bölüm Lider Yardımcısı'])->group(function ()
    {
    ...
    // BÖLÜM YÖNETİM MATRİSİ (Müdür Yardımcıları ve Yetki Dağıtımı)
    Route::middleware(['role:Superadmin|Bölüm Lideri|Direktör|Bölüm Lider Yardımcısı'])->group(function () {
        Route::get('bolum-yonetim-matrisi', [App\Http\Controllers\Admin\BolumYonetimMatrisiController::class, 'index'])->name('bolum-yonetim.index');
        Route::post('bolum-yonetim-matrisi/{user}/update', [App\Http\Controllers\Admin\BolumYonetimMatrisiController::class, 'update'])->name('bolum-yonetim.update');
    });

    // --- MAVİ YAKA PERSONEL YÖNETİMİ (Görüntüleme - Yonetim Dahil) ---
    Route::middleware(['role:Superadmin|Hukuk Admini|Bölüm Lideri|Bölüm Lider Yardımcısı|Yonetim'])->group(function () {
        Route::get('mavi-yaka', [App\Http\Controllers\Admin\MaviYakaController::class, 'index'])->name('mavi-yaka.index');
        Route::get('mavi-yaka/{maviYaka}', [App\Http\Controllers\Admin\MaviYakaController::class, 'show'])->name('mavi-yaka.show');
    });

    // --- MAVİ YAKA PERSONEL YÖNETİMİ (İşlem Yetkisi) ---
    Route::middleware(['role:Superadmin|Hukuk Admini|Bölüm Lideri|Bölüm Lider Yardımcısı'])->group(function ()
    {
        ...
        Route::resource('mavi-yaka', App\Http\Controllers\Admin\MaviYakaController::class)->except(['index', 'show'])->parameters([
            'mavi-yaka' => 'maviYaka'
        ]);
    }
     );
    ...
    // BÖLÜM KATEGORİLERİ (SUPERADMIN VE BÖLÜM KALİTE YÖNETİCİSİ)
    ...
    Route::resource('bolum-kategorileri', BolumKategorisiController::class)
        ->middleware('role:Superadmin|Bölüm Kalite Yöneticisi')
        ...
        // Kullanıcı Yönetimi (Görüntüleme - Yonetim Dahil)
    Route::middleware(['role:Superadmin|Bölüm Kalite Yöneticisi|Yonetim'])->group(function ()
    {
    ...
    // =================================================================
    // === GÜVENLİK DÜZELTMESİ: SUPERADMIN VE KALİTE YÖNETİCİLERİ ERİŞEBİLİR ===
    // =================================================================
Route::middleware(['role:Superadmin'])->group(function () {
    ...
    // =============================================================
    // SAHA KALİTE ANALİSTLERİ (MÜŞTERİ SAHA TEMSİLCİSİ)
    // Superadmin ve Bölüm Kalite Yöneticisi erişebilir
    // =============================================================
    Route::middleware(['role:Superadmin'])->group(function () {
        Route::get('musteri-saha-temsilcileri', [\App\Http\Controllers\Admin\MusteriSahaTemsilcisiController::class, 'index'])->name('musteri-saha-temsilcileri.index');
        Route::post('musteri-saha-temsilcileri', [\App\Http\Controllers\Admin\MusteriSahaTemsilcisiController::class, 'store'])->name('musteri-saha-temsilcileri.store');
        Route::post('musteri-saha-temsilcileri/{user}', [\App\Http\Controllers\Admin\MusteriSahaTemsilcisiController::class, 'update'])->name('musteri-saha-temsilcileri.update');
    });


    // =============================================================
    // GRUP: RAPORLAR (Superadmin, Yönetim ve Direktör Görebilir)
    // =============================================================
    Route::middleware(['role:Superadmin|Yonetim|Direktör'])->group(function ()
    {
    ...
    // =================================================================
    // === MÜŞTERİ HATIRLATMA SİSTEMİ (TICKET) ===
    // =================================================================
    ...
    // Müşteri Şikayet Analiz Raporu (YENİ)
    Route::get('musteri-sikayet-analiz-raporu', [ReportController::class, 'sikayetAnalizRaporu'])
        ->name('musteri-sikayet-analiz-raporu');
    Route::get('musteri-sikayet-karsilastirma', \App\Livewire\Admin\MusteriSikayetKarsilastirma::class)
        ->name('musteri-sikayet-karsilastirma')
        ->middleware(['role:Superadmin|Yonetim']);
    ...
    Route::middleware(['role:Superadmin|Yonetim|Müşteri Şikayeti Kurulu|Müşteri Şikayeti Kurulu - Yurt İçi|Müşteri Şikayeti Kurulu - Yurt Dışı|Müşteri Şikayeti Kurulu Yöneticisi|Müşteri Şikayeti Kurulu Yöneticisi - Yurt İçi|Müşteri Şikayeti Kurulu Yöneticisi - Yurt Dışı|Müşteri Şikayeti Çözüm Lideri|Bölüm Kalite Yöneticisi|Direktör|Bölüm Lideri|Bölüm Lider Yardımcısı|Müşteri Saha Temsilcisi'])->group(function() {
        ...
        Route::get('sikayet-hatirlatma-ayarlari', [SikayetHatirlatmaController::class, 'ayarlar'])->name('sikayet-hatirlatma.ayarlar');
        Route::post('sikayet-hatirlatma-ayarlari', [SikayetHatirlatmaController::class, 'ayarlariKaydet'])->name('sikayet-hatirlatma.ayarlariKaydet');
    });

    // Herkesin (Müşteri Dahil) erişebileceği hatırlatma aksiyonları
    Route::post('musteri-hatirlatmalari/{hatirlatma}/yorum', [SikayetHatirlatmaController::class, 'yorumEkle'])->name('sikayet-hatirlatma.yorum');
    Route::put('musteri-hatirlatmalari/yorum/{yorum}', [SikayetHatirlatmaController::class, 'yorumGuncelle'])->name('sikayet-hatirlatma.yorumGuncelle');
    Route::delete('musteri-hatirlatmalari/yorum/{yorum}', [SikayetHatirlatmaController::class, 'yorumSil'])->name('sikayet-hatirlatma.yorumSil');
    Route::patch('musteri-hatirlatmalari/{hatirlatma}/ikna-oldu', [SikayetHatirlatmaController::class, 'iknaOldu'])->name('sikayet-hatirlatma.iknaOldu');


    // Otomatik Hatırlatıcılar (Modül B)
    Route::middleware(['role:Superadmin'])->group(function() {
        Route::prefix('sistem-ayarlari/hatirlaticilar')->name('sikayet-hatirlaticilar.')->group(function() {
            Route::get('/', [SikayetHatirlaticiController::class, 'index'])->name('index');
            Route::get('/create', [SikayetHatirlaticiController::class, 'create'])->name('create');
            Route::post('/', [SikayetHatirlaticiController::class, 'store'])->name('store');
            Route::get('/{kural}/edit', [SikayetHatirlaticiController::class, 'edit'])->name('edit');
            Route::put('/{kural}', [SikayetHatirlaticiController::class, 'update'])->name('update');
    ...
    Route::middleware(['auth'])
        ...
        ->group(function ()
        {
                ...
                ->group(function ()
            {
                ...
                Route::post('/toplanti/{toplanti}/resend-mails', [App\Http\Controllers\Admin\DisiplinKuruluController::class, 'resendMails'])->name('toplanti.resendMails');
            ...
            Route::get('/raporlar', [App\Http\Controllers\Admin\DisiplinRaporController::class, 'index'])
                ->name('report')
                ->middleware(['role:Superadmin|Yonetim|Bölüm Lideri|Direktör|Hukuk Admini|Hukuk Yöneticisi|Disiplin Kurulu Başkanı|Disiplin Kurulu Üyesi|Bölüm Lider Yardımcısı']);
    ...
    Route::middleware(['role:Superadmin|Hukuk Yöneticisi|Hukuk Admini|Disiplin Kurulu Başkanı|Disiplin Kurulu Üyesi'])
        ...
        ->group(function ()
        {
            ...
            Route::post('/{case}/tekrar-gorusulmeli', [App\Http\Controllers\Admin\DisciplinaryController::class, 'postponeCase'])->name('postpone');
    ...
    // Arabulucular ve Dış Avukatlar (Görüntüleme - Yonetim Dahil)
    Route::group(['middleware' => ['role:Superadmin|Hukuk Admini|Hukuk Yöneticisi|Arabuluculuk Personel|Bölüm Kalite Yöneticisi|Yonetim']], function ()
    {
        Route::get('arabulucular', [ArabulucuController::class, 'index'])->name('arabulucular.index');
        Route::get('arabulucular/{arabulucular}', [ArabulucuController::class, 'show'])->name('arabulucular.show');
        Route::get('dis-avukatlar', [ExternalLawyerController::class, 'index'])->name('dis_avukatlar.index');
        Route::get('dis-avukatlar/{id}', [ExternalLawyerController::class, 'show'])->name('dis_avukatlar.show');
    });

    // prefix => 'admin' ZATEN ÜST GRUPTA VAR, BURADA TEKRAR YAZMAYIN
    Route::group(['middleware' => ['role:Superadmin|Hukuk Admini|Hukuk Yöneticisi|Arabuluculuk Personel|Bölüm Kalite Yöneticisi']], function ()
    {

        // --- LOG ROTASI (En Üste) ---
        Route::get('arabulucular/sistem-loglari', [ArabulucuController::class, 'logs'])
            ->name('arabulucular.logs')
            ->middleware('role:Superadmin|Bölüm Kalite Yöneticisi'); // Sadece Superadmin (ve Kalite Yöneticisi)
        ...
        Route::resource('arabulucular', ArabulucuController::class)->except(['index', 'show']);
...
Route::middleware(['auth'])
    ...
    ->group(function ()
    {
        ...
        // İtiraz Etme
        Route::post('/{case}/itiraz-et', [App\Http\Controllers\Admin\DisciplinaryController::class, 'submitAppeal'])->name('appeal.submit');
    });
...
// [FIX] Dashboard çökmesini önlemek için geçici dummy route
Route::get('/admin/saha-kalite-analistleri', function() {
