    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-bold text-xl text-gray-800 leading-tight flex items-center gap-3">
                ...
                <div>
                    {{ $arabulucu->name }}
                    <span class="block text-xs font-normal text-gray-500">Sicil: {{ $arabulucu->sicil_no }}</span>
                </div>
                
                {{-- Aktif/Pasif Rozeti --}}
                @if($arabulucu->is_active)
            ...
            <div class="flex gap-2">
                ...
                <form action="{{ route('admin.arabulucular.toggleStatus', $arabulucu->id) }}" method="POST">
                    @csrf @method('PATCH')
                    <button type="submit" class="px-4 py-2 text-sm font-bold rounded shadow-sm transition {{ $arabulucu->is_active ? 'bg-gray-200 text-gray-700 hover:bg-gray-300' : 'bg-green-600 text-white hover:bg-green-700' }}">
                        {{ $arabulucu->is_active ? 'Pasife Al' : 'Aktife Al' }}
                    </button>
                </form>

                <a href="{{ route('admin.arabulucular.edit', $arabulucu->id) }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded text-sm font-bold shadow-sm transition">
                    Düzenle
                </a>
                <a href="{{ route('admin.arabulucular.index') }}" class="bg-white border border-gray-300 text-gray-700 hover:bg-gray-50 px-4 py-2 rounded text-sm font-bold shadow-sm transition">
                    Geri Dön
                </a>
            </div>
        </div>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                {{-- SOL KOLON: PROFİL VE İSTATİSTİK --}}
                <div class="space-y-6">
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        ...
                        <ul class="space-y-3 text-sm text-gray-700">
                            <li class="flex items-start gap-3">
                                ...
                                <span>{{ $arabulucu->telefon ?? 'Telefon Yok' }}</span>
                            </li>
                            <li class="flex items-start gap-3">
                                ...
                                <span>{{ $arabulucu->email ?? 'E-posta Yok' }}</span>
                            </li>
                            <li class="flex items-start gap-3">
                                <svg class="w-5 h-5 text-gray-400 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                <span>{{ $arabulucu->adres ?? $arabulucu->sehir }}</span>
                            </li>
                        </ul>
                    </div>

                    {{-- 2. Sistem Kayıt Bilgisi --}}
                    <div class="bg-gray-50 p-6 rounded-xl border border-gray-200">
                        ...
                        <div class="text-xs space-y-2">
                            <div class="flex justify-between">
                                ...
                                <span class="font-mono text-gray-700">{{ $arabulucu->created_at->format('d.m.Y') }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500">Kaydeden:</span>
                                <span class="font-bold text-gray-700">{{ $arabulucu->creator->name ?? 'Sistem' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-gray-500">Son İşlem:</span>
                                <span class="font-mono text-gray-700">{{ $arabulucu->updated_at->diffForHumans() }}</span>
                            </div>
                        </div>
                    </div>

                    {{-- 3. Yıllık İstatistik ve Dosya Payı --}}
                    <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                        <h3 class="font-bold text-gray-800 border-b pb-2 mb-4 flex justify-between items-center">
                            <span>Yıllara Göre Sistem Payı</span>
                            <span class="text-xs font-normal text-gray-400 bg-gray-100 px-2 py-1 rounded">2024 - {{ now()->year }}</span>
                        </h3>

                        @if(count($yillikVeriler) > 0)
                            <div class="space-y-4">
                                @foreach($yillikVeriler as $veri)
                                    {{-- Her Yıl İçin Accordion Yapısı --}}
                                    <div x-data="{ expanded: false }" class="border border-gray-100 rounded-lg overflow-hidden">
                                        
                                        {{-- Başlık ve Bar Kısmı (Tıklanabilir) --}}
                                        <div @click="expanded = !expanded" class="p-3 bg-gray-50 cursor-pointer hover:bg-gray-100 transition">
                                            <div class="flex justify-between items-end mb-2">
                                                <div>
                                                    <span class="font-black text-gray-700 text-lg">{{ $veri['yil'] }}</span>
                                                    <span class="text-xs text-gray-500 ml-1">
                                                        (Sistem: {{ $veri['sistem_toplam'] }} / Bu Kişi: {{ $veri['kendi_toplam'] }})
                                                    </span>
                                                </div>
                                                <div class="flex items-center gap-1">
                                                    <span class="font-bold text-indigo-600 text-sm">%{{ number_format($veri['oran'], 1) }}</span>
                                                    {{-- Ok İkonu (Açılıp kapanınca döner) --}}
                                            ...
                                            <div class="w-full bg-gray-200 rounded-full h-2">
                                                <div class="bg-indigo-500 h-2 rounded-full transition-all duration-1000" style="width: {{ $veri['oran'] }}%"></div>
                                            </div>
                                        </div>

                                        {{-- Açılan Detay Listesi --}}
                                        <div x-show="expanded" x-collapse style="display: none;">
                                            <div class="p-3 bg-white border-t border-gray-100">
                                                @if($veri['dosyalar']->count() > 0)
                                                    <ul class="space-y-2">
                                                        @foreach($veri['dosyalar'] as $dosya)
                                                            <li class="flex justify-between items-center text-xs p-2 hover:bg-gray-50 rounded border border-transparent hover:border-gray-100">
                                                                <div class="flex flex-col">
                                                                    <a href="{{ route('admin.arabuluculuk.show', $dosya->id) }}" class="font-bold text-indigo-600 hover:underline">
                                                                        #{{ $dosya->dosya_no ?? 'No Yok' }}
                                                                    </a>
                                                                    {{-- PERSONEL İSMİ BURAYA EKLENDİ --}}
                                                                    <span class="text-[10px] text-gray-500 mt-0.5">
                                                                        <span class="font-semibold text-gray-600">Personel:</span> {{ $dosya->calisan->name ?? '?' }}
                                                                    </span>
                                                                </div>
                                                                ...
                                                                @endphp
                                                                <span class="px-2 py-0.5 rounded {{ $renk }}">
                                                                    {{ str_replace('_', ' ', ucfirst($dosya->status)) }}
                                                                </span>
                                                            </li>
                                                        @endforeach
                                                    </ul>
                        ...
                        {{-- === YENİ: GRAFİK ALANI === --}}
                        @if(collect($yillikVeriler)->sum('kendi_toplam') > 0)
                ...
                {{-- SAĞ KOLON: DOSYA LİSTESİ VE FİLTRELER --}}
<div class="lg:col-span-2 space-y-6">
    
    {{-- 1. FİLTRELEME ALANI --}}
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        <form action="{{ route('admin.arabulucular.show', $arabulucu->id) }}" method="GET">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                
                {{-- Anlık Personel Arama (JS ile çalışır) --}}
                <div>
                ...
                <div>
                    ...
                    <input type="date" name="baslangic_tarihi" value="{{ request('baslangic_tarihi') }}" class="w-full border-gray-300 rounded text-sm text-gray-600">
                </div>
                ...
                <div>
                    ...
                    <select name="durum" class="w-full border-gray-300 rounded text-sm text-gray-600">
                        ...
                        <option value="kapatildi" {{ request('durum') == 'kapatildi' ? 'selected' : '' }}>Kapatıldı (Tamamlanan)</option>
                        <option value="arabulucuda" {{ request('durum') == 'arabulucuda' ? 'selected' : '' }}>Arabulucuda</option>
                        <option value="odeme_bekliyor" {{ request('durum') == 'odeme_bekliyor' ? 'selected' : '' }}>Ödeme Bekliyor</option>
                    </select>
                </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
            ...
            <span class="bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full text-xs font-bold">
                Toplam: {{ $groupedCases->flatten()->count() }}
            </span>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200" id="casesTable">
                ...
                <tbody class="bg-white">
                    @forelse($groupedCases as $yil => $dosyalar)
                        
                        {{-- YIL BAŞLIĞI (GRUP SATIRI) --}}
                        <tr class="bg-gray-50 border-b border-gray-200 year-header-row" data-year="{{ $yil }}">
                            <td colspan="6" class="px-6 py-2">
                                <div class="flex items-center gap-2">
                                    <span class="text-sm font-black text-gray-700 bg-gray-200 px-2 py-1 rounded">{{ $yil }}</span>
                                    <span class="text-xs text-gray-500 font-medium">({{ $dosyalar->count() }} Dosya)</span>
                                </div>
                            </td>
                        </tr>

                        {{-- O YILA AİT DOSYALAR --}}
                        @foreach($dosyalar as $case)
                            <tr class="hover:bg-indigo-50 transition border-b border-gray-100 case-row" data-personel="{{ strtolower($case->calisan->name ?? '') }}">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-sm font-bold text-gray-900">#{{ $case->dosya_no }}</span>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-2">
                                            <div class="text-xs font-bold text-gray-800">{{ $case->calisan->name ?? 'Silinmiş' }}</div>
                                            <div class="text-[10px] text-gray-500">{{ $case->calisan->email ?? '' }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm text-gray-700 font-mono">
                                    {{ $case->anlasilan_tutar ? number_format($case->anlasilan_tutar, 2) . ' ₺' : '-' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('admin.arabuluculuk.show', $case->id) }}" class="text-indigo-600 hover:text-indigo-900 font-bold hover:underline">İncele</a>
                                </td>
                            </tr>
                        @endforeach
...
<script>
    ...
    {{-- ApexCharts CDN (Grafik Kütüphanesi) --}}
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var rawData = @json($yillikVeriler);
