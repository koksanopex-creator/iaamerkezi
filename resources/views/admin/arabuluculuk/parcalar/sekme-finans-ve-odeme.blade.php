    ...
    <div class="bg-green-50 p-6 rounded-lg border border-green-100">
        ...
        <div class="flex justify-between items-start mb-6 border-b border-green-200 pb-4">
            ...
            <div class="flex flex-col sm:flex-row items-end gap-2">
                @php $dekont = $case->files->where('doc_type', 'dekont')->where('is_active', true)->last(); @endphp
                @if($dekont)
                ...
                @endif
                @php $sonTutanak = $case->files->where('doc_type', 'arabuluculuk_son_tutanak')->where('is_active', true)->first(); @endphp
                @if($sonTutanak)
        ...
        @php $mevcutOdeme = $case->payments->first(); @endphp 
        @if(!$mevcutOdeme || !empty($mevcutOdeme->red_gerekcesi))
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div class="col-span-2 mb-4">
                            ...
                            <div class="flex flex-col sm:flex-row gap-3">
                                ...
                                {{-- DÜZELTME 1: $arabuluculuk yerine $case ve relatedUser yerine calisan --}}
                                <input type="text" class="w-full sm:w-7/12 border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-transparent px-4 py-3 bg-gray-50" name="odenecek_kisi" id="odenecek_kisi_ad_soyad" value="{{ $mevcutOdeme ? $mevcutOdeme->odenecek_kisi : ($case->calisan->name ?? '') }}" placeholder="Ad Soyad Giriniz..." readonly>
                            </div>
                            <div class="mt-2 flex items-start gap-2 text-xs text-gray-600 bg-blue-50 p-3 rounded-lg border border-blue-100">
                                ...
                                {{-- DÜZELTME 2: Sinan Poyraz kaldırıldı, değişkenler düzeltildi --}}
                                <span id="input_aciklama">Ödeme varsayılan olarak ilgili personele ({{ $case->calisan->name ?? 'Çalışan Bilgisi Yok' }}) yapılacaktır.</span>
                            </div>
                        </div>
                        ...
                        <div class="col-span-2 md:col-span-1">
                            ...
                            <select name="banka_adi" x-model="banka" class="w-full border-gray-300 rounded text-sm focus:ring-green-500 focus:border-green-500" required>
                                ...
                                @foreach($bankalar as $b)
                                    <option value="{{ $b }}" {{ ($mevcutOdeme && $mevcutOdeme->banka_adi == $b) ? 'selected' : '' }}>{{ $b }}</option>
                                @endforeach
                        ...
                        <div class="col-span-2 md:col-span-1">
                            ...
                            <input type="text" name="iban" maxlength="34" class="w-full border-gray-300 rounded text-sm font-mono uppercase focus:ring-green-500 focus:border-green-500" value="{{ $mevcutOdeme ? $mevcutOdeme->iban : '' }}" placeholder="TR76 0000..." required oninput="this.value = this.value.toUpperCase()">
            ...
            <div class="bg-white rounded-lg border border-gray-200 p-4 mb-6 shadow-sm">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600 font-mono text-sm tracking-wide bg-gray-100 px-2 py-1 rounded inline-block mt-1">{{ $case->payments->first()->iban }}</p>
                    </div>
                    <div>
                    ...
                    @if($case->payments->first()->son_odeme_tarihi && $case->payments->first()->odeme_durumu != 'odendi')
                        <div class="col-span-2 mt-2">
                            <div class="flex items-center gap-2 p-2 bg-red-50 border border-red-200 rounded animate-pulse">
                                ...
                                <span class="text-red-700 font-bold text-sm">SON ÖDEME TARİHİ: {{ \Carbon\Carbon::parse($case->payments->first()->son_odeme_tarihi)->format('d.m.Y') }}</span>
                            </div>
                        </div>
                    @endif
                </div>
