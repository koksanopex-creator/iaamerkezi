...
<script>
    (function () {
        function initCharts() {
            ...
            // ==============================
            let rawCatSeries = <?php echo json_encode($charts['cat_data'], 15, 512) ?>;
            let rawCatLabels = <?php echo json_encode($charts['cat_labels'], 15, 512) ?>;
            let catSeries = [];
            let catLabels = [];
            ...
            // 3) DURUM ANALİZİ
            // ==============================
            let rawStatusSeries = <?php echo json_encode($charts['status_data'], 15, 512) ?>;
            ...
            // ==============================
            let rawLabels = <?php echo json_encode($bolumMemnuniyeti->pluck('bolum_adi'), 15, 512) ?>;
            let rawOnay = <?php echo json_encode($bolumMemnuniyeti->pluck('onay_sayisi')->map(fn($v) => (int) $v), 15, 512) ?>;
            let rawRed = <?php echo json_encode($bolumMemnuniyeti->pluck('red_sayisi')->map(fn($v) => (int) $v), 15, 512) ?>;
            let rawRevizyon = <?php echo json_encode($bolumMemnuniyeti->pluck('revizyon_sayisi')->map(fn($v) => (int) $v), 15, 512) ?>;
...
</script><?php /**PATH C:\Users\celal.karaman\Desktop\Projelerim\iaa_projesi\resources\views/admin/raporlar/partials/executive/scripts.blade.php ENDPATH**/ ?>