    ...
    <div id="modalHataliBildirim" class="hidden fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                ...
                <form action="<?php echo e(route('proje.markAsFaulty', $iaa->id)); ?>" method="POST">
                    <?php echo csrf_field(); ?>
...
<?php if($iaa->hatali_bildirim_gerekcesi): ?>
    <div
        ...
        <div class="p-5 space-y-6">
            ...
            <div class="flex gap-4 relative">
                ...
                <div class="flex-shrink-0 relative">
                    <img src="<?php echo e($iaa->hataliBildirimLiderUser->profile_photo_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($iaa->hataliBildirimLiderUser->name ?? 'TL') . '&color=7F9CF5&background=EBF4FF'); ?>"
                ...
                <div class="flex-1 bg-white/60 p-3 rounded-xl border border-orange-100 shadow-sm">
                    <div class="flex items-center justify-between mb-1">
                        <span
                            class="text-xs font-bold text-gray-900"><?php echo e($iaa->hataliBildirimLiderUser->name ?? 'Takım Lideri'); ?></span>
                        <span
                            class="text-[10px] text-gray-400 font-medium"><?php echo e($iaa->hatali_bildirim_tarihi ? \Carbon\Carbon::parse($iaa->hatali_bildirim_tarihi)->format('d.m.Y H:i') : '-'); ?></span>
                    </div>
                    <p class="text-xs font-bold text-orange-600 mb-1">RAPORLAMA GEREKÇESİ:</p>
                    <p class="text-sm text-gray-700 italic leading-relaxed">"<?php echo e($iaa->hatali_bildirim_gerekcesi); ?>"</p>
                </div>
            </div>

            
            <?php if($iaa->hatali_bildirim_kalite_notu || $iaa->hataliBildirimKaliteUser): ?>
                <div class="flex gap-4 relative">
                    <div class="absolute left-5 top-10 bottom-0 w-0.5 bg-orange-200/50"></div>
                    <div class="flex-shrink-0 relative">
                        <img src="<?php echo e($iaa->hataliBildirimKaliteUser->profile_photo_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($iaa->hataliBildirimKaliteUser->name ?? 'KY') . '&color=F59E0B&background=FEF3C7'); ?>"
                            class="w-10 h-10 rounded-full border-2 border-white shadow-sm object-cover" alt="KY">
                    </div>
                    <div class="flex-1 bg-white/60 p-3 rounded-xl border border-orange-100 shadow-sm">
                        <div class="flex items-center justify-between mb-1">
                            <span
                                class="text-xs font-bold text-gray-900"><?php echo e($iaa->hataliBildirimKaliteUser->name ?? 'Kalite Yöneticisi'); ?></span>
                            <span
                                class="text-[10px] text-gray-400 font-medium"><?php echo e($iaa->hatali_bildirim_kalite_at ? \Carbon\Carbon::parse($iaa->hatali_bildirim_kalite_at)->format('d.m.Y H:i') : '-'); ?></span>
                        </div>
                        <p class="text-xs font-bold text-yellow-600 mb-1">KALİTE ONAYI NOTU:</p>
                        <p class="text-sm text-gray-700 italic leading-relaxed">
                            "<?php echo e($iaa->hatali_bildirim_kalite_notu ?? 'Not belirtilmedi.'); ?>"</p>
                    </div>
                </div>
            <?php endif; ?>

            
            <?php if($iaa->hatali_bildirim_direktor_notu || $iaa->hataliBildirimDirektorUser): ?>
                <div class="flex gap-4 relative">
                    <div class="absolute left-5 top-10 bottom-0 w-0.5 bg-orange-200/50"></div>
                    <div class="flex-shrink-0 relative">
                        <img src="<?php echo e($iaa->hataliBildirimDirektorUser->profile_photo_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($iaa->hataliBildirimDirektorUser->name ?? 'DR') . '&color=4F46E5&background=E0E7FF'); ?>"
                            class="w-10 h-10 rounded-full border-2 border-white shadow-sm object-cover" alt="DR">
                    </div>
                    <div class="flex-1 bg-white/60 p-3 rounded-xl border border-orange-100 shadow-sm">
                        <div class="flex items-center justify-between mb-1">
                            <span
                                class="text-xs font-bold text-gray-900"><?php echo e($iaa->hataliBildirimDirektorUser->name ?? 'Direktör'); ?></span>
                            <span
                                class="text-[10px] text-gray-400 font-medium"><?php echo e($iaa->hatali_bildirim_direktor_at ? \Carbon\Carbon::parse($iaa->hatali_bildirim_direktor_at)->format('d.m.Y H:i') : '-'); ?></span>
                        </div>
                        <p class="text-xs font-bold text-indigo-600 mb-1">DİREKTÖR KAPANIŞ NOTU:</p>
                        <p class="text-sm text-gray-700 italic leading-relaxed">
                            "<?php echo e($iaa->hatali_bildirim_direktor_notu ?? 'Not belirtilmedi.'); ?>"</p>
                    </div>
                </div>
            <?php endif; ?>

            
            <?php if($iaa->hatali_bildirim_superadmin_notu || $iaa->hataliBildirimSuperadminUser): ?>
                <div class="flex gap-4 relative">
                    <div class="flex-shrink-0 relative">
                        <img src="<?php echo e($iaa->hataliBildirimSuperadminUser->profile_photo_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($iaa->hataliBildirimSuperadminUser->name ?? 'SA') . '&color=DC2626&background=FEE2E2'); ?>"
                            class="w-10 h-10 rounded-full border-2 border-white shadow-sm object-cover" alt="SA">
                    </div>
                    <div class="flex-1 bg-white/60 p-3 rounded-xl border border-orange-100 shadow-sm">
                        <div class="flex items-center justify-between mb-1">
                            <span
                                class="text-xs font-bold text-gray-900"><?php echo e($iaa->hataliBildirimSuperadminUser->name ?? 'Üst Yönetim'); ?></span>
                            <span
                                class="text-[10px] text-gray-400 font-medium"><?php echo e($iaa->hatali_bildirim_superadmin_at ? \Carbon\Carbon::parse($iaa->hatali_bildirim_superadmin_at)->format('d.m.Y H:i') : '-'); ?></span>
                        </div>
                        <p class="text-xs font-bold text-red-600 mb-1">YÖNETİM FİNAL NOTU:</p>
                        <p class="text-sm text-gray-700 italic leading-relaxed">
                            "<?php echo e($iaa->hatali_bildirim_superadmin_notu ?? 'Not belirtilmedi.'); ?>"</p>
                    </div>
                </div>
            <?php endif; ?>
        </div>
...
<?php if($isFaultyPendingQuality && ($isQuality || $isSuperAdmin)): ?>
    <div class="mb-6 bg-yellow-50 border-2 border-yellow-200 rounded-xl p-6 shadow-sm relative overflow-hidden">
        ...
        <form action="<?php echo e(route('proje.decideFaultyByQuality', $iaa->id)); ?>" method="POST" class="mt-4">
            <?php echo csrf_field(); ?>
            <div>
...
<?php if($isFaultyPendingDirector && ($isDirector || $isSuperAdmin)): ?>
    <div class="mb-6 bg-slate-800 text-white rounded-xl p-6 shadow-xl relative overflow-hidden">
        ...
        <form action="<?php echo e(route('proje.decideFaultyByDirector', $iaa->id)); ?>" method="POST" class="mt-4">
            <?php echo csrf_field(); ?>
            <div class="mb-4">
...
<?php if($isFaultyPendingSuperadmin && $isSuperAdmin): ?>
    <div class="mb-6 bg-indigo-900 text-white rounded-xl p-6 shadow-2xl relative overflow-hidden ring-4 ring-indigo-500/20">
        ...
        <form action="<?php echo e(route('proje.decideFaultyBySuperadmin', $iaa->id)); ?>" method="POST" class="mt-4">
            <?php echo csrf_field(); ?>
            <div class="mb-4">
...
<?php if($isLeader && ($isFaultyPendingQuality || $isFaultyPendingDirector || $isFaultyPendingSuperadmin)): ?>
    <div class="mb-6 bg-orange-50 border border-orange-200 rounded-xl p-3 shadow-sm flex items-center justify-between">
        ...
        <form action="<?php echo e(route('proje.recallFaulty', $iaa->id)); ?>" method="POST">
            <?php echo csrf_field(); ?>
            <button type="submit"
...
<?php endif; ?><?php /**PATH C:\Users\celal.karaman\Desktop\Projelerim\iaa_projesi\resources\views/proje-calisma-alani/partials/_faulty-notification.blade.php ENDPATH**/ ?>