...
@if($iaa->visit_planned)
    <div id="ziyaret-bilgileri-alani" class="mt-8 mb-8 bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md">
        ...
        <div class="p-2 bg-white">
            {{-- Ziyaret detaylarını göstermek için PlanVisit bileşenini salt okunur modda çağırıyoruz --}}
            @livewire('project.plan-visit', ['iaa' => $iaa, 'embedded' => true], key('visit-form-card-'.$iaa->id))
        </div>
    </div>
@endif
