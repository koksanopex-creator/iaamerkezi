...
<head>
    ...
    <title>Disiplin Karar Tutanağı - #{{ $case->id }}</title>
    <style>
        @font-face {
...
<body>
    ...
    <div class="page-header">
        ...
        @if($case->durum == 'Karar Verildi' && $case->karar_tarihi)
            <div class="header-date">
                Tarih: {{ $case->karar_tarihi->format('d.m.Y') }}
            </div>
        @endif
    </div>
    ...
    <div class="content">
        @if($case->durum === 'Savunma Bekleniyor')
            ...
            <div style="text-align: center; font-weight: bold; font-size: 16px; margin-top: -20px; margin-bottom: 30px; color: #666;">(Sistem Önerisi: {{ $cezaAdi }})</div>
        @else
            <div class="main-title">DİSİPLİN KURULU KARAR TUTANAĞI</div>
            <div style="text-align: center; font-weight: bold; font-size: 16px; margin-top: -20px; margin-bottom: 30px; color: #666;">({{ $cezaAdi }})</div>
        @endif
        ...
        <div class="info-row">
            ...
            <span class="info-colon">:</span>
            <span class="info-value" style="font-weight:bold; text-decoration: underline;">{{ $case->user->name }}</span>
        </div>
        <div class="info-row">
            <span class="info-label">T.C. Kimlik No</span>
            <span class="info-colon">:</span>
            <span class="info-value">{{ $case->user->tc_kimlik_no ?? '' }}</span>
        </div>
        @if($case->user->bolum)
        <div class="info-row">
            <span class="info-label">Bölümü / Görevi</span>
            <span class="info-colon">:</span>
            <span class="info-value">{{ $case->user->bolum->ad ?? '-' }} / {{ $case->user->unvan ?? '-' }}</span>
        </div>
        @endif
        ...
        <div class="info-row">
            ...
            <span class="info-colon">:</span>
            <span class="info-value">{{ $case->olay_tarihi->format('d.m.Y') }}</span>
        </div>
        <div class="info-row">
            <span class="info-label">Tutanak Tarihi</span>
            <span class="info-colon">:</span>
            <span class="info-value">{{ $case->created_at->format('d.m.Y H:i') }}</span>
        </div>
        ...
        <div>
            ...
            <div style="margin-bottom: 30px; text-align: justify; line-height: 1.6;">
                @if(isset($kararMetni) && !empty($kararMetni))
                    {!! $kararMetni !!}
                @else
                    {!! nl2br(e(trim($case->olay_aciklamasi))) !!}
                @endif
            </div>
        </div>
        ...
        @if(isset($canSeeSensitiveInfo) && $canSeeSensitiveInfo)
            <div class="signatures-area">
                <table class="sig-table">
                    <thead>
                        <tr>
                            @foreach($councilMembers as $member)
                                <th>{{ $member->hasRole('Disiplin Kurulu Başkanı') ? 'BAŞKAN' : 'ÜYE' }}</th>
                            @endforeach
                        </tr>
                    </thead>
                    <tbody>
        ...
        {{-- İmza alanı için yer tutucu boşluk --}}
