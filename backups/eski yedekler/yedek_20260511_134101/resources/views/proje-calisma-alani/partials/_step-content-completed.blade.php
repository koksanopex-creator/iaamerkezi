@props(['progressUpdate', 'step', 'isAssignedToSomeoneElse' => false, 'stepAssignment' => null])
...
<div x-show="open" x-transition class="mt-4 border-t-2 border-gray-100 pt-4 space-y-6">
    ...
    <div class="flex justify-between items-center bg-gray-50 p-3 rounded-lg border border-gray-200">
        ...
        @if(!$isLocked)
            ...
            @else
                @php $sorumluUser = $stepAssignment ? \App\Models\User::find($stepAssignment->user_id) : null; @endphp
                <span class="inline-flex items-center px-3 py-1.5 bg-blue-50 border border-blue-100 rounded-md font-semibold text-xs text-blue-500 uppercase tracking-widest cursor-help" title="Bu adım {{ $sorumluUser ? $sorumluUser->name : 'başkasına' }} atanmıştır. Sadece sorumlu veya lider düzenleyebilir.">
                    ...
                    Sorumlu: {{ $sorumluUser ? Str::limit($sorumluUser->name, 15) : 'Atanmış' }}
                </span>
            @endif
        @else
