...
<?php $component->withAttributes([]); ?>
    <?php $__env->startPush('pageTitle', $case->user->name . ' - Disiplin Dosyası | '); ?>
            ...
            <div class="relative flex flex-col lg:flex-row lg:items-center justify-between gap-8">
                        ...
                        <div @click="showHistory = !showHistory" class="relative group cursor-pointer flex items-center gap-6">
                            <div class="flex flex-col items-center">
                                ...
                                <div class="mt-2 text-[8px] font-bold text-slate-400 uppercase tracking-tighter text-center">
                                    Önceki Kayıt: 
                                    <span class="font-black <?php echo e($prevCount > 3 ? 'text-red-600' : ($prevCount > 0 ? 'text-amber-500' : 'text-slate-900')); ?>">
                                        <?php echo e($prevCount); ?> Adet
                                    </span>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="text-[11px] font-bold text-slate-400 mb-3 flex flex-wrap items-center gap-x-3 gap-y-1">
                                    <?php if($case->user->hire_date): ?>
                                        <div class="flex items-center gap-1.5">
                                            <svg class="w-3.5 h-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                            İşe Giriş: <span class="text-slate-600 font-black uppercase tracking-tight"><?php echo e($case->user->hire_date->format('d.m.Y')); ?></span>
                                        </div>
                                        <?php
                                        ...
                                        <?php if($duration): ?>
                                            <div class="flex items-center gap-1.5 bg-slate-50 px-2 py-0.5 rounded-lg border border-slate-100">
                                                ...
                                                <span class="text-slate-600 font-black uppercase tracking-tight"><?php echo e($duration); ?></span>
                                            </div>
                                        <?php endif; ?>
                                    <?php else: ?>
                                ...
                                <div class="flex flex-col gap-2">
                                    ...
                                    <span class="w-fit px-3 py-1 bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase tracking-widest rounded-lg border border-emerald-100 flex items-center gap-1.5 shadow-sm">
                                        ...
                                        Mevcut Puan: <?php echo e(number_format($case->user->toplam_puan, 0, ',', '.')); ?>
                            ...
                            <div x-show="showHistory" 
                                ...
                                <div class="p-5 bg-gradient-to-r from-slate-50 to-white border-b border-slate-100 flex items-center justify-between">
                                    <div>
                                        ...
                                        <h3 class="text-sm font-black text-slate-800"><?php echo e($case->user->name); ?></h3>
                                    </div>
                                    <span class="px-3 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-black rounded-full border border-indigo-100 italic">
                                        <?php echo e($prevCount > 0 ? $prevCount . ' Önceki Kayıt' : 'Temiz Sicil'); ?>
                                ...
                                <div class="max-h-[350px] overflow-y-auto custom-scrollbar bg-white">
                                    <?php if($prevCount > 0): ?>
                                        <?php $__currentLoopData = $previousCases; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $prev): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <a href="<?php echo e(route('admin.disiplin.show', $prev->id)); ?>" class="flex items-center gap-4 p-5 hover:bg-indigo-50/50 transition-all border-b border-slate-50 last:border-0 group/item">
                                                <div class="w-12 h-12 bg-slate-50 rounded-2xl border border-slate-200 flex flex-col items-center justify-center group-hover/item:border-indigo-200 group-hover/item:bg-white transition-all shadow-sm">
                                                    ...
                                                    <span class="text-xs font-black text-slate-600 group-hover/item:text-indigo-600">#<?php echo e($prev->id); ?></span>
                                                </div>
                                                <div class="flex-1 min-w-0">
                                                    <p class="text-xs font-black text-slate-800 truncate mb-1 uppercase tracking-tight">
                                                        <?php echo e($prev->behavior->category->ad ?? 'Genel İhlal'); ?>

                                                    </p>
                                                    <div class="flex items-center gap-2">
                                                        <span class="text-[10px] font-bold text-slate-400">
                                                            📅 <?php echo e($prev->olay_tarihi->format('d.m.Y')); ?>

                                                        </span>
                                                        <?php if($prev->final_karar): ?>
                                                            ...
                                                            <div class="flex items-center gap-1">
                                                                ...
                                                                <span class="text-[10px] font-black text-rose-600 uppercase tracking-tighter italic">
                                                                    <?php echo e(Str::limit($prev->final_karar, 30)); ?>
                                ...
                                <div class="p-4 bg-indigo-600 text-center relative overflow-hidden">
                                    ...
                                    <p class="relative text-[10px] font-black text-white uppercase tracking-[0.2em]">
                                        SİSTEMDE TOPLAM <?php echo e($prevCount); ?> KAYIT BULUNMAKTADIR
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="bg-indigo-50/50 border border-indigo-100/50 rounded-[2rem] p-6 relative group hover:bg-indigo-50 transition-all shadow-sm">
                        ...
                        <div class="grid grid-cols-3 gap-6 mb-5">
                            <div class="space-y-1.5">
                                ...
                                <span class="text-[11px] font-black text-slate-800 uppercase tracking-tight leading-tight block"><?php echo e($case->behavior->category->ad ?? 'Genel İhlal'); ?></span>
                            </div>
                            <div class="space-y-1.5">
                                <span class="text-[10px] font-black text-indigo-600 uppercase tracking-[0.15em] block">ETKİ VE ŞİDDET</span>
                                <span class="text-[11px] font-black text-slate-800 uppercase tracking-tight leading-tight block"><?php echo e($case->impact->tanim ?? '-'); ?></span>
                            </div>
                            <div class="space-y-1.5">
                                <span class="text-[10px] font-black text-indigo-600 uppercase tracking-[0.15em] block">KAPSAM</span>
                                <span class="text-[11px] font-black text-slate-800 uppercase tracking-tight leading-tight block"><?php echo e($case->scope->tanim ?? '-'); ?></span>
                            </div>
                        </div>
                        ...
                        <div class="pt-4 border-t border-indigo-100/60">
                            <p class="text-slate-600 text-xs md:text-sm font-semibold leading-relaxed italic">
                                ...
                                "<?php echo e($case->behavior->tanim); ?>" maddesinden dolayı açılan disiplin süreci.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col sm:flex-row lg:flex-col items-center lg:items-end gap-6">
                    <div class="flex items-center gap-3 w-full sm:w-auto">
                        ...
                        <?php if($canEdit && $case->durum != 'Karar Verildi' && $case->durum != 'İptal'): ?>
                            <a href="<?php echo e(route('admin.disiplin.edit', $case->id)); ?>"
                        ...
                        <a href="javascript:void(0)" onclick="openPrintModal('print', '<?php echo e(route('admin.disiplin.print', $case->id)); ?>')"
                            id="btn-print"
                        ...
                        <a href="javascript:void(0)" onclick="openPrintModal('pdf', '<?php echo e(route('admin.disiplin.download-pdf', $case->id)); ?>', '<?php echo e($pdfFileName); ?>')"
                            id="btn-download-pdf"
                    ...
                    <div class="text-right hidden lg:block">
                        ...
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <span class="text-xl font-black text-slate-300 italic">#<?php echo e($case->id); ?></span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="py-6">
        <div class="max-w-[1600px] mx-auto sm:px-6 lg:px-8">
            ...
            <?php if($case->durum === 'İtiraz Edildi'): ?>
                    ...
                    <div class="relative flex flex-col md:flex-row items-center gap-6 md:gap-8 p-6 md:p-8">
                        ...
                        <div class="flex-shrink-0">
                            <div class="bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl px-6 py-4 text-center shadow-inner">
                                ...
                                <span class="block text-lg font-black text-white tracking-tight"><?php echo e($case->appeals()->latest()->first()?->created_at->format('d.m.Y') ?? '-'); ?></span>
                                <span class="block text-[10px] font-bold text-indigo-200/60 mt-0.5"><?php echo e($case->appeals()->latest()->first()?->created_at->format('H:i') ?? ''); ?></span>
                            </div>
                        </div>
                    </div>
                </div>
            <?php elseif($case->is_appeal_window_open): ?>
                ...
                ?>
                <div class="relative mb-6 rounded-3xl overflow-hidden group border-2 <?php echo e($isUrgent ? 'border-rose-400 bg-rose-50' : 'border-amber-400 bg-amber-50'); ?> animate-[pulse_3s_infinite]">
                    
                    <div class="absolute inset-0 opacity-40">
                        <div class="absolute -right-12 -top-12 w-48 h-48 <?php echo e($isUrgent ? 'bg-rose-200' : 'bg-amber-200'); ?> rounded-full blur-3xl"></div>
                        <div class="absolute -left-6 -bottom-6 w-32 h-32 <?php echo e($isUrgent ? 'bg-rose-100' : 'bg-amber-100'); ?> rounded-full blur-2xl"></div>
                    </div>
                    ...
                    <div class="relative flex flex-col lg:flex-row items-center gap-4 lg:gap-6 p-4 md:p-6">
                        ...
                        <div class="flex items-center gap-4 flex-shrink-0">
                            <div class="relative">
                                ...
                                <div class="w-20 h-20 md:w-24 md:h-24 relative">
                                    <svg class="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                                        <circle cx="50" cy="50" r="42" fill="none" stroke="<?php echo e($isUrgent ? 'rgba(225,29,72,0.1)' : 'rgba(217,119,6,0.1)'); ?>" stroke-width="6"/>
                                        <circle cx="50" cy="50" r="42" fill="none" stroke="<?php echo e($isUrgent ? '#e11d48' : '#d97706'); ?>" stroke-width="6" stroke-linecap="round"
                                            stroke-dasharray="<?php echo e(2 * 3.14159 * 42); ?>" 
                                            stroke-dashoffset="<?php echo e(2 * 3.14159 * 42 * (1 - ($daysLeft / $totalDays))); ?>" 
                                            class="transition-all duration-1000"/>
                                    </svg>
                                    <div class="absolute inset-0 flex flex-col items-center justify-center">
                                        <span class="text-3xl md:text-4xl font-black <?php echo e($isUrgent ? 'text-rose-600 animate-pulse' : 'text-amber-600'); ?> leading-none"><?php echo e((int)$daysLeft); ?></span>
                                        <span class="text-[7px] md:text-[8px] font-black <?php echo e($isUrgent ? 'text-rose-400' : 'text-amber-400'); ?> uppercase tracking-[0.2em] mt-0.5">Gün</span>
                                    </div>
                                </div>
                                <?php if($isUrgent): ?>
                            ...
                            <div class="hidden lg:block w-px h-16 <?php echo e($isUrgent ? 'bg-rose-200' : 'bg-amber-200'); ?> rounded-full"></div>
                        </div>

                        
                        <div class="flex-1 text-center lg:text-left">
                            <div class="space-y-1">
                                <h4 class="text-lg md:text-xl font-black <?php echo e($isUrgent ? 'text-rose-900' : 'text-amber-900'); ?> tracking-tight uppercase leading-tight">
                                    <?php echo e(Auth::id() == $case->user_id ? 'İtiraz Hakkınız Aktif' : 'Personelin İtiraz Hakkı Aktif'); ?>

                                </h4>
                                <p class="<?php echo e($isUrgent ? 'text-rose-700/70' : 'text-amber-700/70'); ?> font-semibold text-xs md:text-sm max-w-lg">
                                    <?php echo e(Auth::id() == $case->user_id ? 'Verilen karara itiraz etmek için aşağıdaki butonu kullanabilirsiniz.' : 'Personel bu süre zarfında karara itiraz edebilir.'); ?>
                            ...
                            <div class="flex flex-wrap items-center justify-center lg:justify-start gap-2 mt-3">
                                <div class="flex items-center gap-1.5 <?php echo e($isUrgent ? 'bg-rose-100/50 text-rose-700' : 'bg-amber-100/50 text-amber-700'); ?> px-3 py-1.5 rounded-lg border <?php echo e($isUrgent ? 'border-rose-200' : 'border-amber-200'); ?> text-[11px] font-black">
                                    ...
                                    <span class="font-black"><?php echo e($deadline->format('d.m.Y')); ?></span>
                                </div>
                                <div class="flex items-center gap-1.5 <?php echo e($isUrgent ? 'bg-rose-100/50 text-rose-700' : 'bg-amber-100/50 text-amber-700'); ?> px-3 py-1.5 rounded-lg border <?php echo e($isUrgent ? 'border-rose-200' : 'border-amber-200'); ?> text-[11px] font-black">
                        ...
                        <?php if($canAppeal): ?>
                            <div class="flex-shrink-0 w-full lg:w-auto">
                                <button onclick="openAppealModal()" class="w-full lg:w-auto relative <?php echo e($isUrgent ? 'bg-rose-600 hover:bg-rose-700 text-white shadow-rose-200' : 'bg-amber-600 hover:bg-amber-700 text-white shadow-amber-200'); ?> px-8 py-3.5 rounded-2xl font-black shadow-xl transition-all hover:-translate-y-1 active:translate-y-0 flex items-center justify-center gap-3 text-sm md:text-base overflow-hidden">
                                    ...
                                    <span class="uppercase tracking-widest"><?php echo e(Auth::id() == $case->user_id ? 'İtiraz Et' : 'Personel Adına İtiraz Et'); ?></span>
                    ...
                    <div class="relative h-1.5 bg-black/10">
                        <div class="absolute inset-y-0 left-0 bg-white/40 rounded-r-full transition-all duration-1000" style="width: <?php echo e(100 - $progressPercent); ?>%"></div>
                    </div>
                </div>
                ...
                <div id="appealModal" class="fixed inset-0 z-[100] hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                        ...
                        <div class="inline-block align-bottom bg-white rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full border border-slate-200">
                            <form action="<?php echo e(route('disiplin.appeal.submit', $case->id)); ?>" method="POST">
                                <?php echo csrf_field(); ?>
                                    ...
                                    <div class="space-y-4">
                                        <?php if(Auth::id() != $case->user_id): ?>
                                            <div class="bg-amber-50 border-l-4 border-amber-500 p-4 rounded-r-xl">
                                                ...
                                                <p class="text-xs text-amber-600 mt-1 font-medium">Bu itirazı <span class="font-black"><?php echo e($case->user->name); ?></span> adına gerçekleştiriyorsunuz. İşlem log kaydına vekâlet bilgisi ile işlenecektir.</p>
                                            </div>
                                        <?php endif; ?>
            ...
            <div
                class="bg-<?php echo e($durumRenk); ?>-50 border-l-4 border-<?php echo e($durumRenk); ?>-500 p-4 mb-6 rounded-r shadow-sm flex justify-between items-center transition-all duration-500">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="font-bold text-<?php echo e($durumRenk); ?>-800 text-lg">
                            Dosya Durumu: <?php echo e($case->durum == 'Karar Verildi' ? 'Dosya Kapatıldı' : $case->durum); ?>
                            ...
                            <div class="mt-2 flex flex-col gap-1.5">
                                <?php if($isAdmin): ?>
                                    ...
                                    <div class="flex items-center gap-2 bg-white/50 backdrop-blur-sm rounded-lg px-3 py-1 border border-gray-100 shadow-sm w-fit">
                                        ...
                                        <span class="text-xs font-bold text-gray-500 uppercase"><?php echo e($case->sistem_oneri_ceza); ?></span>
                                    </div>
                                    
                                    <?php if($case->manual_penalty_name): ?>
                                        <div class="flex items-center gap-2 bg-rose-50/50 backdrop-blur-sm rounded-lg px-3 py-1 border border-rose-100 shadow-sm w-fit">
                                            ...
                                            <span class="text-xs font-black text-rose-700 uppercase"><?php echo e($case->manual_penalty_name); ?> <span class="ml-1 opacity-70">(-<?php echo e($case->hesaplanan_puan); ?> Puan)</span></span>
                                        </div>
                                    <?php endif; ?>
                                <?php else: ?>
                                    ...
                                    <div class="flex items-center gap-2 bg-rose-50/50 backdrop-blur-sm rounded-lg px-3 py-1 border border-rose-100 shadow-sm w-fit">
                                        ...
                                        <span class="text-xs font-black text-rose-700 uppercase">
                                            ...
                                            <?php if(!$isSavunmaKabul): ?>
                                                <span class="ml-1 opacity-70">(-<?php echo e($case->hesaplanan_puan); ?> Puan)</span>
                                            <?php endif; ?>
                                        </span>
                                    </div>
                                <?php endif; ?>
                        ...
                        <?php else: ?>
                            <p class="text-xs <?php echo e(!$isSavunmaKabul && $case->durum == 'Karar Verildi' ? 'text-red-600' : 'text-'.$durumRenk.'-600'); ?> font-bold <?php echo e((Auth::id() == $case->user_id && $case->durum == 'Savunma Bekleniyor') ? '' : 'uppercase'); ?> tracking-tight"><?php echo e($durumMetni); ?></p>
                        <?php endif; ?>
                    </div>
                </div>
                
                <div class="flex items-center gap-4 flex-shrink-0">
                    <?php if($case->durum == 'Savunma Bekleniyor' && Auth::id() == $case->user_id): ?>
                        <button onclick="const el = document.getElementById('defense_section'); if(el) { el.scrollIntoView({behavior: 'smooth'}); } else { window.location.href='<?php echo e(route('admin.disiplin.show', $case->id)); ?>?tab=detay#defense_section'; }" class="bg-amber-600 hover:bg-amber-700 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-lg shadow-amber-200 transition-all flex items-center gap-2 border border-amber-500 hover:scale-[1.02] active:scale-95 group">
                    ...
                    <?php if($case->durum == 'Yönetici Değerlendirmesi' && (Auth::user()->hasRole(['Superadmin', 'Hukuk Admini']) || Auth::user()->can('disiplin.degerlendirme.kullan'))): ?>
                    ...
                    <?php if(($case->rediscussion_count > 0 || $case->durum == 'Karar Verildi') && Auth::user()->hasRole(['Superadmin', 'Hukuk Admini', 'Disiplin Kurulu Başkanı', 'Disiplin Kurulu Üyesi'])): ?>
                        <?php
                            $isClosed = ($case->durum == 'Karar Verildi');
                            $bannerColor = $isClosed ? 'emerald' : 'amber';
                        ?>
                        <div class="flex-shrink-0 text-right <?php echo e(!$isClosed ? 'animate-pulse' : ''); ?>" style="animation-duration: 3s;">
                            <div class="bg-white/80 backdrop-blur-sm rounded-xl px-3 py-2 border border-<?php echo e($bannerColor); ?>-200 shadow-md shadow-<?php echo e($bannerColor); ?>-100/40">
                                <div class="flex items-center justify-end gap-1.5 mb-0.5">
                                    <span class="inline-block w-1.5 h-1.5 rounded-full bg-<?php echo e($bannerColor); ?>-500 <?php echo e(!$isClosed ? 'animate-ping' : ''); ?>" style="animation-duration: 2s;"></span>
                                    <span class="text-[9px] font-black text-<?php echo e($bannerColor); ?>-800 uppercase tracking-widest">
                                        <?php echo e($isClosed ? 'Görüşme Tamamlandı' : 'Görüşülme Durumu'); ?>

                                    </span>
                                </div>
                                <div class="text-base font-black text-<?php echo e($bannerColor); ?>-700">
                                    <?php echo e($case->rediscussion_count + 1); ?>. <?php echo e($isClosed ? 'Tur Kararı' : 'Kez'); ?>
                                ...
                                <?php if($isClosed): ?>
                                    <div class="mt-1 pt-1 border-t border-emerald-100">
                                        ...
                                        <div class="text-[11px] font-black text-emerald-700 flex items-center justify-end gap-1">
                                            ...
                                            <?php echo e(($case->oylama_bitti_at ?? $case->updated_at)->format('d.m.Y H:i')); ?>

                                        </div>
                                    </div>
                                <?php elseif($case->toplanti_tarihi): ?>
                                    <div class="mt-1 pt-1 border-t border-amber-100">
                                        ...
                                        <div class="text-[11px] font-black text-blue-700 flex items-center justify-end gap-1">
                                            ...
                                            <?php echo e(\Carbon\Carbon::parse($case->toplanti_tarihi)->format('d.m.Y H:i')); ?>
            ...
            ?>
            <?php if($case->oylama_aktif && $case->durum != 'Karar Verildi' && $activeTab !== 'kurul'): ?>
                <?php
                ...
                <?php if($isAuthorizedToVote): ?>
                    <div class="relative overflow-hidden bg-white border-2 border-indigo-500 rounded-2xl p-4 mb-6 shadow-xl animate-in zoom-in duration-300">
                        ...
                        <div class="relative flex flex-col md:flex-row items-center justify-between gap-4">
                            ...
                            <a href="<?php echo e(route('admin.disiplin.show', $case->id)); ?>?tab=kurul" class="w-full md:w-auto bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl font-bold transition-all shadow-lg shadow-indigo-200 flex items-center justify-center gap-2 group">
                                Oylama Odasına Git
            ...
            <?php if($case->durum == 'Kurulda' && !Auth::user()->hasRole(['Superadmin', 'Hukuk Yöneticisi', 'Hukuk Admini', 'Disiplin Kurulu Başkanı', 'Disiplin Kurulu Üyesi'])): ?>
                ...
                <div class="mt-4 bg-orange-50/50 border border-orange-200 rounded-3xl p-4 mb-4 shadow-xl shadow-orange-100/20 relative overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-500">
                    ...
                    <div class="relative flex flex-col md:flex-row items-center gap-5">
                            ...
                            <?php if($case->is_appealed): ?>
                                ...
                                <?php if($case->toplanti_tarihi): ?>
                                    <div class="mt-2 flex items-center justify-center md:justify-start gap-2">
                                        ...
                                        <span class="px-2 py-0.5 bg-orange-600 text-white text-[9px] font-bold rounded-lg shadow-sm">📅 <?php echo e($case->toplanti_tarihi->format('d.m.Y')); ?></span>
                                    </div>
                                <?php endif; ?>
                            <?php else: ?>
                                <div class="flex items-center justify-center md:justify-start gap-3 mb-1">
                                ...
                                <div class="mt-3 flex flex-wrap justify-center md:justify-start gap-2">
                                    <?php $__currentLoopData = $meetings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $meeting): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <div class="flex items-center gap-1 px-2 py-0.5 bg-white/60 border border-orange-100 rounded text-[9px] font-bold text-orange-800">
                                            <span class="opacity-50"><?php echo e($index + 1); ?>. Tur:</span>
                                            <?php echo e($meeting->baslangic_tarihi->format('d.m.Y')); ?>
                        ...
                        <?php if($case->is_appealed && $latestAppeal): ?>
                            <div class="flex-shrink-0 w-full md:w-64 bg-white/60 backdrop-blur-sm rounded-xl p-3 border border-orange-100 text-left">
                                <div class="flex items-center justify-between mb-1.5 border-b border-orange-100 pb-1">
                                    ...
                                    <span class="text-[9px] font-bold text-orange-400"><?php echo e($latestAppeal->created_at->format('d.m.Y')); ?></span>
                                </div>
                                <p class="text-orange-800 italic font-medium text-[11px] line-clamp-2" title="<?php echo e($latestAppeal->reason); ?>">"<?php echo e($latestAppeal->reason); ?>"</p>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            ?>
            <?php if((Auth::user()->hasRole(['Superadmin', 'Hukuk Admini', 'Disiplin Kurulu Başkanı', 'Disiplin Kurulu Üyesi']) || Auth::user()->can('disiplin.kurul.portal.gor')) && ( $case->durum === 'Kurulda' || ($case->durum === 'Karar Verildi' && $hasMeeting) )): ?>
                <?php
                    $activeTab = request()->get('tab', session('tab', 'detay'));
                ?>
                <div class="flex gap-1 mb-6 bg-slate-100 p-1 rounded-xl w-fit no-print">
                    <a href="<?php echo e(route('admin.disiplin.show', $case->id)); ?>?tab=detay"
                        class="px-5 py-2.5 rounded-lg text-sm font-bold transition-all duration-200 flex items-center gap-2 <?php echo e($activeTab === 'detay' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'); ?>">
                    ...
                    <a href="<?php echo e(route('admin.disiplin.show', $case->id)); ?>?tab=kurul"
                        class="px-5 py-2.5 rounded-lg text-sm font-bold transition-all duration-200 flex items-center gap-2 <?php echo e($activeTab === 'kurul' ? 'bg-gradient-to-r from-indigo-600 to-violet-600 shadow-lg shadow-indigo-200/60 text-white' : 'text-slate-500 hover:text-slate-700'); ?>">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <?php $activeTab = request()->get('tab', session('tab', 'detay')); ?>
            
            <?php if($activeTab === 'detay'): ?>
            ...
            <?php elseif($activeTab === 'kurul' && (Auth::user()->hasRole(['Superadmin', 'Hukuk Admini', 'Disiplin Kurulu Başkanı', 'Disiplin Kurulu Üyesi']) || Auth::user()->can('disiplin.kurul.portal.gor')) && ($case->durum === 'Kurulda' || ($case->durum === 'Karar Verildi' && $hasMeeting))): ?>
...
[$__name, $__params] = $__split('admin.disiplin.disiplin-oylama-paneli', ['case' => $case]);

$__html = app('livewire')->mount($__name, $__params, 'voting-tab-'.$case->id, $__slots ?? [], get_defined_vars());
...
?>
        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-12">
            <?php echo $__env->make('admin.disiplin.partials.logs', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
        </div>
    </div>
                    ...
                    <div class="sm:flex sm:items-start">
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            ...
                            <div class="mt-4">
                                ...
                                <div class="space-y-3">
                                    <label class="flex items-start p-3 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors <?php if(!$hasTemplate): ?> opacity-50 <?php endif; ?>">
                                        <div class="flex items-center h-5">
                                            <input type="radio" name="print_text_type" value="sablon" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300" onchange="updateTextarea()"
                                                <?php if($hasTemplate): ?> checked <?php else: ?> disabled <?php endif; ?>>
                                        </div>
                                        <div class="ml-3 text-sm">
                                            ...
                                            <?php if(!$hasTemplate): ?> <p class="text-red-500 text-xs mt-1">Bu ceza skalası için şablon bulunamadı.</p> <?php endif; ?>
                                        </div>
                                    </label>

                                    <label class="flex items-start p-3 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors <?php if(!$hasManualText): ?> opacity-50 <?php endif; ?>">
                                        <div class="flex items-center h-5">
                                            <input type="radio" name="print_text_type" value="manuel" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300" onchange="updateTextarea()"
                                                <?php if(!$hasTemplate && $hasManualText): ?> checked <?php elseif(!$hasManualText): ?> disabled <?php endif; ?>>
                                        </div>
                                        <div class="ml-3 text-sm">
                                            ...
                                            <?php if(!$hasManualText): ?> <p class="text-red-500 text-xs mt-1">Nihai karar gerekçesi girilmemiş.</p> <?php endif; ?>
                                        </div>
                                    </label>
                                </div>
    ...
    <script>
        ...
        const rawSablon = <?php echo json_encode($rawSablon ?? ''); ?>;
        const rawManuel = <?php echo json_encode($rawManuel ?? ''); ?>;
...
<?php endif; ?><?php /**PATH /var/www/kys_koksan/iaa/resources/views/admin/disiplin/show.blade.php ENDPATH**/ ?>