...
class IaaYonetimController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // --- YETKİ KONTROLÜ ---
        if (!$user->hasRole(['Superadmin', 'Direktör'])) {
    ...
    public function bolumOnayiVer(Request $request, Iaa $iaa)
    {
        ...
        try {
            ...
            if ($iaa->musteriSikayeti) {
                ...
                $temsilciler = collect();
                if ($sikayet->yetkili_user_id) $temsilciler->push($sikayet->yetkili_user);
    ...
    private function finalizeProjectApproval(Iaa $iaa, User $approver, string $approverTitle)
    {
        ...
        try {
            DB::transaction(function () use ($iaa, $takim, $projePuani, $approver, $approverTitle, &$puanVerildi, &$logAciklama, &$sikayetGuncellendi) {
                ...
                $isSafIaa = !$iaa->musteriSikayeti; // Şikayet bağlı değilse Saf İAA'dır
                ...
                if (is_null($iaa->onaylanma_tarihi) && $takim && $projePuani > 0) {
                    ...
                    if ($isSafIaa) {
                        ...
                        $logAciklama = "Proje {$approverTitle} tarafından onaylandı. " . $projePuani . ' puan, ' . $takim->ad . ' hanesine ve tüm takım üyelerine (' . $uyeIdleri->count() . ' kişi) eklendi.';
                    } else {
                ...
                if ($iaa->musteriSikayeti) {
                    ...
                    if ($msSikayet->yetkili_user_id) $msTemsilciler->push($msSikayet->yetkili_user);
    ...
    public function updateScore(Request $request, Iaa $iaa)
    {
        ...
        if ($iaa->durum === 'Tamamlandı') {
            try {
                ...
                \Illuminate\Support\Facades\Log::error('Puan güncelleme sonrası senkronizasyon hatası: ' . $e->getMessage());
            }
        }
