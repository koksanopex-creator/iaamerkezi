<x-app-layout>
    ...
    <div class="py-8 bg-gray-50/50 min-h-screen">
        <div class="max-w-[98%] mx-auto space-y-6">
            ...
            {{-- BOŞ ALPINE SARMALAYICI (GEREKİRSE GENEL STATE İÇİN) --}}
            <div x-data="{ mode: 'month' }" class="flex items-center gap-3 mb-4"></div>

            {{-- KPI + Son Gelen Şikayet --}}
<div 
    x-data="{
        mode: 'month',
        modes: ['month','all'],
...
<div class="rounded-lg border border-red-200 bg-red-50/60 p-3 mb-4 shadow-sm">
    ...
    <div class="flex justify-between items-center mb-2">
        ...
        <a href="{{ route('admin.sikayetler.show', $last->id) }}"
           class="text-[11px] font-semibold text-indigo-600 hover:underline">
            → Detay
        </a>
    </div>

    {{-- 2 Satırlık Ultra Kompakt Bilgi Alanı --}}
    <div class="grid grid-cols-1 md:grid-cols-6 gap-2 text-[11px] leading-tight">
        ...
        <div class="p-2 bg-white rounded border text-gray-700">
            ...
            <p class="font-bold truncate">{{ $last->musteri_adi }}</p>
        </div>

        {{-- Kategori --}}
        <div class="p-2 bg-white rounded border text-gray-700">
            <p class="text-[9px] uppercase text-gray-400 font-semibold">Kategori</p>
            <p class="font-bold truncate">{{ $last->sikayetKategori->ad ?? 'Genel' }}</p>
        </div>

        {{-- Sorumlu Birim --}}
        <div class="p-2 bg-white rounded border text-gray-700">
            <p class="text-[9px] uppercase text-gray-400 font-semibold">Sorumlu Birim</p>
            <p class="font-bold truncate">{{ $last->cozumTakimi->ad ?? 'Atanmamış' }}</p>
        </div>
        ...
        <div class="p-2 bg-white rounded border text-gray-700 col-span-2">
            ...
            <p class="font-bold truncate">{{ $last->musteri_sikayet_konusu }}</p>
        </div>
        ...
        <div class="p-2 bg-white rounded border text-gray-700">
            ...
            <p class="font-bold">
                {{ \Carbon\Carbon::parse($last->musteri_sikayet_tarihi)->locale('tr')->isoFormat('D MMM YYYY') }}
            </p>
        </div>
...
@endif
                ...
                {{-- FİLTRE SLIDER (Otomatik Geçişli) --}}
<div 
    x-data="{
        mode: 'month',
        modes: ['month', 'all'],
...
>
    ...
    <div class="text-center mt-1 mb-3">
        ...
        >
            Bu Ay ({{ now()->locale('tr')->translatedFormat('F Y') }})
        </span>
        ...
        >
            ...
            @if($sonSikayetler->isNotEmpty())
                <div class="bg-white rounded-xl shadow-sm border border-indigo-100 p-3 flex items-center gap-4 relative overflow-hidden"
                    ...
                    <div class="flex-1 h-8 relative overflow-hidden" @mouseenter="stopTimer()" @mouseleave="startTimer()">
                        @foreach($sonSikayetler->take(5) as $index => $sikayet)
                            @php
                            ...
                            <a href="{{ route('admin.sikayetler.show', $sikayet->id) }}" target="_blank"
                               class="absolute top-0 left-0 w-full h-full flex items-center px-3 rounded-lg border transition-all duration-500 ease-in-out transform cursor-pointer hover:brightness-95 {{ $satirStili }}"
                               x-show="activeNews === {{ $index }}"
                               ...
                               x-transition:leave-end="opacity-0 -translate-y-6">
                                <div class="flex items-center gap-1 text-sm w-full overflow-hidden font-medium">
                                    <span class="font-bold uppercase text-xs opacity-80">[{{ $kategori }}]</span>
                                    <span class="mx-1 opacity-50">-</span>
                                    <span class="font-bold whitespace-nowrap">{{ $tarih }}</span>
                                    <span class="whitespace-nowrap opacity-80">&nbsp;tarihinde&nbsp;</span>
                                    <span class="font-bold whitespace-nowrap">
                                        {{ Str::limit($sikayet->musteri_adi, 20) }}
                                    </span>
            ...
            {{-- 3. SIRA: SON ŞİKAYETLER LİSTESİ (DİKEY KAYAN AKIŞ / MARQUEE) --}}
                                ...
                                <div class="flex justify-between items-start">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        @if($ilkDosya && str_contains($ilkDosya->mime_tipi, 'image'))
                                            <img src="{{ asset('storage/'.$ilkDosya->dosya_yolu) }}"
                                    ...
                                    <div class="text-right">
                                        <span class="text-[11px] font-bold px-2 py-0.5 rounded
                                            {{ $isGecikmis ? 'bg-red-200 text-red-800' : 'bg-white/60 text-gray-800 border border-gray-200' }}">
                                            {{ $sikayet->musteri_durum }}
                                        </span>
            ...
            {{-- 4. SIRA: BÖLÜM PERFORMANS KARNESİ (SLIDER) --}}
            <div 
                x-data="{
                    slide: 0,
                        ...
                        @foreach(array_chunk($bolumPerformansi->toArray(), 2) as $chunk)
                            <div class="min-w-full grid grid-cols-1 md:grid-cols-2 gap-4 px-1">
                                @foreach($chunk as $bolum)
                                    ...
                                    {{-- Yıllık Mini Özet (Bu Yıl + Geçen Yıl + Önceki Yıl) --}}
                                    <div class="absolute top-0 right-0 flex gap-1 p-1">
                                        ...
                                        @foreach($yearsToShow as $yy)
                                            <div class="bg-orange-100 text-orange-700 rounded-bl-lg px-2 py-0.5 text-[9px] border-b border-l text-center">
                                                ...
                                                <span class="block text-[8px]">
                                                    T: {{ $bolum['yillik_detay'][$yy]['toplam'] ?? 0 }}
                                                    | Ç: {{ $bolum['yillik_detay'][$yy]['cozulen'] ?? 0 }}
                                                </span>
                                            </div>
                                        @endforeach
                                        ...
                                        <div class="mb-2">
                                            <div class="flex justify-between text-xs mb-1">
                                                ...
                                                <span class="font-bold {{ $bolum['basari_orani'] < 50 ? 'text-red-600' : 'text-green-600' }}">
                                                    %{{ $bolum['basari_orani'] }}
                                                </span>
                                            </div>
                                            <div class="w-full bg-gray-100 rounded-full h-2">
                                                <div class="bg-gradient-to-r from-blue-500 to-indigo-600 h-2 rounded-full" 
                                                     style="width: {{ $bolum['basari_orani'] }}%">
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        {{-- === YILLIK DETAYLI TABLO (BÜYÜK GÖRÜNÜM) === --}}
<div class="mt-4 border-t pt-3">
    ...
    <table class="w-full text-[11px] text-gray-700 border border-gray-200 rounded-lg overflow-hidden">
        ...
        <tbody>
            ...
            {{-- 5. SIRA: GRAFİK SLIDER (APEXCHARTS) --}}
            <div 
                x-data="{
                    gSlide: 0,
                    total: 4,
    ...
    {{-- CSS Animasyonu (Dikey Kaydırma İçin) --}}
    <style>
    ...
    @push('scripts')
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                // 2) Kategori Dağılımı (Pie)
                // ==============================
                const catSeries = @json($charts['cat_data']);
                const catLabels = @json($charts['cat_labels']);
    ...
    colors: [
        ...
        '#6366F1', '#8B5CF6', '#22D3EE', '#A3E635'
                ...
                // 3) Durum Analizi (Donut)
                // ==============================
                const statusSeries = @json($charts['status_data']);
