@php
    ...
    if (auth()->check() && $isComplaint) {
        ...
        if ($isLeaderOrAdminHeader || auth()->user()->hasRole('Bölüm Kalite Yöneticisi') || auth()->user()->hasRole('Kurul Üyesi') || auth()->user()->hasRole('Superadmin')) {
            $canSeeEvaluating = true;
        }
    }
...
@if($iaa->musteriSikayeti)
    {{-- Lider İçin Talep Formu (MODAL) --}}
    @if($isLeaderOrAdmin && in_array($durum, [null, 'Reddedildi']))
        <div x-data="{ modalOpen: false }" @open-extension-modal.window="modalOpen = true" x-show="modalOpen" x-cloak
            ...
            <div @click.outside="modalOpen = false"
                ...
                {{-- Üst Kısım: Başlık & Geçmiş Butonu --}}
                <div class="bg-orange-50 px-6 py-4 border-b border-orange-100 flex justify-between items-center">
                    ...
                    <div class="flex items-center space-x-2">
                        {{-- Ek Süre Geçmişi Butonu --}}
                        @if(isset($ekSureLoglari) && $ekSureLoglari->count() > 0)
                ...
                <div class="p-6">
                    <form action="{{ route('proje.talep.extension.request', $iaa->id) }}" method="POST">
                        @csrf
                        <div class="space-y-4">
                            <div>
    ...
    {{-- MODAL: Kullanıcı İçin Talep Durumu Bekleniyor Modalı --}}
    @if($durum == 'Talep Edildi' && (isset($canSeeEvaluating) && $canSeeEvaluating))
            ...
            <div @click.outside="modalOpen = false"
                ...
                <div class="p-6">
                    ...
                    <div class="p-3 bg-gray-50 rounded-lg border border-gray-200 text-sm text-gray-600 mb-4">
                        {{ $iaa->musteriSikayeti->ek_sure_talep_aciklamasi }}
                    </div>
                    ...
                    <div class="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg mt-4">
                        <p class="text-sm text-blue-700 font-medium">
                            ...
                            gelmezse proje bitiş tarihi "{{ $sonTarih ? $sonTarih->format('d.m.Y H:i') : '' }}" olarak geçerli
                            olacaktır.
                        </p>
                    </div>
    ...
    {{-- MODAL: Yönetici İçin Onay Bekliyor Modalı --}}
    @if($durum == 'Talep Edildi')
                    ...
                    <div class="p-6">
                        <div class="mb-5 space-y-2">
                            <p class="text-sm"><strong>Talep Eden:</strong> <span
                                    class="text-gray-700">{{ $iaa->atananTakim->lider->name ?? 'Belirsiz' }} (Takım Lideri)</span>
                            </p>
                            <p class="text-sm"><strong>Tarih:</strong> <span
                                    class="text-gray-700">{{ $iaa->musteriSikayeti->updated_at->format('d.m.Y H:i') }}</span></p>
                        ...
                        <div class="mt-6 flex flex-col sm:flex-row gap-3">
                            @if($direktorOnayiAktif && $isDirector)
                                <div class="flex-1 w-full" x-show="!isRejecting">
                                    <form action="{{ route('proje.talep.extension.director', $iaa->id) }}" method="POST">
                                        @csrf
                            ...
                            @elseif($isAdmin)
                                <div class="flex-1 w-full" x-show="!isRejecting">
                                    <form action="{{ route('proje.talep.extension.superadmin', $iaa->id) }}" method="POST">
                                        @csrf
                                        <input type="hidden" name="action" value="approve">
                                        <button type="submit"
                                            class="w-full justify-center bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-lg font-medium transition-colors shadow-sm flex items-center">
    ...
    {{-- MODAL: Ek Süre Onaylandı Sonuç Modalı --}}
    @if($durum == 'Onaylandı' && (isset($canSeeEvaluating) && $canSeeEvaluating))
        <div x-data="{ modalOpen: false }" @open-approved-extension-modal.window="modalOpen = true" x-show="modalOpen" x-cloak
            ...
            <div @click.outside="modalOpen = false"
                ...
                <div class="p-6">
                    ...
                    @if($iaa->musteriSikayeti->ek_sure_talep_aciklamasi)
                        <div class="p-3 bg-gray-50 rounded-lg border border-gray-200 text-sm text-gray-600 italic">
                            Talep Edilen Gerekçe: "{{ $iaa->musteriSikayeti->ek_sure_talep_aciklamasi }}"
                        </div>
                    @endif
                    <div class="mt-6 flex justify-end">
                        <button type="button" @click="modalOpen = false"
    ...
    {{-- MODAL: Ek Süre Reddedildi Sonuç Modalı --}}
    @if($durum == 'Reddedildi' && (isset($canSeeEvaluating) && $canSeeEvaluating))
