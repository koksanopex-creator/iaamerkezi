...
>
    ...
    <div class="<?php echo e($embedded ? 'p-4' : 'bg-white overflow-hidden shadow-xl sm:rounded-lg p-6 border-t-4 border-indigo-500'); ?>">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 <?php echo e($embedded ? 'bg-indigo-50/50 p-4 rounded-xl border border-indigo-100' : ''); ?>">
            <h3 class="<?php echo e($embedded ? 'text-md' : 'text-lg'); ?> font-bold text-gray-800 flex items-center mb-2 sm:mb-0">
            ...
            <!--[if BLOCK]><![endif]--><?php if(!$savedVisit): ?>
                <button wire:click="toggleForm" wire:loading.attr="disabled" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:border-indigo-900 focus:ring ring-indigo-300 disabled:opacity-50 transition ease-in-out duration-150 shadow-sm relative overflow-hidden">
                    <span wire:loading.remove wire:target="toggleForm"><?php echo e($isOpen ? 'Formu Kapat' : 'Yeni Ziyaret Planı Oluştur'); ?></span>
            ...
            <div class="bg-indigo-50 rounded-2xl p-6 border-2 border-indigo-100 shadow-sm">
                ...
                <div class="mt-6 pt-4 border-t border-indigo-100 flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <!--[if BLOCK]><![endif]--><?php if(in_array($status, ['İptal Edildi', 'Revize İsteniyor']) && $canEdit): ?>
                            <button wire:click="undoStatusAction" onclick="confirm('<?php echo e($status); ?> kararını geri alıp ziyaret planını tekrar Beklemede aşamasına çekmek istediğinize emin misiniz?') || event.stopImmediatePropagation()" class="px-4 py-2 bg-gray-100 text-gray-700 text-xs font-bold rounded-lg border border-gray-200 hover:bg-gray-200 transition shadow-sm">
                                <?php echo e($status === 'İptal Edildi' ? 'İptali Geri Al' : 'Revizyonu Geri Al'); ?>
...
unset($__errorArgs, $__bag); ?><!--[if ENDBLOCK]><![endif]-->
                            ...
                            <div x-data="{ 
                                search: '', 
                                open: false, 
                                selected: <?php if ((object) ('formData.visit_reason') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('formData.visit_reason'->value()); ?>')<?php echo e('formData.visit_reason'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('formData.visit_reason'); ?>')<?php endif; ?>.live,
                                reasons: <?php echo e(json_encode($customerData['visit_reasons'] ?? ['Şikayet İnceleme', 'Ürün Denemesi', 'Teknik Destek', 'Periyodik Ziyaret', 'Rutin Ziyaret', 'Diğer'])); ?>,
                                toggle(item) {
                                    if (<?php echo e($isReadOnly ? 'true' : 'false'); ?>) return;
                                ...
                                <div @click="<?php echo e($isReadOnly ? '' : 'open = !open'); ?>" @click.away="open = false" class="w-full text-sm border border-gray-300 rounded-xl px-3 py-2.5 bg-white cursor-pointer flex justify-between items-center" :class="{ 'opacity-50 cursor-not-allowed': <?php echo e($isReadOnly ? 'true' : 'false'); ?> }">
                            ...
                            <div x-data="{ 
                                search: '', 
                                open: false, 
                                selected: <?php if ((object) ('formData.customer_product_id') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('formData.customer_product_id'->value()); ?>')<?php echo e('formData.customer_product_id'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('formData.customer_product_id'); ?>')<?php endif; ?>.live,
                                products: <?php echo e(json_encode($customerData['products'] ?? [])); ?>,
                                toggle(id) {
                                    if (<?php echo e($isReadOnly ? 'true' : 'false'); ?>) return;
                                    if (!Array.isArray(this.selected)) this.selected = [];
                                    let strId = String(id);
                    ...
                    <div class="space-y-6">
                        ...
                        <?php if(($savedVisit['status'] ?? '') === 'Revize İsteniyor'): ?>
                        ...
                        <div class="bg-gray-50/50 p-5 rounded-3xl border border-gray-100 space-y-4 <?php echo e($isEnteringResults ? 'opacity-75 pointer-events-none' : ''); ?> mt-6">
                            ...
                            <div class="pt-2">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <!--[if BLOCK]><![endif]--><?php if($savedVisit && isset($savedVisit['visit_file']) && is_array($savedVisit['visit_file']) && count($savedVisit['visit_file']) > 0): ?>
...
unset($__errorArgs, $__bag); ?><!--[if ENDBLOCK]><![endif]-->
                ...
                <div class="pt-8 border-t border-gray-100 flex justify-end items-center gap-4">
                    ...
                    <!--[if BLOCK]><![endif]--><?php if($savedVisit && ($savedVisit['status'] ?? 'Beklemede') === 'Beklemede' && !$isReadOnly): ?>
                    ...
                    <!--[if BLOCK]><![endif]--><?php if(!$isReadOnly): ?>
                        <!--[if BLOCK]><![endif]--><?php if($isEnteringResults || ($savedVisit['status'] ?? '') === 'Onaylandı'): ?>
