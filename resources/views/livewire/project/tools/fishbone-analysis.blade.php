            ...
            <div class="mb-6">
                ...
                @else
                    <div class="w-full text-base bg-white border border-gray-200 rounded-md p-3 text-gray-800 font-semibold">
                        {{ $problem_statement ?: 'Problem tanımı girilmemiş.' }}
                    </div>
                @endif
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach($categoryLabels as $key => $label)
                    <div class="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden flex flex-col">
                        <div class="px-3 py-2 border-b border-gray-200 flex items-center {{ $categoryColors[$key] }}">
                            <h6 class="text-sm font-bold">{{ $label }}</h6>
                        </div>
                        <div class="p-3 flex-1 flex flex-col">
                            <ul class="space-y-2 mb-3 flex-1">
                                @forelse($categories[$key] as $index => $item)
                                    <li class="flex items-start text-sm group">
                                        ...
                                        @if($canManage)
                                            <button wire:click="removeItem('{{ $key }}', {{ $index }})" class="text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity ml-1">
                            ...
                            @if($canManage)
                                <div class="mt-auto pt-2 border-t border-gray-100">
                                    <div class="flex">
                                        <input type="text" wire:model="newItems.{{ $key }}" wire:keydown.enter="addItem('{{ $key }}')" class="block w-full text-xs border-gray-300 rounded-l-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" placeholder="Yeni neden yazıp Enter'a basın">
                                        <button wire:click="addItem('{{ $key }}')" class="bg-gray-100 text-gray-600 border border-gray-300 border-l-0 rounded-r-md px-2 hover:bg-gray-200">
