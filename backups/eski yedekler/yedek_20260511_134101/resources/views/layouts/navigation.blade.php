@php
    ...
    if (Auth::check()) {
        ...
        if ($user->hasRole('Superadmin')) {
            $pendingUserCount = \App\Models\User::where('onaylandi_mi', 0)->count();
                        ...
                        <div class="relative h-full flex items-center" x-data="{ open: false }" @click.away="open = false">
                            ...
                            <div x-show="open" x-cloak x-transition.origin.top.right
                                ...
                                @if($disiplinYetkisi)
                                    ...
                                    @if(Auth::user()->can('disiplin.kurul.portal.gor') || Auth::user()->hasRole(['Disiplin Kurulu Başkanı', 'Disiplin Kurulu Üyesi']))
                    ...
                    <div class="flex items-center h-full">
                        <div class="relative h-full flex items-center" x-data="{ open: false }" @click.away="open = false">
                            <button type="button" @click="open = !open"
                                ...
                                <span class="relative">
                                    Müşteri
                                    @if($sikayetApprovalCount > 0 || $pendingSikayetGorevCount > 0 || $unassignedSikayetCount > 0)
                                        <span class="absolute -top-1.5 -right-2.5 flex h-2.5 w-2.5 focus:outline-none">
                                            <span class="animate-ping absolute inline-flex h-full w-full rounded-full {{ $unassignedSikayetCount > 0 ? 'bg-rose-500' : 'bg-rose-400' }} opacity-75"></span>
                                            <span class="relative inline-flex rounded-full h-2.5 w-2.5 {{ $unassignedSikayetCount > 0 ? 'bg-rose-600' : 'bg-rose-500' }}"></span>
                                        </span>
                                    @endif
                                </span>
                                <svg class="w-3 h-3 opacity-50 transition-transform" :class="{ 'rotate-180': open }" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                            ...
                            <div x-show="open" x-cloak x-transition.origin.top.right
                                ...
                                @if(Auth::user()->canSeeMusteriOperasyonlari() || $kullaniciSikayetTakimindaMi)
                                    ...
                                    @if(Auth::user()->hasRole(['Superadmin', 'Yonetim', 'Müşteri Temsilcisi', 'Müşteri Şikayeti Kurulu', 'Müşteri Şikayeti Çözüm Lideri', 'Direktör', 'Bölüm Lideri']))
                                ...
                                @if(Auth::user()->canSeeMusteriOperasyonlari())
                                    ...
                                    <x-dropdown-link :href="route('admin.ziyaretler')">Müşteri Ziyaretleri</x-dropdown-link>
                                @endif
                                
                                @if(Auth::user()->hasZiyaretGorevi())
                                    <x-dropdown-link :href="route('admin.ziyaret-planlarim')">Ziyaret Planlarım</x-dropdown-link>
                                @endif
                    ...
                    <div class="flex items-center h-full">
                        <div class="relative h-full flex items-center" x-data="{ open: false }" @click.away="open = false">
                            <button type="button" @click="open = !open"
                                ...
                                <span class="relative">
                                    Yönetici
                                    @if($iaaApprovalCount > 0 || $pendingUserCount > 0 || $istekCount > 0)
                                        <span class="absolute -top-1.5 -right-2.5 flex h-2.5 w-2.5 focus:outline-none">
                                            <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-rose-400 opacity-75"></span>
                                            <span class="relative inline-flex rounded-full h-2.5 w-2.5 bg-rose-500"></span>
                                        </span>
                            ...
                            <div x-show="open" x-cloak x-transition.origin.top.right
                                ...
                                @if(Auth::user()->hasAnyRole(['Superadmin', 'Yonetim', 'Direktör']))
                                    ...
                                    <x-dropdown-link :href="Auth::user()->hasRole(['Yonetim', 'Yönetim']) ? route('admin.raporlar.index') : route('admin.iaa-yonetim.index')" class="flex items-center justify-between font-bold">
                                        ...
                                        @if($iaaApprovalCount > 0)
                                            <span class="inline-flex items-center justify-center px-1.5 py-0.5 text-[10px] font-bold leading-none text-white bg-rose-500 rounded-full">{{ $iaaApprovalCount }}</span>
                                        @endif
                                    </x-dropdown-link>
            ...
            <div class="pt-2 pb-3 space-y-1 px-2">
                ...
                @if($hukukMenuYetkisi)
                    ...
                    @if($disiplinYetkisi)
                        ...
                        @if(Auth::user()->hasAnyRole(['Superadmin', 'Yonetim', 'Bölüm Lideri', 'Hukuk Admini', 'Hukuk Yöneticisi', 'Disiplin Kurulu Başkanı', 'Disiplin Kurulu Üyesi']))
                            <x-responsive-nav-link :href="route('admin.disiplin.report')" class="text-indigo-600 font-semibold">
                                Disiplin Raporu
                            </x-responsive-nav-link>
                        @endif
