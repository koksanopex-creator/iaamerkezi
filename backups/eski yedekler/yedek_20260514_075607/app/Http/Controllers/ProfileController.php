...
class ProfileController extends Controller
{
    ...
    public function show($user): View
    {
        ...
        if ($user->hasRole($yasakliRoller) && !$currentUser->hasRole('Superadmin')) {
        ...
        // Müşteri temsilcileri için puan hesaplanmaz. Pasif kullanıcılar için de hesaplanmaz (save hatası almamak için)
        if ($user->is_personnel && !$user->hasRole('Müşteri Temsilcisi') && !$user->trashed()) {
            $guncelPuan = $this->puanService->calculateTotalScore($user);
            if ($user->toplam_puan != $guncelPuan) {
                $user->toplam_puan = $guncelPuan;
                $user->save();
    ...
    private function getProfileData($user)
    {
        ...
        $kullaniciProjeleri = Iaa::where(function ($q) use ($user) {
            ...
            ->with(['atananTakim', 'projeEkibi', 'musteriSikayeti'])
            ->latest('updated_at')
            ->get();
        ...
        return [
            ...
            'bolumManager' => $user->bolum_id ? User::role('Bölüm Lideri')->where('bolum_id', $user->bolum_id)->first() : null,
