...
<div x-data="{ editMode: <?php echo e($canEditDetails && !$case->anlasilan_tutar ? 'true' : 'false'); ?> }" class="bg-indigo-50 border border-indigo-200 rounded-lg p-6 mb-6 shadow-sm">
    ...
    <div x-show="!editMode">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white p-4 rounded border border-indigo-100 shadow-sm">
                ...
                <p class="text-2xl font-bold text-indigo-700"><?php echo e($case->anlasilan_tutar ? number_format($case->anlasilan_tutar, 2) . ' TL' : '---'); ?></p>
            </div>
            <div class="col-span-2 bg-white p-4 rounded border border-indigo-100 shadow-sm">
                ...
                <?php if($case->anlasma_maddeleri): ?>
                    <div class="text-sm text-gray-700 space-y-2 leading-relaxed">
                        <?php echo nl2br(e($case->anlasma_maddeleri)); ?>
    ...
    <?php if($canEditDetails): ?>
        <div x-show="editMode" class="mt-4 pt-4 border-t border-indigo-200">
            <form action="<?php echo e(route('admin.arabuluculuk.update', $case->id)); ?>" method="POST">
                <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?>
                <div class="mb-6">
                    <label class="block text-sm font-bold mb-2 text-gray-700">Anlaşma Tutarı (TL)</label>
                    <input type="number" step="0.01" name="anlasilan_tutar" value="<?php echo e($case->anlasilan_tutar); ?>" class="w-full md:w-1/3 border-gray-300 rounded focus:ring-indigo-500 focus:border-indigo-500 font-bold text-lg" placeholder="0.00">
                </div>
                <div class="mb-6">
                    <label class="block text-xs font-bold text-gray-500 uppercase mb-2">
                        Anlaşma Maddeleri (Zorunlu Seçim) <span class="text-red-500">*</span>
                    </label>
                    <?php if(isset($anlasmaMaddeleri) && $anlasmaMaddeleri->count() > 0): ?>
                        <div class="bg-white border border-gray-200 rounded-lg p-4 max-h-60 overflow-y-auto space-y-3 shadow-inner">
                            <?php $__currentLoopData = $anlasmaMaddeleri; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $madde): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <label class="flex items-start gap-3 p-2 hover:bg-gray-50 rounded cursor-pointer transition select-none">
                                    <input type="checkbox" name="maddeler_secim[]" value="<?php echo e($madde->icerik); ?>" 
                                        <?php echo e(Str::contains($case->anlasma_maddeleri ?? '', $madde->icerik) ? 'checked' : ''); ?>

                                        class="mt-1 w-5 h-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                                    <div class="text-sm text-gray-700">
                                        <span class="font-medium"><?php echo e($madde->icerik); ?></span>
                                        <?php if($madde->hukuki_dayanak): ?>
                                            <span class="block text-xs text-gray-400 mt-0.5">(<?php echo e($madde->hukuki_dayanak); ?>)</span>
                                        <?php endif; ?>
                                    </div>
                                </label>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="ek_notlar" class="w-full border-gray-300 rounded text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500" rows="3" placeholder="Listede olmayan özel bir durum varsa buraya yazınız..."><?php echo e($mevcutNot); ?></textarea>
                </div>
    ...
    <?php endif; ?>
</div><?php /**PATH C:\Users\celal.karaman\Desktop\Projelerim\iaa_projesi\resources\views/admin/arabuluculuk/parcalar/genel-anlasma-detaylari.blade.php ENDPATH**/ ?>