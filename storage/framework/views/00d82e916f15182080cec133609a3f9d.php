...
<div id="onay-durum-paneli" class="mt-8 mb-8 bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden transition-all duration-300 hover:shadow-md relative">
    ...
    <?php if($submissionLog && !in_array($iaa->durum, ['Atandı', 'Devam Ediyor', 'Revize Ediliyor', 'Çalışılıyor'])): ?>
        <div class="mb-6 p-4 bg-indigo-50 border border-indigo-100 rounded-xl flex items-center justify-between animate-fade-in">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm text-indigo-900 font-semibold text-wrap">
                        Proje, <span class="text-indigo-700 underline decoration-indigo-300 underline-offset-4"><?php echo e($submissionLog->user->name ?? 'Lider'); ?></span> tarafından 
                        <span class="text-indigo-700"><?php echo e($submissionLog->created_at->format('d.m.Y')); ?></span> günü 
                        saat <span class="text-indigo-700"><?php echo e($submissionLog->created_at->format('H:i')); ?></span>'de onay sürecine gönderilmiştir.
                    </p>
                </div>
            </div>
            <div class="hidden md:block shrink-0 px-3 py-1 bg-white/50 rounded-full border border-indigo-200 text-[10px] font-bold text-indigo-500 uppercase tracking-tighter">
                <?php echo e($iaa->durum == 'Tamamlandı' ? 'Süreç Tamamlandı' : 'Süreç Başlatıldı'); ?>
                ...
                <div class="flex items-start gap-4 mt-2">
                    ...
                    <div class="flex-1">
                        ...
                        <?php if($direktorDurum['tip'] == 'success' && $iaa->durum == 'Tamamlandı'): ?>
                            <?php if(auth()->check() && (auth()->user()->hasRole('Superadmin') || auth()->user()->hasRole('Direktör'))): ?>
        ...
        <div
            ...
            <div class="absolute top-0 right-0 flex flex-col items-end z-10">
                ...
                <?php if($adminDurum['tarih'] && !$adminDurum['locked'] && $adminDurum['tip'] != 'waiting'): ?>
                    <span
                        class="inline-flex items-center px-3 py-1 rounded-bl-xl text-[10px] font-bold uppercase tracking-widest bg-white/60 border-l border-b <?php echo e($aRenk['border']); ?> <?php echo e($aRenk['title']); ?> shadow-sm">
                        <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
...
</div><?php /**PATH /var/www/kys_koksan/iaa/resources/views/proje-calisma-alani/partials/_project-final-status.blade.php ENDPATH**/ ?>