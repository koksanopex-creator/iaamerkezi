    ...
    <div class="bg-gradient-to-br from-white to-gray-50 p-6 rounded-xl shadow-lg border border-gray-200">
        <div class="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-5 border-b border-gray-100 pb-4">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                @if($iaa->visit_planned)
                    <a href="#ziyaret-bilgileri-alani" onclick="document.getElementById('ziyaret-bilgileri-alani').scrollIntoView({behavior: 'smooth'})" class="inline-flex items-center px-3 py-1.5 bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 rounded-lg text-xs font-bold transition-all shadow-sm transform hover:scale-105">
        ...
        <dl class="grid grid-cols-1 md:grid-cols-{{ $iaa->musteriSikayeti ? 4 : 5 }} gap-4">
                            ...
                            <div class="flex flex-col gap-1">
                                <a href="{{ route('profile.show', $displayLider->id) }}" class="hover:text-indigo-600 hover:underline transition-colors flex items-center gap-1">
                                    ...
                                    @if(method_exists($displayLider, 'trashed') && $displayLider->trashed())
                                        <span class="text-[10px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded border border-red-200">Pasif</span>
                                    @endif
                                    <svg class="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>
                                </a>
                                ...
                                @if($leaderChanged)
                                    <div class="inline-flex items-center gap-1.5 px-2 py-1 bg-orange-50 text-orange-700 border border-orange-100 rounded text-[10px] font-bold leading-none w-fit">
                                        ...
                                        <span>Önceki Lider: {{ $originalLider->name }} ({{ $originalLider->trashed() ? 'İşten Ayrıldı' : 'Görev Değişikliği' }})</span>
                                    </div>
                                @endif
                            </div>
                        @else
            ...
            <div class="bg-gradient-to-r from-red-50 to-orange-50 rounded-lg p-3 shadow-sm border-2 border-red-200 flex flex-col justify-between"
                ...
                @php
                    $isLeaderOrAdminHeader = auth()->check() && (($iaa->atananTakim && auth()->id() == $iaa->atananTakim->lider_user_id) || auth()->user()->hasRole('Superadmin'));
                    ...
                    if (auth()->check() && $isComplaint) {
                        ...
                        // Lider, Admin, Kalite Yöneticisi, Kurul veya Direktör ise sonuç/süreç modallarını görebilir
                        if ($isLeaderOrAdminHeader || auth()->user()->hasRole('Bölüm Kalite Yöneticisi') || auth()->user()->hasRole('Kurul Üyesi') || auth()->user()->hasRole('Superadmin') || auth()->user()->hasRole('Direktör')) {
                            $canSeeEvaluating = true;
                        }
                    }
                @endphp
        ...
        <div class="mt-8 mb-4 animate-fade-in-up">
            @php
                $durum = $iaa->durum;
                $color = $iaa->durum_rengi ?? 'slate'; // Modelden gelen merkezi renk: blue, indigo, emerald, pink, purple, orange, red, gray
                
                // Tailwind renk eşleşmelerinde 'red' yerine daha yumuşak olan 'rose' kullanımı (Referans 2'ye uyum için)
                if ($color === 'red') $color = 'rose';
                if ($color === 'emerald') $color = 'emerald'; // Emerald zaten modelde tanımlı
                
                $baseClasses = "px-4 py-1.5 rounded-full text-sm font-bold tracking-wide flex items-center gap-2 shadow-sm border transition-colors";
                
                // 1. Badge Stilleri (DİNAMİK)
                $badgeStyles = "bg-{$color}-50/90 text-{$color}-700 border-{$color}-200/80 ring-1 ring-inset ring-{$color}-500/10";
                
                // 2. Alert Container Stilleri (DİNAMİK)
                $alertContainerStyles = "bg-{$color}-50/40 border-{$color}-100";
                
                // 3. Dot (Nokta) Stilleri (DİNAMİK)
                $dotStyles = "bg-{$color}-500";
                
                $isAnimated = in_array($durum, ['Devam Ediyor', 'Beklemede', 'Revize Ediliyor']) || str_contains($durum, 'bekliyor');
                
                $displayLabel = match($durum) {
                    'talep_onayi_bekliyor_kalite' => '🟣 Müşterinin Talebidir, Kalite Onayı Bekliyor',
                    'talep_onayi_bekliyor_direktor' => '🟣 Müşterinin Talebidir, Direktör Onayı Bekliyor',
                    'talep_onayi_bekliyor_superadmin' => '🟣 Müşterinin Talebidir, Yönetim Onayı Bekliyor',
                    'hatali_bildirim_onayi_bekliyor_kalite' => '🟠 Hatalı Bildirimdir, Kalite Onayı Bekliyor',
                    'hatali_bildirim_onayi_bekliyor_direktor' => '🟠 Hatalı Bildirimdir, Direktör Onayı Bekliyor',
                    'hatali_bildirim_onayi_bekliyor_superadmin' => '🟠 Hatalı Bildirimdir, Yönetim Onayı Bekliyor',
                    'talep_olarak_kapatildi' => '🟣 Müşterinin Talebidir Olarak Kapatıldı',
                    'hatali_bildirim_olarak_kapatildi' => '🟠 Hatalı Bildirim Olarak Kapatıldı',
                    'Bölüm Onayı Bekliyor' => 'Bölüm Kalite Yöneticisi Onayı Bekliyor',
                    'Yönetici Onayı Bekliyor' => 'Final Onay Bekliyor',
                    default => $durum
                };
            @endphp

            <div class="relative overflow-hidden rounded-2xl border {{ $alertContainerStyles }} p-4 sm:p-5 shadow-sm transition-all duration-300 hover:shadow-md flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                
                <div class="flex items-center gap-4">
                    <div class="flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center {{ str_replace('bg-', 'bg-', $alertContainerStyles) }} border border-white bg-white/60 shadow-sm">
                        @if($durum == 'Tamamlandı')
                    ...
                    <div>
                        ...
                        <span class="{{ $baseClasses }} {{ $badgeStyles }}">
                            @if($isAnimated)
                            <span class="relative flex h-2 w-2">
                                <span class="animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 {{ $dotStyles }}"></span>
                                <span class="relative inline-flex rounded-full h-2 w-2 {{ $dotStyles }}"></span>
                            </span>
                            @else
                            <span class="inline-block h-2 w-2 rounded-full {{ $dotStyles }} shadow-sm"></span>
                            @endif
                            {{ $displayLabel }}
                        </span>
                    </div>
