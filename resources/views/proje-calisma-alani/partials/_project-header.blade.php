    ...
    <div class="bg-gradient-to-br from-white to-gray-50 p-6 rounded-xl shadow-lg border border-gray-200">
        ...
        <dl class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="bg-white rounded-lg p-3 shadow-sm border border-gray-100">
                ...
                <dd class="text-sm font-bold text-gray-900">{{ $takim->ad }}</dd>
            </div>
            ...
            <div class="bg-gradient-to-r from-red-50 to-orange-50 rounded-lg p-3 shadow-sm border-2 border-red-200">
                ...
                <dd class="text-sm font-bold text-red-700">{{ \Carbon\Carbon::parse($assignment->due_date)->format('d.m.Y') }}</dd>
            </div>
        </dl>
                ...
                <div class="flex">
                    ...
                    <div class="ml-3 flex-1">
                        <div class="flex justify-between items-start">
                            <div>
                                <h4 class="text-sm font-bold text-{{ $kutuAyar['renk'] }}-800">{{ $kutuAyar['baslik'] }}</h4>
                                <div class="mt-2 text-sm text-{{ $kutuAyar['renk'] }}-700">
                                    <p class="whitespace-pre-wrap">{{ $kutuAyar['mesaj'] }}</p>
                                </div>
                            </div>
                            @if(!empty($statusDate))
                            <div class="ml-4 flex-shrink-0 text-xs text-{{ $kutuAyar['renk'] }}-600 font-medium flex items-center space-x-1.5 whitespace-nowrap">
                                ...
                                <span>{{ \Carbon\Carbon::parse($statusDate)->format('d.m.Y H:i') }}</span>
                            </div>
                            @endif
                        </div>
                    </div>
