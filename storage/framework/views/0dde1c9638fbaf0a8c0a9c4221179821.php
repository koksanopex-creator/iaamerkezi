<div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden mt-6">
    <div class="bg-gray-50 px-6 py-4 border-b border-gray-100 flex justify-between items-center">
        ...
        <span class="bg-gray-200 text-gray-600 text-xs px-2 py-1 rounded-full"><?php echo e($case->comments->count()); ?> Mesaj</span>
    </div>

    <div class="p-6">
        
        <div class="space-y-6 mb-8 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
        <?php $__empty_1 = true; $__currentLoopData = $case->comments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $comment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                ...
                <div x-data="{ editing: false }" class="flex gap-4 <?php echo e($comment->user_id == Auth::id() ? 'flex-row-reverse' : ''); ?> <?php echo e($containerOpacity); ?>">
                    ...
                    <div class="flex flex-col max-w-[85%] <?php echo e($comment->user_id == Auth::id() ? 'items-end' : 'items-start'); ?>">
                        
                        <div class="flex items-center gap-2 mb-1">
                            <span class="text-xs font-bold text-gray-700"><?php echo e($comment->user->name); ?></span>
                            <span class="text-[10px] text-gray-400"><?php echo e($comment->created_at->format('d.m.Y H:i')); ?></span>
                            
                            
                            <?php if($comment->histories->count() > 0): ?>
                        ...
                        <div x-show="!editing" class="relative px-4 py-3 rounded-2xl text-sm shadow-sm <?php echo e($messageBg); ?> <?php echo e($comment->user_id == Auth::id() ? 'rounded-tr-none' : 'rounded-tl-none'); ?>">
                            <p class="whitespace-pre-line">
                                <?php if($isDeleted): ?>
                                    <span class="italic font-bold">[Bu yorum silinmiştir]</span><br>
                                <?php endif; ?>
                                <?php echo e($comment->yorum); ?>

                            </p>

                            
                            <?php if(!empty($comment->dosyalar)): ?>
                                <div class="mt-3 pt-2 border-t border-black/10 flex flex-col gap-1">
                                    <?php $__currentLoopData = $comment->dosyalar; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dosya): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <a href="<?php echo e(asset('storage/'.$dosya)); ?>" target="_blank" class="flex items-center gap-2 text-xs hover:underline">
                                            📎 <?php echo e(basename($dosya)); ?>
                        ...
                        <?php if(!$isDeleted): ?>
                            <div x-show="editing" x-cloak class="w-full min-w-[300px] bg-white border border-gray-200 rounded-xl p-3 shadow-lg z-10">
                                <form action="<?php echo e(route('admin.disiplin.comment.update', $comment->id)); ?>" method="POST">
                                    <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?>
                                    <textarea name="yorum" rows="3" class="w-full text-sm border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 text-gray-800"><?php echo e($comment->yorum); ?></textarea>
                        ...
                        <?php if(!$isDeleted): ?>
                            <div class="flex gap-3 mt-1">
                                ...
                                <?php if(Auth::id() == $comment->user_id || Auth::user()->hasRole(['Superadmin', 'Hukuk Admini'])): ?>
                                    <form action="<?php echo e(route('admin.disiplin.comment.destroy', $comment->id)); ?>" method="POST">
                        ...
                        <?php if(Auth::user()->hasRole(['Superadmin', 'Hukuk Admini']) && $comment->histories->count() > 0): ?>
                            <div x-data="{ showHistory: false }" class="mt-1 w-full text-right">
                                <button @click="showHistory = !showHistory" class="text-[9px] text-indigo-400 hover:text-indigo-600 underline">
                                    Düzenleme Geçmişi (<?php echo e($comment->histories->count()); ?>)
                                </button>
                                <div x-show="showHistory" class="mt-2 bg-yellow-50 border border-yellow-200 p-2 rounded text-[10px] text-gray-700 space-y-2 text-left w-full">
                                    <?php $__currentLoopData = $comment->histories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $history): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <div class="border-b border-yellow-200 pb-1 last:border-0">
                                            <strong class="block text-yellow-800"><?php echo e($history->created_at->format('d.m.Y H:i')); ?> - <?php echo e($history->user->name ?? 'Bilinmiyor'); ?></strong>
                                            <p class="italic text-gray-600">"<?php echo e($history->eski_yorum); ?>"</p>
                                        </div>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </div>
                            </div>
        ...
        <div class="bg-gray-50 p-4 rounded-xl border border-gray-200" x-data="{ files: [] }">
            <form action="<?php echo e(route('admin.disiplin.comment.store', $case->id)); ?>" method="POST" enctype="multipart/form-data">
                <?php echo csrf_field(); ?>
                <div class="mb-3">
...
</div><?php /**PATH C:\Users\celal.karaman\Desktop\Projelerim\iaa_projesi\resources\views/admin/disiplin/partials/comments.blade.php ENDPATH**/ ?>