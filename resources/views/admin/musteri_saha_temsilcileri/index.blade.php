...
<x-app-layout>
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h2 class="font-black text-2xl text-slate-800 leading-tight tracking-tight uppercase">
                    {{ __('Müşteri Saha Temsilcileri Yönetimi') }}
                </h2>
    ...
    <div class="py-12" x-data="{ selectedUserId: {{ $temsilciler->first() ? $temsilciler->first()->id : 'null' }}, searchTerm: '' }">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row gap-6 h-[700px]">
                        ...
                        @else
                            <ul class="divide-y divide-gray-100">
                                @foreach ($temsilciler as $temsilci)
                                    <li x-show="searchTerm === '' || '{{ strtolower($temsilci->name) }}'.includes(searchTerm.toLowerCase())">
                                        <button @click="selectedUserId = {{ $temsilci->id }}" 
                                                class="w-full text-left px-6 py-4 focus:outline-none transition-colors duration-150"
                                                :class="{ 'bg-indigo-50 border-l-4 border-indigo-600': selectedUserId === {{ $temsilci->id }}, 'hover:bg-gray-50 border-l-4 border-transparent': selectedUserId !== {{ $temsilci->id }} }">
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0">
                                                    <div class="h-10 w-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold border border-indigo-200">
                                                        {{ strtoupper(substr($temsilci->name, 0, 1)) }}
                                                    </div>
                                                </div>
                                                <div class="ml-4 flex-1">
                                                    <p class="text-sm font-bold text-gray-900 truncate">{{ $temsilci->name }}</p>
                                                    <p class="text-xs text-gray-500 mt-1">
                                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-green-100 text-green-800">
                                                            {{ $temsilci->musteriSahaTemsilcisiOlduguBolumler->count() }} Bölüm
                                                        </span>
                                                    </p>
                                                </div>
                                                <div class="ml-2">
                                                    <svg class="h-5 w-5 text-gray-400" :class="{ 'text-indigo-500': selectedUserId === {{ $temsilci->id }} }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="w-full md:w-2/3 bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col overflow-hidden relative">
                    ...
                    @foreach ($temsilciler as $temsilci)
                        <div x-show="selectedUserId === {{ $temsilci->id }}" x-cloak
                            ...
                            <div class="px-6 py-5 border-b border-gray-200 flex justify-between items-center bg-gray-50">
                                <div>
                                    <h3 class="text-lg font-bold text-gray-900">{{ $temsilci->name }}</h3>
                            ...
                            {{-- Form İçeriği --}}
                            <form id="update-form-{{ $temsilci->id }}" action="{{ route('admin.musteri-saha-temsilcileri.update', $temsilci->id) }}" method="POST" class="flex flex-col flex-1 min-h-0">
                                @csrf
                                            ...
                                            <label class="relative flex items-start p-4 rounded-lg border border-gray-200 hover:bg-gray-50 cursor-pointer transition-colors duration-200 group"
                                                ...
                                                <div class="flex items-center h-5">
                                                    <input type="checkbox" 
                                                           name="bolumler[]" 
                                                           value="{{ $bolum->id }}" 
                                                           {{ $temsilci->musteriSahaTemsilcisiOlduguBolumler->contains($bolum->id) ? 'checked' : '' }}
                                                ...
                                                <div class="ml-3 text-sm">
                                                    <span class="font-medium text-gray-900 group-hover:text-indigo-700">{{ $bolum->ad }}</span>
                                                </div>
                                            </label>
                                        @endforeach
                                    </div>
    ...
    {{-- Yeni Temsilci Ekle Modal --}}
    <x-modal name="temsilci-ekle-modal" maxWidth="md">
        <form method="POST" action="{{ route('admin.musteri-saha-temsilcileri.store') }}" class="p-6">
            @csrf
            ...
            <div class="mb-4">
                ...
                {{-- Eğer listemizi tüm userlardan çekersek sunucuyu yorabilir. Canlı aramalı select (select2) tavsiye edilir. --}}
                <select name="user_id" id="user_id" class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                    ...
                    @foreach(\App\Models\User::orderBy('name')->get() as $u)
                        @unless($u->hasRole('Müşteri Saha Temsilcisi'))
