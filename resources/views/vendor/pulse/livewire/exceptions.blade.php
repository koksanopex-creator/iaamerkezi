<x-pulse::card :cols="$cols" :rows="$rows" :class="$class">
    <x-pulse::card-header
        name="Hatalar (Exceptions)"
        x-bind:title="`Time: {{ number_format($time) }}ms; Run at: ${formatDate('{{ $runAt }}')};`"
        details="son {{ str_replace(['1 hour', '6 hours', '24 hours', '7 days'], ['1 saat', '6 saat', '24 saat', '7 gün'], $this->periodForHumans()) }}"
    >
        <x-slot:icon>
    ...
    <x-pulse::scroll :expand="$expand" wire:poll.5s="">
        ...
        @else
            <x-pulse::table>
                ...
                <x-pulse::thead>
                    <tr>
                        <x-pulse::th>Hata Tipi</x-pulse::th>
                        <x-pulse::th class="text-right">Son Tarih</x-pulse::th>
                        <x-pulse::th class="text-right">Sayı</x-pulse::th>
                    </tr>
                </x-pulse::thead>
                <tbody>
