{{-- resources/views/proje-calisma-alani/partials/_talep-notification.blade.php --}}
...
{{--
================================================================
1. LİDER PANELİ (TALEP OLUŞTURMA)
================================================================
--}}
...
@if($iaa->talep_gerekcesi)
    <div
        ...
        <div class="bg-white/50 backdrop-blur-sm px-5 py-3 border-b border-indigo-100 flex items-center justify-between">
            ...
            @elseif(!Str::startsWith($iaa->durum, 'talep_onayi_bekliyor'))
                <span
        ...
        <div class="p-5 space-y-6">
            ...
            <div class="flex gap-4 relative">
                ...
                <div class="flex-shrink-0 relative">
                    <img src="{{ $iaa->hataliBildirimLiderUser->profile_photo_url ?? 'https://ui-avatars.com/api/?name=' . urlencode('Lider') . '&color=7F9CF5&background=EBF4FF' }}"
                ...
                <div class="flex-1 bg-white/60 p-3 rounded-xl border border-indigo-100 shadow-sm">
                    <div class="flex items-center justify-between mb-1">
                        ...
                        <span
                            class="text-[10px] text-gray-400 font-medium">{{ $talepBaslatLog ? $talepBaslatLog->created_at->format('d.m.Y H:i') : ($iaa->onaya_gonderilme_tarihi ? \Carbon\Carbon::parse($iaa->onaya_gonderilme_tarihi)->format('d.m.Y H:i') : ($iaa->created_at ? $iaa->created_at->format('d.m.Y H:i') : '-')) }}</span>
                    </div>
                    <p class="text-xs font-bold text-indigo-600 mb-1">TALEP GEREKÇESİ:</p>
                    <p class="text-sm text-gray-700 italic leading-relaxed">"{{ $iaa->talep_gerekcesi }}"</p>
                </div>
            </div>

            {{-- 2. Kalite Yöneticisi --}}
            @if($iaa->talep_kalite_notu || $talepKaliteLog)
                <div class="flex gap-4 relative">
                    <div class="absolute left-5 top-10 bottom-0 w-0.5 bg-indigo-200/50"></div>
                    <div class="flex-shrink-0 relative">
                        {{-- Kalite avatarı --}}
                    ...
                    <div class="flex-1 bg-white/60 p-3 rounded-xl border border-indigo-100 shadow-sm">
                        <div class="flex items-center justify-between mb-1">
                            <span class="text-xs font-bold text-gray-900">{{ $talepKaliteLog->user->name ?? 'Kalite Yöneticisi' }}</span>
                            <span
                                class="text-[10px] text-gray-400 font-medium">{{ $talepKaliteLog ? $talepKaliteLog->created_at->format('d.m.Y H:i') : '-' }}</span>
                        </div>
                        @if($iaa->talep_kalite_notu)
                            <p class="text-xs font-bold text-yellow-600 mb-1">KALİTE ONAYI NOTU:</p>
                            <p class="text-sm text-gray-700 italic leading-relaxed">
                                "{{ $iaa->talep_kalite_notu }}"</p>
                        @else
                ...
                <div class="flex gap-4 relative">
                    ...
                    <div class="flex-1 bg-white/60 p-3 rounded-xl border border-indigo-100 shadow-sm">
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <span
                                class="text-[10px] text-gray-400 font-medium">{{ $iaa->talep_direktor_at ? \Carbon\Carbon::parse($iaa->talep_direktor_at)->format('d.m.Y H:i') : '-' }}</span>
                        </div>
                        <p class="text-xs font-bold text-purple-600 mb-1">DİREKTÖR ONAY NOTU:</p>
                        <p class="text-sm text-gray-700 italic leading-relaxed">
                            "{{ $iaa->talep_direktor_notu }}"</p>
                    </div>
                </div>
            @endif

            {{-- 4. Superadmin --}}
            @if($iaa->talep_admin_notu)
                <div class="flex gap-4 relative">
                    <div class="flex-shrink-0 relative">
                        <div
                    ...
                    <div class="flex-1 bg-white/60 p-3 rounded-xl border border-indigo-100 shadow-sm">
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <span class="text-[10px] text-gray-400 font-medium">{{ $talepAdminLog ? $talepAdminLog->created_at->format('d.m.Y H:i') : ($iaa->updated_at ? $iaa->updated_at->format('d.m.Y H:i') : '-') }}</span>
                        </div>
                        <p class="text-xs font-bold text-red-600 mb-1">YÖNETİM FİNAL NOTU:</p>
                        <p class="text-sm text-gray-700 italic leading-relaxed">
                            "{{ $iaa->talep_admin_notu }}"</p>
                    </div>
                </div>
            @endif
        </div>
...
{{-- Superadmin İçin Geri Alma (Talep Kapatıldıysa) --}}
@if($isSuperAdmin && $iaa->durum == 'talep_olarak_kapatildi')
    <div class="mb-6 bg-indigo-50 border border-indigo-200 rounded-xl p-3 shadow-sm flex items-center justify-between">
        ...
        <form action="{{ route('proje.recallRequestBySuperadmin', $iaa->id) }}" method="POST">
            @csrf
            <button type="submit"
...
@if($isRequestPendingQuality && ($isQuality || $isSuperAdmin))
    <div class="mb-6 bg-purple-50 border-2 border-purple-200 rounded-xl p-6 shadow-sm relative overflow-hidden">
        ...
        <form action="{{ route('proje.decideRequestByQuality', $iaa->id) }}" method="POST" class="mt-4">
            @csrf
            <div>
...
{{-- B. DİREKTÖR ONAYI --}}
@if($isRequestPendingDirector && ($isDirector || $isSuperAdmin))
    <div class="mb-6 bg-slate-800 text-white rounded-xl p-6 shadow-xl relative overflow-hidden">
        ...
        <form action="{{ route('proje.decideRequestByDirector', $iaa->id) }}" method="POST" class="mt-4">
            @csrf
            <div class="mb-4">
...
{{-- C. SUPERADMIN ONAYI --}}
@if($isRequestPendingSuperadmin && $isSuperAdmin)
    <div class="mb-6 bg-indigo-900 text-white rounded-xl p-6 shadow-2xl relative overflow-hidden ring-4 ring-indigo-500/20">
        ...
        <form action="{{ route('proje.decideRequestBySuperadmin', $iaa->id) }}" method="POST" class="mt-4">
            @csrf
            <div class="mb-4">
