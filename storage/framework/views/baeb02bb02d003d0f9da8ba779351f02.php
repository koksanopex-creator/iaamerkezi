<div id="defense_section" class="mt-6">
    ...
    <?php if($case->durum == 'Savunma Bekleniyor' || $case->durum == 'Yönetici Değerlendirmesi'): ?>
        <?php
        ...
        <div x-data="{ isEditing: <?php echo e($hasDefense ? 'false' : 'true'); ?> }">
            <?php if(Auth::user()->hasRole(['Yonetim', 'Yönetim']) && !$canEdit): ?>
                <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-6 shadow-sm text-center">
                    ...
                    <h3 class="text-lg font-bold text-yellow-800">Personel Savunması <?php echo e($case->durum == 'Savunma Bekleniyor' ? 'Bekleniyor' : 'Değerlendiriliyor'); ?></h3>
                    <p class="text-yellow-700 mt-2 italic"><?php echo e($case->durum == 'Savunma Bekleniyor' ? 'Bu dosya için henüz savunma girilmemiştir.' : 'Personel savunmasını girdi, hukuk onayı bekleniyor.'); ?></p>
                </div>
            <?php elseif($canEdit): ?>
                ...
                <template x-if="!isEditing">
                    <div class="bg-white border-t-4 border-blue-500 rounded-lg shadow-sm p-6 mb-6">
                        <div class="flex justify-between items-center border-b pb-4 mb-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 font-medium italic">Son Güncelleme: <?php echo e($case->savunma_tarihi ? $case->savunma_tarihi->format('d.m.Y H:i') : '-'); ?></p>
                                </div>
                            </div>
                            <div class="flex gap-2">
                                <?php if($case->durum == 'Yönetici Değerlendirmesi' || $case->durum == 'Savunma Bekleniyor'): ?>
                        ...
                        <div class="bg-indigo-50/30 p-5 rounded-2xl border border-indigo-100 mb-4 shadow-sm">
                            <div class="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap font-medium">"<?php echo e($savunmaMetni); ?>"</div>
                            <?php if($notKismi): ?>
                                <div class="mt-4 pt-4 border-t border-indigo-100/50">
                                    <div class="flex items-start gap-3 bg-white/50 p-3 rounded-xl border border-indigo-50">
                                        ...
                                        <div class="text-[11px] text-slate-500 leading-relaxed italic"><?php echo $notKismi; ?></div>
                                    </div>
                                </div>
                            <?php endif; ?>
                        </div>

                        <?php if(!empty($case->savunma_dosyalari)): ?>
                            <h4 class="text-xs font-bold text-gray-500 uppercase mb-3 border-b pb-1">Savunma Ekleri:</h4>
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                                <?php $__currentLoopData = $case->savunma_dosyalari; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dosya): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <?php
                                        $url = asset('storage/' . $dosya);
                                        $ext = strtolower(pathinfo($dosya, PATHINFO_EXTENSION));
                                        $isImage = in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'webp']);
                                    ?>
                                    <a href="<?php echo e($url); ?>" target="_blank" class="group relative border rounded-lg overflow-hidden bg-white hover:shadow-md transition block">
                                        <div class="h-24 w-full flex items-center justify-center bg-gray-50 overflow-hidden relative">
                                            <?php if($isImage): ?>
                                                <img src="<?php echo e($url); ?>" class="w-full h-full object-cover group-hover:scale-105 transition duration-500">
                                            <?php else: ?>
                                        ...
                                        <div class="p-2 border-t bg-white truncate text-[10px] text-gray-500"><?php echo e(basename($dosya)); ?></div>
                                    </a>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div>
                        <?php endif; ?>
                    ...
                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-6 shadow-sm mb-6">
                        <div class="flex justify-between items-center mb-4 border-b border-yellow-200 pb-2">
                            <h3 class="text-lg font-bold text-yellow-800 flex items-center gap-2">
                                ...
                                ?>
                                <?php echo e($case->savunma_tarihi ? $title : $ilkGirisBaslik); ?>
                        ...
                        <?php if(!$isOwner): ?>
                            <div class="mb-3 px-3 py-2 bg-amber-100 border border-amber-300 rounded-lg flex items-center gap-2">
                                ...
                                <p class="text-xs font-bold text-amber-800">Bu savunmayı <span class="font-black underline"><?php echo e($personnelName); ?></span> adına siz yazıyorsunuz.</p>
                            </div>
                        <?php endif; ?>
                        <p class="text-sm text-yellow-700 mb-4">
                            <?php echo $case->savunma_tarihi ? $desc : $ilkGirisDesc; ?>

                        </p>

                        <form action="<?php echo e(route('disiplin.defense.store', $case->id)); ?>" method="POST" enctype="multipart/form-data"
                            x-data="{ 
                                upfiles: [],
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="savunma_aciklamasi" rows="6" class="w-full border-gray-300 rounded-md shadow-sm focus:border-yellow-500 focus:ring-yellow-500" placeholder="Olayla ilgili açıklamanızı buraya yazınız..." required><?php echo e(old('savunma_aciklamasi', $case->savunma_aciklamasi)); ?></textarea>
                            </div>
                            <div class="mb-6">
                                <label class="block text-sm font-bold text-gray-700 mb-2">Savunma Ekleri <?php echo e($case->savunma_dosyalari ? '(Mevcutlara eklenir)' : ''); ?></label>
                            ...
                            <div class="flex justify-end gap-3">
                                ...
                                <button type="submit" class="bg-yellow-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-yellow-700 shadow transition flex items-center gap-2">
                                    ...
                                    <?php if($case->savunma_tarihi): ?>
                                        Değişiklikleri Kaydet
                                    <?php elseif(!$isOwner): ?>
                                        <?php echo e($personnelName); ?> Adına Savunmayı Kaydet ve Gönder
                                    <?php else: ?>
    ...
    <?php elseif(in_array($case->durum, ['Karar Verildi', 'Kurulda', 'İtiraz Edildi', 'Yönetici Değerlendirmesi'])): ?>
        <div class="bg-white border-t-4 border-blue-500 rounded-lg shadow p-6 mb-6">
            <div class="flex justify-between items-start border-b pb-4 mb-4">
                <div>
                    ...
                    <?php if($case->savunma_tarihi): ?>
                        <p class="text-xs text-gray-500">Tarih: <?php echo e($case->savunma_tarihi->format('d.m.Y H:i')); ?></p>
                    <?php endif; ?>
                </div>
            ...
            <div class="bg-indigo-50/30 p-5 rounded-2xl border border-indigo-100 mb-4 shadow-sm">
                <?php if($case->savunma_aciklamasi): ?>
                    <div class="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap font-medium">"<?php echo e($savunmaMetni); ?>"</div>
                <?php else: ?>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <?php $__currentLoopData = $case->savunma_dosyalari; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dosya): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <a href="<?php echo e($url); ?>" target="_blank" class="group relative border rounded-lg overflow-hidden bg-white hover:shadow-md transition block">
                            ...
                            <div class="p-2 border-t bg-white italic text-[10px] text-gray-400 truncate"><?php echo e(basename($dosya)); ?></div>
                        </a>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            <?php endif; ?>
    ...
    <?php endif; ?>
</div><?php /**PATH C:\Users\celal.karaman\Desktop\Projelerim\iaa_projesi\resources\views/admin/disiplin/partials/defense-section.blade.php ENDPATH**/ ?>