...
<div class="p-6 bg-gray-50 min-h-screen">
                        ...
                        <div class="bg-indigo-50/30 p-6 rounded-2xl mb-8 border border-indigo-100 shadow-sm relative overflow-hidden">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-start relative z-10">
                                <div class="md:col-span-3">
                                    ...
                                    @error('new_rep_name') <span class="text-red-500 text-[10px] block mt-1 font-bold">{{ $message }}</span> @enderror
                                </div>
                                ...
                                <div class="md:col-span-3">
                                    ...
                                    @error('new_rep_email') <span class="text-red-500 text-[10px] block mt-1 font-bold">{{ $message }}</span> @enderror
                                </div>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @forelse($selectedCustomer->users as $rep)
                                        @php $isRepActive = $rep->pivot->is_active; @endphp
                                        <tr wire:key="rep-row-{{ $rep->id }}" class="transition-colors {{ !$isRepActive ? 'bg-amber-50/50' : 'hover:bg-gray-50' }}">
                                            @if($editingRepId === $rep->id)
                                                <td class="px-6 py-4 align-top bg-white border-x-2 border-indigo-200 shadow-inner" colspan="3">
                                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                        <div>
                                                            ...
                                                            @error('edit_rep_name') <span class="text-red-500 text-[10px] block mt-1 font-bold">{{ $message }}</span> @enderror
                                                        </div>
                                                        <div>
                                                            ...
                                                            @error('edit_rep_email') <span class="text-red-500 text-[10px] block mt-1 font-bold">{{ $message }}</span> @enderror
                                                        </div>
                                                        <div>
                                                ...
                                                <td class="px-6 py-4 text-right whitespace-nowrap align-top bg-white border-r-2 border-indigo-200 shadow-inner">
                                                    <div class="flex justify-end gap-2 mt-4" wire:key="edit-actions-{{ $rep->id }}">
                                                        <button wire:click="updateRepresentative" wire:key="btn-save-{{ $rep->id }}" class="bg-emerald-500 text-white p-2.5 rounded-xl hover:bg-emerald-600 shadow-lg shadow-emerald-500/20 transition-all duration-200 transform hover:-translate-y-0.5" title="Kaydet">
                                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                                        </button>
                                                        <button wire:click="cancelEditRepresentative" wire:key="btn-cancel-{{ $rep->id }}" class="bg-gray-100 text-gray-500 p-2.5 rounded-xl hover:bg-gray-200 transition-all duration-200" title="İptal">
                                            ...
                                            @else
                                                <td class="px-6 py-4">
                                                    <div class="text-sm font-black {{ !$isRepActive ? 'text-amber-900 opacity-60' : 'text-gray-900' }} tracking-tight">{{ $rep->name }}</div>
                                                    <div class="text-[10px] text-gray-500 mt-1 font-bold uppercase tracking-widest flex items-center gap-1.5">
                                                        <span class="bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200">
                                                            {{ $rep->pivot->unvan ?? $rep->unvan ?? '-' }}
                                                        </span>
                                                        @if(!$isRepActive) <span class="text-amber-600 font-black flex items-center gap-1"><span class="w-1.5 h-1.5 bg-amber-500 rounded-full animate-pulse"></span>PASİF</span> @endif
                                                        @if($rep->trashed()) <span class="text-red-600 font-black flex items-center gap-1"><span class="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse"></span>SİLİNMİŞ</span> @endif
                                                    </div>
                                                </td>
                                                ...
                                                <td class="px-6 py-4 text-right whitespace-nowrap">
                                                    @if($isAdmin)
                                                    <div class="flex justify-end space-x-2" wire:key="view-actions-{{ $rep->id }}">
                                                        {{-- Aktif/Pasif Toggle Butonu --}}
                                                        <button wire:click="toggleRepresentativeStatus({{ $rep->id }})" 
                                                                wire:key="btn-toggle-{{ $rep->id }}"
                                                                wire:confirm="{{ !$isRepActive ? 'Yetkiliyi bu firma için tekrar aktif etmek istiyor musunuz?' : 'Yetkiliyi bu firma için pasife almak istiyor musunuz?' }}"
                                                                class="p-2 rounded-lg transition-colors {{ !$isRepActive ? 'text-green-600 hover:bg-green-50' : 'text-gray-500 hover:bg-gray-100' }}" 
                                                                title="{{ !$isRepActive ? 'Firmada Aktifleştir' : 'Firmada Pasife Al' }}">
                                                            @if(!$isRepActive)
                                                        ...
                                                        @if($isRepActive)
                                                            <button wire:click="editRepresentative({{ $rep->id }})" wire:key="btn-edit-{{ $rep->id }}" class="text-indigo-600 hover:text-indigo-900 p-2 rounded-lg hover:bg-indigo-50 transition-colors" title="Düzenle">
                                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                                            </button>
                                                            
                                                            @if(!auth()->user()->hasRole('Müşteri Şikayeti Kurulu'))
                                                                <button wire:click="deleteRepresentative({{ $rep->id }})" wire:key="btn-delete-{{ $rep->id }}" wire:confirm="Bu yetkiliyi bu firmadan kaldırmak istediğinize emin misiniz? (Kullanıcı kaydı silinmez)" class="text-rose-600 hover:text-rose-900 p-2 rounded-lg hover:bg-rose-50 transition-colors" title="Firmadan Kaldır">
    ...
    {{-- MÜŞTERİ OLUŞTURULDUKTAN SONRAKİ PREMIUM BİLDİRİM KARTI --}}
    @if(session('generated_password') && !$showRepModal)
        <div class="mb-6 bg-gradient-to-br from-indigo-900 via-indigo-950 to-slate-900 rounded-2xl p-6 shadow-xl border border-indigo-500/30 relative overflow-hidden animate-fadeIn">
            ...
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <h4 class="text-xl font-bold text-white mb-1">
                            {{ session('generated_customer_name') ?? 'Müşteri' }}
                        </h4>
                        <p class="text-indigo-200/80 text-xs">
                            İlk yetkili temsilci <strong>{{ session('generated_user_email') }}</strong> e-posta adresi ile oluşturuldu ve şifre e-posta ile gönderildi.
                        </p>
                    </div>
                </div>
                ...
                <div class="bg-black/30 rounded-xl p-4 border border-white/10 flex items-center justify-between gap-6 w-full md:w-auto min-w-[280px]">
                    ...
                    <button onclick="navigator.clipboard.writeText('{{ session('generated_password') }}'); const btn = this; btn.innerText='Kopyalandı!'; btn.classList.add('bg-emerald-600', 'text-white'); setTimeout(() => { btn.innerText='Kopyala'; btn.classList.remove('bg-emerald-600', 'text-white'); }, 2000);" 
        ...
        @forelse($customers as $customer)
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden" wire:key="mobile-customer-{{ $customer->id }}">
                ...
                <div class="bg-gray-50 px-4 py-3 flex items-center justify-around border-t border-gray-100">
                    @if($isAdmin || auth()->user()->hasAnyRole(['Yonetim', 'Yönetim']))
                        @if($isAdmin && auth()->user()->hasRole(['Superadmin', 'Müşteri Şikayeti Kurulu']))
