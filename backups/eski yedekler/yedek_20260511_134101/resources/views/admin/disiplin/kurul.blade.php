<x-app-layout>
    @push('pageTitle', 'Disiplin Kurulu Portalı | ')
    @push('styles')
    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
            ...
            <div class="flex items-center gap-3" x-data="{}">
                ...
                @endif
                @unless(auth()->user()->hasRole('Yonetim'))
    ...
    <div class="py-6 space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8" x-data="{ 
        activeTab: '{{ $activeTab !== 'overview' ? $activeTab : (request()->has('activeTab') ? request('activeTab') : '') }}' || localStorage.getItem('kurulActiveTab') || 'overview',
        ...
        @if(session('success'))
            <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)" class="bg-emerald-50 border border-emerald-200 text-emerald-700 px-6 py-4 rounded-3xl flex items-center justify-between shadow-xl shadow-emerald-500/10 animate-fade-in-down">
                <div class="flex items-center gap-3">
                    ...
                    <span class="font-black text-sm uppercase tracking-wide">{{ session('success') }}</span>
                </div>
        ...
        <div class="bg-white/60 backdrop-blur-xl rounded-[2rem] border border-white/40 p-5 shadow-2xl shadow-indigo-500/5 ring-1 ring-black/5">
            <form action="{{ route('admin.disiplin.kurul.index') }}" method="GET" class="flex flex-wrap items-end gap-6">
                <div class="space-y-2 min-w-[180px]">
                    ...
                    <div class="relative group">
                        <input type="date" name="baslangic" value="{{ $baslangic }}" class="w-full bg-indigo-50/30 border-none ring-2 ring-indigo-100 focus:ring-4 focus:ring-indigo-400 text-indigo-900 rounded-2xl transition-all text-sm py-2.5">
                    </div>
                </div>
                <div class="space-y-2 min-w-[180px]">
                    <label class="text-[10px] font-black text-indigo-400 uppercase tracking-widest ml-2">Bitiş Tarihi</label>
                    <div class="relative group">
                        <input type="date" name="bitis" value="{{ $bitis }}" class="w-full bg-indigo-50/30 border-none ring-2 ring-indigo-100 focus:ring-4 focus:ring-indigo-400 text-indigo-900 rounded-2xl transition-all text-sm py-2.5">
                    </div>
                </div>
                ...
                <div class="flex gap-3 ml-auto md:ml-0">
                    ...
                    <a href="{{ route('admin.disiplin.kurul.index') }}" class="p-3 bg-indigo-100 hover:bg-indigo-200 text-indigo-600 rounded-2xl transition-all">
            ...
            <div @click="setActiveTab('members'); window.scrollTo({top: document.getElementById('kurul-tabs').offsetTop - 100, behavior: 'smooth'})" 
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div>
                        ...
                        <h3 class="text-4xl font-black text-gray-800 mt-2 tracking-tighter">{{ $tumUyeler->count() }}</h3>
                    </div>
                ...
                <div class="mt-6 flex items-center gap-2 relative z-10">
                    <span class="px-2.5 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-black rounded-lg uppercase whitespace-nowrap">{{ $baskanlar->count() }} Başkan</span>
                    <span class="px-2.5 py-1 bg-gray-50 text-gray-500 text-[10px] font-black rounded-lg uppercase whitespace-nowrap">{{ $uyeler->count() }} Üye</span>
                </div>
            </div>
            ...
            <div @click="setActiveTab('meetings'); meetingStatusFilter = 'tamamlandı'; window.scrollTo({top: document.getElementById('kurul-tabs').offsetTop - 100, behavior: 'smooth'})" 
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div>
                        ...
                        <h3 class="text-4xl font-black text-gray-800 mt-2 tracking-tighter">{{ $tamamlananToplantiSayisi }}</h3>
                    </div>
            ...
            <div @click="setActiveTab('overview'); document.getElementById('kurul-gundemi').scrollIntoView({ behavior: 'smooth', block: 'start' })" 
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div>
                        ...
                        <h3 class="text-4xl font-black text-gray-800 mt-2 tracking-tighter">{{ $bekleyenGundemSayisi }}</h3>
                    </div>
            ...
            <div @click="setActiveTab('meetings'); meetingStatusFilter = 'planlandı'; window.scrollTo({top: document.getElementById('kurul-tabs').offsetTop - 100, behavior: 'smooth'})" 
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div>
                        ...
                        <h3 class="text-4xl font-black text-gray-800 mt-2 tracking-tighter">{{ $planlananToplantiSayisi }}</h3>
                    </div>
            ...
            <div @click="setActiveTab('members'); window.scrollTo({top: document.getElementById('kurul-tabs').offsetTop - 100, behavior: 'smooth'})" 
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div>
                        ...
                        <h3 class="text-4xl font-black text-gray-800 mt-2 tracking-tighter">%{{ $ortalamKatilim }}</h3>
                    </div>
                ...
                <div class="mt-6 flex items-center gap-2 relative z-10">
                    <div class="flex -space-x-2">
                        @foreach($tumUyeler->take(5) as $u)
                            <img src="{{ $u->profile_photo_url }}" alt="{{ $u->name }}" class="w-7 h-7 rounded-full border-2 border-white shadow-sm ring-1 ring-amber-100 object-cover">
                        @endforeach
                    </div>
            ...
            <div @click="setActiveTab('archive'); window.scrollTo({top: document.getElementById('kurul-tabs').offsetTop - 100, behavior: 'smooth'})" 
                ...
                <div class="flex justify-between items-start relative z-10">
                    <div>
                        ...
                        <h3 class="text-4xl font-black text-gray-800 mt-2 tracking-tighter">{{ $kararVerilenDosya }} / {{ $toplamDosya }}</h3>
                    </div>
                ...
                <div class="mt-6 space-y-2 relative z-10">
                    <div class="flex justify-between items-center px-1">
                        ...
                        @php $yuzde = $toplamDosya > 0 ? ($kararVerilenDosya / $toplamDosya) * 100 : 0; @endphp
                        <span class="text-[10px] text-purple-600 font-black">%{{ round($yuzde) }}</span>
                    </div>
                    <div class="w-full bg-purple-50 h-2.5 rounded-full overflow-hidden p-0.5 ring-1 ring-purple-100">
                        <div class="bg-gradient-to-r from-purple-400 to-purple-600 h-full rounded-full transition-all duration-1000" style="width: {{ $yuzde }}%"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="min-h-[600px]">
                    ...
                    <div class="lg:col-span-2 space-y-6" id="kurul-gundemi">
                        <div class="flex items-center justify-between px-2">
                            <h4 class="text-gray-800 font-black text-xl flex items-center gap-3">
                                ...
                                Kurul Gündemi & Toplantılar
                            </h4>
                            <div class="px-3 py-1 bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase tracking-widest rounded-full ring-1 ring-emerald-100">
                                {{ $yaklasanToplantılar->count() }} Aktif Bekleyen
                            </div>
                        </div>
                        
                        <div class="grid grid-cols-1 gap-4">
                            <!-- Durumu Kurulda Olan Dosyalar -->
                            @foreach($kuruldakiDosyalar as $case)
                                <a href="{{ route('admin.disiplin.show', $case) }}" class="group relative bg-white hover:bg-indigo-50/30 border border-indigo-100 p-5 rounded-[2rem] transition-all hover:shadow-2xl hover:shadow-indigo-500/10 hover:-translate-y-1">
                                    <div class="flex flex-col sm:flex-row items-center gap-6">
                                        <div class="flex-shrink-0 flex flex-col items-center justify-center p-3 bg-gradient-to-br from-indigo-50 to-white rounded-2xl border border-indigo-100 w-20 h-20 shadow-inner group-hover:border-indigo-200 transition-colors">
                                            <span class="text-[10px] font-black uppercase text-indigo-400 tracking-tighter">KURULDA</span>
                                            <svg class="w-8 h-8 text-indigo-600 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                        </div>
                                        <div class="flex-1 text-center sm:text-left">
                                            <div class="flex flex-wrap items-center justify-center sm:justify-start gap-2 mb-2">
                                                <span class="px-2.5 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-wider bg-indigo-500/10 text-indigo-600 ring-1 ring-indigo-500/20">
                                            ...
                                            <h5 class="text-lg font-black text-gray-800 group-hover:text-indigo-600 transition-colors">{{ $case->user->name }}</h5>
                                            <p class="text-xs text-gray-500 mt-1 flex items-center justify-center sm:justify-start gap-1 font-medium">
                                                {{ $case->behavior->name ?? 'Disiplin Vukuatı' }}
                                            </p>
                                        </div>
                                        <div class="sm:border-l border-gray-100 sm:pl-6 hidden sm:block">
                                            <div class="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-[10px] font-black uppercase tracking-widest group-hover:bg-indigo-600 group-hover:text-white transition-all">Görüşülmeyi Bekliyor</div>
                                        </div>
                                    </div>
                                </a>
                            @endforeach
                            ...
                            <!-- Planlı Toplantılar -->
                            @forelse($yaklasanToplantılar as $toplanti)
                                <a href="{{ route('admin.disiplin.kurul.toplanti.show', $toplanti) }}" class="group relative bg-white hover:bg-emerald-50/30 border border-gray-100 p-5 rounded-[2rem] transition-all hover:shadow-2xl hover:shadow-emerald-500/10 hover:-translate-y-1">
                                    <div class="flex flex-col sm:flex-row items-center gap-6">
                                        <div class="flex-shrink-0 flex flex-col items-center justify-center p-3 bg-gradient-to-br from-gray-50 to-white rounded-2xl border border-gray-100 w-20 h-20 shadow-inner group-hover:border-emerald-200 transition-colors">
                                            <span class="text-[10px] font-black uppercase text-gray-400 tracking-tighter">{{ $toplanti->baslangic_tarihi->translatedFormat('M') }}</span>
                                            <span class="text-3xl font-black text-gray-800 leading-none mt-1">{{ $toplanti->baslangic_tarihi->format('d') }}</span>
                                        </div>
                                        <div class="flex-1 text-center sm:text-left">
                                            <div class="flex flex-wrap items-center justify-center sm:justify-start gap-2 mb-2">
                                                <span class="px-2.5 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-wider bg-emerald-500/10 text-emerald-600 ring-1 ring-emerald-500/20">
                                                    {{ $toplanti->tur }}
                                                </span>
                                                <span class="text-[10px] font-bold text-gray-400 flex items-center gap-1">
                                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                                    {{ $toplanti->baslangic_tarihi->format('H:i') }}
                                                </span>
                                            ...
                                            <h5 class="text-lg font-black text-gray-800 group-hover:text-emerald-600 transition-colors">{{ $toplanti->baslik }}</h5>
                                            <p class="text-xs text-gray-500 mt-1 flex items-center justify-center sm:justify-start gap-1 font-medium">
                                                <svg class="w-3.5 h-3.5 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                                {{ $toplanti->yer ?: 'Yerleşke Belirtilmemiş' }}
                                            </p>
                                        </div>
                                        <div class="flex items-center gap-6">
                                            <div class="flex -space-x-3 group-hover:-space-x-1 transition-all duration-500">
                                                @foreach($toplanti->katilimcilar->take(4) as $kat)
                                                    @if($kat->user)
                                                        <img src="{{ $kat->user->profile_photo_url }}" class="w-10 h-10 rounded-full border-4 border-white shadow-lg ring-1 ring-gray-100" title="{{ $kat->user->name }}">
                                                    @endif
                                                @endforeach
                                                @if($toplanti->katilimcilar->count() > 4)
                                                    <div class="w-10 h-10 rounded-full bg-indigo-50 border-4 border-white shadow-lg flex items-center justify-center text-[10px] text-indigo-600 font-black ring-1 ring-indigo-100">+{{ $toplanti->katilimcilar->count() - 4 }}</div>
                                                @endif
                                            </div>

                                            @if(Auth::user()->hasAnyRole(['Superadmin', 'Hukuk Admini', 'Disiplin Kurulu Başkanı']) && $toplanti->durum !== 'tamamlandı')
                                                <form action="{{ route('admin.disiplin.kurul.toplanti.resendMails', $toplanti) }}" method="POST" class="inline-block" onclick="event.stopPropagation();">
                                                    @csrf
                                                    <button type="submit" class="flex items-center gap-2 px-3 py-2 bg-indigo-50 hover:bg-indigo-600 text-indigo-600 hover:text-white rounded-xl transition-all shadow-sm ring-1 ring-indigo-200" title="Davet maillerini tekrar gönder">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                                                        <span class="text-[9px] font-black uppercase tracking-widest">MAİL TEKRAR</span>
                                                    </button>
                                                </form>
                                            @endif

                                            <div class="sm:border-l border-gray-100 sm:pl-6 hidden sm:block">
                                                <div class="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center text-gray-400 group-hover:bg-emerald-600 group-hover:text-white transition-all">
                            ...
                            @empty
                                <div class="bg-white/40 border-2 border-dashed border-gray-200 p-16 rounded-[2.5rem] flex flex-col items-center justify-center text-center group">
                                    <div class="p-6 bg-gray-50 rounded-full text-gray-300 group-hover:scale-110 transition-transform duration-500">
                                    ...
                                    <h6 class="text-gray-400 font-black text-lg mt-6 tracking-tight uppercase">Planlanmış Toplantı Yok</h6>
                                    <p class="text-gray-400 text-sm mt-1 max-w-xs font-medium">Kurul gündemine dair henüz bir oturum planlanmamış görünüyor.</p>
                                </div>
                            @endforelse
                        </div>
                    </div>
                                    ...
                                    <div class="flex items-center gap-4 relative z-10">
                                        <div class="relative">
                                            <div class="w-12 h-12 rounded-2xl overflow-hidden ring-4 ring-purple-50 group-hover:ring-purple-100 transition-all">
                                                @if($karar->user)
                                                    <img src="{{ $karar->user->profile_photo_url }}" class="w-full h-full object-cover">
                                                @else
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <h6 class="text-sm font-black text-gray-800 truncate">{{ $karar->user->name ?? 'Silinmiş Personel' }}</h6>
                                            
                                            @php
                                                $iMeeting = $karar->toplantilar()->where('durum', 'tamamlandı')->latest()->first();
                                            @endphp

                                            @if($iMeeting && $iMeeting->kararMaddeleri->count() > 0)
                                                <div class="mt-1 space-y-1">
                                                    @foreach($iMeeting->kararMaddeleri->take(2) as $m)
                                            ...
                                            @else
                                                <p class="text-[9px] font-black text-purple-400 uppercase tracking-tighter truncate mt-0.5">{{ $karar->behavior->name ?? 'Disiplin Vukuatı' }}</p>
                                            @endif

                                            <div class="flex items-center gap-2 mt-2">
                                                <span class="text-[10px] text-gray-400 font-bold font-mono">{{ $karar->karar_tarihi ? $karar->karar_tarihi->format('d.m.Y') : $karar->created_at->format('d.m.Y') }}</span>
                                                <a href="{{ route('admin.disiplin.show', $karar) }}" class="text-[10px] font-black text-indigo-500 hover:text-indigo-700 underline decoration-indigo-200 underline-offset-2">Arşive Git</a>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="bg-white rounded-[2.5rem] border border-gray-100 overflow-hidden shadow-2xl shadow-indigo-500/5 group hover:border-indigo-400 transition-all hover:scale-[1.02]">
                            ...
                            <div class="p-6 bg-gradient-to-br from-gray-50/50 to-white border-b border-gray-100 relative">
                                <div class="absolute top-4 right-4">
                                    <span class="px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-wider {{ $stat['rol'] === 'Başkan' ? 'bg-amber-100 text-amber-600 ring-1 ring-amber-200' : 'bg-gray-100 text-gray-500 ring-1 ring-gray-200' }}">
                                        {{ $stat['rol'] }}
                                    </span>
                                </div>
                                <div class="flex items-center gap-5">
                                    <div class="relative shrink-0">
                                        <img src="{{ $stat['user']->profile_photo_url }}" alt="{{ $stat['user']->name }}" class="w-20 h-20 rounded-[1.5rem] object-cover ring-4 ring-indigo-50 shadow-xl group-hover:ring-indigo-100 transition-all">
                                    ...
                                    <div class="min-w-0">
                                        <h5 class="text-xl font-black text-gray-800 truncate">{{ $stat['user']->name }}</h5>
                                        <p class="text-indigo-600 text-[10px] font-black uppercase tracking-widest mt-1">{{ $stat['user']->bolum->ad ?? 'Genel Yönetim' }}</p>
                                        <div class="flex items-center gap-1.5 mt-2">
                                            ...
                                            <span class="text-[10px] text-gray-400 font-bold italic">{{ $stat['katilim_tarihi'] ? $stat['katilim_tarihi']->translatedFormat('d M Y') : '-' }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="p-6 space-y-6">
                                <div class="grid grid-cols-2 gap-6">
                                    <div class="space-y-2">
                                        ...
                                        <div class="flex items-baseline gap-1">
                                            <span class="text-3xl font-black text-gray-800">%{{ $stat['katilim_orani'] }}</span>
                                        </div>
                                        <div class="w-full bg-gray-100 h-2 rounded-full mt-2 overflow-hidden shadow-inner">
                                            <div class="bg-gradient-to-r from-indigo-500 to-indigo-700 h-full rounded-full transition-all duration-1000" style="width: {{ $stat['katilim_orani'] }}%"></div>
                                        </div>
                                    </div>
                                    <div class="space-y-2">
                                        <p class="text-[9px] font-black text-gray-400 uppercase tracking-widest">Toplam Oylama</p>
                                        <div class="flex items-baseline gap-1">
                                            <span class="text-3xl font-black text-gray-800">{{ $stat['oy_kullanilanSayisi'] }}</span>
                                ...
                                <div class="bg-gray-50/50 p-4 rounded-3xl border border-gray-100 grid grid-cols-3 gap-2 ring-1 ring-black/5">
                                    <div class="text-center group-hover:scale-110 transition-transform">
                                        <p class="text-sm font-black text-emerald-600">{{ $stat['leh_oy'] }}</p>
                                    ...
                                    <div class="text-center border-x border-gray-200 group-hover:scale-110 transition-transform">
                                        <p class="text-sm font-black text-rose-600">{{ $stat['aleyh_oy'] }}</p>
                                        <p class="text-[8px] text-gray-400 uppercase font-black tracking-tighter mt-1">Ceza Verilsin</p>
                                    </div>
                                    <div class="text-center group-hover:scale-110 transition-transform">
                                        <p class="text-sm font-black text-gray-500">{{ $stat['cekimser_oy'] }}</p>
                                ...
                                @if($uyeYonetimiYetkisi)
                                    <div class="pt-4 border-t border-gray-100 flex justify-end">
                                        <form action="{{ route('admin.disiplin.kurul.uye.cikar', $stat['user']) }}" method="POST" id="remove-member-grid-{{ $stat['user']->id }}">
                                            @csrf @method('DELETE')
                                            <button type="button" 
                                                    onclick="Swal.fire({
                                                        title: 'Emin misiniz?',
                                                        ...
                                                        if (result.isConfirmed) {
                                                            document.getElementById('remove-member-grid-{{ $stat['user']->id }}').submit();
                                                        }
                                                    })"
                            ...
                            <tbody class="divide-y divide-gray-50">
                                @foreach($uyeStats as $stat)
                                    <tr class="hover:bg-indigo-50/30 transition-colors group">
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-3">
                                                <img src="{{ $stat['user']->profile_photo_url }}" class="w-10 h-10 rounded-xl object-cover ring-2 ring-indigo-50 group-hover:ring-indigo-200 transition-all">
                                                <div>
                                                    <p class="text-sm font-black text-gray-800">{{ $stat['user']->name }}</p>
                                                    <p class="text-[9px] text-gray-400 italic">{{ $stat['katilim_tarihi'] ? $stat['katilim_tarihi']->translatedFormat('d M Y') : '-' }}'den beri</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <span class="px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-wider {{ $stat['rol'] === 'Başkan' ? 'bg-amber-100 text-amber-600' : 'bg-gray-100 text-gray-500' }}">
                                                {{ $stat['rol'] }}
                                            </span>
                                            <p class="text-[10px] text-indigo-600 font-bold mt-1">{{ $stat['user']->bolum->ad ?? 'Genel Yönetim' }}</p>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-3">
                                                <span class="text-sm font-black text-gray-800">%{{ $stat['katilim_orani'] }}</span>
                                                <div class="w-20 bg-gray-100 h-1.5 rounded-full overflow-hidden">
                                                    <div class="bg-indigo-500 h-full rounded-full" style="width: {{ $stat['katilim_orani'] }}%"></div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-2">
                                                <span class="px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded text-[10px] font-black" title="Ceza Verilmesin">{{ $stat['leh_oy'] }}</span>
                                                <span class="px-2 py-0.5 bg-rose-50 text-rose-600 rounded text-[10px] font-black" title="Ceza Verilsin">{{ $stat['aleyh_oy'] }}</span>
                                                <span class="px-2 py-0.5 bg-gray-50 text-gray-500 rounded text-[10px] font-black" title="Çekimser">{{ $stat['cekimser_oy'] }}</span>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 text-right">
                                            @if($uyeYonetimiYetkisi)
                                                <form action="{{ route('admin.disiplin.kurul.uye.cikar', $stat['user']) }}" method="POST" class="inline" id="remove-member-list-{{ $stat['user']->id }}">
                                                    @csrf @method('DELETE')
                                                    <button type="button" 
                                                            onclick="Swal.fire({
                                                                title: 'Üye Çıkarılsın mı?',
                                                                text: '{{ $stat['user']->name }} isimli üyeyi kuruldan çıkarmak istediğinize emin misiniz?',
                                                                icon: 'warning',
                                                                showCancelButton: true,
                                                                confirmButtonColor: '#e11d48',
                                                                cancelButtonColor: '#64748b',
                                                                ...
                                                                if (result.isConfirmed) {
                                                                    document.getElementById('remove-member-list-{{ $stat['user']->id }}').submit();
                                                                }
                                                            })"
            ...
            <div x-show="activeTab === 'meetings'" x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 translate-y-8" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-8">
                <div class="flex items-center justify-between px-2">
                    <h4 class="text-gray-800 font-black text-xl flex items-center gap-3">
                        <span class="w-2 h-6 bg-gradient-to-b from-emerald-400 to-emerald-600 rounded-full"></span>
                        Toplantı Geçmişi & Arşivi
                    </h4>
                </div>

                <div class="flex flex-col md:flex-row items-center gap-4 px-2">
                    <div class="relative flex-1 w-full">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                            <svg class="h-5 w-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                        </div>
                        <input type="text" x-model="meetingSearch" placeholder="Toplantı başlığı, gündem veya katılımcı ara..." class="block w-full pl-11 pr-4 py-3.5 bg-white border-none ring-2 ring-gray-100 focus:ring-4 focus:ring-emerald-500/20 text-gray-900 rounded-[1.5rem] transition-all font-medium text-sm">
                    </div>
                    <div class="w-full md:w-64">
                        <select x-model="meetingStatusFilter" class="block w-full px-4 py-3.5 bg-white border-none ring-2 ring-gray-100 focus:ring-4 focus:ring-emerald-500/20 text-gray-900 rounded-[1.5rem] transition-all font-black text-xs uppercase tracking-widest appearance-none">
                            <option value="">TÜM DURUMLAR</option>
                            <option value="planlandı">PLANLANDI</option>
                        ...
                        <table class="w-full text-left border-collapse min-w-[1000px]">
                            <thead>
                                <tr class="bg-gray-50/50 border-b border-gray-100">
                                    ...
                                    <th class="px-4 py-5 font-black text-gray-400 uppercase tracking-[0.2em] text-[10px]">Toplantı / Gündem</th>
                                    ...
                                    <th class="px-4 py-5 font-black text-gray-400 uppercase tracking-[0.2em] text-[10px]">Planlayan</th>
                            ...
                            <tbody class="divide-y divide-gray-50">
                                @forelse(collect($yaklasanToplantılar)->merge($toplantıGecmisi)->sortByDesc('baslangic_tarihi') as $t)
                                    <tr class="hover:bg-indigo-50/20 transition-all group" 
                                        x-show="(meetingSearch === '' || '{{ strtolower($t->baslik) }}'.includes(meetingSearch.toLowerCase()) || '{{ strtolower($t->icerik) }}'.includes(meetingSearch.toLowerCase())) && (meetingStatusFilter === '' || '{{ $t->durum }}' === meetingStatusFilter)">
                                        <td class="px-6 py-6 text-center">
                                            <span class="text-xs font-black text-gray-300">#{{ $loop->iteration }}</span>
                                        </td>
                                        <td class="px-5 py-6">
                                            <div class="font-black text-gray-800 text-base group-hover:text-indigo-600 transition-colors">{{ $t->baslik }}</div>
                                            @if($t->disiplinDosyasi)
                                                <div class="text-[10px] text-indigo-400 font-black mt-1.5 flex items-center gap-1.5 uppercase tracking-tighter">
                                                    <span class="p-0.5 bg-indigo-50 rounded">REF: #{{ $t->disiplinDosyasi->id }}</span>
                                                    {{ $t->disiplinDosyasi->user->name }}
                                                </div>
                                            @endif
                                        </td>
                                        <td class="px-5 py-6 whitespace-nowrap">
                                            <div class="text-gray-800 font-black">{{ $t->baslangic_tarihi->translatedFormat('d F Y') }}</div>
                                            <div class="text-[11px] text-gray-400 font-bold mt-0.5">{{ $t->baslangic_tarihi->format('H:i') }}</div>
                                        </td>
                                        <td class="px-5 py-6">
                                            <div class="px-2 py-0.5 inline-block rounded-lg bg-gray-100 text-[10px] font-black uppercase text-gray-500 mb-1.5">{{ $t->tur }}</div>
                                        ...
                                        <td class="px-5 py-6">
                                            <div class="flex items-center gap-2">
                                                @if($t->olusturan)
                                                    <img src="{{ $t->olusturan->profile_photo_url }}" class="w-6 h-6 rounded-full border border-gray-100">
                                                    <a href="{{ url('/kullanici-profil/'.$t->olusturan_user_id) }}" class="text-[11px] font-black text-gray-700 hover:text-indigo-600 transition-colors">{{ $t->olusturan->name }}</a>
                                                @else
                                                    ...
                                                    <span class="text-[11px] font-black text-gray-400 italic">Sistem / Silinmiş</span>
                                                @endif
                                            </div>
                                        </td>
                                        <td class="px-5 py-6">
                                            <div class="flex -space-x-3 hover:-space-x-1 transition-all duration-300">
                                                @foreach($t->katilimcilar->take(5) as $kat)
                                                    @if($kat->user)
                                                        <a href="{{ url('/kullanici-profil/'.$kat->user_id) }}" title="{{ $kat->user->name }}" class="relative group">
                                                            <img src="{{ $kat->user->profile_photo_url }}" class="w-8 h-8 rounded-full border-2 border-white shadow-sm ring-1 ring-gray-100 group-hover:ring-indigo-400 group-hover:scale-110 transition-all">
                                                        </a>
                                                    @endif
                                                @endforeach
                                                @if($t->katilimcilar->count() > 5)
                                                    <div class="w-8 h-8 rounded-full bg-gray-50 border-2 border-white shadow-sm flex items-center justify-center text-[8px] text-gray-500 font-black">+{{ $t->katilimcilar->count() - 5 }}</div>
                                                @endif
                                            </div>
                                        </td>
                                        <td class="px-5 py-6">
                                            @php
                                                $color = match($t->durum) {
                                                    ...
                                                    'tamamlandı' => 'bg-emerald-50 text-emerald-600 ring-emerald-200',
                                                    'iptal' => 'bg-rose-50 text-rose-600 ring-rose-200',
                                            ...
                                            <div class="flex items-center gap-2">
                                                {{-- Mail İletildi Simgesi (Hukuk Admin ve Superadmin için) --}}
                                                @if(Auth::user()->hasAnyRole(['Superadmin', 'Hukuk Admini']))
                                                    @if($t->davet_maili_gonderildi)
                                                        <span class="p-1 bg-blue-50 text-blue-500 rounded-lg ring-1 ring-blue-100" title="Davet mailleri gönderildi">
                                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                                                        </span>
                                                    @else
                                                        <span class="p-1 bg-rose-50 text-rose-500 rounded-lg ring-1 ring-rose-100" title="Davet mailleri henüz gönderilmedi">
                                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                                                        </span>
                                                    @endif
                                                @endif

                                                <span class="px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-[0.1em] border-none ring-1 {{ $color }}">
                                                    {{ $t->durum }}
                                                </span>
                                            </div>
                                        </td>
                                        <td class="px-5 py-6">
                                            <div class="flex items-center justify-end gap-2">
                                                <a href="{{ route('admin.disiplin.kurul.toplanti.show', $t) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-black text-white hover:bg-indigo-600 transition-all" title="Toplantı Odasına Git">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6"/></svg>
                                                </a>

                                                @if(auth()->user()->hasRole('Superadmin') || $t->olusturan_user_id == auth()->id())
                                                    <button @click="window.dispatchEvent(new CustomEvent('open-modal', { detail: 'edit-meeting-modal-{{ $t->id }}' }))" class="w-8 h-8 flex items-center justify-center bg-indigo-50 text-indigo-600 rounded-xl hover:bg-indigo-600 hover:text-white transition-all">
                                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                                                    </button>
                                                    <form action="{{ route('admin.disiplin.kurul.toplanti.destroy', $t) }}" method="POST" id="delete-meeting-{{ $t->id }}">
                                                        @csrf @method('DELETE')
                                                        <button type="button" 
                                                                onclick="Swal.fire({
                                                                    title: 'Toplantı Silinsin mi?',
                                                                    text: 'Bu toplantı kaydı kalıcı olarak silinecektir.',
                                                                    icon: 'error',
                                                                    showCancelButton: true,
                                                                    ...
                                                                    if (result.isConfirmed) {
                                                                        document.getElementById('delete-meeting-{{ $t->id }}').submit();
                                                                    }
                                                                })"
                                                                class="w-8 h-8 flex items-center justify-center bg-rose-50 text-rose-600 rounded-xl hover:bg-rose-600 hover:text-white transition-all">
                                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                                        </button>
                                                    </form>
                                                @endif

                                                {{-- Manuel Mail Gönderme Butonu --}}
                                                @if(Auth::user()->hasAnyRole(['Superadmin', 'Hukuk Admini', 'Disiplin Kurulu Başkanı']) && $t->durum !== 'tamamlandı')
                                                    <form action="{{ route('admin.disiplin.kurul.toplanti.resendMails', $t) }}" method="POST" class="inline-block">
                                                        @csrf
                                                        <button type="submit" class="w-8 h-8 flex items-center justify-center bg-blue-50 text-blue-600 rounded-xl hover:bg-blue-600 hover:text-white transition-all" title="Davet maillerini tekrar gönder">
                                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                                                        </button>
                                                    </form>
                                                @endif
                                ...
                                @empty
                                    <tr><td colspan="6" class="px-8 py-20 text-center text-gray-400 font-black text-sm uppercase tracking-widest italic">Kurul kaydı bulunamadı.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
            ...
            <div x-show="activeTab === 'archive'" x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 translate-y-8" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-8">
                <div class="flex items-center justify-between px-2">
                    ...
                    <div class="px-4 py-1.5 bg-purple-50 text-purple-600 text-[10px] font-black uppercase tracking-widest rounded-full ring-1 ring-purple-100">
                        {{ $arsivdekiDosyalar->count() }} Toplam Kayıt
                    </div>
                </div>
                ...
                <div class="bg-white/70 backdrop-blur-2xl rounded-[2.5rem] border border-white/50 shadow-2xl shadow-indigo-500/5 ring-1 ring-black/[0.02] overflow-hidden">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse min-w-[1000px]">
                            ...
                            <tbody class="divide-y divide-gray-50">
                                @forelse($arsivdekiDosyalar as $case)
                                    <tr class="hover:bg-purple-50/20 transition-all group">
                                        <td class="px-6 py-6 text-center">
                                            <span class="text-xs font-black text-gray-300">#{{ $case->id }}</span>
                                        </td>
                                        <td class="px-6 py-6">
                                            <div class="flex items-center gap-4">
                                                <img src="{{ $case->user->profile_photo_url }}" class="w-10 h-10 rounded-2xl object-cover ring-2 ring-gray-100 shadow-sm">
                                                <div>
                                                    <div class="font-black text-gray-800 text-sm group-hover:text-purple-600 transition-colors">{{ $case->user->name }}</div>
                                                    <div class="text-[10px] text-gray-400 font-bold uppercase tracking-tighter">{{ $case->user->bolum->ad ?? 'Bölüm Belirtilmemiş' }}</div>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-6">
                                            <div class="text-xs font-black text-gray-700 leading-relaxed">{{ $case->behavior->name ?? 'Belirtilmemiş' }}</div>
                                            @if($case->subject)
                                                <div class="text-[10px] text-gray-400 mt-1 font-medium italic line-clamp-1">"{{ $case->subject }}"</div>
                                            @endif
                                        </td>
                                        <td class="px-6 py-6 text-xs font-bold text-gray-500">
                                            {{ $case->created_at->translatedFormat('d M Y') }}
                                            <div class="text-[10px] text-gray-300 mt-0.5">{{ $case->created_at->format('H:i') }}</div>
                                        </td>
                                        <td class="px-6 py-6">
                                            @php
                                            ...
                                            @endphp
                                            <span class="px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest ring-1 {{ $statusColor }}">
                                                {{ $case->durum }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-6 text-right">
                                            <a href="{{ route('admin.disiplin.show', $case) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-black text-white hover:bg-purple-600 rounded-xl transition-all shadow-lg shadow-black/10 hover:shadow-purple-500/30">
            ...
            <div x-show="activeTab === 'history'" x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 translate-y-8" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-8">
                ...
                <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
                    <table class="w-full text-left border-collapse table-auto">
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @forelse($uyelikGecmisi as $gecmis)
                                <tr class="hover:bg-amber-50/10 transition-colors">
                                    <td class="px-8 py-6">
                                        <div class="flex items-center gap-4">
                                            ...
                                            @if($gecmis->user && !$gecmis->user->trashed())
                                                <img src="{{ $gecmis->user->profile_photo_url }}" class="w-10 h-10 rounded-2xl object-cover ring-2 ring-gray-100">
                                                <div>
                                                    <div class="font-black text-gray-800">{{ $displayName }}</div>
                                                    <div class="text-[10px] text-gray-400 font-bold uppercase">{{ $gecmis->user->bolum->ad ?? '-' }}</div>
                                                </div>
                                            @else
                                                ...
                                                <div class="opacity-60">
                                                    <div class="font-black text-gray-400 italic strike-through">{{ $displayName }}</div>
                                                    <div class="text-[10px] text-gray-300 font-bold uppercase">{{ $gecmis->user ? ($gecmis->user->bolum->ad ?? 'Bölüm Yok') : 'SİSTEMDEN SİLİNDİ' }}</div>
                                                </div>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-8 py-6">
                                        <span class="px-2.5 py-1 rounded-xl text-[10px] font-black uppercase tracking-tighter {{ $gecmis->rol === 'baskan' ? 'bg-amber-100 text-amber-600' : 'bg-gray-100 text-gray-500' }}">
                                            {{ $gecmis->rol === 'baskan' ? 'Şura Başkanı' : 'Kurul Üyesi' }}
                                        </span>
                                    </td>
                                    <td class="px-8 py-6">
                                        <div class="text-xs font-mono font-black space-y-1">
                                            <div class="text-emerald-500 flex items-center gap-1.5"><span class="w-1.5 h-1.5 bg-emerald-500 rounded-full {{ !$isDeleted ? 'animate-pulse' : '' }}"></span> {{ $gecmis->katilim_tarihi->format('d.m.Y') }}</div>
                                            @if($gecmis->aktif && !$isDeleted)
                                                <div class="text-indigo-400 text-[9px] uppercase tracking-widest">Hizmet Veriyor</div>
                                            @else
                                                <div class="text-rose-400 flex items-center gap-1.5"><span class="w-1.5 h-1.5 bg-rose-400 rounded-full"></span> {{ $gecmis->ayrilma_tarihi ? $gecmis->ayrilma_tarihi->format('d.m.Y') : ($isDeleted ? 'İşten Ayrıldı' : 'Bilinmiyor') }}</div>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-8 py-6">
                                        <div class="text-[10px] space-y-1.5">
                                            <div class="flex items-center gap-1.5"><span class="w-1 h-1 bg-gray-300 rounded-full"></span> <span class="text-gray-400 font-bold uppercase">Tanımlayan:</span> <span class="text-gray-700 font-black">{{ $gecmis->ekleyen->name ?? 'SYSTEM' }}</span></div>
                                            @if(!$gecmis->aktif)
                                                <div class="flex items-center gap-1.5"><span class="w-1 h-1 bg-gray-300 rounded-full"></span> <span class="text-gray-400 font-bold uppercase">Azleden:</span> <span class="text-gray-700 font-black">{{ $gecmis->cikaran->name ?? '-' }}</span></div>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-8 py-6">
    ...
    <!-- MODALLAR -->
    <x-modal name="add-member-modal" :show="false" focusable>
        <form action="{{ route('admin.disiplin.kurul.uye.ekle') }}" method="POST" class="bg-white rounded-[3rem] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.1)]">
            @csrf
            ...
            <div class="p-8 -mt-8 bg-white rounded-t-[3rem] relative z-20 space-y-8">
                <div class="space-y-2.5">
                    ...
                    <select name="user_id" required class="w-full bg-gray-50 border-none ring-2 ring-gray-100 focus:ring-4 focus:ring-indigo-500 text-gray-800 rounded-2xl py-3.5 transition-all font-bold">
                        ...
                        @foreach(\App\Models\User::orderBy('name')->get() as $user)
                            <option value="{{ $user->id }}">{{ $user->name }} ({{ $user->bolum->ad ?? '-' }})</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2.5">
                        ...
                        <input type="date" name="katilim_tarihi" value="{{ now()->format('Y-m-d') }}" required class="w-full bg-gray-50 border-none ring-2 ring-gray-100 focus:ring-4 focus:ring-indigo-500 text-gray-800 rounded-2xl py-3.5 transition-all font-bold">
                    </div>
                </div>
    ...
    <x-modal name="plan-meeting-modal" :show="false" focusable>
        <form action="{{ route('admin.disiplin.kurul.toplanti.store') }}" method="POST" class="bg-white rounded-3xl overflow-hidden" x-data="{ modalTab: 'kurul' }">
            @csrf
                ...
                <div class="px-8 py-5">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-56 overflow-y-auto pr-1">
                            @foreach($tumUyeler as $uye)
                                <label class="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-gray-50 hover:bg-emerald-50 border border-transparent hover:border-emerald-200 cursor-pointer transition-all group">
                                    <input type="checkbox" name="katilimcilar[]" value="{{ $uye->id }}" checked class="w-4 h-4 rounded text-emerald-600 border-gray-300 focus:ring-emerald-500 flex-shrink-0"/>
                                    <img src="{{ $uye->profile_photo_url }}" alt="{{ $uye->name }}" class="w-8 h-8 rounded-full object-cover border border-gray-200 flex-shrink-0">
                                    <div class="min-w-0">
                                        <p class="text-sm font-semibold text-gray-800 truncate group-hover:text-emerald-700">{{ $uye->name }}</p>
                                        <p class="text-[10px] text-gray-400 uppercase tracking-wide">
                                            @if($baskanlar->contains($uye)) Başkan @else Kurul Üyesi @endif
                                        </p>
                                    </div>
                                </label>
                            @endforeach
                    ...
                    <div x-show="modalTab === 'sistem'" x-data="{ search: '' }" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-1" x-transition:enter-end="opacity-100 translate-y-0">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1">
                            @foreach($tumPersonel as $sUser)
                                <label x-show="'{{ strtolower($sUser->name) }} {{ strtolower($sUser->bolum->ad ?? '') }}'.includes(search.toLowerCase())" class="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-gray-50 hover:bg-indigo-50 border border-transparent hover:border-indigo-200 cursor-pointer transition-all group">
                                    <input type="checkbox" name="sistem_katilimcilari[]" value="{{ $sUser->id }}" class="w-4 h-4 rounded text-indigo-600 border-gray-300 focus:ring-indigo-500 flex-shrink-0"/>
                                    <img src="{{ $sUser->profile_photo_url }}" alt="{{ $sUser->name }}" class="w-8 h-8 rounded-full object-cover border border-gray-200 flex-shrink-0">
                                    <div class="min-w-0">
                                        <p class="text-sm font-semibold text-gray-800 truncate group-hover:text-indigo-700">{{ $sUser->name }}</p>
                                        <p class="text-[10px] text-gray-400 uppercase tracking-wide">{{ $sUser->bolum->ad ?? 'Bölüm yok' }}</p>
                                    </div>
                                </label>
                            @endforeach
                        </div>
    ...
    {{-- TOPLANTI DÜZENLEME MODALLARI (DÖNGÜ) --}}
    @foreach(collect($yaklasanToplantılar)->merge($toplantıGecmisi) as $t)
        <x-modal name="edit-meeting-modal-{{ $t->id }}" :show="false" focusable>
            <form action="{{ route('admin.disiplin.kurul.toplanti.update', $t) }}" method="POST" class="bg-white rounded-3xl overflow-hidden" x-data="{ modalTab: 'kurul' }">
                @csrf @method('PUT')
                {{-- HEADER --}}
                ...
                <div class="divide-y divide-gray-100">
                    <div class="px-8 py-5">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Toplantı Başlığı</label>
                        <input type="text" name="baslik" value="{{ $t->baslik }}" required class="w-full bg-gray-50 border border-gray-200 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 text-gray-800 rounded-xl px-4 py-3 text-sm font-medium transition-all outline-none"/>
                    </div>
                    ...
                    <div class="px-8 py-5 grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div>
                            ...
                            <select name="tur" class="w-full bg-gray-50 border border-gray-200 focus:border-indigo-400 rounded-xl px-3 py-3 text-sm font-medium transition-all appearance-none cursor-pointer outline-none">
                                <option value="olağan" {{ $t->tur == 'olağan' ? 'selected' : '' }}>Olağan</option>
                                <option value="olağanüstü" {{ $t->tur == 'olağanüstü' ? 'selected' : '' }}>Olağanüstü</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Süre (dk)</label>
                            <input type="number" name="planlanan_sure_dk" value="{{ $t->planlanan_sure_dk }}" required class="w-full bg-gray-50 border border-gray-200 focus:border-indigo-400 rounded-xl px-3 py-3 text-sm font-medium outline-none"/>
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Tarih</label>
                            <input type="datetime-local" name="baslangic_tarihi" value="{{ $t->baslangic_tarihi->format('Y-m-d\TH:i') }}" required class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-3 text-sm font-medium outline-none"/>
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Yer</label>
                            <input type="text" name="yer" value="{{ $t->yer }}" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-3 text-sm font-medium outline-none"/>
                        </div>
                    </div>

                    <div class="px-8 py-5">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">İçerik / Gündem</label>
                        <textarea name="icerik" rows="3" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm font-medium transition-all outline-none resize-none">{{ $t->icerik }}</textarea>
                    </div>
                </div>
    ...
    @push('scripts')
        <script>
            ...
            (function() {
                const count = {{ $bekleyenGundemSayisi }};
                if (count > 0) {
