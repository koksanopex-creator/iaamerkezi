...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        if (!$user->is_personnel) {
            ...
            $stats = $this->musteriService->getStats($user, $startDate, $endDate);
        ...
        $onlineQuery = User::where('last_seen_at', '>=', \Carbon\Carbon::now()->subMinutes(5))
            ->with(['bolum', 'loginActivities' => function($q) { $q->latest()->take(7); }]);
        ...
        $topPerformers = $this->puanService->getRankings($start_date, $end_date, null, 5, $excludeRoles);
        ...
        if ($user->is_personnel && $birthdayIsActive == '1') {
            ...
            $internalUsers = User::where('is_personnel', true)
        ...
        if ($user->is_personnel && $anniversaryIsActive == '1') {
            ...
            foreach ($internalUsers as $u) {
                ...
                if ($years <= 0 && !$annivThisYear->isToday()) continue; // Bugün girmişse 0. yıl kutlanabilir belki ama genellikle 1. yıldan başlar.
    ...
    public function puanDurumu(Request $request)
    {
        ...
        if ($start_date || $end_date) {
            ...
            foreach($topUsers as $u) $u->period_puan = $u->toplam_puan;
        }
    ...
    public function tumPersonel(Request $request)
    {
        ...
        if ($start_date || $end_date) {
            ...
            foreach($allUsers as $u) $u->period_puan = $u->toplam_puan;
        }
    ...
    private function getBolumData(Request $request)
    {
        ...
        if ($start_date || $end_date) {
            ...
            $allUsers = User::where('is_personnel', true)
                ->whereDoesntHave('roles', function ($q) use ($excludeRoles) {
                    $q->whereIn('name', $excludeRoles);
                })
                ->get();
