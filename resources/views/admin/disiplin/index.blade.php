        ...
        <div class="flex justify-between items-center">
            <div>
                <h2 class="font-bold text-xl text-gray-800 leading-tight flex items-center gap-2">
                    ...
                    @if(isset($filterMessage) && str_contains($filterMessage, 'Kendi'))
                        Disiplin Dosyalarım
                    @else
                        Disiplin Dosyaları & Tutanaklar
                    @endif
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition group">
                    ...
                    <div class="text-xs font-bold text-gray-500 uppercase tracking-wider">
                        {{ (isset($filterMessage) && str_contains($filterMessage, 'Kendi')) ? 'Dosya Sayım' : 'Toplam Tutanak' }}
                    </div>
                ...
                {{-- Tamamlanan / Bitti --}}
                <a href="{{ route('admin.disiplin.index', ['durum' => 'Karar Verildi']) }}" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition group">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <div class="text-3xl font-black text-emerald-600">{{ $stats['karar_verildi'] }}</div>
                    </div>
                ...
                {{-- Oylama Başlatılanlar --}}
                <a href="{{ route('admin.disiplin.index', ['oylama_aktif' => 1]) }}" class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition group">
                    <div class="flex items-center justify-between mb-3">
                        <div class="bg-red-100 p-2.5 rounded-xl text-red-600 group-hover:bg-red-200 transition {{ ($stats['oylama_baslatilanlar'] ?? 0) > 0 ? 'animate-pulse' : '' }}">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5" />
                            </svg>
                        </div>
                        <div class="text-3xl font-black text-red-600">{{ $stats['oylama_baslatilanlar'] ?? 0 }}</div>
                    </div>
                    <div class="text-xs font-bold text-gray-500 uppercase tracking-wider">Oylama Başlatılanlar</div>
                    <div class="mt-2">
                        @if(($stats['oylama_baslatilanlar'] ?? 0) > 0)
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                            ...
                            <div>
                                ...
                                <select name="itiraz_durumu" class="w-full py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500">
                                    <option value="">Tümü</option>
                                    <option value="itiraz_hakki_olanlar" {{ request('itiraz_durumu') == 'itiraz_hakki_olanlar' ? 'selected' : '' }}>İtiraz Hakkı Olanlar</option>
                                    <option value="itiraz_edilenler" {{ request('itiraz_durumu') == 'itiraz_edilenler' ? 'selected' : '' }}>İtiraz Edilenler</option>
                                    <option value="itiraz_kabul" {{ request('itiraz_durumu') == 'itiraz_kabul' ? 'selected' : '' }}>İtirazı Kabul Edilenler</option>
                                    <option value="itiraz_red" {{ request('itiraz_durumu') == 'itiraz_red' ? 'selected' : '' }}>İtirazı Kabul Edilmeyenler</option>
                                    <option value="itiraz_hakki_yok" {{ request('itiraz_durumu') == 'itiraz_hakki_yok' ? 'selected' : '' }}>İtiraz Hakkı Olmayanlar</option>
                                </select>
                            </div>

                            {{-- Toplantı Sayısı (Dinamik) --}}
                            <div>
                                ...
                                <select name="toplanti_sayisi" class="w-full py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-red-500 focus:border-red-500">
                                    ...
                                    @endphp
                                    @for($i = 1; $i <= $totalPossibleRounds; $i++)
                                        <option value="{{ $i }}" {{ request('toplanti_sayisi') == $i ? 'selected' : '' }}>{{ $i }}. Kez Toplananlar</option>
                                    @endfor
                                </select>
                            </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200 table-fixed">
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            @forelse($cases as $case)
                                ...
                                @endphp
                                <tr class="hover:bg-red-50/20 transition-all group {{ $isLiveVoting ? 'live-voting-row' : '' }}">
                                    {{-- No / Raporlayan --}}
                                    <td class="px-2 py-3 whitespace-nowrap">
                                        <div class="flex items-center gap-2">
                                            ...
                                            @endif
                                            <div class="text-sm font-bold text-gray-900 {{ $isLiveVoting ? 'text-red-700' : '' }}">#{{ $case->id }}</div>
                                    ...
                                    <td class="px-2 py-3">
                                        <div class="text-sm text-gray-900 font-semibold line-clamp-1"
                                            title="{{ $case->behavior_text }}">
                                            {{ Str::limit($case->behavior_text, 35) }}
                                        </div>
                                        <div class="text-xs text-gray-400 mt-0.5">{{ $case->category_text ?? '-' }}
                                        </div>
                                    </td>

                                    {{-- Puan --}}
                                    <td class="px-2 py-3 text-center cursor-help" title="{{ $matrisBilgi }}">
                                        <div
                                            ...
                                            @if($case->manual_penalty_name && Auth::user()->hasRole(['Superadmin', 'Yonetim', 'Yönetim', 'Hukuk Admini', 'Disiplin Kurulu Başkanı', 'Disiplin Kurulu Üyesi']))
                                                <div class="flex flex-col items-center gap-0.5 py-1">
                                                    <div class="flex items-center gap-1">
                                                        ...
                                                        <span class="text-[8px] font-bold text-gray-500 uppercase leading-none">{{ $case->sistem_oneri_ceza }}</span>
                                                    </div>
                                                    <div class="flex items-center gap-1 mt-0.5">
                                                        <span class="text-[7px] font-black text-indigo-400 uppercase tracking-tighter">MANUEL:</span>
                                                        <span class="text-[9px] font-black text-indigo-700 uppercase leading-none">{{ $case->manual_penalty_name }}</span>
                                                    </div>
                                            ...
                                            @else
                                                <span
                                                    class="text-[9px] text-red-600 font-bold uppercase tracking-wide mt-0.5">{{ $case->manual_penalty_name ?: $case->sistem_oneri_ceza }}</span>
                                            @endif
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-2 py-3 whitespace-nowrap text-center">
                                        ...
                                        @endif
                                        {{-- TEKRAR GÖRÜŞME ETİKETİ (Yetkili rollere özel) --}}
                                        @if($case->rediscussion_count > 0 && Auth::user()->hasRole(['Superadmin', 'Hukuk Admini', 'Disiplin Kurulu Başkanı', 'Disiplin Kurulu Üyesi']))
                                            <div class="mt-1.5">
                                        ...
                                        @elseif($case->is_appeal_window_open)
                                            ...
                                            <div class="mt-1.5 flex justify-center">
                                                <div class="inline-flex flex-col items-center px-2 py-1 rounded-lg bg-amber-50 text-amber-700 border border-amber-200 shadow-sm leading-tight animate-pulse hover:animate-none transition-all">
                                                    ...
                                                    <span class="text-[10px] font-black whitespace-nowrap">{{ (int)$daysLeft }} Gün</span>
                                                </div>
                                            </div>
                                        @elseif($case->durum == 'Karar Verildi' && !$case->is_appealed)
                                            <div class="mt-1.5 flex justify-center">
