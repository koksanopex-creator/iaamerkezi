...
{{-- SONUÇ EKRANI --}}
@if(in_array($case->durum, ['Karar Verildi', 'Kurulda', 'İtiraz Edildi']))
            ...
            <div class="relative flex flex-col md:flex-row justify-between items-start gap-6">
                <div class="flex-1 w-full">
                    ...
                    <div class="flex items-center gap-4 mb-6">
                        ...
                        @endphp
                        <div class="flex-shrink-0 w-14 h-14 bg-gradient-to-br from-{{ $themeColor }}-500 to-{{ $themeColor }}-700 rounded-2xl flex items-center justify-center shadow-lg transform -rotate-2">
                            <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @elseif($cardIcon == 'reply') <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"/>
                                @elseif($cardIcon == 'search') <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                                @else <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
                                @endif
                            </svg>
                        </div>
                        
                        <div>
                            <h3 class="text-2xl font-black text-{{ $themeColor }}-900 leading-tight uppercase tracking-tight italic">{{ $cardTitle }}</h3>
                            
                            <div class="flex flex-wrap items-center gap-2 mt-2">
                                {{-- İşlem Tarihi (Dinamik Etiket ve Fallback) --}}
                                @php
                                ...
                                @endphp
                                <span class="inline-flex items-center px-3 py-1 rounded-md text-xs font-bold bg-white text-{{ $themeColor }}-700 border border-{{ $themeColor }}-200 shadow-sm">
                                    ...
                                    {{ $mainDateLabel }}: {{ $mainDate ? $mainDate->format('d.m.Y H:i') : 'Tarih Yok' }}
                                </span>

                                {{-- TOPLANTI / KARAR TARİHİ --}}
                                @php
                                ...
                                @endphp
                                @if($dateToShow)
                                    <span class="inline-flex items-center px-3 py-1 rounded-md text-xs font-bold {{ $isClosed ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-' . $themeColor . '-50 text-' . $themeColor . '-700 border-' . $themeColor . '-100' }} border shadow-sm">
                                        <svg class="w-3.5 h-3.5 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                        {{ $dateLabel }}: {{ $dateToShow->format('d.m.Y H:i') }}
                                    </span>
                                @endif

                                {{-- RE-DISCUSSION BADGE --}}
                                @if($case->rediscussion_count > 0)
                                    @php $isClosed = ($case->durum == 'Karar Verildi'); @endphp
                                    <span class="inline-flex items-center px-3 py-1 rounded-md text-xs font-black {{ $isClosed ? 'bg-slate-700' : 'bg-orange-600 animate-pulse' }} text-white shadow-sm">
                                        ...
                                        {{ $case->rediscussion_count + 1 }}. KEZ GÖRÜŞÜL{{ $isClosed ? 'DÜ' : 'ÜYOR' }}
                                    </span>
                                @endif
                            </div>
                        </div>
                    ...
                    @endphp
                    @if($meetings->count() > 1)
                        <div class="mb-5 bg-white/40 rounded-xl p-3 border border-{{ $themeColor }}-100">
                            <h4 class="text-[9px] font-black text-{{ $themeColor }}-600 uppercase tracking-widest mb-2 flex items-center gap-2">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                                Kurul Görüşme Geçmişi
                            </h4>
                            <div class="flex flex-wrap gap-2">
                                @foreach($meetings as $index => $meeting)
                                    <div class="flex items-center gap-1.5 px-2 py-1 bg-white border border-{{ $themeColor }}-100 rounded-lg shadow-sm">
                                        <span class="text-[8px] font-black text-{{ $themeColor }}-400">{{ $index + 1 }}. Tur:</span>
                                        <span class="text-[10px] font-bold text-slate-700">{{ $meeting->baslangic_tarihi->format('d.m.Y') }}</span>
                    ...
                    @if($case->rediscussion_count > 0 && $case->rediscussion_reason && $canSeeRediscussionReason)
                        <div class="mb-5 bg-orange-50/50 border border-orange-200 rounded-xl p-4 shadow-sm border-l-4 border-l-orange-500">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-orange-800 font-medium leading-relaxed">{{ $case->rediscussion_reason }}</p>
                                    @php
                                        $isClosed = ($case->durum == 'Karar Verildi');
                                    ...
                                    @if($finalDate)
                                        <div class="mt-2 text-[10px] font-bold text-orange-600 flex items-center gap-1">
                                            ...
                                            {{ $isClosed ? 'Karar Bağlanma Tarihi' : 'Yeni Toplantı Tarihi' }}: {{ $finalDate->format('d.m.Y H:i') }}
                                        </div>
                                    @endif
                                </div>
                            </div>
                    ...
                    <div class="space-y-5">
                        ...
                        {{-- İtiraz Bilgisi --}}
                        @if($case->is_appealed)
                            @php $appeal = $case->appeals()->latest()->first(); @endphp
                            @if($appeal)
                                <div class="bg-white/60 backdrop-blur-sm rounded-xl p-4 border border-indigo-200">
                                    <div class="flex items-center gap-2 mb-2">
                                        ...
                                        <span class="text-[10px] text-indigo-400 font-bold">{{ $appeal->created_at->format('d.m.Y H:i') }}</span>
                                    </div>
                                    <p class="text-xs text-indigo-900 font-bold mb-1 italic">"{{ $appeal->reason }}"</p>
                                    <p class="text-[9px] text-indigo-500 font-medium">İtiraz Eden: <span class="font-bold">{{ $appeal->user->name }}</span></p>
                                </div>
                            @endif
                        @endif
                        ...
                        {{-- Nihai Karar Özeti (Görseldeki gibi eklendi) --}}
                        ...
                        {{-- Açıklama (Sadece yetkililer ve personel dışındakiler görebilir) --}}
                        @if($case->yonetici_notu && $canSeeManagerNotes)
                            @php
                            ...
                            <div class="w-full bg-white/60 backdrop-blur-sm rounded-xl p-5 border border-{{ $cardColor }}-200 relative group shadow-sm">
                                <div class="mb-0">
                                    <span class="block text-[10px] font-bold text-{{ $cardColor }}-400 uppercase tracking-wider mb-2">YÖNETİCİ / KURUL AÇIKLAMASI</span>
                                    <p class="text-sm text-gray-700 italic leading-relaxed">"{{ $yoneticiNotu }}"</p>
                                </div>

                                @if($signatureData)
                                    <div class="mt-4 pt-3 border-t border-{{ $cardColor }}-50 flex justify-end">
                                        <div class="text-right">
                                            <div class="flex items-center justify-end gap-2 text-[11px] font-medium text-rose-600">
                                                ...
                                                @if(isset($signatureData['id']))
                                                    <a href="{{ url('/kullanici-profil/'.$signatureData['id']) }}" class="text-rose-700 font-black hover:underline">
                                                        {{ $signatureData['name'] }}
                                                    </a>
                                                @else
                                                    <span class="text-rose-700 font-black uppercase">{{ $signatureData['name'] }}</span>
                                                @endif
                                            </div>
                        ...
                        {{-- Karar Dosyaları --}}
                        @if($case->karar_dosyasi && count($case->karar_dosyasi) > 0)
                            <div class="mt-4">
                ...
                @endphp
                @if((Auth::user()->hasRole(['Superadmin', 'Hukuk Admini']) || Auth::user()->can('disiplin.degerlendirme.kullan')) && $isRevertible)
...
{{-- ========================================================== --}}
@if(in_array($case->durum, ['Yönetici Değerlendirmesi', 'İtiraz Edildi']))
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div class="lg:col-span-5 space-y-6">
                        ...
                        <div class="bg-white/80 rounded-xl p-4 border border-indigo-100 shadow-sm">
                                    ...
                                    <div class="flex flex-col gap-3 relative z-10">
                                        <div class="flex items-start gap-3">
                                            ...
                                            <div class="text-[11px] font-bold leading-tight">
                                                ...
                                                <span class="font-black text-amber-900 text-xs mt-1 block tracking-tight">{{ $existingMeeting->baslangic_tarihi->format('d.m.Y H:i') }}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex flex-wrap gap-2">
                                            ...
                                            @if($case->durum != 'Kurulda')
                                                <button type="button" 
                                                        onclick="managerSubmitForm('{{ route('admin.disiplin.board.send', $case->id) }}', 'Dosya durumu KURULDA olarak güncellenecek. Emin misiniz?')"
                                            ...
                                            {{-- Toplantı Detay Linki --}}
                                            <a href="{{ route('admin.disiplin.kurul.toplanti.show', $existingMeeting->id) }}" 
                                               target="_blank"
                                            ...
                                            {{-- Online Toplantı Linki (Eğer yer alanı link ise) --}}
                                            @if(Str::startsWith($existingMeeting->yer, ['http://', 'https://']))
                                                <a href="{{ $existingMeeting->yer }}" 
                                                   target="_blank"
                            ...
                            @endif
                            <input type="datetime-local" id="toplanti_tarihi_input" name="toplanti_tarihi" value="{{ $existingMeeting ? $existingMeeting->baslangic_tarihi->format('Y-m-d\TH:i') : '' }}" class="w-full bg-gray-50 border border-gray-200 text-gray-800 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block p-2.5 mb-4">
                            ...
                            <input type="text" id="toplanti_yeri_input" name="toplanti_yeri" value="{{ $existingMeeting ? $existingMeeting->yer : '' }}" placeholder="Örn: Toplantı Odası 1 veya Zoom Linki" class="w-full bg-gray-50 border border-gray-200 text-gray-800 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block p-2.5">
                        </div>
                    </div>
                </div>
