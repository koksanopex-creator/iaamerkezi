        ...
        <div @click="squadOpen = !squadOpen" class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2 pb-2 border-b border-gray-100 cursor-pointer select-none group">
             ...
             <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap items-center gap-2 mt-1.5">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-gray-100 text-gray-800">Toplam: <?php echo e($toplamUye); ?></span>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-green-100 text-green-800"><span class="w-2 h-2 bg-green-500 rounded-full mr-1.5"></span><?php echo e($aktifSayisi); ?> Aktif</span>
                        <?php if($bekleyenSayisi > 0): ?>
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-amber-100 text-amber-800 animate-pulse"><span class="w-2 h-2 bg-amber-500 rounded-full mr-1.5"></span><?php echo e($bekleyenSayisi); ?> Bekliyor</span>
                        <?php endif; ?>
                        <?php if($reddedenSayisi > 0): ?>
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-red-100 text-red-800"><span class="w-2 h-2 bg-red-500 rounded-full mr-1.5"></span><?php echo e($reddedenSayisi); ?> Reddetti</span>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <?php if($isLeader): ?>
                ...
                <div class="flex-shrink-0" @click.stop>
                    <?php if(!$kilitliMi): ?>
                        <button onclick="Livewire.dispatch('openSquadModal', { iaaId: <?php echo e($iaa->id); ?> })" class="flex items-center px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-lg transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
