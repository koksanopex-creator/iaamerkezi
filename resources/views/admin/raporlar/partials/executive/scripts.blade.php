@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const rawCatSeries = @json($charts['cat_data']);
            const rawCatLabels = @json($charts['cat_labels']);
            ...
            const rawStatusSeries = @json($charts['status_data']); // [Açık, Çözülen, Geciken]
            ...
            const rawLabels   = @json($bolumMemnuniyeti->pluck('bolum_adi'));
            const rawOnay     = @json($bolumMemnuniyeti->pluck('onay_sayisi')->map(fn($v) => (int)$v));
            const rawRed      = @json($bolumMemnuniyeti->pluck('red_sayisi')->map(fn($v) => (int)$v));
            const rawRevizyon = @json($bolumMemnuniyeti->pluck('revizyon_sayisi')->map(fn($v) => (int)$v));
