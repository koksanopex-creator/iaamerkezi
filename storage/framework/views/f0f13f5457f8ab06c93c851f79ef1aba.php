...
<?php $component->withAttributes([]); ?>
    <?php $__env->startPush('pageTitle', 'Disiplin Ayarları | '); ?>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6" 
                    ...
                    <ul class="space-y-2 max-h-60 overflow-y-auto">
                        ...
                        <li class="flex justify-between items-center text-sm bg-gray-50 p-2 rounded border">
                            ...
                            <div class="flex gap-1.5">
                                <button @click="isEdit=true; id=<?php echo e($c->id); ?>; ad='<?php echo e(addslashes($c->ad)); ?>'; action='<?php echo e(route('admin.disiplin.settings.category.update', $c->id)); ?>'" 
                    ...
                    <ul class="space-y-2 max-h-60 overflow-y-auto">
                        ...
                        <li class="flex justify-between items-center text-sm bg-gray-50 p-2 rounded border">
                            ...
                            <div class="flex items-center gap-1.5">
                                <span class="font-bold text-indigo-600 text-xs mr-1 bg-indigo-50 px-1.5 py-0.5 rounded"><?php echo e($i->puan); ?>p</span>
                                <button @click="isEdit=true; id=<?php echo e($i->id); ?>; tanim='<?php echo e(addslashes($i->tanim)); ?>'; puan='<?php echo e($i->puan); ?>'; action='<?php echo e(route('admin.disiplin.settings.impact.update', $i->id)); ?>'" 
                                        class="p-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all shadow-sm" title="Düzenle">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </button>
                ...
                <div class="bg-white shadow sm:rounded-lg p-4 h-fit"
                    ...
                    <ul class="space-y-2 max-h-60 overflow-y-auto">
                        ...
                        <li class="flex justify-between items-center text-sm bg-gray-50 p-2 rounded border">
                            ...
                            <div class="flex items-center gap-1.5">
                                <span class="font-bold text-indigo-600 text-xs mr-1 bg-indigo-50 px-1.5 py-0.5 rounded"><?php echo e($s->puan); ?>p</span>
                                <button @click="isEdit=true; id=<?php echo e($s->id); ?>; tanim='<?php echo e(addslashes($s->tanim)); ?>'; puan='<?php echo e($s->puan); ?>'; action='<?php echo e(route('admin.disiplin.settings.scope.update', $s->id)); ?>'" 
                                        class="p-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all shadow-sm" title="Düzenle">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </button>
            ...
            <div x-show="activeTab === 'suclar'" class="space-y-6" style="display: none;">
                ...
                <div class="p-4 bg-white border border-indigo-100 rounded-xl shadow-sm">
                    ...
                    <form action="<?php echo e(route('admin.disiplin.settings.behavior.store')); ?>" method="POST" class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                        <?php echo csrf_field(); ?>
                ...
                <div class="overflow-x-auto bg-white rounded-xl shadow-sm border border-gray-100">
                    <table class="min-w-full divide-y divide-gray-100">
                        ...
                        <tbody class="bg-white divide-y divide-gray-50" x-data="{ editingId: null }">
                            <?php $__currentLoopData = $behaviors; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $categoryName => $group): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr class="bg-indigo-50/30">
                                    <td colspan="5" class="px-4 py-2 text-[10px] font-black text-indigo-800 uppercase tracking-widest border-y border-indigo-100/50">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="ml-auto bg-white/80 text-indigo-600 px-2 py-0.5 rounded-full text-[9px] shadow-sm border border-indigo-100"><?php echo e($group->count()); ?> Kayıt</span>
                                        </div>
                                    </td>
                                </tr>
                                ...
                                <tr class="hover:bg-gray-50/50 transition-colors group" :class="editingId === <?php echo e($b->id); ?> ? 'bg-amber-50/50' : ''">
                                    
                                    <template x-if="editingId === <?php echo e($b->id); ?>">
                                        <td colspan="5" class="p-0">
                                            <form action="<?php echo e(route('admin.disiplin.settings.behavior.update', $b->id)); ?>" method="POST" class="grid grid-cols-1 md:grid-cols-12 gap-3 p-3 bg-amber-50/50 border-y border-amber-100 items-center">
                                                ...
                                                <div class="md:col-span-2">
                                                    <select name="category_id" class="w-full text-xs rounded-lg border-amber-200 focus:ring-amber-500 bg-white" required>
                                                        <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                            <option value="<?php echo e($c->id); ?>" <?php echo e($b->category_id == $c->id ? 'selected' : ''); ?>><?php echo e($c->ad); ?></option>
                                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                                    </select>
                                                </div>
                                                <div class="md:col-span-6">
                                                    <input type="text" name="tanim" value="<?php echo e($b->tanim); ?>" class="w-full text-xs rounded-lg border-amber-200 focus:ring-amber-500 bg-white" required>
                                                </div>
                                                <div class="md:col-span-2">
                                                    <input type="text" name="yasal_dayanak" value="<?php echo e($b->yasal_dayanak); ?>" class="w-full text-xs rounded-lg border-amber-200 focus:ring-amber-500 bg-white">
                                                </div>
                                    ...
                                    <template x-if="editingId !== <?php echo e($b->id); ?>">
                                        <td class="px-4 py-3 text-[10px] text-gray-400 font-mono"><?php echo e($loop->parent->iteration); ?>.<?php echo e($loop->iteration); ?></td>
                                    </template>
                                    <template x-if="editingId !== <?php echo e($b->id); ?>">
                                    ...
                                    <template x-if="editingId !== <?php echo e($b->id); ?>">
                                        <td class="px-4 py-3">
                                            <div class="text-sm text-gray-800 font-medium leading-relaxed"><?php echo e($b->tanim); ?></div>
                                            <?php if($b->created_at->diffInDays() < 7): ?>
                                                <span class="text-[9px] bg-green-100 text-green-600 px-1.5 py-0.5 rounded font-black uppercase mt-1 inline-block">Yeni</span>
                                            <?php endif; ?>
                                        </td>
                                    </template>
                                    <template x-if="editingId !== <?php echo e($b->id); ?>">
                                        <td class="px-4 py-3 text-xs text-gray-500 italic"><?php echo e($b->yasal_dayanak ?: '-'); ?></td>
                                    </template>
                                    <template x-if="editingId !== <?php echo e($b->id); ?>">
                                        <td class="px-4 py-3 text-right">
                                            <div class="flex justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                                                <button @click="editingId = <?php echo e($b->id); ?>" 
                                                ...
                                                <form action="<?php echo e(route('admin.disiplin.settings.behavior.delete', $b->id)); ?>" method="POST" onsubmit="return confirm('Bu suç tanımını silmek istediğinize emin misiniz?')">
                                                    <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
            ...
            <div x-show="activeTab === 'katsayilar'" class="space-y-6" style="display: none;">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="bg-yellow-50/50 p-4 rounded border border-yellow-100" 
                         x-data="{ editMode: false, formAction: '<?php echo e(route('admin.disiplin.settings.scale.store')); ?>', method: 'POST', min: '', max: '', ad: '', karar_metni: '' }">
                        <div class="flex justify-between items-center mb-4">
                            <h4 class="font-bold text-gray-700" x-text="editMode ? 'Skala Düzenle' : 'Ceza Puan Skalası'"></h4>
                            <button x-show="editMode" @click="editMode=false; method='POST'; formAction='<?php echo e(route('admin.disiplin.settings.scale.store')); ?>'; min=''; max=''; ad=''; karar_metni=''" class="text-[10px] text-red-500 underline">Vazgeç</button>
                        </div>
                            ...
                            <?php $__currentLoopData = $scales; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <li class="flex items-center justify-between p-2 hover:bg-gray-50 group">
                                    <div class="flex items-center gap-3">
                                        <span class="font-mono font-bold text-gray-600 w-16"><?php echo e($s->min_puan); ?> - <?php echo e($s->max_puan); ?></span>
                                        <span class="font-bold text-gray-800"><?php echo e($s->ceza_adi); ?></span>
                                    </div>
                                    <div class="flex items-center gap-1.5">
                                        <button @click="editMode=true; method='PUT'; formAction='<?php echo e(route('admin.disiplin.settings.scale.update', $s->id)); ?>'; min='<?php echo e($s->min_puan); ?>'; max='<?php echo e($s->max_puan); ?>'; ad='<?php echo e(addslashes($s->ceza_adi)); ?>'; karar_metni='<?php echo e(preg_replace('/\r|\n/', '\n', addslashes($s->karar_metni))); ?>'" 
                                                class="p-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all shadow-sm" title="Düzenle">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </button>
                                        <form action="<?php echo e(route('admin.disiplin.settings.scale.delete', $s->id)); ?>" method="POST" onsubmit="return confirm('Silmek istediğinize emin misiniz?')">
                                            <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
...
<?php endif; ?><?php /**PATH /var/www/kys_koksan/iaa/resources/views/admin/disiplin/settings.blade.php ENDPATH**/ ?>