<div class="space-y-8 animate-fade-in relative">
    ...
    <div id="print-header" class="hidden print:flex flex-col items-center justify-center p-8 border-b-2 border-slate-900 mb-8">
        ...
        <div class="flex items-center gap-2 px-4 py-1.5 bg-slate-100 rounded-full text-xs font-black text-slate-700 uppercase italic">
            Rapor Dönemi: <?php echo e(\Carbon\Carbon::parse($startDate)->format('d.m.Y')); ?> - <?php echo e(\Carbon\Carbon::parse($endDate)->format('d.m.Y')); ?>
        ...
        <div class="p-4 flex flex-wrap items-center justify-between gap-4 bg-gradient-to-r from-slate-50 to-white">
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex items-center gap-1.5 p-1 bg-slate-100 rounded-xl border border-slate-200">
                    <button wire:click="setFilterPeriod('all')" class="px-3 py-1 text-[10px] font-black uppercase rounded-lg transition-all <?php echo e($startDate == null && $endDate == null ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'); ?>">Hepsi</button>
                    <button wire:click="setFilterPeriod('month')" class="px-3 py-1 text-[10px] font-black uppercase rounded-lg transition-all <?php echo e($startDate == Carbon\Carbon::now()->startOfMonth()->format('Y-m-d') ? 'bg-rose-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-700'); ?>">Bu Ay</button>
                    <button wire:click="setFilterPeriod('quarter')" class="px-3 py-1 text-[10px] font-black uppercase rounded-lg transition-all <?php echo e($startDate == Carbon\Carbon::now()->startOfQuarter()->format('Y-m-d') ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-700'); ?>">Bu Çeyrek</button>
                    <button wire:click="setFilterPeriod('year')" class="px-3 py-1 text-[10px] font-black uppercase rounded-lg transition-all <?php echo e($startDate == Carbon\Carbon::now()->startOfYear()->format('Y-m-d') ? 'bg-amber-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-700'); ?>">Bu Yıl</button>
                </div>
                ...
                <div class="relative">
                    <select wire:model.live="selectedBolumId" class="pl-10 pr-8 py-2 text-sm border-slate-200 bg-white rounded-xl shadow-sm focus:ring-2 focus:ring-rose-500 min-w-[200px] text-slate-700 font-bold appearance-none transition-all">
                        ...
                        <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $bolumler; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $bolum): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($bolum->id); ?>"><?php echo e($bolum->ad); ?></option>
                ...
                <div class="relative">
                    <select wire:model.live="selectedReason" class="pl-10 pr-8 py-2 text-sm border-slate-200 bg-white rounded-xl shadow-sm focus:ring-2 focus:ring-rose-500 min-w-[200px] text-slate-700 font-bold appearance-none transition-all">
                        ...
                        <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $reasons; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $reason): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 flex items-center gap-4 hover:shadow-md transition-all">
            ...
            <div>
                ...
                <div class="text-lg font-black text-slate-900"><?php echo e(number_format($totalIadeCount)); ?> <span class="text-xs text-slate-400">Kayıt</span></div>
            </div>
        </div>

        
        <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $unitTotals; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $unitTotal): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div wire:click="setFilterUnit('<?php echo e($unitTotal->birim); ?>')" 
                 class="bg-white rounded-2xl p-4 shadow-sm border <?php echo e($selectedUnit === $unitTotal->birim ? 'border-indigo-500 ring-2 ring-indigo-50' : 'border-slate-100'); ?> flex items-center gap-4 hover:shadow-md transition-all cursor-pointer group">
                <div class="w-12 h-12 <?php echo e($selectedUnit === $unitTotal->birim ? 'bg-indigo-600 text-white' : 'bg-indigo-50 text-indigo-600'); ?> rounded-2xl flex items-center justify-center shrink-0 transition-colors">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
                </div>
                <div>
                    <h3 class="text-[10px] font-black <?php echo e($selectedUnit === $unitTotal->birim ? 'text-indigo-600' : 'text-slate-400'); ?> uppercase tracking-widest mb-0.5">Toplam Miktar (<?php echo e($unitTotal->birim); ?>)</h3>
                    <div class="text-lg font-black text-slate-900"><?php echo e(number_format($unitTotal->total_amount, 2)); ?> <span class="text-xs text-slate-400"><?php echo e($unitTotal->birim); ?></span></div>
                </div>
            </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><!--[if ENDBLOCK]><![endif]-->

        
        <div wire:click="setFilterReason('<?php echo e($topReason->iade_sebebi ?? ''); ?>')" 
             class="bg-white rounded-2xl p-4 shadow-sm border <?php echo e($selectedReason === ($topReason->iade_sebebi ?? '') ? 'border-rose-500 ring-2 ring-rose-50' : 'border-slate-100'); ?> flex items-center gap-4 hover:shadow-md transition-all cursor-pointer group">
            ...
            <div>
                ...
                <div class="text-sm font-black text-slate-900"><?php echo e($topReason->iade_sebebi ?? '-'); ?></div>
                <div class="text-[10px] text-slate-500 font-bold">(<?php echo e($topReason->count ?? 0); ?> kez tekrarlandı)</div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white rounded-3xl p-6 shadow-lg border border-slate-100 flex flex-col">
            ...
            <p class="text-[10px] text-slate-400 mb-6 ml-3.5">(<?php echo $iadeDateStr; ?> İşlem Tarihli)</p>
        ...
        <div class="bg-white rounded-3xl p-6 shadow-lg border border-slate-100 lg:col-span-2 flex flex-col">
            <div class="flex items-start justify-between mb-6">
                <div>
                    ...
                    <p class="text-[10px] text-slate-400 ml-3.5">(<?php echo $iadeDateStr; ?> Açılış Tarihli)</p>
                </div>
                <span id="returnRateBadge" class="px-2 py-1 bg-rose-100 text-rose-700 text-xs font-black rounded-full uppercase italic">İade Oranı: %<?php echo e($returnRate); ?></span>
            </div>
        ...
        <div class="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden lg:col-span-2">
            <div class="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div class="space-y-1">
                    ...
                    <p class="text-[10px] text-slate-500 font-bold flex items-center gap-1">
                        (<?php echo $iadeDateStr; ?> İşlem Tarihli)
                    </p>
                </div>
            </div>
            ...
            <div class="p-6 bg-slate-50/30">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <!--[if BLOCK]><![endif]--><?php $__empty_1 = true; $__currentLoopData = $deptReasonStats ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dept => $data): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 flex flex-col">
                            <div class="flex items-center justify-between mb-4 border-b border-slate-100 pb-3">
                                <h5 class="font-black text-slate-800 uppercase tracking-wide text-[13px] truncate pr-2" title="<?php echo e($dept); ?>"><?php echo e($dept); ?></h5>
                                <span class="px-2.5 py-1 bg-rose-50 text-rose-600 text-[10px] font-black rounded-lg border border-rose-100 whitespace-nowrap"><?php echo e($data['total']); ?> İade</span>
                            </div>
                            <div class="space-y-4 max-h-[280px] overflow-y-auto pr-2 custom-scrollbar">
                                <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $data['reasons']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $reason): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div>
                                        <div class="flex justify-between items-end mb-1.5">
                                            <span class="font-bold text-slate-600 text-xs truncate pr-2" title="<?php echo e($reason['sebep']); ?>"><?php echo e($reason['sebep']); ?></span>
                                            <div class="flex items-center gap-1.5 whitespace-nowrap">
                                                <span class="font-black text-slate-900 text-sm"><?php echo e($reason['count']); ?></span>
                                                <span class="text-[9px] px-1.5 py-0.5 bg-slate-100 text-slate-500 font-bold rounded">
                                                    %<?php echo e($reason['percent']); ?>

                                                </span>
                                            </div>
                                        </div>
                                        <div class="w-full bg-slate-100 rounded-full h-1.5">
                                            <div class="bg-indigo-500 h-1.5 rounded-full" style="width: <?php echo e($reason['percent']); ?>%"></div>
                                        </div>
                                    </div>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><!--[if ENDBLOCK]><![endif]-->
                            </div>
    ...
    <div class="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden">
        <div class="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
            <div class="space-y-1">
                <h4 class="text-slate-900 font-black flex items-center gap-3">
                    ...
                    <span class="px-2.5 py-0.5 bg-rose-100 text-rose-700 text-[10px] font-black rounded-full uppercase">Toplam <?php echo e($totalIadeCount); ?> Kayıt</span>
                </h4>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100">
                    <!--[if BLOCK]><![endif]--><?php $__empty_1 = true; $__currentLoopData = $iadeler; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $iade): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50/80 transition-colors group">
                            <td class="px-6 py-4">
                                <div class="flex flex-col gap-1.5">
                                    <div class="flex items-center gap-1.5" title="İade Tarihi">
                                        ...
                                        <span class="text-xs font-black text-slate-900"><?php echo e($iade->iade_tarihi ? $iade->iade_tarihi->format('d.m.Y') : $iade->created_at->format('d.m.Y')); ?></span>
                                    </div>
                                    <div class="flex items-center gap-1.5" title="Şikayet Açılış Tarihi">
                                        ...
                                        <span class="text-xs font-bold text-slate-600"><?php echo e($iade->musteriSikayeti->created_at->format('d.m.Y')); ?></span>
                                    </div>
                                    <div class="flex items-center gap-1.5" title="Kapanış Tarihi">
                                        ...
                                        <span class="text-xs font-bold text-slate-600">
                                            <!--[if BLOCK]><![endif]--><?php if(in_array($iade->musteriSikayeti->musteri_durum, ['Çözümlendi', 'Kapatıldı', 'Talep Olarak Kapatıldı', 'Hatalı Bildirim Olarak Kapatıldı', 'talep_olarak_kapatildi', 'hatali_bildirim_olarak_kapatildi'])): ?>
                                                <?php echo e($iade->musteriSikayeti->updated_at->format('d.m.Y')); ?>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <!--[if BLOCK]><![endif]--><?php if($iade->musteriSikayeti->customer && $iade->musteriSikayeti->customer->logo_path): ?>
                                        <img src="<?php echo e(asset('storage/' . $iade->musteriSikayeti->customer->logo_path)); ?>" class="w-8 h-8 rounded-lg object-cover shadow-sm border border-slate-200" alt="Logo">
                                    <?php else: ?>
                                    ...
                                    <div class="flex flex-col">
                                        <!--[if BLOCK]><![endif]--><?php if($iade->musteriSikayeti->customer_id): ?>
                                            <a href="<?php echo e(route('musteri.profil.show', $iade->musteriSikayeti->customer_id)); ?>" class="text-sm font-bold text-indigo-600 hover:text-rose-600 transition-colors tracking-tight">
                                                <?php echo e($iade->musteriSikayeti->musteri_adi); ?>

                                            </a>
                                        <?php else: ?>
                                            <div class="text-sm font-bold text-slate-700 tracking-tight"><?php echo e($iade->musteriSikayeti->musteri_adi); ?></div>
                                        <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                                        <div class="text-[11px] text-slate-400 font-medium truncate max-w-[200px]" title="<?php echo e($iade->musteriSikayeti->musteri_sikayet_konusu); ?>">
                                            #<?php echo e($iade->musteriSikayeti->id); ?> - <?php echo e($iade->musteriSikayeti->musteri_sikayet_konusu); ?>
                            ...
                            <td class="px-6 py-4">
                                <span class="px-2.5 py-1 bg-indigo-50 text-indigo-700 text-[10px] font-black uppercase rounded-lg border border-indigo-100">
                                    <?php echo e($iade->musteriSikayeti->sikayetKategori->bolum->ad ?? 'Genel'); ?>

                                </span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="text-sm font-black text-slate-900"><?php echo e(number_format($iade->miktar, 2)); ?></div>
                                <div class="text-[10px] text-slate-400 font-bold uppercase"><?php echo e($iade->birim); ?></div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <span class="w-2 h-2 rounded-full bg-rose-500"></span>
                                    <span class="text-xs font-bold text-slate-700"><?php echo e($iade->iade_sebebi); ?></span>
                                </div>
                                <div class="text-[10px] text-slate-400 truncate max-w-[150px] italic mt-0.5"><?php echo e($iade->aciklama); ?></div>
                            </td>
                            <td class="px-6 py-4 flex items-center gap-1">
                                <a href="<?php echo e(route('admin.sikayetler.show', $iade->musteriSikayeti->id)); ?>" class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all inline-block" title="Şikayet Detayı">
                                ...
                                <a href="<?php echo e(route('proje.workspace.show', $iade->musteriSikayeti->iaaProjesi->id)); ?>" class="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all inline-block" title="Proje Çalışma Alanı">
        ...
        <div class="px-6 py-4 bg-slate-50/50 border-t border-slate-100 flex justify-center">
            <!--[if BLOCK]><![endif]--><?php if($totalIadeCount > 5): ?>
                <button wire:click="toggleShowAll" class="flex items-center gap-2 px-6 py-2 bg-white border border-slate-200 text-slate-600 text-xs font-black uppercase rounded-full shadow-sm hover:bg-slate-50 hover:text-indigo-600 hover:border-indigo-100 transition-all group">
                    ...
                    <?php else: ?>
                        ...
                        <span>Devamını Görüntüle (<?php echo e($totalIadeCount - 5); ?> Kayıt Daha)</span>
                    <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                </button>
            <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
        </div>
    ...
    <script>
        document.addEventListener('livewire:initialized', () => {
            ...
            const reasonsChart = new ApexCharts(document.querySelector("#reasonsChart"), {
                ...commonOptions,
                series: <?php echo json_encode($reasonDistribution->pluck('count'), 15, 512) ?>,
                labels: <?php echo json_encode($reasonDistribution->pluck('iade_sebebi'), 15, 512) ?>,
            ...
            const deptChart = new ApexCharts(document.querySelector("#deptChart"), {
                ...commonOptions,
                series: [{ name: 'İade Adedi', data: <?php echo json_encode($deptDistribution->pluck('count'), 15, 512) ?> }],
                ...
                xaxis: { categories: <?php echo json_encode($deptDistribution->pluck('bolum_adi'), 15, 512) ?>, labels: { style: { colors: '#64748b', fontWeight: 600 } } },
            ...
            const trendChart = new ApexCharts(document.querySelector("#trendChart"), {
                ...commonOptions,
                series: [{ name: 'İade Adedi', data: <?php echo json_encode($trendCounts, 15, 512) ?> }],
                ...
                xaxis: { categories: <?php echo json_encode($trendDates, 15, 512) ?>, labels: { style: { colors: '#64748b', fontWeight: 600 } } },
                yaxis: { labels: { style: { colors: '#64748b' } } },
                grid: { borderColor: '#f1f5f9', strokeDashArray: 4 }
            });
            trendChart.render();
            ...
            const ratioChart = new ApexCharts(document.querySelector("#ratioChart"), {
                ...commonOptions,
                series: [{ name: 'Şikayet Sayısı', data: <?php echo json_encode($ratioData, 15, 512) ?> }],
                chart: { ...commonOptions.chart, type: 'bar', height: 350 },
...
<?php /**PATH C:\Users\celal.karaman\Desktop\Projelerim\iaa_projesi\resources\views/livewire/sikayet-iadesi-raporu.blade.php ENDPATH**/ ?>