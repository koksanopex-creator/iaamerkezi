    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="md:flex md:items-center md:justify-between mb-8">
            ...
            <div class="mt-4 flex md:mt-0 md:ml-4">
                <a href="{{ route('admin.sikayet-raporlari.tablo') }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-xl shadow-sm text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition duration-150">
                ...
                <table class="w-full table-fixed" style="border-collapse: separate; border-spacing: 0 10px;">
                    ...
                    <tbody>
                        @forelse($visits as $index => $visit)
                            ...
                            @endphp
                            <tr class="bg-white hover:bg-indigo-50/30 transition duration-150 group rounded-lg" style="{{ $outlineStyle }}">
                                {{-- # Numara --}}
                                <td class="px-2 py-4 text-center">
                                    <span class="inline-flex items-center justify-center h-7 w-7 rounded-full bg-gray-100 text-[11px] font-black text-gray-500">{{ $rowNumber }}</span>
                                </td>
                                {{-- Tarihler --}}
                                <td class="px-4 py-4">
                                    <div class="flex flex-col">
                                        ...
                                        <div class="flex flex-wrap items-center gap-1">
                                            <span class="text-sm font-bold text-gray-900 whitespace-nowrap">{{ Carbon\Carbon::parse($visit['visit_date'])->translatedFormat('d F Y') }}</span>
                                            <span class="text-[10px] text-gray-500 font-medium whitespace-nowrap">{{ Carbon\Carbon::parse($visit['visit_date'])->format('H:i') }}</span>
                                        </div>
                                        ...
                                        <div class="flex flex-wrap items-center gap-1">
                                            @if(!empty($visit['estimated_return_date']))
                                                <span class="text-xs font-bold text-emerald-700 whitespace-nowrap">{{ Carbon\Carbon::parse($visit['estimated_return_date'])->translatedFormat('d F Y') }}</span>
                                            @else
                                                @if(in_array($visit['status'], ['Onaylandı', 'Tamamlandı']))
                                        ...
                                        <div class="flex flex-wrap items-center gap-1">
                                            <span class="text-xs font-bold text-indigo-700 whitespace-nowrap">{{ Carbon\Carbon::parse($visit['created_at'])->translatedFormat('d F Y') }}</span>
                                            <span class="text-[10px] text-indigo-500/70 whitespace-nowrap">{{ Carbon\Carbon::parse($visit['created_at'])->format('H:i') }}</span>
                                        </div>
                                    </div>
                                </td>
                                {{-- Müşteri / Ürün --}}
                                <td class="px-4 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="flex-shrink-0 h-10 w-10 bg-white rounded-lg border border-gray-100 p-1 flex items-center justify-center overflow-hidden shadow-sm">
                                            @if(!empty($visit['customer']['logo_path']))
                                                <img src="{{ asset('storage/' . $visit['customer']['logo_path']) }}" alt="Logo" class="max-h-full max-w-full object-contain">
                                            @else
                                                <div class="h-full w-full bg-indigo-50 rounded flex items-center justify-center text-indigo-700 font-bold text-xs">
                                                    {{ strtoupper(substr($visit['customer']['name'] ?? 'M', 0, 1)) }}
                                                </div>
                                            @endif
                                        </div>
                                        <div class="flex flex-col min-w-0">
                                            <span class="text-sm font-bold text-gray-800 truncate" title="{{ !empty($visit['customer']['name']) ? $visit['customer']['name'] : 'Müşteri Belirtilmedi' }}">{{ !empty($visit['customer']['name']) ? $visit['customer']['name'] : 'Müşteri Belirtilmedi' }}</span>
                                            @if(!empty($visit['product']['name']))
                                                <span class="text-[11px] text-indigo-600 font-bold bg-indigo-50 self-start px-2 py-0.5 rounded-full mt-1 truncate max-w-full" title="{{ $visit['product']['name'] }}">{{ $visit['product']['name'] }}</span>
                                            @else
                                ...
                                <td class="px-4 py-4">
                                    <div class="flex flex-col w-full">
                                        <span class="text-xs font-black text-gray-500 uppercase tracking-tight truncate" title="{{ $visit['visit_reason'] }}">{{ $visit['visit_reason'] }}</span>
                                        @if($visit['visit_notes'])
                                            <span class="text-[11px] text-gray-500 line-clamp-2 mt-0.5" title="{{ $visit['visit_notes'] }}">{{ $visit['visit_notes'] }}</span>
                                        @endif
                                    </div>
                                </td>
                                {{-- Personel --}}
                                <td class="px-4 py-4">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-8 w-8 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-700 font-bold text-xs">
                                            {{ strtoupper(substr($visit['user']['name'] ?? 'P', 0, 1)) }}
                                        </div>
                                        <div class="ml-3">
                                            <div class="text-xs font-bold text-gray-900">{{ $visit['user']['name'] ?? 'Belirtilmedi' }}</div>
                                        </div>
                                    </div>
                                </td>
                                {{-- İlgili Şikayet --}}
                                <td class="px-4 py-4">
                                    @if($complaint)
                                        <div class="flex flex-col gap-1">
                                            <span class="text-xs font-bold text-gray-700 line-clamp-2" title="#{{ $complaint->id }} - {{ $complaint->musteri_sikayet_konusu }}">#{{ $complaint->id }} - {{ $complaint->musteri_sikayet_konusu }}</span>
                                            <div class="flex items-center gap-1.5 mt-0.5">
                                                @if($complaint->bolum && $complaint->bolum->logo_yolu)
                                ...
                                <td class="px-4 py-4">
                                    ...
                                    <div class="flex flex-col items-start gap-1.5">
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-black border uppercase tracking-widest {{ $statusClass }}">
                                            {{ $statusStr ?: 'Bilinmiyor' }}
                                        </span>
                                        @php
                                            $statusDate = null;
                                            $statusLabel = null;
                                        ...
                                        @if($statusDate)
                                            <div class="flex flex-col">
                                                <span class="text-[9px] text-gray-400 font-bold uppercase tracking-widest">{{ $statusLabel }}</span>
                                                <span class="text-[10px] text-gray-600 font-medium whitespace-nowrap">{{ Carbon\Carbon::parse($statusDate)->format('d.m.Y H:i') }}</span>
                                            </div>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-4 text-center text-sm font-medium relative overflow-visible">
                                    @if($waitingText)
                                        <div class="absolute -top-[2px] -right-[2px] z-10">
                                            <div class="px-2.5 py-1 rounded-bl-lg text-[9px] font-black uppercase whitespace-nowrap {{ $waitingColor }} shadow-lg animate-pulse" style="letter-spacing: 0.3px;">
                                                ⚠ {{ $waitingText }}
                                            </div>
                                        </div>
                                    @endif
                                    <div class="flex justify-center items-center gap-2">
                                        @if($complaint)
                                            <a href="{{ route('admin.sikayetler.show', $complaint->id) }}" target="_blank" class="p-2 text-blue-600 hover:bg-blue-600 hover:text-white rounded-xl transition shadow-sm border border-blue-100">
                                        ...
                                        @endif
                                        @if($complaint && $complaint->iaa_id)
                                            <a href="{{ route('proje.workspace.show', $complaint->iaa_id) }}" target="_blank" class="p-2 text-indigo-600 hover:bg-indigo-600 hover:text-white rounded-xl transition shadow-sm border border-indigo-100">
        ...
        <div class="lg:hidden space-y-4">
            @forelse($visits as $index => $visit)
                @php
                    ...
                    $rowNumber = (($pagination['current_page'] - 1) * $pagination['per_page']) + $index + 1;
                @endphp
                <div class="relative rounded-xl border-2 {{ $outlineColor ?: 'border-gray-200' }} {{ $bgTint }} shadow-sm overflow-hidden">
                    {{-- Waiting Badge --}}
                    @if($waitingText)
                        <div class="w-full px-3 py-1.5 {{ $waitingColor }} text-[10px] font-black uppercase text-center animate-pulse" style="letter-spacing: 0.3px;">
                            ⚠ {{ $waitingText }}
                        </div>
                    @endif

                    <div class="p-4 space-y-3">
                        {{-- Header: # + Müşteri + Durum --}}
                        <div class="flex items-start justify-between gap-3">
                            <div class="flex items-center gap-3 min-w-0 flex-1">
                                <span class="flex-shrink-0 inline-flex items-center justify-center h-7 w-7 rounded-full bg-gray-100 text-[11px] font-black text-gray-500">{{ $rowNumber }}</span>
                                <div class="flex-shrink-0 h-10 w-10 bg-white rounded-lg border border-gray-100 p-1 flex items-center justify-center overflow-hidden shadow-sm">
                                    @if(!empty($visit['customer']['logo_path']))
                                        <img src="{{ asset('storage/' . $visit['customer']['logo_path']) }}" alt="Logo" class="max-h-full max-w-full object-contain">
                                    @else
                                ...
                                <div class="min-w-0 flex-1">
                                    <p class="text-sm font-bold text-gray-900 truncate">{{ !empty($visit['customer']['name']) ? $visit['customer']['name'] : 'Müşteri Belirtilmedi' }}</p>
                                    @if(!empty($visit['product']['name']))
                                        <span class="text-[10px] text-indigo-600 font-bold bg-indigo-50 px-2 py-0.5 rounded-full inline-block mt-0.5 truncate max-w-full">{{ $visit['product']['name'] }}</span>
                                    @endif
                                </div>
                            </div>
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-black border uppercase tracking-widest flex-shrink-0 {{ $statusClass }}">
                                {{ $statusStr ?: 'Bilinmiyor' }}
                            </span>
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-2 bg-gray-50/80 rounded-lg p-2.5">
                            <div>
                                ...
                                <span class="text-xs font-bold text-gray-900">{{ Carbon\Carbon::parse($visit['visit_date'])->translatedFormat('d M Y') }}</span>
                                <span class="text-[10px] text-gray-500 block">{{ Carbon\Carbon::parse($visit['visit_date'])->format('H:i') }}</span>
                            </div>
                            <div>
                                <span class="text-[9px] text-emerald-500 font-bold uppercase tracking-widest block">Tahmini Dönüş</span>
                                @if(!empty($visit['estimated_return_date']))
                                    <span class="text-xs font-bold text-emerald-700">{{ Carbon\Carbon::parse($visit['estimated_return_date'])->translatedFormat('d M Y') }}</span>
                                @else
                            ...
                            <div>
                                ...
                                <span class="text-xs font-bold text-indigo-700">{{ Carbon\Carbon::parse($visit['created_at'])->translatedFormat('d M Y') }}</span>
                                <span class="text-[10px] text-indigo-500/70 block">{{ Carbon\Carbon::parse($visit['created_at'])->format('H:i') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                ...
                                <span class="text-xs font-black text-gray-600 uppercase">{{ $visit['visit_reason'] }}</span>
                                @if($visit['visit_notes'])
                                    <p class="text-[11px] text-gray-500 line-clamp-2 mt-0.5">{{ $visit['visit_notes'] }}</p>
                                @endif
                            </div>
                            <div>
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-xs font-bold text-gray-900">{{ $visit['user']['name'] ?? 'Belirtilmedi' }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        @if($complaint)
                            <div class="bg-white rounded-lg border border-gray-100 p-2.5">
                                ...
                                <p class="text-xs font-bold text-gray-700 line-clamp-2">#{{ $complaint->id }} - {{ $complaint->musteri_sikayet_konusu }}</p>
                        ...
                        <div class="flex items-center justify-end gap-2 pt-1 border-t border-gray-100">
                            @if($complaint)
                                <a href="{{ route('admin.sikayetler.show', $complaint->id) }}" target="_blank" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition">
                                    <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                    Şikayet
                                </a>
                            @endif
                            @if($complaint && $complaint->iaa_id)
                                <a href="{{ route('proje.workspace.show', $complaint->iaa_id) }}" target="_blank" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition">
                ...
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm text-gray-500">
                            Toplam <span class="font-bold text-indigo-600">{{ $pagination['total'] }}</span> kayıttan 
                            <span class="font-bold">{{ ($pagination['current_page'] - 1) * $pagination['per_page'] + 1 }}</span> ile 
                            <span class="font-bold">{{ min($pagination['current_page'] * $pagination['per_page'], $pagination['total']) }}</span> arası gösteriliyor.
                        </p>
                    </div>
                    <div>
                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                            <button wire:click="previousPage" {{ $pagination['current_page'] == 1 ? 'disabled' : '' }} class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:opacity-50">
                            ...
                            @for($i = 1; $i <= $pagination['last_page']; $i++)
                                <button wire:click="gotoPage({{ $i }})" class="relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm font-bold {{ $pagination['current_page'] == $i ? 'text-indigo-600 bg-indigo-50 border-indigo-500 z-10' : 'text-gray-700 hover:bg-gray-50' }}">
                                    {{ $i }}
                                </button>
                            @endfor

                            <button wire:click="nextPage" {{ $pagination['current_page'] == $pagination['last_page'] ? 'disabled' : '' }} class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 disabled:opacity-50">
