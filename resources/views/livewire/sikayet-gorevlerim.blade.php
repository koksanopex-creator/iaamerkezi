<div>
    {{-- Bu dosya, app/Livewire/SikayetGorevlerim.php içindeki #[Layout] direktifi sayesinde 
         otomatik olarak ana layout'unuzun (layouts.app) içine yerleşecek. --}}

    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Devam Eden Şikayet Görevlerim') }}
        </h2>
    </x-slot>
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4 text-lg text-gray-600 leading-7 font-semibold">
                                        <a href="{{ route('proje.workspace.show', $proje->id) }}">{{ $proje->baslik }}</a>
                                    </div>
                                </div>
                                <div class="text-sm text-gray-500">
                                    {{ $proje->onaylanma_tarihi->format('d.m.Y') }}
                                </div>
                            </div>
                            ...
                            <div class="ml-12 mt-2">
                                <div class="text-sm text-gray-500">
                                    @if($proje->musteriSikayeti)
                                        <strong>Müşteri:</strong> {{ $proje->musteriSikayeti->musteri_adi }}
                                    @else
                                ...
                                <div class="mt-3 flex items-center gap-3">
                                    @if($proje->musteriSikayeti)
                                    {{-- Detay linki admin paneline gidiyor, bu doğru çünkü SikayetController->show metodu yetkilendirme yapacaktır --}}
                                    <a href="{{ route('admin.sikayetler.show', $proje->musteriSikayeti->id) }}"
