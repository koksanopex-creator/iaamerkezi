        ...
        @else
            <div class="space-y-6">
                @foreach($tools as $tool)
                    <div class="bg-white rounded-lg shadow border border-gray-200 overflow-hidden" wire:key="tool-{{ $tool->id }}">
                        <div class="bg-gray-50 px-4 py-3 border-b border-gray-200 flex justify-between items-center">
                            <div class="flex items-center">
                                {!! $availableTools[$tool->tool_type]['icon'] ?? '' !!}
                                <h4 class="ml-2 text-base font-medium text-gray-900">{{ $tool->title }}</h4>
                                <span class="ml-3 inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800">
                                    {{ $tool->user->name ?? 'Bilinmeyen' }} tarafından
                                </span>
                            </div>
                            @if($this->canManageTools)
                                <div class="flex items-center gap-2">
                                    <div class="flex items-center border border-gray-200 rounded overflow-hidden bg-white shadow-sm">
                                        <button wire:click="moveToolUp({{ $tool->id }})" class="hover:bg-gray-100 px-1.5 py-1 text-gray-500 {{ $loop->first ? 'opacity-30 cursor-not-allowed' : 'hover:text-indigo-600' }}" {{ $loop->first ? 'disabled' : '' }} title="Yukarı Taşı">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" /></svg>
                                        </button>
                                        <div class="h-4 w-px bg-gray-200"></div>
                                        <button wire:click="moveToolDown({{ $tool->id }})" class="hover:bg-gray-100 px-1.5 py-1 text-gray-500 {{ $loop->last ? 'opacity-30 cursor-not-allowed' : 'hover:text-indigo-600' }}" {{ $loop->last ? 'disabled' : '' }} title="Aşağı Taşı">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
                                        </button>
                                    </div>
                                    <div class="h-6 w-px bg-gray-300 mx-1"></div>
                                    <button wire:click="removeTool({{ $tool->id }})" wire:confirm="Bu aracı ve içindeki verileri silmek istediğinizden emin misiniz?" class="text-red-500 hover:text-red-700 transition-colors p-1" title="Aracı Sil">
                        ...
                        <div class="p-4">
                            @if($tool->tool_type === '5why')
                                @livewire('project.tools.five-whys-analysis', ['tool' => $tool, 'canManage' => $this->canManageTools], key('5why-'.$tool->id))
                            @elseif($tool->tool_type === 'chart')
                                @livewire('project.tools.chart-analysis', ['tool' => $tool, 'canManage' => $this->canManageTools], key('chart-'.$tool->id))
                            @elseif($tool->tool_type === 'swot')
                                @livewire('project.tools.swot-analysis', ['tool' => $tool, 'canManage' => $this->canManageTools], key('swot-'.$tool->id))
                            @elseif($tool->tool_type === 'checklist')
                                @livewire('project.tools.checklist-tool', ['tool' => $tool, 'canManage' => $this->canManageTools], key('checklist-'.$tool->id))
                            @elseif($tool->tool_type === 'action_list')
                                @livewire('project.tools.action-list-tool', ['tool' => $tool, 'canManage' => $this->canManageTools], key('actionlist-'.$tool->id))
                            @elseif($tool->tool_type === 'fishbone')
                                @livewire('project.tools.fishbone-analysis', ['tool' => $tool, 'canManage' => $this->canManageTools], key('fishbone-'.$tool->id))
                            @elseif($tool->tool_type === 'pareto')
                                @livewire('project.tools.pareto-analysis', ['tool' => $tool, 'canManage' => $this->canManageTools], key('pareto-'.$tool->id))
                            @elseif($tool->tool_type === '4m_report')
                                @livewire('project.tools.four-m-report', ['tool' => $tool, 'canManage' => $this->canManageTools], key('4m-'.$tool->id))
                            @else
                                <div class="p-4 bg-yellow-50 text-yellow-700 rounded-md">
                                    Bu araç tipi ({{ $tool->tool_type }}) henüz desteklenmiyor veya yapım aşamasında.
                                </div>
                            @endif
                        </div>
                    </div>
                        ...
                        <div>
                            ...
                            <div class="mt-6 text-left max-w-full">
                                ...
                                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                    @foreach($availableTools as $type => $info)
                                        <div wire:click="addTool('{{ $type }}')" 
                                            ...
                                            <div wire:loading wire:target="addTool('{{ $type }}')" class="absolute inset-0 bg-white/70 flex items-center justify-center rounded-xl backdrop-blur-sm z-10">
