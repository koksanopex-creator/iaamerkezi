                        ...
                        @if($case->yonetici_notu)
                                    ...
                                    @if($signatureData)
                                        <div class="mt-4 pt-3 border-t border-{{ $cardColor }}-100 flex justify-end">
                                            <div class="text-right">
                                                <div class="flex items-center justify-end gap-2 text-[11px] font-medium text-{{ $cardColor }}-600">
                                                    <span class="opacity-60 text-[9px] font-bold uppercase tracking-tighter">İşlemi Yapan:</span>
                                                    @if(isset($signatureData['id']))
                                                        <a href="{{ route('profile.show', $signatureData['id']) }}" class="text-{{ $cardColor }}-700 font-black hover:underline decoration-{{ $cardColor }}-400 underline-offset-2">
                                                            {{ $signatureData['name'] }}
                                                        </a>
                                                    @else
                                                        <span class="text-{{ $cardColor }}-700 font-black">{{ $signatureData['name'] }}</span>
                                                    @endif
                                                    <span class="w-1 h-1 rounded-full bg-{{ $cardColor }}-200"></span>
                            ...
                            @elseif($signatureData)
                                <div class="flex justify-end mt-2">
                                    <div class="flex items-center gap-2 text-[11px] font-medium text-{{ $cardColor }}-600">
                                        <span class="opacity-60">İşlemi Yapan:</span>
                                        @if(isset($signatureData['id']))
                                            <a href="{{ route('profile.show', $signatureData['id']) }}" class="text-{{ $cardColor }}-700 font-black hover:underline">
                                                {{ $signatureData['name'] }}
                                            </a>
                                        @else
                                            <span class="text-{{ $cardColor }}-700 font-black">{{ $signatureData['name'] }}</span>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div class="lg:col-span-5 space-y-6">
                        ...
                        <div class="bg-white/80 rounded-xl p-4 border border-indigo-100 shadow-sm">
                            ...
                            @if($existingMeeting)
                                <div class="mb-2 flex items-center gap-2 bg-amber-50 text-amber-700 px-3 py-2 rounded-lg border border-amber-100 shadow-sm">
                                    ...
                                    <div class="text-[10px] font-bold leading-tight">
                                        ...
                                        <span class="font-medium opacity-80">({{ $existingMeeting->baslangic_tarihi->format('d.m.Y H:i') }})</span>
                                    </div>
                                </div>
                            @endif

                            <input type="datetime-local" id="toplanti_tarihi_input" name="toplanti_tarihi" 
                                value="{{ $existingMeeting ? $existingMeeting->baslangic_tarihi->format('Y-m-d\TH:i') : '' }}"
                                class="w-full bg-gray-50 border border-gray-200 text-gray-800 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block p-2.5">
                        </div>
                    </div>
                </div>
