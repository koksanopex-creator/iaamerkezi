                            ...
                            <tbody class="divide-y divide-slate-100">
                                @forelse($users as $user)
                                    ...
                                    <tr class="bg-white table-row-hover transition-all">
                                            ...
                                            <div class="flex items-center gap-3">
                                                <div class="relative flex-shrink-0">
                                                    @if($user->profile_photo_path)
                                                        <img src="{{ asset('storage/' . $user->profile_photo_path) }}" alt="{{ $user->name }}" class="w-10 h-10 rounded-full object-cover shadow-md border border-white">
                                                    @else
                                        ...
                                        <td class="px-3 py-3">
                                            @if($tab === 'musteri')
                                                <span class="inline-block px-2 py-1 rounded-lg text-xs font-bold bg-amber-50/50 border border-amber-100/30 text-amber-700 max-w-[140px] break-words text-left" title="{{ $user->firma_adlari }}">
                                                    {{ $user->firma_adlari ?: '-' }}
                                                </span>
                                            @else
                                                <span class="inline-block px-2 py-1 rounded-lg text-xs font-bold bg-indigo-50/50 border border-indigo-100/30 text-indigo-700 max-w-[140px] break-words text-left" title="{{ $user->bolum->ad ?? '-' }}">
                                                    {{ $user->bolum->ad ?? '-' }}
                                                </span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-3 py-3 text-right">
                                            <a href="{{ route('logs.login.show', $user->id) }}"
                                                class="inline-flex items-center px-2.5 py-1.5 bg-indigo-50 hover:bg-indigo-600 text-indigo-600 hover:text-white text-[10px] font-black uppercase tracking-wider rounded-lg transition-all shadow-sm whitespace-nowrap gap-1">
                            ...
                            <div class="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm space-y-4">
                                ...
                                <div class="grid grid-cols-2 gap-3 pt-3 border-t border-slate-50 text-xs">
                                    ...
                                    <div>
                                        ...
                                        @if($tab === 'musteri')
                                            <span class="inline-block px-2.5 py-1 rounded-lg font-bold bg-amber-50/50 border border-amber-100/30 text-amber-700 text-[11px] text-left truncate w-full" title="{{ $user->firma_adlari }}">
                                                {{ $user->firma_adlari ?: '-' }}
                                            </span>
                                        @else
                                            <span class="inline-block px-2.5 py-1 rounded-lg font-bold bg-indigo-50/50 border border-indigo-100/30 text-indigo-700 text-[11px] text-left truncate w-full" title="{{ $user->bolum->ad ?? '-' }}">
                                                {{ $user->bolum->ad ?? '-' }}
                                            </span>
                                        @endif
                                    </div>
