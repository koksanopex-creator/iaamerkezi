...
{{-- DÜZELTME: Tüm JS mantığı 'x-data' dışına, bir script etiketine taşındı --}}
            ...
            <div class="flex flex-wrap gap-3">
                @foreach($formData[$index]['files'] as $filePath)
                    ...
                    <div class="relative group w-24">
                        @if($isImage)
                            <a href="{{ Storage::url($filePath) }}" data-fancybox="gallery-active-{{$index}}" data-caption="{{ basename($filePath) }}">
                                <img src="{{ Storage::url($filePath) }}" alt="{{ basename($filePath) }}" class="h-24 w-24 object-cover rounded-lg border border-gray-200 shadow-sm">
                            </a>
                        @else
                            <a href="{{ Storage::url($filePath) }}" target="_blank" class="h-24 w-24 flex flex-col items-center justify-center bg-gray-100 rounded-lg border border-gray-200 p-2" title="{{ basename($filePath) }}">
...
{{-- DÜZELTME: AlpineJS mantığı buraya taşındı (pushOnce DEĞİL, her widget için çalışmalı) --}}
@push('scripts')
<script>
