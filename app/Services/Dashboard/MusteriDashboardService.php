...
{
    public function getStats(User $user, $startDate = null, $endDate = null, $customerId = null)
    {
        $stats = [];
        
        // Eğer dışarıdan ID gelmediyse session veya default'tan alacağız (bu kontrolü controller yapacak ama burada da fallback olsun)
        ...
        if ($customerId) {
            ...
            $stats['son_sikayetler'] = (clone $baseQuery)
                ->with(['iaaProjesi', 'sikayetKategori', 'cozumTakimi.lider', 'aktifHatirlatma'])
                ->latest()
                ->take(10)
                ->get();
        }
