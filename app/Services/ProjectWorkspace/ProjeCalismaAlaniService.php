...
class ProjeCalismaAlaniService
{
    ...
    public function getProjectData(Iaa $iaa)
    {
        ...
        if (!$statusDate)
        {
            ...
            elseif (in_array($iaa->durum, ['talep_olarak_kapatildi', 'hatali_bildirim_olarak_kapatildi']))
            {
                $statusDate = $iaa->tamamlanma_tarihi ?? $iaa->updated_at;
            }
            else
            {
    ...
    public function authorizeUser(Iaa $iaa)
    {
        ...
        if (Auth::check())
        {
            ...
            if (!$isYetkiliKullanici && Auth::user()->hasRole('Müşteri Saha Temsilcisi'))
            {
                $allowedBolumIds = Auth::user()->getAllowedBolumIds();
                
                // 1. Doğrudan proje bölümü
                if (in_array($iaa->bolum_id, $allowedBolumIds)) {
                    return true;
                }
                
                // 2. Şikayet kategorisi departmanı üzerinden
                if ($iaa->musteriSikayeti && $iaa->musteriSikayeti->sikayetKategori && in_array($iaa->musteriSikayeti->sikayetKategori->bolum_id, $allowedBolumIds)) {
                    return true;
                }
            }
    ...
    private function checkGeneralAuthorization(Iaa $iaa)
    {
        ...
        // Ziyaretçilerin Bölüm ID'lerini Topla
        $visitorBolumIds = [];
        if ($iaa->ziyaretPlani) {
            $visitorsData = $iaa->ziyaretPlani->visitors;
            $vIds = [];
            if ($visitorsData) {
                $vIds = is_string($visitorsData) ? json_decode($visitorsData, true) : (is_array($visitorsData) ? $visitorsData : []);
            } elseif ($iaa->ziyaretPlani->visitor_id) {
                $vIds[] = $iaa->ziyaretPlani->visitor_id;
            }
            if (!empty($vIds)) {
                $visitorBolumIds = \App\Models\User::whereIn('id', $vIds)->pluck('bolum_id')->filter()->unique()->toArray();
            }
        }

        // 1.6 Ziyaretçilerin Yöneticileri ve Proje Lider Yardımcıları
        if ($user->hasRole('Direktör')) {
            $yonetilenBolumIds = $user->yonetilenBolumler()->pluck('bolumler.id')->toArray();
            if (count(array_intersect($visitorBolumIds, $yonetilenBolumIds)) > 0) return true;
        }

        if ($user->hasRole('Bölüm Lideri') && $user->bolum_id) {
            if (in_array($user->bolum_id, $visitorBolumIds)) return true;
        }

        if ($user->hasRole('Bölüm Lider Yardımcısı') && $user->bolum_id) {
            $hasAccess = false;
            // Kendi bölümünün projesi mi?
            if ($iaa->bolum_id == $user->bolum_id || ($iaa->musteriSikayeti && $iaa->musteriSikayeti->sikayetKategori && $iaa->musteriSikayeti->sikayetKategori->bolum_id == $user->bolum_id)) {
                if ($user->hasPermissionTo('bolum.iaa.gor') || $user->hasPermissionTo('bolum.ziyaret.gor')) {
                    $hasAccess = true;
                }
            }
            // Ziyaretçinin yöneticisi mi?
            if (in_array($user->bolum_id, $visitorBolumIds) && $user->hasPermissionTo('bolum.ziyaret.gor')) {
                $hasAccess = true;
            }
    ...
    private function canEditProject(Iaa $iaa)
    {
        ...
        // Müşteri Saha Temsilcisi sadece read-only
