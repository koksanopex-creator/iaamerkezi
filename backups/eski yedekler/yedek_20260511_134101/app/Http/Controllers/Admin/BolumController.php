...
class BolumController extends Controller
{
    ...
    public function dashboard(Request $request, Bolum $bolum)
    {
        ...
        // 1. Yetki Kontrolü: Superadmin, Yönetim, Bölüm Lideri (Kendi Bölümü) veya Direktör (Kendi Bölümü)
        ...
        if (!$isSuperAdmin && !$isYonetim && !$isLider && !$isDirektor)
        {
            abort(403, 'Bu bölümün paneline erişim yetkiniz yok.');
        }

        // Direktörler için salt okunur modu bayrağı
        $isReadOnly = $isDirektor && !$isSuperAdmin && !$isYonetim && !$isLider;
