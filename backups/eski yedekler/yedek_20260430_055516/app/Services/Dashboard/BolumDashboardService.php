...
class BolumDashboardService
{
    ...
    public function getQualityStats(User $user, array $filters = [])
    {
        ...
        $stats = [
            'bolum_onay_sayisi' => Iaa::whereIn('durum', ['Bölüm Onayı Bekliyor', 'talep_onayi_bekliyor_kalite'])
        ...
        $statuses = [
            'onay_bekleyen' => ['Havuzda', 'Onay Bekliyor', 'Bölüm Onayı Bekliyor', 'Yönetici Onayı Bekliyor', 'Direktör Onayı Bekliyor', 'talep_onayi_bekliyor_kalite', 'talep_onayi_bekliyor_superadmin'],
    ...
    public function getLeaderStats(User $user, array $filters = [])
    {
        ...
        // --- 1. PROJELER ---
        $bolumProjeleriQuery = Iaa::whereIn('durum', ['Atandı', 'Devam Ediyor', 'Revize Ediliyor', 'Bölüm Onayı Bekliyor', 'Yönetici Onayı Bekliyor', 'talep_onayi_bekliyor_kalite'])
        ...
        $statuses = [
            ...
            'onay_bekleyen' => ['Onay Bekliyor', 'Bölüm Onayı Bekliyor', 'Yönetici Onayı Bekliyor', 'Direktör Onayı Bekliyor', 'talep_onayi_bekliyor_kalite', 'talep_onayi_bekliyor_superadmin']
        ];
    ...
    public function getDirectorAggregateStats($bolumIds, array $filters = [])
    {
        ...
        foreach ($bolumler as $bolum) {
            ...
            $projeQ = Iaa::whereIn('durum', ['Atandı', 'Devam Ediyor', 'Revize Ediliyor', 'Bölüm Onayı Bekliyor', 'Yönetici Onayı Bekliyor', 'Direktör Onayı Bekliyor', 'talep_onayi_bekliyor_kalite'])
                ->where(function ($q) use ($inclusiveQuery, $bolum) {
                    $inclusiveQuery($q);
                    $q->orWhereHas('musteriSikayeti.sikayetKategori', function ($sq) use ($bolum) {
                        $sq->where('bolum_id', $bolum->id);
