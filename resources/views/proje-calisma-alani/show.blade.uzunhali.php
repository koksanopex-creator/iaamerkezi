<x-app-layout>
    ...
    <div class="py-8 bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            {{-- ========================================================= --}}
            {{-- === SQUAD (PROJE EKİBİ) ALANI - DETAYLI & KARTLI GÖRÜNÜM === --}}
            {{-- ========================================================= --}}
            @if($iaa->musteriSikayeti)
                ...
                {{-- x-data ile durum kontrolü ekledik (squadOpen: false -> Default Kapalı) --}}
                <div x-data="{ squadOpen: false }" class="bg-white rounded-xl shadow-sm border border-indigo-100 p-6 animate-fade-in-up mb-8">
                    
                    {{-- 1. BAŞLIK VE ÖZET İSTATİSTİKLER (Tıklanabilir Alan) --}}
                    {{-- cursor-pointer ve click eventi eklendi --}}
                    <div @click="squadOpen = !squadOpen" class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2 pb-2 border-b border-gray-100 cursor-pointer select-none group">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <div class="flex items-center gap-2">
                                    ...
                                    {{-- Açılıp Kapanma İkonu (Ok İşareti) --}}
                                    <svg class="w-5 h-5 text-gray-400 transform transition-transform duration-200" 
                                        :class="{'rotate-180': squadOpen}" 
                                ...
                                <div class="flex flex-wrap items-center gap-2 mt-1.5">
                                    ...
                                    @if($bekleyenSayisi > 0)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-md text-xs font-medium bg-amber-100 text-amber-800 animate-pulse">
                        ...
                        {{-- Yönetim Butonu (Sadece Lider Görür) --}}
                        {{-- @click.stop ekledik ki butona basınca pencere kapanmasın --}}
                        @if($isLeader)
                            @php
                            ...
                            <div class="flex-shrink-0" @click.stop>
                                ...
                                @else
                                    <span class="flex items-center px-4 py-2 bg-gray-100 text-gray-500 text-sm font-medium rounded-lg cursor-not-allowed border border-gray-200">
                    ...
                    {{-- 2. ÜYE LİSTESİ (KARTLAR) --}}
                    {{-- x-show ile görünürlüğü bağladık, x-transition ile animasyon verdik --}}
                                ...
                                <div class="relative flex items-start gap-4 p-4 rounded-xl border transition-all duration-200 hover:shadow-md {{ $cardClass }} group">
                                    ...
                                    <a href="{{ route('profile.show', $uye->id) }}" class="flex-shrink-0 relative block">
                                        @if($uye->profile_photo_path)
                                            <img class="h-14 w-14 rounded-full object-cover border-2 {{ $isLider ? 'border-indigo-200' : 'border-white' }} shadow-sm group-hover:scale-105 transition-transform duration-200 {{ $durum == 'reddedildi' ? 'grayscale' : '' }}" 
                                                src="{{ asset('storage/'.$uye->profile_photo_path) }}" 
                                                alt="{{ $uye->name }}">
                                        @else
                                            <div class="h-14 w-14 rounded-full border-2 {{ $isLider ? 'border-indigo-200' : 'border-white' }} bg-gray-100 flex items-center justify-center text-lg font-bold text-gray-600 shadow-sm group-hover:scale-105 transition-transform duration-200 {{ $durum == 'reddedildi' ? 'grayscale' : '' }}">
                                                {{ substr($uye->name, 0, 1) }}
                                            </div>
                                        @endif
                                    ...
                                    <div class="flex-1 min-w-0 pt-0.5">
                                        <div class="flex items-start justify-between">
                                            <div>
                                                {{-- İsim --}}
                                                <a href="{{ route('profile.show', $uye->id) }}" class="text-sm font-bold text-gray-900 hover:text-indigo-600 transition-colors block leading-tight">
                                                    {{ $uye->name }}
                                                </a>
            ...
            {{-- ========================================================= --}}
                ...
                {{-- ========================================================= --}}
                {{-- === MÜŞTERİ BİLGİLENDİRME PANELİ (YENİ) === --}}
                {{-- ========================================================= --}}
                @if($iaa->musteriSikayeti)
                    @php
                        $sikayet = $iaa->musteriSikayeti;
                        // Önce Auth::check() ile giriş yapılmış mı bakıyoruz, sonra role bakıyoruz.
                    ...
                    @if($canNotify)
                        <div class="bg-white rounded-xl shadow-sm border border-indigo-100 mb-6 overflow-hidden">
                            ...
                            <div class="p-5">
                                {{-- ŞİFRE GÖSTERİM ALANI (Sadece işlem sonrası 1 kere görünür) --}}
                    ...
                    <div class="relative border-l-2 border-gray-200">
                        ...
                        @foreach ($steps as $step)
                            ...
                            @include('proje-calisma-alani.partials._step-item', [
                                ...
                                'canEdit' => $canEdit  // <--- BU SATIRI MUTLAKA EKLEYİN
                            ])
                        @endforeach
