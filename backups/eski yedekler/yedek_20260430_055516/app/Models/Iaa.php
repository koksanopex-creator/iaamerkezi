...
class Iaa extends Model
{
    ...
    public function getRealCompletionDateAttribute()
    {
        ...
        $onayLog = \App\Models\IaaLog::where('iaa_id', $this->id)
    ...
    public function getCompletionDurationInDaysAttribute()
    {
        ...
        if ($baslangicTarihi && $bitisTarihi) {
            try {
                $baslangic = \Carbon\Carbon::parse($baslangicTarihi);
                $bitis = \Carbon\Carbon::parse($bitisTarihi);
    ...
    public function getAktifAdimAttribute()
    {
        ...
        if ($assignment && !empty($assignment->workflow_snapshot)) {
            ...
            $completedStepIds = \App\Models\IaaProgressUpdate::where('iaa_talep_id', $assignment->id)
    ...
    public function getIlerlemeVerisiAttribute()
    {
        ...
        $totalSteps = count($assignment->workflow_snapshot);
        
        // Tamamlanmış adım sayısını bul
        $completedSteps = \App\Models\IaaProgressUpdate::where('iaa_talep_id', $assignment->id)
            ->whereNotNull('completed_at')
            ->count();
        ...
        $gecenGun = ceil(\Carbon\Carbon::parse($baslangicTarihi)->diffInDays(now()));
    ...
    public function getAktifAsamaMetniAttribute()
    {
        ...
        if ($veriler['kapanis_bekleniyor']) {
            return match ($this->durum) {
                'Bölüm Onayı Bekliyor' => 'Bölüm Onayı Bekleniyor',
                'Direktör Onayı Bekliyor' => 'Direktör Onayı Bekleniyor',
                'Yönetici Onayı Bekliyor' => 'Yönetici Onayı (Final) Bekleniyor',
                'talep_onayi_bekliyor_kalite' => 'Talep Onayı (Kalite) Bekleniyor',
                'talep_onayi_bekliyor_superadmin' => 'Talep Onayı (Yönetim) Bekleniyor',
                'hatali_bildirim_onayi_bekliyor_kalite' => 'Hatalı Bildirim Onayı (Kalite) Bekleniyor',
                'hatali_bildirim_onayi_bekliyor_direktor' => 'Hatalı Bildirim Onayı (Direktör) Bekleniyor',
                'hatali_bildirim_onayi_bekliyor_superadmin' => 'Hatalı Bildirim Onayı (Yönetim) Bekleniyor',
                default => 'Kapanış İşlemleri Bekleniyor'
            };
        }
    ...
    public function getBeklemeSuresiMetniAttribute()
    {
        ...
        if ($tarih) {
            $tarihObj = \Carbon\Carbon::parse($tarih);
