...
<div class="relative flex items-center gap-2 px-3 py-1.5 rounded-full border transition-all duration-200 hover:shadow-sm {{ $cardClass }} group w-auto">
    
    {{-- Avatar (Tıklanabilir) --}}
    <a href="{{ route('profile.show', $uye->id) }}" class="flex-shrink-0 relative block">
        @if($uye->profile_photo_path)
            <img class="h-8 w-8 rounded-full object-cover border {{ $isLider ? 'border-indigo-200' : 'border-white' }} shadow-sm group-hover:scale-105 transition-transform duration-200 {{ ($durum == 'reddedildi' || $uye->trashed()) ? 'grayscale' : '' }}" 
                    src="{{ asset('storage/'.$uye->profile_photo_path) }}" 
                    alt="{{ $uye->name }}">
        @else
            <div class="h-8 w-8 rounded-full border {{ $isLider ? 'border-indigo-200' : 'border-white' }} bg-gray-100 flex items-center justify-center text-xs font-bold text-gray-600 shadow-sm group-hover:scale-105 transition-transform duration-200 {{ ($durum == 'reddedildi' || $uye->trashed()) ? 'grayscale' : '' }}">
                {{ substr($uye->name, 0, 1) }}
            </div>
        @endif
        ...
        @elseif(($durum == 'reddedildi' || $uye->trashed()) && !$isLider)
            <span class="absolute bottom-0 right-0 flex items-center justify-center h-3 w-3 rounded-full ring-1 ring-white bg-red-500 text-white text-[6px] font-bold" title="{{ $uye->trashed() ? 'İşten Ayrıldı' : 'Reddetti' }}">X</span>
        @endif
    </a>

    {{-- Bilgiler --}}
    <div class="flex flex-col min-w-0 pr-1">
        <a href="{{ route('profile.show', $uye->id) }}" class="text-xs font-bold text-gray-900 hover:text-indigo-600 transition-colors flex items-center gap-1 leading-tight whitespace-nowrap">
            {{ $uye->name }}
            @if($uye->trashed())
        ...
        <p class="text-[10px] text-gray-500 font-medium truncate max-w-[140px]" title="{{ optional($uye->bolum)->ad }} - {{ $isLider ? 'Proje Lideri' : 'Ekip Üyesi' }}">
            {{ $isLider ? 'Lider' : 'Üye' }} &bull; {{ optional($uye->bolum)->ad ?? 'Bölüm Yok' }}
            @if($durum == 'bekliyor')
    ...
    @if($durum == 'bekliyor' && isset($isLeader) && $isLeader && isset($isLocked) && !$isLocked)
