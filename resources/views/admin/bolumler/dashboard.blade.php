...
<x-app-layout>
    ...
    @mousemove="if(hoveredUser) { $refs.hoverCard.style.left = ($event.clientX + 20) + 'px'; $refs.hoverCard.style.top = ($event.clientY + 20) + 'px'; }">
        ...
        <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            @if(Auth::user()->hasAnyRole(['Superadmin', 'Bölüm Lideri']) && !($isReadOnly ?? false))
                <div class="flex gap-2">
                    <a href="{{ route('admin.bolum-yonetim.index', ['bolum_id' => $bolum->id]) }}" class="inline-flex items-center px-5 py-2.5 bg-indigo-600 border border-transparent rounded-xl shadow-sm text-sm font-bold text-white hover:bg-indigo-700 transition-all gap-2">
        ...
        <div class="hero-premium">
            ...
            <div class="hero-content">
                ...
                <div class="flex-grow">
                    ...
                    <div class="flex flex-wrap gap-4">
                        ...
                        {{-- YARDIMCILAR (BÖLÜM LİDER YARDIMCISI) --}}
                        @foreach($bolum->yardimcilar as $yardimci)
                            <a href="{{ route('profile.show', $yardimci->id) }}" 
                               @mouseenter="hoveredUser = { 
                                   name: '{{ $yardimci->name }}', 
                                   email: '{{ $yardimci->email }}', 
                                   phone: '{{ $yardimci->telefon ?? 'Belirtilmemiş' }}', 
                                   unvan: '{{ $yardimci->unvan ?? 'Bölüm Lider Yardımcısı' }}',
                                   photo: '{{ $yardimci->profile_photo_url }}'
                               }" 
                               @mouseleave="hoveredUser = null"
                               class="inline-flex items-center gap-2 group relative">
                                <div class="relative">
                                    @if($yardimci->profile_photo_path)
                                        <img src="{{ asset('storage/' . $yardimci->profile_photo_path) }}" class="w-8 h-8 rounded-full border-2 border-white shadow-sm ring-1 ring-emerald-200">
                                    @else
                                        <div class="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-600 text-xs font-bold border-2 border-white shadow-sm ring-1 ring-emerald-200">
                                            {{ substr($yardimci->name, 0, 1) }}
                                        </div>
                                    @endif
                                </div>
                                <div class="flex flex-col">
                                    <span class="text-[10px] font-bold text-gray-400 uppercase leading-none mb-1">Yardımcı</span>
                                    <span class="text-sm font-bold text-gray-700 group-hover:text-emerald-600 transition">{{ $yardimci->name }}</span>
                                </div>
                            </a>
                        @endforeach
                    </div>
            ...
            <div class="stats-grid">
                @if($bolum->sikayet_kategorileri_count > 0 && $canSeeSikayet)
                <div class="stat-card">
                ...
                <div class="stat-card">
                    <span class="stat-val text-indigo-600">{{ $beyazYakaCount }}</span>
                    <span class="stat-tit">Beyaz Yaka</span>
                </div>
                <div class="stat-card">
                    <span class="stat-val text-blue-500">{{ $maviYakaCount }}</span>
                    <span class="stat-tit">Mavi Yaka</span>
                </div>

                @if($bolum->has_machines && $canSeeMakine)
                <div class="stat-card">
        ...
        <div>
            ...
            <div x-show="mainTab === 'projeler'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-8" x-cloak>
                ...
                @if($canSeeIaa)
                    @livewire('admin.bolum-iaa-listesi', ['bolumId' => $bolum->id])
                @endif

                @if($bolum->sikayet_kategorileri_count > 0 && $canSeeSikayet)
                    @livewire('admin.bolum-sikayet-listesi', ['bolumId' => $bolum->id])
                @endif
            </div>
            @endif
                    ...
                    <div class="lg:col-span-1">
                        <div class="premium-card p-6">
                            <div class="flex items-center justify-between mb-6">
                                <h3 class="p-card-title">Personel Listesi ({{ $toplamPersonelCount }})</h3>
                                <div class="flex gap-1 bg-gray-100 p-1 rounded-lg">
                                    <button @click="personelTab = 'beyaz-yaka'" :class="personelTab === 'beyaz-yaka' ? 'bg-white shadow-sm' : ''" class="px-3 py-1 rounded-md text-[10px] font-bold transition">BEYAZ YAKA ({{ $beyazYakaCount }})</button>
                                    <button @click="personelTab = 'mavi-yaka'" :class="personelTab === 'mavi-yaka' ? 'bg-white shadow-sm' : ''" class="px-3 py-1 rounded-md text-[10px] font-bold transition">MAVİ YAKA ({{ $maviYakaCount }})</button>
                                </div>
                            </div>

                            @if(($pendingUsersCount ?? 0) > 0 && (Auth::user()->hasRole('Bölüm Lideri') || Auth::user()->hasRole('Superadmin')))
                            <div class="mb-4">
                                <a href="{{ route('admin.users.onay_bekleyenler') }}" class="w-full flex items-center justify-between bg-red-50 border border-red-100 px-4 py-3 rounded-xl hover:bg-red-100 transition shadow-sm group">
                                    ...
                                    <span class="bg-red-500 text-white text-xs font-black px-2.5 py-1 rounded-lg group-hover:scale-110 transition-transform">{{ $pendingUsersCount }}</span>
                                </a>
                            </div>
                            @endif

                            @if(!($isReadOnly ?? false) || $user->hasRole('Bölüm Lideri'))
                            ...
                            <div class="space-y-3 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                                <template x-if="personelTab === 'beyaz-yaka'">
                                    <div class="space-y-2">
                                        @foreach($users->where('is_mavi_yaka', false) as $u)
                                            <div x-show="userSearch === '' || '{{ strtolower($u->name) }}'.includes(userSearch.toLowerCase())" 
                                                 class="flex items-center gap-3 p-3 {{ $u->hasRole('Bölüm Lideri') ? 'bg-indigo-50 border border-indigo-100' : 'hover:bg-gray-50' }} rounded-xl transition">
                                                <div class="relative">
                                                    <img src="{{ $u->profile_photo_url }}" class="w-10 h-10 rounded-full border-2 border-white shadow-sm">
                                                    @if($u->hasRole('Bölüm Lideri'))
                                                ...
                                                <div class="flex flex-col flex-1">
                                                    <div class="flex items-center justify-between">
                                                        <a href="{{ route('profile.show', $u->id) }}" class="text-sm font-bold text-gray-900 hover:text-indigo-600 truncate">{{ $u->name }}</a>
                                                        @if($u->hasRole('Bölüm Lideri'))
                                                            <span class="bg-yellow-100 text-yellow-700 text-[8px] px-1.5 py-0.5 rounded font-black uppercase tracking-tighter">LİDER</span>
                                                        @elseif($u->hasRole('Bölüm Lider Yardımcısı'))
                                                    ...
                                                    <span class="text-[10px] text-gray-400 font-bold uppercase">{{ $u->unvan ?? 'Personel' }}</span>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                                ...
                                <template x-if="personelTab === 'mavi-yaka'">
                                    <div class="space-y-2">
                                            @foreach($users->where('is_mavi_yaka', true) as $u)
                                                <div x-show="userSearch === '' || '{{ strtolower($u->name) }}'.includes(userSearch.toLowerCase())" class="flex items-center gap-3 p-2 hover:bg-blue-50 rounded-xl transition">
                                                    <img src="{{ $u->profile_photo_url }}" class="w-10 h-10 rounded-full border-2 border-white shadow-sm">
                                                    <div class="flex flex-col">
                                                        <a href="{{ route('profile.show', $u->id) }}" class="text-sm font-bold text-gray-900 hover:text-blue-600 truncate">{{ $u->name }}</a>
                                                        <span class="text-[10px] text-blue-400 font-bold uppercase">Sicil: {{ $u->sicil_no ?? '-' }}</span>
                                                    </div>
                                                </div>
                                            @endforeach
                                        </div>
            ...
            <div x-show="mainTab === 'teknik'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-8" x-cloak>
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div class="lg:col-span-2">
                        <div class="premium-card">
                            <div class="p-card-header">
                                <h3 class="p-card-title">Makine Envanteri ({{ $machines->count() }})</h3>
                                @if(!$isReadOnly || $user->hasBolumAuthority('bolum.makine.yonet'))
                    ...
                    <div class="premium-card">
                        <div class="p-card-header">
                            <h3 class="p-card-title flex items-center gap-2">
                                ...
                                Hammaddeler ({{ $hammaddeler->count() }})
                            </h3>
                            @if(!$isReadOnly || $user->hasBolumAuthority('bolum.hammadde.yonet'))
                    ...
                    <div class="premium-card">
                        <div class="p-card-header">
                            <h3 class="p-card-title flex items-center gap-2">
                                ...
                                Ürün Versiyonları ({{ $versiyonlar->count() }})
                            </h3>
                            @if(!$isReadOnly || $user->hasBolumAuthority('bolum.hammadde.yonet'))
