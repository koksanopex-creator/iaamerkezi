        ...
        if ($this->iaaProjesi) {
            ...
            if (str_contains(strtolower($projeDurum), 'talep')) {
                if (in_array(strtoupper($projeDurum), ['TALEP_OLARAK_KAPATILDI', 'TALEP_OLARAK_KAPATİLDİ'])) {
                    return '<span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-purple-100 text-purple-700 border border-purple-200">🟣 Talep Olarak Kapatıldı</span>';
                }
                
                $text = "🟣 Müşteri Talebi (Onay Sürecinde)";
                if ($projeDurum == 'talep_onayi_bekliyor_kalite') $text = "🟣 Müşteri Talebi (Kalite Onayında)";
                if ($projeDurum == 'talep_onayi_bekliyor_direktor') $text = "🟣 Müşteri Talebi (Direktör Onayında)";
                if ($projeDurum == 'talep_onayi_bekliyor_superadmin') $text = "🟣 Müşteri Talebi (Yönetim Onayında)";
                
                return '<span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-purple-100 text-purple-800 border border-purple-200 animate-pulse">'.$text.'</span>';
            }

            // --- HATALI BİLDİRİM SÜREÇLERİ (TURUNCU) ---
            if (str_contains(strtolower($projeDurum), 'hatali_bildirim')) {
                if ($projeDurum == 'hatali_bildirim_olarak_kapatildi') {
                    return '<span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-orange-100 text-orange-600 border border-orange-200 line-through">🟠 Hatalı Bildirim Olarak Kapatıldı</span>';
                }

                $text = "🟠 Hatalı Bildirim (Onay Sürecinde)";
                if ($projeDurum == 'hatali_bildirim_onayi_bekliyor_kalite') $text = "🟠 Hatalı Bildirim (Kalite Onayında)";
                if ($projeDurum == 'hatali_bildirim_onayi_bekliyor_direktor') $text = "🟠 Hatalı Bildirim (Direktör Onayında)";
                if ($projeDurum == 'hatali_bildirim_onayi_bekliyor_superadmin') $text = "🟠 Hatalı Bildirim (Yönetim Onayında)";

                return '<span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-orange-100 text-orange-800 border border-orange-200 animate-pulse">'.$text.'</span>';
            }
